...
<script type="text/javascript" src="<?php echo WEBPATH; ?>js/batch_details.js"></script>
      ...
      <div class="container">
        ...
        <div class="navbar-collapse collapse">
          <ul class="nav navbar-nav">
            ...
            <li><a href="<?php echo PATHURL; ?>batch/details/" class="nav-items-link-active"><i class="fa fa-fw fa-file-text"></i> BCH11</a></li>
		...
		<div class="row">
			<div class="col-lg-12">
				<h3 class="page-header">
					<span class="page-header-text">BCH11 Batch details Lookup</span>
					<div class="filter-div-settings text-right">
						...
						<a href="<?php echo PATHURL; ?>batch/details/" class="btn btn-transparent-outlined"><i class="fa fa-fw fa-undo"></i> Reset Filter</a>
				...
				<div class="well filter-div">
					<form id="submitSearch" name="signinform" action="<?php echo PATHURL; ?>batch/details/" method="POST" class="form-signin" role="form" onsubmit="">
					<input type="hidden" name="data[UNMASK]" value="<?php echo isset($data['UNMASK'])? $data['UNMASK']:0;?>" id="MASKUNMASK">
						<div class="row">
						<div class="col-sm-12 col-md-3 mb-3">
								<div class="form-group">
									<label>Device Id</label>
									<input type="text" maxlength="10" id="DEVICE_ID" class="form-control" placeholder="Device Id" name="data[DEVICE_ID]" value="<?php echo isset($data['DEVICE_ID'])? $data['DEVICE_ID']:'';?>" >
								</div>
							</div>
							<div class="col-sm-12 col-md-3 mb-3">
								<div class="form-group">
									<label>Merchant Id</label>
									<input type="text" maxlength="15" id="MERCHANT_ID" class="form-control" placeholder="Merchant Id" name="data[MERCHANT_ID]" value="<?php echo isset($data['MERCHANT_ID'])? $data['MERCHANT_ID']:'';?>">
								</div>
							</div>
							...
							<div class="col-sm-12 col-md-3 mb-3">
								<div class="form-group">
									...
									<input type="text" maxlength="10" id="BATCH_NBR" class="form-control" placeholder="Batch Number" name="data[BATCH_NBR]" value="<?php echo isset($data['BATCH_NBR'])? $data['BATCH_NBR']:'';?>">
								</div>
							</div>
							<div class="col-sm-12 col-md-3 mb-3">
								<div class="form-group">
									<label> Retrieval Ref No</label>
									<input type="text" maxlength="15" id="RETRIEVAL_REF_NUM" class="form-control" placeholder="Retrieval Ref No" name="data[RETRIEVAL_REF_NUM]" value="<?php echo isset($data['RETRIEVAL_REF_NUM'])? $data['RETRIEVAL_REF_NUM']:'';?>">
								</div>
							</div>
							<div class="col-sm-12 col-md-3 mb-3">
								<div class="form-group">
									...
									<div>
										<div class="input-group date form_date" data-date="" data-date-format="yyyy-mm-dd" data-link-field="dtp_input1" data-link-format="yyyy-mm-dd">
											<input class="form-control" size="16" type="text" name="data[TRAN_DATE]" value="<?php echo isset($data['TRAN_DATE'])? $data['TRAN_DATE']:'';?>" readonly>
								...
								<div class="form-group">
									...
									<div>
										<div class="input-group date form_date" data-date="" data-date-format="yyyy-mm-dd" data-link-field="dtp_input2" data-link-format="yyyy-mm-dd">
											<input class="form-control" size="16" type="text" readonly name="data[OPEN_DATE]" value="<?php echo isset($data['OPEN_DATE'])? $data['OPEN_DATE']:'';?>">
											<span class="input-group-addon"><span class="glyphicon glyphicon-remove"></span></span>
											<span class="input-group-addon"><span class="glyphicon glyphicon-calendar"></span></span>
										</div>
				...
				<?php if(isset($result['data']) && !empty($result['data'])): ?>
					<div class="overflow-visible">
						...
						<?php foreach($result['data'] as $val): ?>
							<tr>
								...
								<td>
									<?php if(isset($data['UNMASK']) )  {?>
									<?php $maskCardNo = ""; 
									if(!empty($val['CARD_NBR']) && strlen($val['CARD_NBR']) > 10) {
								...
								<td align="right"><?php 
								if(!empty($val["TRAN_AMOUNT"])) { 
									    if($val["TRAN_AMOUNT"]!="000000000000"){ 
										$TRAN_AMOUNT = ltrim($val["TRAN_AMOUNT"], 0);
										$TRAN_AMOUNT = substr_replace($TRAN_AMOUNT, '.', -2, 0);
										}else{$TRAN_AMOUNT = '0';}
									} else { 
												if($val["TRAN_AMOUNT"]!=""){
													$TRAN_AMOUNT = '';
									...
									echo number_format($TRAN_AMOUNT,2);
									/* if(isset($val['TRAN_AMOUNT']) && !empty($val['TRAN_AMOUNT'])) {
										$temp = ltrim($val['TRAN_AMOUNT'], 0);
										echo number_format(substr_replace($temp, '.', -2, 0), 2, '.',',');
									} else {
										echo number_format($val['TRAN_AMOUNT'], 2, '.',',');
									} */
								?></td>
								<td align="center"><?php echo $val['RESPONSE_CODE']; ?></td>
					...
					<?php endforeach; ?>
						...
						<ul class="pager">
							...
							<?php else: ?>
								<li><a href="<?php echo PATHURL.'batch/details/1'; ?>"><i class="fa fa-step-backward"></i> First</a></li>
								<li><a href="<?php echo PATHURL.'batch/details/'.($page-1); ?>"><i class="fa fa-backward"></i> Previous</a></li>
							<?php endif;?>
							
							<li><a href="#"><?php echo $page.'/'.$result['page_number'];?></a></li>
							<?php if(isset($result['page_number']) && $result['page_number'] > $page): ?>
								<li><a href="<?php echo PATHURL.'batch/details/'.($page+1); ?>">Next <i class="fa fa-forward"></i></a></li>
								<li><a href="<?php echo PATHURL.'batch/details/'.$result['page_number']; ?>">Last <i class="fa fa-step-forward"></i></a></li>
							<?php else: ?>
			...
			<div class="modal-dialog">
				<div class="modal-content" >
					<?php if(!isset($result['Total']) || $result['Total'] == 0): ?>
							<div class="alert alert-info" role="alert">No records found:(</div>
						<?php else: ?>
							<form id="SubmitExportCsv" name="signinform" action="<?php echo PATHURL; ?>batch/csv/details/" method="POST" class="form-signin" role="form">
					<div class="modal-header">
						<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
