$(document).ready(function () {
	...
	/*function openpopup(Transaction_ID) {
        $.ajax({
            url: "<?php echo PATHURL.'batch/detailinfo/';?>"+Transaction_ID,
            type: 'GET',
            cache: false,
	...
	*/	
	$(function () {
		...
		$('#BATCH_NBR, #RETRIEVAL_REF_NUM, #MERCHANT_ID, #DEVICE_ID').keypress(function (e) {
			if ($.inArray(e.keyCode, [46, 8, 9, 27, 13, 110, 190]) !== -1 ||
				 // Allow: Ctrl+A
				 (e.keyCode == 65 && e.ctrlKey === true) || 
				// Allow: Ctrl+C
	...
	function checkfilter() {
		if($('#DEVICE_ID').val() == '' && $('#BATCH_NBR').val() == '' && $('#RETRIEVAL_REF_NUM').val() == '' && $('#MERCHANT_ID').val() == ''
			&& $('#dtp_input1').val() == '' && $('#dtp_input2').val() == '') {
	...
	$("p.anchor").click(function() {
		...
		var Transaction_ID = event.srcElement.innerHTML;
		
		$.ajax({
            url: mypath+'batch/detailinfo/'+Transaction_ID,
            type: 'GET',
            cache: false,
            success: function(data) {
                $('#modalContent').html(data);
