...
/*function openpopup(device_id, batch_num) {
        $.ajax({
            url: "<?php echo PATHURL.'batch/headinfo/';?>"+device_id+'/'+batch_num,
            type: 'GET',
            cache: false,
...
/* 		$("#DEVICE_ID").keydown(function (e) {
		...
		$('#BATCH_NBR,#DEVICE_ID').keypress(function (e) {
			if ($.inArray(e.keyCode, [46, 8, 9, 27, 13, 110, 190]) !== -1 ||
				 // Allow: Ctrl+A
				 (e.keyCode == 65 && e.ctrlKey === true) || 
				// Allow: Ctrl+C
	...
	function checkfilter() {
		if($('#DEVICE_ID').val() == '' && $('#BATCH_NBR').val() == '') {
	...
	$("p.anchor").click(function() {
		...
		var batch_number =$(this).data("batch");
		var device_id = event.srcElement.innerHTML;	
	
		$.ajax({
				url: mypath+'batch/headinfo/'+device_id+'/'+batch_number,
				type: 'GET',
				cache: false,
				success: function(data) {
					$('#modalContent').html(data);
