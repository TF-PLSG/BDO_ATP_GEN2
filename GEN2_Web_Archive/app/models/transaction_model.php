	...
	function process_data($data) {
		...
		if(!empty($data)) {
			...
			if(!empty($data['CARD_NUM'])) {
				$data['CARD_NUM_MASK'] = substr($data['CARD_NUM'], 0, 6) . str_repeat("X", strlen($data['CARD_NUM']) - 10) . substr($data['CARD_NUM'], -4);
			} else {
				$data['CARD_NUM_MASK'] = "";
			}
		...
		$data['TRAN_ID'] = $tranid = self::getTranID($data['CARD_NUM'], $data);//var_dump($tranid);die();
		/* $data['TRAN_ID1'] = $tranid["0"];
		$data['TRAN_ID2'] = $tranid["1"]; */
		$data['TRAN_ID1'] =isset($tranid[0]) ? $tranid[0] : null;
		$data['TRAN_ID2'] = isset($tranid[1]) ? $tranid[1] : null;
