      ...
      if ( timer_reply_size != Auth_Tx_Size )
	   {
         sprintf( err_buf,
                 "Error retrieving auth_tx from timer message. Lookup key = %s",
                  lookup_key );
         nccup_log_message( 1, 2, err_buf, "process_timeout_msg",0 );
	   }
	   else
      {
         ...
         strcpy( err_buf,"Txn to CUP timed out." );
         nccup_log_message( 1, 1, err_buf, "process_timeout_msg",0 );
         ...
         if ( false == increment_timeout_count(err_buf) )
         {
             ...
             */
            nccup_log_message( 1, 2, err_buf, "process_timeout_msg",1 );
...
INT set_timer( pCHAR lookup_key, INT timer_length, INT type_of_timer )
{
   ...
   if ( false == ret_val )
      nccup_log_message( 2, 2, err_buf, "set_timer" ,0);
...
INT clear_timer( pCHAR lookup_key )
{
   ...
   if ( false == ret_val )
      nccup_log_message( 2, 2, err_buf, "clear_timer",0 );
