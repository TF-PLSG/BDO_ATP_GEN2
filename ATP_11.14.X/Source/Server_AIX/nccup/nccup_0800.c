...
char masked_Key_value_cwk[49]={0};
...
void process_monitor_request( pPTE_MSG p_msg )
{
   ...
   else if ( nm_type == ST1_ECHOTEST )
   {
	   nmi_type = CUP_ECHO;
	   //HCTS1-D
	   nccup_log_message( 1, 1, "Sending ECHO TEST request to host.", "process_monitor_request",0 );
   }
   else if (nm_type == ST1_CUP_KEY_REQUEST)
   {
	  nmi_type = CUP_KEY_EXCHANGE;
   }
   /*Active Active trigger from Monitor*/
   else if (nm_type == ST1_CUP_ACTIVE_ACTIVE_TRIGGER)
   {
	   if((0 == strncmp(AppName,"nccup3",6)))
	   {
		   nccup_active_active_trigger_handler_from_monitor();
	   }
	   return;
   }
   ...
   else
   {
      ...
      sprintf( err_buf,
              "Unsupported request from Monitor. Subtype1 = %c",
               nm_type );
      nccup_log_message( 1, 2, err_buf, "process_monitor_request",0 );
   }
...
void issue_req_08X0( INT nm_type )
{
   ...
   else if ( nm_type == CUP_LOGON )
   {
      if ( 0 != strcmp( ONLINE, current_state ) )
      {
         ...
         nccup_log_message( 1, 1, err_buf, "issue_req_0820",0 );
   ...
   else
   {
      ...
      sprintf( err_buf,
              "Received request for unsupported request Type = %d",
               nm_type );
      nccup_log_message( 1, 1, err_buf, "issue_req_08X0",0 );
      ret_val = false;
   }
   ...
   if ( ret_val == true )
   {
      ...
      if ( false == build_08X0_msg_request(err_buf , nm_type) )
         nccup_log_message( 1, 2, err_buf, "issue_req_08X0",0 );
...
INT issue_rsp_08X0( INT nm_type )
{
   ...
   {
      nccup_log_message( 1, 1, "Sending Log ON response to host.",
    		  	  	  	  "issue_rsp_0820",0 );
   }
   else if ( nm_type == CUP_LOGOFF )
   {
      nccup_log_message( 1, 1, "Sending Log OFF response to host.",
    		  	  	  	  "issue_rsp_0820" ,0);
   }
   ...
   if ( false == build_08X0_msg_response(err_buf,nm_type ) )
      nccup_log_message( 1, 2, err_buf, "issue_rsp_08X0", 0 );
...
void process_08X0_request()
{
   ...
   {
      type = CUP_LOGON;
      nccup_log_message( 1, 1, "Received Log ON request from host.",
                  	  	  "process_08X0_request",0 );
      if ( 0 == strcmp(ONLINE, current_state) )
         nccup_log_message( 1, 1, "Already online.", "process_0820_request",0 );
      else
   ...
   {
      type = CUP_LOGOFF;
      nccup_log_message( 1, 1, "Received Log OFF request from host.",
                  	  "process_0820_request",0 );
      if ( 0 == strcmp(OFFLINE, current_state) )
         nccup_log_message( 1, 1, "Not online.", "process_0820_request",0 );
      else
   ...
   else
   {
      ...
      if ( 0 == strcmp(CUP_NMI_KEYCHANGE, NMI_Code) )
         nccup_log_message( 1, 1, "Key Change request from host not supported.",
        		 	 	 	 "process_0820_request",0 );

      else if ( 0 == strcmp(CUP_NMI_STR_DATE_SWITCH , NMI_Code) )
         nccup_log_message( 1, 1, "Unionpay starts date switch (0820) request from host not supported.",
                     	 	 "process_0820_request",0 );

      else if ( 0 == strcmp(CUP_NMI_END_DATE_SWITCH , NMI_Code) )
         nccup_log_message( 1, 1, "Unionpay end date switch (0820) request from host not supported.",
                     	 	 "process_0820_request",0 );
	  
	  else if ( 0 == strcmp(CUP_STND_IN_AUTH_ECHO , NMI_Code) )
			   nccup_log_message( 1, 1, "Stand in Echo test (0820) request from host not supported.",
						   	   	   "process_0820_request",0 );
	   else if ( 0 == strcmp(CUP_STND_IN_AUTH_TRA_ACT  , NMI_Code) )
			   nccup_log_message( 1, 1, "Stand in Authorization Tra Transmission active (0820) request from host not supported.",
						   	   	   "process_0820_request",0 );
	   else if ( 0 == strcmp(CUP_STND_IN_PARA_UPD , NMI_Code) )
			   nccup_log_message( 1, 1, "Stand in Parameters (0820) request from host not supported.",
					   	   	   	   "process_0820_request",0 );
      else
      {
         memset( buffer, 0x00, sizeof(buffer) );
         sprintf( buffer,
                 "Received unsupported 0820 request from host. NMI = %s",
                  NMI_Code );
         nccup_log_message( 1, 1, buffer, "process_08X0_request",0 );
      }
   }
      ...
      if ( true == issue_rsp_08X0( type ) )
      {
         if ( 0 != strcmp(new_state, same_state) )
         {
            ...
            if ( false == set_host_state( new_state, err_buf ) )
            {
               ...
               nccup_log_message( 1, 3, buffer, "process_08X0_request",0 );
            }
         }
      }
   }
...
void process_08X0_response()
{
   ...
   if ( 0 == strcmp(CUP_NMI_LOGON, NMI_Code) )
   {
      ...
      if ( 0 != strcmp(ONLINE, current_state) )
      {
	    ...
	    {
			nccup_log_message(1, 3,"Error sending network status to Txcntl",
	   ...
	   if ( ret_val < 0 )
	   {
           ...
           nccup_log_message(1, 3,buffer,"process_0820_response",0);
	   }

       nccup_log_message( 1, 1, "Received Log ON response.",
                     	 "process_0820_response",0 );

     //  if ( true == set_host_state( ONLINE, err_buf ) )
      // {
       // We can not make cup module online state untill key translation is successful
            if((0 == strncmp(AppName,"nccup3",6)))
			{
				   /* We received sign on response. Issue key change request. */
                   nccup_log_message( 1, 1, "Issuing Key Change message.",
                              	  	  "process_0820_response",0 );
					issue_req_08X0( CUP_KEY_EXCHANGE );
        ...
        /* else
         {
            ...
            strcat( buffer, err_buf );
            nccup_log_message( 3, 2, buffer, "process_08X0_response",0 );
        }*/
    }
   ...
   else if ( 0 == strcmp(CUP_NMI_LOGOFF, NMI_Code) )
   {
      ...
      if ( 0 != strcmp(OFFLINE, current_state) )
      {
         nccup_log_message( 1, 1, "Received Log OFF response. Setting state offline.",
                     	 	 "process_0820_response",0 );
      ...
      // We have to make other CUP link also offlin
	  if((0 == strncmp(AppName,"nccup3",6)))
	  {
		 nccup_send_logoff_message_to_nccup();
	  }
   ...
   else if ( 0 == strcmp(CUP_NMI_ECHO, NMI_Code) )
   {
      ...
      if ( false == clear_request_timer(echo_req_timer_key) )
      {
         ...
         nccup_log_message(2, 2, err_buf, "process_0820_response",0 );
      }
    ...
    if ( NULL_PTR == p_msg_ins )
		   ...
		   if ( ret_val < 0 )
		   {
			   pteipc_get_errormsg( ret_val, err_buf );
               sprintf(buffer,"Failed transmit to que: %s. %s",applnk_que_name,err_buf);
               nccup_log_message(1, 3,buffer,"process_08X0_response",0);
		   }
         ...
         /*if ( false == set_host_state( ONLINE, err_buf ) )
         {
            strcpy( buffer, "Unable to update host state to online. " );
            strcat( buffer, err_buf );
            nccup_log_message( 3, 2, buffer, "process_08X0_response" );
         }*/
      }
   ...
   {
      nccup_log_message( 1, 1,
                  "Received response to key change request.  ATP is in now waiting for the key from Host.",
                  "process_0820_response",0 );
      // We will now receive key exchange from CUP/UPI, enable the Key_exchange_enable_Flag
      if( 0 == strncmp(AppName,"nccup3",6))
      {
      	// NCCUP issuing module
    	  process_logon_Reset_Key_exchange_enable_flag();
      }
   }
   else
   {
      ...
      nccup_log_message( 1, 1, buffer, "process_08X0_response",0 );
   }
...
void process_net_management_from_host()
{
   ...
   else
   {
      ...
      nccup_log_message( 1, 2, buffer, "process_net_management_from_host",0 );
   }
   return;
}
...
INT build_08X0_msg_request( pCHAR err_buf , int cmd)
{
   ...
   if(0 == strncmp(Auth_Tx.TLF01_details.message_type, "0820",4))
   {
		...
		memcpy( HOST_MESSAGE.fwd_inst_id,"27240608" , 8 );
		len = strlen( HOST_MESSAGE.fwd_inst_id );
...
void process_monitor_query( pPTE_MSG p_msg )
{
   ...
   {
      nccup_log_message( 2, 3, "Unable to build PTE message for Monitor query",
   ...
   else
   {
      ...
      if (ConnectionId[0] != 0)
      {
         ...
         if ( false == send_msg( p_msg_query, applnk_que_name, err_buf ))
            nccup_log_message( 2, 3, err_buf, "process_monitor_query",0 );
      }
      free( p_msg_query );
   }
   return;
...
/*
 * * This Function is called by nccup module only * */
...
void process_key_request_response(pPTE_MSG p_msg_in)
{
	...
	strcpy(Buffer,"Nccup3 module received new key from CUP host, Lets updated nccup key as well");
	nccup_log_message( 1, 1, Buffer, "process_key_request_response",0 );
	// empty privious key and Mac Value
	strncpy(static_Rtdcwk_Key,"",48);
	strncpy(MAC,"",16);
	Get_Key_from_ncf01_table();
	if ((strlen(static_Rtdcwk_Key)> 0) && (strlen(MAC)> 0))
	{
		// We need to do key change and then store into data base only for nccup acquiring module
		memcpy(Ncf01_I.kek,MAC,strlen(MAC));
		memcpy(Auth_Tx.TLF01_details.source_key,static_Rtdcwk_Key,strlen(static_Rtdcwk_Key));
	...
	else
	{
		...
		nccup_log_message( 1, 3, ErrorMsg, "process_key_request_response",0 );
	}
...
/**
 *  This function is called by nccup3 module only
 * */
void nccup_process_key_update_request_from_Acq(pPTE_MSG p_msg_in)
{
	pBYTE          p_data;
	...
	memset( &Ncf01_Nccup3_I, 0x00, sizeof(  Ncf01_Nccup3_I ) );
	memcpy( &Ncf01_Nccup3_I, p_data + sizeof(AUTH_TX) ,sizeof( Ncf01_Nccup3_I) );

	memcpy(Ncf01_I.retired_cwk,Ncf01_Nccup3_I.retired_cwk,strlen(Ncf01_Nccup3_I.retired_cwk));
	memcpy(static_Rtdcwk_Key,Ncf01_Nccup3_I.retired_cwk,strlen(Ncf01_Nccup3_I.retired_cwk));

	Mask_Keys(static_Rtdcwk_Key);
	sprintf(key_msg1,"Key received from nccup is %s",masked_Key_value_cwk);
	nccup_log_message( 2, 1, key_msg1,"nccup_process_key_update_request_from_Acq",0 );
	sprintf(key_msg,"Key received from CUP is %s",masked_Key_value_cwk);
	nccup_log_message( 2, 1, key_msg,"nccup_process_key_update_request_from_Acq",0 );
	memcpy(Ncf01_I.kek,Ncf01_Nccup3_I.kek,strlen(Ncf01_Nccup3_I.kek));
	Auth_Tx.TLF01_details.tx_key=AUTH_PIN_CHANGE_RESPONSE;
	update_ncf01(&auth_tx);

}

void nccup_process_logon_after_key_transalation()
{
	INT     ret_val = true;
	CHAR    buffer[200]  = {0};
	CHAR    err_buf[200] = {0};
	...
	if((0 == strncmp(AppName,"nccup3",6)))
	{
		if( 0 == strncmp(Auth_Tx.TLF01_details.product_codes[16].code,"ISSLNK",6))
		{
			...
			Auth_Tx.TLF01_details.tx_key = AUTH_PIN_CHANGE_RESPONSE;
			nccup_log_message( 1, 1, "Send response to CUP Host for KEY Request",
									"nccup_process_logon_after_key_transalation",0 );
	...
	{
		if( 0 == strncmp(Auth_Tx.TLF01_details.product_codes[16].code,"ACQLNK",6))
		{
			memset(Auth_Tx.TLF01_details.product_codes[16].code,0x00,6);
			memset(Auth_Tx.TLF01_details.response_code,0x00,
				   (sizeof(Auth_Tx.TLF01_details.response_code) - 1));
	...
	if ( 0 != strcmp(ONLINE, current_state) )
	{
		if ( true == set_host_state( ONLINE, err_buf ) )
		{
			nccup_log_message( 1, 1, "CUP Key translation is successful. Setting state online.",
								"nccup_process_logon_after_key_transalation",0 );
		}
		else
		{
			strcpy( buffer, "Unable to update host state to online. " );
			strcat( buffer, err_buf );
			nccup_log_message( 1, 3, buffer,
							"nccup_process_logon_after_key_transalation",0 );
		}
	}
	else
	{
		nccup_log_message( 1, 1, "Setting state online.",
						"nccup_process_logon_after_key_transalation",0 );
	}

	if((0 == strncmp(AppName,"nccup3",6)))
	{
		send_update_Key_message_to_nccup2();
	}
	else
	{
		// Lets update nccup2 module to update its details for Pin transalation
		send_update_Pin_Transalate_info_to_nccup2();
	}

}

void nccup_process_logoff_after_key_transalation()
{
	INT     ret_val = true;
	CHAR    buffer[200]  = {0};
	CHAR    err_buf[200] = {0};
	...
	if((0 == strncmp(AppName,"nccup3",6)))
	{
		...
		{
			memset(Auth_Tx.TLF01_details.product_codes[16].code,0x00,6);
			nccup_log_message( 1, 1, "CUP Key translation failed,Send response to CUP Host for KEY Request",
								"nccup_process_logoff_after_key_transalation",0 );
			memcpy(Auth_Tx.TLF01_details.response_code,CUP_KEY_TRANSLATION_ERROR,CUP_RESPONSEP_CODE_LEN);
			Auth_Tx.TLF01_details.tx_key = AUTH_PIN_CHANGE_RESPONSE;
			hhutil_send_net_mngmnt_response(&Auth_Tx);
			// Send Log off meesage to CUP HOST
			issue_req_08X0(CUP_LOGOFF);
		}
		// We have to make nccup2 module also offline:
		nccup_send_logoff_request_to_nccup2_after_key_transalation_failed();
	...
	if ( 0 != strcmp(OFFLINE, current_state) )
	{
		if ( true == set_host_state( OFFLINE, err_buf ) )
		{
			sprintf(buffer,"%s module is OFFLINE",AppName);
		...
		else
		{
			strcpy( buffer, "Unable to update host state to offline. " );
			strcat( buffer, err_buf );
			nccup_log_message( 1, 3, buffer, "nccup_process_logoff_after_key_transalation",0 );
		}
	}
	else
	{
		nccup_log_message( 1, 1, "State is OFFLINE", "nccup_process_logoff_after_key_transalation",0 );
	}

}

void nccup_process_logoff_after_logoff_message(void)
{
	INT     ret_val = true;
	CHAR    buffer[200]  = {0};
	CHAR    err_buf[200] = {0};
	...
	if ( 0 != strcmp(OFFLINE, current_state) )
	{
		if ( true == set_host_state( OFFLINE, err_buf ) )
		{
			sprintf(buffer,"%s module is OFFLINE",AppName);
			nccup_log_message( 1, 1, buffer,
							"nccup_process_logoff_after_logoff_message",0 );
		}
		else
		{
			strcpy( buffer, "Unable to update host state to offline. " );
	...
	{
		nccup_log_message( 1, 1, "Setting state offline.",
								"nccup_process_logoff_after_logoff_message",0 );
	}
}

/*
 * This function will reload the CUP and update the CUP keys for CUP ISSUING LINKE
 * Key will be reload only when status of module is online
 *
 */


INT nccup_active_active_trigger_handler_from_monitor()
{
	INT     ret_val = true;
	CHAR    buffer[200]  = {0};
	CHAR    err_buf[200] = {0};
	...
	if ( 0 == strcmp(ONLINE, current_state) )
	{
		ret_val == nccup_read_incoming_cup_key_from_nccup3_ncf01_issuing_table();
		if(true == ret_val)
		{
			if(true == nccup_send_active_active_trigger_to_nccup_module())
			{
				strcpy( buffer, "Send Active active trigger to nccup module Success" );
				nccup_log_message( 1, 1, buffer,
							 	 	 "nccup_active_active_trigger_handler_from_monitor",0 );
				if(true == nccup_send_active_active_trigger_to_nccup2_module())
				{
					strcpy( buffer, "Send Active active trigger to nccup module is Successful, Lets send to nccup2" );
					nccup_log_message( 1, 1, buffer,
										"nccup_active_active_trigger_handler_from_monitor",0 );
				}
				else
				{
					strcpy( buffer, "Send Active active trigger to nccup2 module failed,Making Sate OFFLINE" );
					nccup_log_message( 1, 1, buffer,
										"nccup_active_active_trigger_handler_from_monitor",0 );
				}
			}
			...
			{
				strcpy( buffer, "Send Active active trigger to nccup module failed,Making Sate OFFLINE" );
				nccup_log_message( 1, 1, buffer,
							 	 	 "nccup_active_active_trigger_handler_from_monitor",0 );
				//Send message to nccup2 and nccup3 to go offline.
			}
		}
		else
		{
			strcpy( buffer, " Active active trigger failed, nccup, nccup2 and nccup3 going OFFLINE" );
			nccup_log_message( 1, 3, buffer,
						 	 	 "nccup_active_active_trigger_handler_from_monitor",0 );
	...
	{
		sprintf( buffer, "%s state is offline can't enable Active Active setup", AppName);
		nccup_log_message( 1, 3, buffer,
							"nccup_active_active_trigger_handler_from_monitor",0 );
	}
}
/*
 *
 *This Function will make nccup module to go offline as nccup3 module was not
 *able to read key from DB
 */

void nccup_make_nccup_offline_after_active_active_triger_failed()
{
	INT     ret_val = true;
	CHAR    buffer[200]  = {0};
	CHAR    err_buf[200] = {0};
	...
	{
		// Send Log off meesage to CUP HOST
		//issue_req_08X0(LOGOFF);
		if ( true == set_host_state( OFFLINE, err_buf ) )
		{
			sprintf(buffer,"%s module is OFFLINE",AppName);
			nccup_log_message( 1, 2, buffer,
						 "nccup_make_nccup_offline_after_active_active_triger_failed",0 );
		}
		else
		{
			strcpy( buffer, "Unable to update host state to offline. " );
...
/*
 *
 *This Function will make nccup3 module to go offline as it was not
 *able to read key from DB
 */


INT nccup_make_nccup3_offline_after_active_active_triger_failed()
{
	INT     ret_val = true;
	CHAR    buffer[200]  = {0};
	CHAR    err_buf[200] = {0};
	...
	if ( 0 != strcmp(OFFLINE, current_state) )
	{
		...
		if ( true == set_host_state( OFFLINE, err_buf ) )
		{
			sprintf(buffer,"%s module is OFFLINE",AppName);
			nccup_log_message( 1, 2, buffer,
						 "nccup_make_nccup3_offline_after_active_active_triger_failed",0 );
		}
		else
		{
			strcpy( buffer, "Unable to update host state to offline. " );
...
void process_logon_Key_Reset_Reject_Transaction(void)
{
	...
	memcpy( Auth_Tx.TLF01_details.response_code,
		    CUP_KEY_TRANSLATION_ERROR,
			CUP_RESPONSEP_CODE_LEN);
	Auth_Tx.TLF01_details.tx_key = AUTH_PIN_CHANGE_RESPONSE;
	nccup_log_message( 1, 1, "Key exchange request is Rejected with RC:A7",0 );
	hhutil_send_net_mngmnt_response(&Auth_Tx);
}
...
void process_logon_Reset_Key_exchange_disable_flag(void)
{
	...
	Ncf01_I.prev_src_station[0] = '0';
	Auth_Tx.TLF01_details.tx_key = CUP_KEY_DESIABLE_FLAG;
	update_ncf01(&Auth_Tx);
	nccup_log_message( 1,1,"Key_exchange_enable_Flag is disable to nccup3 Table","process_logon_Reset_Key_exchange_disable_flag",0 );
}
void process_logon_Reset_Key_exchange_disable_flag_KEY_TRANSLATION_SUCCESS(void)
{
	Key_exchange_enable_Flag = 0;
	Ncf01_I.prev_src_station[0] = '0';
	Auth_Tx.TLF01_details.tx_key = CUP_KEY_DESIABLE_FLAG;
	update_ncf01(&Auth_Tx);
	nccup_log_message( 1,1,"Key_exchange_enable_Flag is disable to nccup3 Table","process_logon_Reset_Key_exchange_disable_flag_KEY_TRANSLATION_SUCCESS",0 );
}
void process_logon_Reset_Key_exchange_disable_flag_KEY_TRANSLATION_FAILED(void)
{
	Key_exchange_enable_Flag = 0;
	Ncf01_I.prev_src_station[0] = '0';
	Auth_Tx.TLF01_details.tx_key = CUP_KEY_DESIABLE_FLAG;
	update_ncf01(&Auth_Tx);
	nccup_log_message( 1,1,"Key_exchange_enable_Flag is disable to nccup3 Table","process_logon_Reset_Key_exchange_disable_flag_KEY_TRANSLATION_FAILED",0 );
}

void process_logon_Reset_Key_exchange_enable_flag(void)
{
	Key_exchange_enable_Flag = 1;
	Ncf01_I.prev_src_station[0] = '1';
	Auth_Tx.TLF01_details.tx_key = CUP_KEY_ENABLE_FLAG;
	update_ncf01(&Auth_Tx);
	nccup_log_message( 1,1,"Key_exchange_enable_Flag is enable to nccup3 Table","process_logon_Reset_Key_exchange_enable_flag",0 );
}
void Mask_Keys(char* key_value)
{
	int key_valuelen = 0;
	int nIndex 	= 0;
	memset(masked_Key_value_cwk ,0x00, sizeof(masked_Key_value_cwk));
	key_valuelen = strlen( key_value );
	if (key_valuelen > 0)
	{
		/* Masking card number first 5 and last 4 in clear form remaining all masked. */
		strncpy( masked_Key_value_cwk, key_value, key_valuelen);
		for ( nIndex = key_valuelen-5; nIndex >= key_valuelen-11; nIndex-- )
		{
			masked_Key_value_cwk[nIndex] = '*';
		}
	}
}
