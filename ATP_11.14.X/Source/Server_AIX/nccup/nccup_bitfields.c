...
extern CHAR   MAC[17];
extern char cup_scheme_timeout_rc;
...
INT build_request_field_4( pINT result_flag, pCHAR err_buf )
{
   ...
   if ( *result_flag == 0 )
   {
      ...
      else if ( isnum(Auth_Tx.TLF01_details.total_amount) )
      {
         ...
         {
            memcpy(HOST_MESSAGE.amount,Auth_Tx.TLF01_details.orig_amount,CUP_DE4_SIZE);
         }
      }
      else
      {
...
INT build_request_field_7( pINT result_flag, pCHAR err_buf )
{
   ...
   {
	   // Its issuing transaction coming from CUP ,
	   //echo what ever is there in bit 7 which we have received
	   // If time stamp filed is empty its network transction create time stamp and send
	   ...
	   memcpy( HOST_MESSAGE.gmt_time,
			   Auth_Tx.TLF01_details.transmission_timestamp, CUP_DE7_SIZE );
	   if ( is_reversal() )
	   {
...
INT build_request_field_14( pINT result_flag, pCHAR err_buf )
{
	...
	if(	0 == strncmp(Auth_Tx.TLF01_details.handler_queue,"nccup",5) )
	{
		...
		else if ( Auth_Tx.TLF01_details.tx_key==AUTH_PRE_AUTHORIZATION_CANCLE_RESPONSE)
		{
			// No need to any thing
		}
		else 
		{
			memcpy( HOST_MESSAGE.date_expd,temp_exp_date,CUP_DE14_SIZE );
		}
	}
...
INT build_request_field_22( pINT result_flag, pCHAR err_buf )
{
	...
	else if ( *result_flag == 0 )
	{
...
#if BDOR_68_REQ_1_UPI /* MobilePOS NFC Tap2Phone Transactions */
		else if(true == nccup_check_if_trx_is_upi_MobilePOS_NFC_Tap2Phone_trx_with_pin())
		{
		   strncpy( HOST_MESSAGE.posentry,
				    UPI_MOBILE_NFC_TAP2PHONE_TXN_WITH_PIN_PEM,
					CUP_DE22_SIZE );
		}
		else if(true == nccup_check_if_trx_is_upi_MobilePOS_NFC_Tap2Phone_trx_without_pin())
		{
		   strncpy( HOST_MESSAGE.posentry,
	   ...
	   else if(true == nccup_check_if_trx_is_upi_Swiftpass_BDO_Pay_credit_trx_with_pin())
	   {
		   strncpy( HOST_MESSAGE.posentry,
				    UPI_SWIFTPASS_BDO_PAY_CREDIT_TXN_WITH_PIN_PEM,
					CUP_DE22_SIZE );
	   }
	   else if(true == nccup_check_if_trx_is_upi_Swiftpass_BDO_Pay_credit_trx_without_pin())
	   {
		   strncpy( HOST_MESSAGE.posentry,
			...
			else if(true == nccup_check_if_trx_is_upi_ecommerce_swiftpass_BDOPay_trx ())
			{
			   strncpy( HOST_MESSAGE.posentry,
	...
	memcpy(Auth_Tx.TLF01_details.product_codes[9].code,HOST_MESSAGE.posentry,sizeof(HOST_MESSAGE.posentry));
	return( ret_val );
}
   ...
   INT   ret_val = true;
	...
	else if ( *result_flag == 0 )
	{
		...
		(strlen(Auth_Tx.TLF01_details.pos_condition_code) <= CUP_DE25_SIZE))
		{
			...
			if(true == nccup_Check_If_CUP_MOTO_TRN())
			{
				memcpy( HOST_MESSAGE.pos_condcd,
						CUP_MOTO_POS_COND_CODE,
						CUP_DE25_SIZE);
			}
			else if (true == nccup_Check_If_CUP_RPS_TRN())
			{
				memcpy( HOST_MESSAGE.pos_condcd,
...
#endif
			/* POS condtion code sent to HOST will be stored to dipsplay it on Tranlookup page */
			memcpy(Auth_Tx.TLF01_details.product_codes[4].quantity ,
...
{
	INT   ret_val = true;
	/*if there is no PIN DATA. No need to send this field*/
	...
	else if ( *result_flag == 0 )
	{
		...
		(strlen(Auth_Tx.TLF01_details.product_codes[17].code) <= CUP_DE26_SIZE))
		{
			...
			CUP_DE26_SIZE);
   	 		  ...
   	 		  (Auth_Tx.TLF01_details.tx_key == AUTH_VOID_SALE))
   		{
			//Pin code for viod going to CUP for acquring is 00
...
INT build_request_field_37( pINT result_flag, pCHAR err_buf )
{
	...
	if( Auth_Tx.TLF01_details.tx_key == AUTH_REVERSAL || Auth_Tx.TLF01_details.tx_key == AUTH_REVERSAL_ADVICE )
		{
			if ((strlen(Auth_Tx.TLF01_details.orig_retrieval_ref_num) > 0) && 
			(strlen(Auth_Tx.TLF01_details.orig_retrieval_ref_num) <= CUP_DE37_SIZE))
			{
	...
	{	
		...
		if(strlen( Auth_Tx.TLF01_details.response_code)> 0)
		{
			if(0 == strncmp(Auth_Tx.TLF01_details.response_code,
							...
							CUP_DE39_SIZE))
			{
				if(strlen( cup_scheme_timeout_rc)>0)
				{
...
INT build_request_field_42( pINT result_flag, pCHAR err_buf )
{
   ...
   if ( *result_flag == 0 )
   {
	    ...
	    }else if ((strlen(Auth_Tx.TLF01_details.merchant_id) > 0) && 
   ...
   if ( *result_flag == 0 )
   {
	 if(0== strncmp(Auth_Tx.TLF01_details.handler_queue,"nccup",5)) 
	 {
		 ...
		 memcpy( HOST_MESSAGE.cardaccname+25,
				 ...
				 CUP_CITY_LEN_FEILD_43 );// Max 12 Byte	one byte will over right on Country Name
...
INT build_request_field_48( pINT result_flag, pCHAR err_buf )
{
	...
	if(0 == strncmp(Auth_Tx.TLF01_details.handler_queue,"nccup",5))
	{
		if(strlen(Auth_Tx.TLF01_details.voice_auth_text)>0)
		{
			...
			memcpy( HOST_MESSAGE.addl_data,Auth_Tx.TLF01_details.voice_auth_text + CUP_DE48_LEN_SIZE,(len - CUP_DE48_LEN_SIZE));
		}
	}
...
INT build_request_field_53 ( pINT result_flag, pCHAR err_buf )
{
   ...
   if ( *result_flag == 0 )
   {
		if ( strlen (Auth_Tx.TLF01_details.pin_block ) > 0)
		{
			 strncpy(HOST_MESSAGE.sec_cntl,"2600000000000000" ,CUP_DE53_SIZE);
		}else if(Auth_Tx.TLF01_details.tx_key == AUTH_PIN_CHANGE_RESPONSE)
		{
			strncpy(HOST_MESSAGE.sec_cntl,"1600000000000000" ,CUP_DE53_SIZE);
		}
   }
   return( ret_val );
}
...
INT build_request_field_54 ( pINT result_flag, pCHAR err_buf )
{
		...
		(Auth_Tx.TLF01_details.tx_key == AUTH_BALANCE_INQUIRY_RESPONSE))
    {
		if ( 0 == strncmp(Auth_Tx.TLF01_details.response_code, "00",2) )
		{
			...
			/*Prepare Balance*/
			strncpy(tem_str + position_count, "000000000000", 12);
			...
			strncpy(tem_str + position_count, Auth_Tx.ACF01_details.available,12);
			position_count = position_count + 12;
...
INT build_request_field_55 ( pINT result_flag, pCHAR err_buf )
{
		...
		//HOST_MESSAGE.IccData_len
		 if(0== strncmp(Auth_Tx.TLF01_details.handler_queue,"nccup",5) )// Issuing tx, Coming from CUP		 
		 {
			...
			memcpy(HOST_MESSAGE.IccData_len,Auth_Tx.EMV_details.future_field1,3);
			strncpy(temp_ascii_len,HOST_MESSAGE.IccData_len,3);
			len = atoi(temp_ascii_len);
			memcpy(HOST_MESSAGE.IccData,Auth_Tx.EMV_details.future_field1+3,len);
...
INT build_request_field_60 ( pINT result_flag, pCHAR err_buf )
{
			...
			if(strlen(Auth_Tx.future_use_4)>0)
			{
				...
				if(len <= CUP_MAX_DE60_SIZE)
				{
					if(len==27)
					{
						...
						/*
						 * Prepare 60.3.9 and 60.3.10 for incoming transactions irrespective of it being present in
						 * the request. If present copy it, if not prepare it. These values are being used by vertexon.
						 * UPI is flagging this issue. Hence, changes are made.
						 */
						...
						sprintf(Len_temp,"%03d",len);

						memcpy(HOST_MESSAGE.self_def_feild_len ,Len_temp, sizeof(HOST_MESSAGE.self_def_feild_len));
					}
					else
					{
						memcpy( HOST_MESSAGE.self_def_feild,
					...
					/*Storing data for sbatch transaction*/
	   ...
	   {
			/*Prepare feild 60.1*/
				 strncpy(HOST_MESSAGE.self_def_feild,
						"0000",
						CUP_MESSAGE_REPONSE_CODE_FEILD_60_1_LEN);
		   
		   /*Prepare feild 60.2*/
	
				 /*Prepare feild 60.2.1 Reserved filled in with 0 as we are not aware of it*/
				 /* Check for MOTO transaction and build Field 61*/
					if((true == nccup_Check_If_CUP_MOTO_TRN()) ||
					   (true == nccup_Check_If_CUP_RPS_TRN()))
					{
				...
				*/
					...
					switch(temp_posentry)
					{
					   ...
					   break;
						...
						case 01:
							...
							else if(true == nccup_Check_If_CUP_MOTO_TRN())
							{
									if(true == nccup_is_voice_transaction())
									{
										memcpy(HOST_MESSAGE.self_def_feild + 5,"0",1);
									}
									else
									...
									(true == nccup_Check_If_CUP_BDO_PAY()))
							{
								memcpy(HOST_MESSAGE.self_def_feild + 5,"0",1);
							}
							else
					...
					/*Prepare feild 60.2.2 for Void and Reversal for Magstip transaction*/
			 		if( temp_posentry == 05 ||
						temp_posentry == 12 ||
						temp_posentry == 01 ||
						temp_posentry == 80 ||
						...
						temp_posentry == 98)
			 		{
						/* Do nothing Its EMV transaction*/
					}
				   ...
				   /* Prepare field 60.2.3 for Reversal and Fallback reversal*/
					...
					else if (true == nccup_Check_If_CUP_BDO_PAY())
					{
						memcpy(HOST_MESSAGE.self_def_feild + 8,"08", 2);
						/*Prepare feild 60.2.9 Interactive mode indicator*/
						memcpy(HOST_MESSAGE.self_def_feild + 14,"0", 1);
					...
					if(true == nccup_check_if_trx_is_upi_MobilePOS_NFC_Tap2Phone_trx_with_pin())
					{
					   memcpy( HOST_MESSAGE.self_def_feild + 4,
	...
	else if ( *result_flag == 0 )
	{
		...
		else if ( strlen(Auth_Tx.TLF01_details.orig_message)> 0)
		{
			...
			if((orig_data[1] == '0') && (Auth_Tx.TLF01_details.message_type[1]=='4'))
			{
				...
				nccup_log_message( 2, 3, err_buf, "build_request_field_90",1 );
...
INT build_request_field_100 ( pINT result_flag, pCHAR err_buf )
{
	...
	else if( *result_flag == 0 )
	{
		 if( (0 == strncmp(Auth_Tx.TLF01_details.handler_queue,"dcp",3))&&
   	 		  (Auth_Tx.TLF01_details.tx_key == AUTH_VOID_SALE))
   		{
   			//This field is not needed for POS ACQ void sale
			return ret_val;
		}
		else if( Auth_Tx.TLF01_details.tx_key == AUTH_PRE_AUTHORIZATION ||
				  Auth_Tx.TLF01_details.tx_key == AUTH_CARD_VERIFICATION ||
   ...
   if ( *result_flag == 0 )
   {
   	    if(strlen(HOST_MESSAGE.card_seq_num)> 0)
   	    {
   	    	memcpy( Auth_Tx.EMV_details.pan_sequence_number , 
					...
					CUP_DE23_SIZE );
   	    	memcpy( Auth_Tx.host2_message_seq_nbr ,
   	    					    HOST_MESSAGE.card_seq_num,
   	    						CUP_DE23_SIZE );
   ...
   if ( *result_flag == 0 )
   {
   	  if(strlen(HOST_MESSAGE.track2)> 0)
   	  {
	      ...
	      if ( len <= CUP_MAX_DE35_SIZE )
	      {
			...
			if((true == nccup_get_service_code(  service_code,  error_msg )))
			{
				...
				if(strcmp(HOST_MESSAGE.responsecd,"00") != 0)
				{
     				sprintf( error_msg,
            			 "Non-alphanumeric Auth Number (field 38) : %s,",
              		 		Auth_Tx.TLF01_details.auth_number );
					nccup_log_message( 1, 2, error_msg, "move_field_38" ,1);
				}
      	  ...
      	  if ( ret_val == false )
      	  {
		      	   sprintf( error_msg,
		                "Non-alphanumeric response code (field 39) : %s,",
		                 Auth_Tx.TLF01_details.response_code);
				   nccup_log_message( 1, 2, error_msg, "move_field_39",1 );
		  }
      	   /* Store the response code sent by CUP to display in tranlookup*/
      	  if(strncmp(Auth_Tx.TLF01_details.message_type,"0120",4) ||
      		 strncmp(Auth_Tx.TLF01_details.message_type,"0420",4))
      	  {
   ...
   if ( *result_flag == 0 )
   {
		   ...
		   /*
		 	...
		 	memcpy( Auth_Tx.TLF01_details.ncf01_retired_cwk,
					...
					CUP_CITY_LEN_FEILD_43 );// Max 12 Byte	1 byte we will over write on country name
					
			memcpy( Auth_Tx.TLF01_details.ncf01_retired_cwk+12," ",1);
...
INT move_field_48( pINT result_flag, pCHAR err_buf )
{
   ...
   if ( *result_flag == 0 )
   {
		...
		if(strlen(HOST_MESSAGE.addl_data_len)> 0)
		{
			...
			{
				/* Store length */
				memcpy(Auth_Tx.TLF01_details.voice_auth_text,HOST_MESSAGE.addl_data_len,CUP_DE48_LEN_SIZE);
   ...
   if ( *result_flag == 0 )
   {
   		if(strlen(HOST_MESSAGE.tran_curr_cd)> 0)
   		{
	   		memcpy( Auth_Tx.TLF01_details.currency_code,
					...
					CUP_DE49_SIZE );
	   		//store the original currency code
...
INT move_field_51( pINT result_flag, pCHAR err_buf )
{
   ...
   if ( *result_flag == 0 )
   {
   		if(strlen(HOST_MESSAGE.card_curr_cd)>0)
   		{
   			...
   			if(0 != strncmp(Auth_Tx.TLF01_details.currency_code, PESO, CUP_DE49_SIZE) &&
			   0 != strncmp(Auth_Tx.TLF01_details.currency_code, USD, CUP_DE49_SIZE))
			{
				strncpy(Auth_Tx.TLF01_details.product_codes[7].code, Auth_Tx.ch_billing_curr_code, CUP_DE51_SIZE);
				strncpy(Auth_Tx.TLF01_details.product_codes[7].amount, Auth_Tx.ch_billing_amt, CUP_DE6_SIZE);
			}
   ...
   if ( *result_flag == 0 )
   {
   		if ( strlen(HOST_MESSAGE.self_def_feild)> 0)
   		{
			...
			if(len <= CUP_MAX_DE60_SIZE)
			{
				...
				// Lets store e-commerc indicate
                if(len >= CUP_DE60_LEN_WITH_TERM_TYPR )
                {
                	if(0 == strncmp(HOST_MESSAGE.self_def_feild + CUP_DE60_TERM_TYPE_POS_IN_DE60,
                	                					CUP_ECOM_TERM_TYPE,
                										CUP_DE60_TERM_TYPR_LEN) ||
                	                			(0 == strncmp(HOST_MESSAGE.self_def_feild + CUP_DE60_TERM_TYPE_POS_IN_DE60,
                								...
                								(0 == strncmp(HOST_MESSAGE.self_def_feild + CUP_DE60_TERM_TYPE_POS_IN_DE60,
                										CUP_UPI3DS_AUTH_MODE_UPI_ATTEMPT,
                												CUP_DE60_TERM_TYPR_LEN)	))
                	{
                		// Its ecom tranasction lets store Terminal type for STIP processing
                		strncpy(Auth_Tx.TLF01_details.product_codes[3].quantity,
                				HOST_MESSAGE.self_def_feild + CUP_DE60_TERM_TYPE_POS_IN_DE60,
								CUP_DE60_TERM_TYPR_LEN);
                		nccup_log_message( 2, 1, "received E-com tranasction", "move_field_60",1 );
                		if(len >= CUP_DE60_LEN_WITH_ECOM_IDENTIFER )
                		{
                			strncpy(Auth_Tx.TLF01_details.profile,
                					HOST_MESSAGE.self_def_feild + CUP_DE60_ECOM_IDENTIFER_POS_IN_DE60,
									CUP_DE60_ECOM_IDENTIFER_LEN);
                		}
                		else
                		{
                			nccup_log_message( 2, 1, "Could not receive Ecom identifier in incoming ECOM tranasction", "move_field_60",1 );;
                		}
                	}
                }
			}
		...
		//Store Fields 60.2 and Field 60.3 data to be show for POS data on TranLookup
		//Currently we will store 60.2 11 byte
	  	strncpy(Auth_Tx.TLF01_details.product_codes[16].amount,HOST_MESSAGE.self_def_feild  + 4 ,11);
   	}
   return( ret_val );
}
...
INT move_field_61( pINT result_flag, pCHAR err_buf )
{
   ...
   if ( *result_flag == 0 )
   {
   		if( strlen(HOST_MESSAGE.card_holder_auth_info)> 0)
		{
			...
			// Lets check if AM usage tag is present or not.
			if(len > CUP_DE61_AM_USAGE_TAG_POS)
			{
				// lets check if AM usage tag is present.
				if(0 == strncmp(HOST_MESSAGE.card_holder_auth_info + CUP_DE61_AM_USAGE_TAG_POS,
								CUP_DE61_AM_USAGE_TAG,
								CUP_DE_61_AM_USAGE_TAG_LEN))
				{
					// We have received AM usgae tag Store bit map for validation
					strncpy(Auth_Tx.TLF01_details.resp_source,
							HOST_MESSAGE.card_holder_auth_info + CUP_DE_61_AM_USAGE_BITMAP_POS,
							CUP_DE_61_AM_USAGE_BITMAP_LEN);
				}
			}

			if(0==strncmp(Auth_Tx.TLF01_details.resp_source[AVN_INDICATOR_POS],'1',1))
			{
				nccup_Compute_AVN_Value(AVN);
				strncpy( Auth_Tx.TLF01_details.product_codes[1].amount,AVN,12);
				strncpy( Auth_Tx.TLF01_details.product_codes[2].amount,AVN+12,12);
				strncpy( Auth_Tx.TLF01_details.product_codes[8].amount,AVN+24,12);
				strncpy( Auth_Tx.TLF01_details.product_codes[0].code,AVN+36,4);
			}

			if(0==strncmp(Auth_Tx.TLF01_details.resp_source[CVN2_INDICATOR_POS],'1',1))
			{
				strncpy( Auth_Tx.TLF01_details.product_codes[5].code,Auth_Tx.future_use_2+CVN2_VALUE_POS,CVN2_VALUE_LEN);
			}
		}
   	}
   return( ret_val );
   ...
   if ( *result_flag == 0 )
   {
		if(strlen(HOST_MESSAGE.net_manag_info_code)>0)
		{
			memcpy( Auth_Tx.function_code,
					HOST_MESSAGE.net_manag_info_code,
					CUP_DE70_SIZE);
		}
   }
   return( ret_val );
 ...
 *  NAME:         move_field_117
 *
 *  DESCRIPTION:  This function will move the Payment Facilitator from the Host
 *                Message structure according to  BDO Cadencie host formatting
 *                specifications.
 *
 *  INPUTS:       Host_Message -  BDO Cadencie host specific structure (Global)
...
INT move_field_117( pINT result_flag, pCHAR err_buf )
{
   ...
   if ( *result_flag == 0 )
   {
		if(strlen(HOST_MESSAGE.cup_payFacl_len)>0)
		{
			// DE 117 LENGTH
			memcpy(Auth_Tx.TLF01_details.product_codes[7].quantity,
					HOST_MESSAGE.cup_payFacl_len,CUP_DE117_LEN);
			// DE117 sub filed Tag
			memcpy(Auth_Tx.TLF01_details.product_codes[7].code,
					HOST_MESSAGE.cup_payFacl, CUP_DE117_SUBFIELD_TAG_LEN);
			// DE117 sub filed Length
			memcpy(byte_len, HOST_MESSAGE.cup_payFacl + CUP_DE117_SUBFIELD_BYTE_LEN_POS,
					CUP_DE117_SUBFIELD_BYTE_LEN);
			len = atoi(byte_len);
			if(len <= CUP_DE117_PAYMENT_FACILTATOR_MAX_LEN)
			{
				//DE117 The Payment Facilitator ID assigned by UPI
				memcpy(Auth_Tx.TLF01_details.product_codes[7].amount,
				HOST_MESSAGE.cup_payFacl + CUP_DE117_PAYMENT_FACILTATOR_POS,len);
			}
			else
			{
				 *result_flag = 1;
				 sprintf( err_buf,
						  "Invalid Payment Facilitator length (field 117) : %d,",len );
			}
		}
...
INT move_field_125( pINT result_flag, pCHAR err_buf )
{
   INT   ret_val = true;
   INT   De125_len     = 0;
   BYTE  temp_len[4]="";
   ...
   if ( *result_flag == 0 )
   {
	   De125_len=atoi(HOST_MESSAGE.ds_transactionID_len);
	   if(De125_len> 0)
	     {
	   		if(0==strncmp(HOST_MESSAGE.ds_transactionID,"TF",2)&& De125_len==43)
	   		{
	   			memcpy( Auth_Tx.TLF01_details.product_codes[12].amount,
	   				   				   HOST_MESSAGE.ds_transactionID,
	   				   					7);
	   			memcpy( Auth_Tx.TLF01_details.product_codes[13].amount,
	   				   HOST_MESSAGE.ds_transactionID+7,
	   					12);
	   			memcpy( Auth_Tx.TLF01_details.product_codes[18].amount,
	   				   		HOST_MESSAGE.ds_transactionID+19,
	   				   		12);
	   			memcpy( Auth_Tx.TLF01_details.product_codes[19].amount,
	   				   	HOST_MESSAGE.ds_transactionID+31,
	   				   	12);

	   			}
	   		else if(0==strncmp(HOST_MESSAGE.ds_transactionID,"IP",2))
	   		{

	   			memcpy(Auth_Tx.TLF01_details.product_codes[15].code,HOST_MESSAGE.ds_transactionID+9,3);
   ...
   if ( *result_flag == 0 )
   {
   		if(strlen(HOST_MESSAGE.mess_auth_code)> 0)
   		{
			memcpy( Auth_Tx.TLF01_details.product_codes[0].amount,
					...
					CUP_DE128_SIZE);
			bin_to_ascii (HOST_MESSAGE.mess_auth_code,MAC,CUP_DE128_SIZE );
		}
   }
   return( ret_val );
}
