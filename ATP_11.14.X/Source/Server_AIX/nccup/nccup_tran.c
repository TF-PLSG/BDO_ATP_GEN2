/******************************************************************************
    ...
    TITLE:  CUP Acquiring Host Handler Driver
 
 	This module is the host handler for the CUP host.  It
...
/*Need to feild with correct cup value*/
CHAR  LOGON_NMI_ECHO_TEST[5] 				= "301";
CHAR  LOGON_NMI_LOGON[5] 					= "001";
CHAR  LOGON_NMI_LOGOFF[5] 					= "002";
CHAR  LOGON_NMI_CUP_START_DATE_SWITCH[5] 	= "201";
CHAR  LOGON_NMI_CUP_END_DATE_SWITCH[5] 		= "202";
...
CHAR  LOGON_RESP_INDICATOR[4] =     "001";
CHAR  LOGOFF_REQUEST[7] =           "LOGOFF";
CHAR vLogoff_NMI_Code[4];
CHAR vLogoff_Resp_Indicator[6];
CHAR  LOGOFF_NMI_START[4] =          "002";
CHAR  LOGOFF_RESP_INDICATOR[4] =     "002";
CHAR  LOGON_ECHO_INDICATOR[4] =     "301";
CHAR  LOGON_NMI_CUP_START_DATE_SWITCH_START_INDICATOR[4] =          "201";
CHAR  LOGON_NMI_CUP_END_DATE_SWITCH_START_INDICATOR[4] =            "202";
CHAR  LOGON_NMI_CUP_KEY_RESET_REQUEST_INDICATOR[4] =            "101";
...
//praneeth    update_txn_stats( &Txn_Counts, (pAUTH_TX)NULL_PTR, RECEIVED );
   ...
   if ( false == reset_echo_timer(err_buf) )
      nccup_log_message(2, 2, err_buf, "process_host_msg",0 );
   ...
   copy_to_req_resp_buffer( p_msg_in );
	...
	{ sprintf(err_buf,"Invalid length or Big msg recvd from Host:  %04d", buflen);
      nccup_log_message( 1, 2, err_buf, "process_host_msg",0 );
	  return;
	}
  ...
  (void)get_originator_info( p_msg_in, Auth_Tx.TLF01_details.originator_queue,
   ...
   else
   {
	  ...
	  TranFlow = determine_txn_flow( Auth_Tx.TLF01_details.message_type, CUP_HOST );
	  ...
	  {
		  strcpy( Auth_Tx.TLF01_details.response_code, CUP_FORMAT_ERROR );
		  ...
		  strcpy( Auth_Tx.TLF01_details.response_text,
				 "Received CUP Reject Msg on the Request" );
	 }
   ...
   if ( ret_val == false )
   {
	  ...
	  {
	  	nccup_log_message( 1, 2, err_buf, "process_host_msg",1 );
	  }
   }
   return;
}
...
///////////////////////////////////////////////////////////////////////////////
    ...
    if(AUTH_INVALID == Auth_Tx.TLF01_details.tx_key)
   {
      /*+-------------------------+
        ...
        +-------------------------+*/
      strcpy( Buffer, "Unsupported transaction from CUP. Message type = ");
   ...
   /*Sign on Request 0820 message type*/
   if(0 == strcmp(Auth_Tx.TLF01_details.message_type, "0820"))
   {
	  strcpy( Auth_Tx.TLF01_details.handler_queue,application_que_name);
   	  if(strncmp(NMI_Code,"001", 3)==0)
   	  {
      	perform_host_logon_request(LOGON_NMI_LOGON,SOURCE_IS_HOST,&Auth_Tx);
		send_network_status_to_monitor ();
   	  }
	  else if (strncmp(NMI_Code,"002", 3)==0)
	  {
		perform_host_logon_request(LOGON_NMI_LOGOFF,SOURCE_IS_HOST,&Auth_Tx);
		send_network_status_to_monitor ();
	  }
	  else if(strncmp(NMI_Code,"301", 3)==0)
	  {
		  perform_host_logon_request(LOGON_NMI_ECHO_TEST,SOURCE_IS_HOST,&Auth_Tx);

	  }
	  else if(strncmp(NMI_Code,"201", 3)==0)
	  {
		  perform_host_logon_request(LOGON_NMI_CUP_START_DATE_SWITCH,SOURCE_IS_HOST,&Auth_Tx);

	  }
	  else if(strncmp(NMI_Code,"202", 3)==0)
	  {
		  perform_host_logon_request(LOGON_NMI_CUP_END_DATE_SWITCH,SOURCE_IS_HOST,&Auth_Tx);
   ...
   else if(0 == strncmp(Auth_Tx.TLF01_details.message_type, "0830",4))
   {  
  	  strcpy( Auth_Tx.TLF01_details.handler_queue,application_que_name);
	...
	/*Key Reset transaction 0800 message type coming from CUP */
   ...
   {  
       strcpy( Auth_Tx.TLF01_details.handler_queue,application_que_name);
   	   if(0 == strncmp(NMI_Code,"101", 3))
   	   {
   		    ...
   		    {
   		    	if(Key_exchange_enable_Flag == 1)
   		    	{
   		    		//process_logon_Reset_Key_exchange_disable_flag();
   		    		Key_exchange_enable_Flag = 0;
   		    		process_logon_Key_Reset_Transaction();
   		    	}
   		    	else
   		    	{
   		    		process_logon_Key_Reset_Reject_Transaction();
   		    	}
   		    }
   		    else
   		    {
   		    	// Get Key_exchange_enable_Flag from DB
   		    	nccup_Key_exchange_enable_Flag_value_from_nccup3_table();
   		    	if(Key_exchange_enable_Flag == 1)
				{
   		    		Key_exchange_enable_Flag = 0;
      		process_logon_Key_Reset_Transaction();
      ...
      (Auth_Tx.TLF01_details.tx_key <= AUTH_INVALID))
   {
      ...
      //strcpy(Auth_Tx.TLF01_details.source_key, Ncf01_I.retired_cwk);
	  if((0 != strncmp(AppName,"nccup3",6)))
		{
		// This is nccup module code
		strcpy(Auth_Tx.TLF01_details.source_key, Ncf01_I.cwk);
		}
		else
		{
		strcpy(Auth_Tx.TLF01_details.source_key,Ncf01_I.cwk);
...
void process_request_for_host_after_security()
{
   ...
   switch ( tx_key )
   {
      ...
      case AUTH_REVERSAL_ADVICE:
         ...
         strcpy( Buff, "Reversal going to CUP" );
         nccup_log_message( 3, 2, Buff, "process_request_for_host_after_security",1 );
	  ...
	  case AUTH_OFFLINE_SALE:
		  ret_val = perform_standin_processing( CUP_APPROVED,
				  	  	  	  	  	  	  	  	GS_TXN_COMPLETED_OFF,
											    err_buf );
	   if ( false == ret_val )
		  nccup_log_message(2,2,err_buf,"process_request_for_host_after_security",1);
      ...
      default:
         ...
         ret_val = perform_standin_processing( CUP_INVALID_TXN,
...
/////////////////////////////////////
   ...
   if ( false == ret_val )
   {
      nccup_log_message( 1, 2, err_buf, "process_request_for_host_after_currency",1 );
	  /*TF Praneeth revert back tx_key  while stip transaction */
	  if ((Auth_Tx.TLF01_details.voice_auth_text[0] == AUTH_AUTHORIZATION_OVERRIDE) || (Auth_Tx.TLF01_details.voice_auth_text[0] == AUTH_CASH_ADVANCE_AUTHORIZATION_OVERRIDE))
	  {
		  Auth_Tx.TLF01_details.tx_key = Auth_Tx.TLF01_details.voice_auth_text[0];
      ...
      ret_val = perform_standin_processing( "01",
                                            ...
                                            err_buf );
      if ( false == ret_val )
         nccup_log_message(2,2,err_buf,"process_request_for_host_after_currency",1);
   }
   else
   {
...
void process_response_from_host()
{
   ...
   if ( false == decrement_active_count( err_buf ) )
      nccup_log_message(1, 2, err_buf, "process_response_from_host",1 );
   ...
   if(Auth_Tx.TLF01_details.tx_key != AUTH_INVALID)
   {
      ...
      else
      {
         ...
         if ( false == clear_request_timer(lookup_key) )
         {
            ...
            nccup_log_message(2, 2, err_buf, "process_response_from_host",1 );
         }
      }
   }
   return;
      ...
      if( ptemsg_get_result_code( p_msg_in ) == PTEMSG_NOT_FOUND )
      {
         ...
         sprintf( err_buf,
                 "Unable to retrieve timer data for lookup key %s",
                  lookup_key );
         nccup_log_message( 1, 3, err_buf, "process_clear_timer_reply",0 );
      }
      ...
      else if ( timer_reply_size != Auth_Tx_Size )
      {
         sprintf( err_buf,
                 "Did not receive auth_tx data with timer reply. Key = %s",
                  lookup_key );
         nccup_log_message( 2, 2, err_buf, "process_clear_timer_reply",0 );
      }
      else
      {
         ...
         else
         {
            ...
            else if ( false == send_msg_to_authorizer(MT_AUTH_RESPONSE) )
            {
               ...
               nccup_log_message( 1, 3, err_buf, "process_clear_timer_reply",1 );
            }
         }
      }
   }
...
void copy_resp_into_request( pAUTH_TX host_auth_tx )
{
   ...
   /* if response don't have authid, request auth id will be sent, don't alter it.*/
   /*else
   {
   ...
   if ( 0 != strcmp(Auth_Tx.TLF01_details.response_code,CUP_ISO_APPROVED) )
   {
      strcpy( Auth_Tx.TLF01_details.response_text,
...
/*
	...
	if ( false == ret_val )
   {
    	nccup_log_message( 1, 2, err_msg, "process_1304_from_host",0 );
   }*/
...
INT delete_orig_ncf30( NCF30 ncf30, pAUTH_TX p_auth_tx )
{
   ...
   if ( NULL_PTR == p_msg  )
   {
	   nccup_log_message( 1, 3, "Insufficient Memory to build NCF30 message",
	       	        				"delete_orig_ncf30",1 );
      return( false );
   }
   ...
   if ( 0 > ret_code )
   {
      ...
      nccup_log_message( 1, 3, Buff,"delete_orig_ncf30",1 );
      return( false );
   }
   return(true);
 }
 ...
 BOOLEAN process_logon_response(CHAR nmi_code[4],pAUTH_TX p_auth_tx)
 {
	...
	if (strcmp(LOGON_NMI_LOGON ,p_auth_tx->function_code) == 0)
	{
	   if ( strcmp( p_auth_tx->TLF01_details.response_code, "00" ) )
	   {
		  ...
		  nccup_log_message( 1, 1, "LOGON Request successful.",
				  	  	  	  	  "process_logon_response",0 );
	   }
	   else
	   {
		   nccup_log_message( 1, 1, "LOGON Request DENIED.",
		   				  	  	  	 "process_logon_response",0 );
	   }
	}
	...
	else if (strcmp(LOGON_NMI_LOGOFF,p_auth_tx->function_code) == 0)
	{
	   if ( strcmp( p_auth_tx->TLF01_details.response_code, "00" ) )
	   {
		  ...
		  update_host_status = true;
		  nccup_log_message( 1, 1, "LOGOFF Request successful.",
		  		   				  	"process_logon_response",0 );
	   }
	   else
	   {
		   nccup_log_message( 1, 1, "LOGOFF Request DENIED.",
		   		  		   			"process_logon_response",0 );
	   }
	}
	...
	else if (strcmp(LOGON_NMI_ECHO_TEST,p_auth_tx->function_code) == 0)
	{
	   if ( strcmp( p_auth_tx->TLF01_details.response_code, "00" ) )
	   {
		  strcpy(current_state, "1");
		  update_host_status = true;
		  nccup_log_message( 1, 1, "ECHO Request successful.",
		  		   		  		   	"process_logon_response",0 );
	   }
	   else
	   {
		   nccup_log_message( 1, 1, "ECHO Request DENIED.",
		   		  		   		  	  "process_logon_response",0 );
	   }
	}
	...
	{
		nccup_log_message( 1, 1, "Insufficient Memory to build insert TLF01 message.",
				   		  		   "process_logon_response",0 );
		return(false);
	}
	else
 ...
 ******************************************************************************/
 INT process_incoming_message(pPTE_MSG p_msg)
 {
		  ...
		  (Auth_Tx.TLF01_details.tx_key > AUTH_INVALID))  /* a response */
	   {
		  ...
		  if(Auth_Tx.TLF01_details.tx_key > 100 )
		  {
			  if ( ((strlen(Auth_Tx.reconciliation_cur_code) > 0 ) &&
					(isnum(Auth_Tx.reconciliation_cur_code))))
			  {
				if(strlen(Auth_Tx.TLF01_details.orig_amount))
				{
					strncpy(Auth_Tx.TLF01_details.product_codes[9].amount,Auth_Tx.TLF01_details.total_amount,CUP_DE4_SIZE);
					memcpy(Auth_Tx.TLF01_details.total_amount,Auth_Tx.TLF01_details.orig_amount,
							CUP_DE4_SIZE);
				}
				else
				{
					memcpy( Auth_Tx.TLF01_details.total_amount,Auth_Tx.TLF01_details.total_amount,
							CUP_DE4_SIZE );
				}

				if( 0 == strncmp(Auth_Tx.settlement_curr_code, PESO,3) ||
					0 == strncmp(Auth_Tx.ch_billing_curr_code, PESO,3))
				{
					strcpy( Auth_Tx.TLF01_details.host_start_time, "5.000");
				}
				else if( 0 == strncmp(Auth_Tx.settlement_curr_code, USD,3) ||
						 0 == strncmp(Auth_Tx.ch_billing_curr_code, USD,3))
				{
		  ...
		  if((0 == strcmp(Auth_Tx.TLF01_details.response_code, "00")) || (0 == strcmp(Auth_Tx.TLF01_details.response_code, "55")))
		  {
			  ...
			  {
				  nccup_log_message( 1, 3, "Unable to update database tables.",
						  	  	  	  "process_incoming_message",1 );
				 /* Unable to complete a transaction.  Inform host; log to TLF01. */
				 if(false == incoming_other_host_response_db_reply( ) )
				 {
					 nccup_log_message( 1, 3, "Unable to send response to host.",
					 						  "process_incoming_message",1 );
				 }
			  }
	...
	{
	/*TF Phani- we need some specific changes to CUP
		- NCF01 -> CWK is contains TPK values , Whcihc is source key . User will provide a value.

		- NCF01 -> Retired CWK contains destination key, whchic is CUP key.
			whenever nccup recvied KEY from CUP, it will be stored in retd CWk.

		- NCf01 -> KEK, this contains MAC value from CUP

		Note : each day when key request by nccup, Retd CWk and KEK will be updated.
	*/
		strcpy(Auth_Tx.TLF01_details.dest_key, ncf01.retired_cwk);
		strcpy(Auth_Tx.TLF01_details.dest_pin_format, "1");
		strcpy(Auth_Tx.TLF01_details.source_key, ncf01.cwk);
		strcpy(Auth_Tx.TLF01_details.source_pin_format, "1");
	   /* PIN Translation is required */
	   if(false == hhutil_process_debit_transaction(&Auth_Tx))
	   {
		   nccup_log_message( 1, 3, "Failed to process debit transaction.",
		  					 		 "process_incoming_message",1 );
		   perform_error_processing( &Auth_Tx );
		   strcpy(Auth_Tx.TLF01_details.response_code,"91");
		   Auth_Tx.TLF01_details.tx_key =
 ...
 ******************************************************************************/
 BOOLEAN send_it_back_to_terminal(BYTE resp_code[3], pAUTH_TX p_auth_tx )
 {
	...
	if ( NULL_PTR == p_msg_term_handler_out )
	 {
		nccup_log_message( 1, 3, "Insufficient Memory to build response to txcntl.",
				  				  "send_it_back_to_terminal",1 );
		return(false);
	 }
	...
	if ( ret_code < 0 )
	{
		 ...
		 nccup_log_message( 2, 1, Buffer,"send_it_back_to_terminal",1 );
		 free(p_msg_term_handler_out);
		 return(false);
	}
	else
	{
		nccup_log_message( 2,1, "Response sent successfully to terminal handler\n",
							"send_it_back_to_terminal",1 );
	}
 	free(p_msg_term_handler_out);
	return(true);
 ...
 ******************************************************************************/
 BOOLEAN hhutil_process_debit_transaction(pAUTH_TX p_auth_tx)
 {
	...
	if ( NULL_PTR == p_msg )
	 {
		nccup_log_message( 1,3, "Insufficient Memory to build pin translation message",
								"hhutil_process_debit_transaction",1 );
	    return(false);
	 }
	 ...
	 if( ret_code < 0 )
	 {
		...
		nccup_log_message( 1,3,Buffer,"hhutil_process_debit_transaction",1 );
		free( p_msg );
		return(false);
	 }
 ...
 INT incoming_terminal_request(pAUTH_TX p_auth_tx)
 {
	...
	if (false == verify_host_app_status(p_auth_tx))
	{
	   /* build response_for_terminal */
		nccup_log_message( 1,2,"Host Inactive","incoming_terminal_request",1 );
		p_auth_tx->TLF01_details.tx_key = match_request_to_response(p_auth_tx->TLF01_details.tx_key);
 
		if (0 == strcmp(current_state,"2"))  /* check if we are error state */
	   {		 
			nccup_log_message( 1,2,"Host Inactive: current_state!=0",
									"incoming_terminal_request",1 );
	...
	switch (source)
	 {
		 case SOURCE_IS_FSVISA:
		 {
			 ...
			 {
				 nccup_log_message( 1,1,"Sending logon request to host:",
				 						"perform_host_logon_request",0 );
		 ...
		 case SOURCE_IS_STATE:
		 {
			 ...
			 {
				 nccup_log_message( 1,1,"Sending logon ECHO request to host:",
				 				 		"perform_host_logon_request",0 );
				 p_auth_tx->TLF01_details.tx_key = AUTH_LOGON;
	   ...
	   case SOURCE_IS_OPERATOR:
	   {
		 ...
		 case SOURCE_IS_HOST:
		 {
			 ...
			 if (strcmp(LOGON_NMI_START , request_type)==0)
			 {
				 ...
				 {
					 nccup_log_message( 1,1,"Received logon request from host:",
					 				 		"perform_host_logon_request",0 );
					 p_auth_tx->TLF01_details.tx_key = AUTH_LOGON_RESPONSE;
					 strcpy(vLogon_NMI_Code, LOGON_NMI_START);
					 // We will make CUP link Online only after CUP Key exchnge successfully
				 ...
				 {
					 nccup_log_message( 1,1,"Received logon request from host,Already Online:",
										 	"perform_host_logon_request",0 );
					 p_auth_tx->TLF01_details.tx_key = AUTH_LOGON_RESPONSE;
					 strcpy(vLogon_NMI_Code, LOGON_NMI_START);
					 perform_op_net_mngmnt_response(request_type, p_auth_tx);
			 ...
			 else if (strcmp(LOGOFF_NMI_START , request_type)==0)
			 {
				 nccup_log_message( 1,1,"Received logoff request from host:",
				 						"perform_host_logon_request",0 );
				 p_auth_tx->TLF01_details.tx_key = AUTH_LOGON_RESPONSE;
				 strcpy(vLogoff_NMI_Code, LOGOFF_NMI_START);
				 ...
				 if((0 == strncmp(AppName,"nccup3",6)))
				 {
					 nccup_send_logoff_message_to_nccup();
				 }
				 else
				 {
					 nccup_send_logoff_message_to_nccup3();
				 }
			 }
   		     else if (strcmp(LOGON_NMI_CUP_START_DATE_SWITCH , request_type)==0)
			 {
   		    	nccup_log_message( 1,1,"Received UnionPay starts date switch request from host:",
   		    					 		"perform_host_logon_request",0 );
				p_auth_tx->TLF01_details.tx_key = AUTH_LOGON_RESPONSE;
				strcpy(vLogon_NMI_Code, LOGON_NMI_CUP_START_DATE_SWITCH_START_INDICATOR);
				perform_op_net_mngmnt_response(request_type, p_auth_tx);
			 }

			 else if (strcmp(LOGON_NMI_CUP_END_DATE_SWITCH , request_type)==0)
			 {
				 nccup_log_message( 1,1,"Received UnionPay ends date switch request from host:",
				    		    		"perform_host_logon_request",0 );
				 p_auth_tx->TLF01_details.tx_key = AUTH_LOGON_RESPONSE;
				 strcpy(vLogon_NMI_Code, LOGON_NMI_CUP_END_DATE_SWITCH_START_INDICATOR);
				 perform_op_net_mngmnt_response(request_type, p_auth_tx);
			 }
			 ...
			 else
			 {
				 if (0 == atoi(current_state))
				 {
					 //nccup_log_message( 1,1,"Received network request from host:",
					 //				    	 "perform_host_logon_request",0 );
					 p_auth_tx->TLF01_details.tx_key = AUTH_LOGON_RESPONSE;
					 strcpy(vLogon_NMI_Code, LOGON_NMI_ECHO_TEST);
					 strcpy(vLogon_Resp_Indicator, LOGON_NMI_ECHO_TEST);
					 perform_op_net_mngmnt_response(request_type, p_auth_tx);
				 }
				 else
					 return(false);
 ...
 ******************************************************************************/
 INT  verify_host_app_status(pAUTH_TX p_auth_tx)
 {
	...
	{
		nccup_log_message( 1,2,"Host app not in state 0, cannot process tran:",
							 	 "verify_host_app_status",0 );
		return(false);
	}
 
	if (2 == atoi(current_state))  
	{  
		nccup_log_message( 1,2,"Maximum consecutive timeouts reached:",
									"verify_host_app_status",1 );
	   	strcpy(current_state,"2");
		return(false);
	}
	if (atoi(active_txns_ctr) >= atoi(ncf01.max_active_txns))
	{
		nccup_log_message( 1,2,"Maximum active transactions reached:",
								"verify_host_app_status",1 );
	    return(false);
	}
 ...
 INT perform_op_net_mngmnt_request( char request_type[], pAUTH_TX p_auth_tx )
 {
	...
	strncpy( cup_response_code, "000", 3);
	...
	{
		nccup_log_message( 1,1,"Sending LOGON Request to host.",
								"perform_op_net_mngmnt_request",0 );
	   strcpy(vLogon_NMI_Code, LOGON_NMI_LOGON);
	}
	else if (0 == strcmp(request_type, LOGOFF_REQUEST))
	{
		nccup_log_message( 1,1,"Sending LOGOFF Request to host.",
								"perform_op_net_mngmnt_request",0 );
	...
	if (false == hhutil_send_net_mngmnt_request(&auth_tx, cup_response_code))
	   return(false);
	else
	   return(true);
 ...
 ******************************************************************************/
 int perform_op_net_mngmnt_response(char nmi_code[], pAUTH_TX p_auth_tx)
 {
	...
	p_auth_tx->TLF01_details.tx_key = AUTH_LOGON_RESPONSE;
	if(0 == strncmp(p_auth_tx->TLF01_details.message_type,"0800",4))
	{
		strncpy(p_auth_tx->TLF01_details.message_type,"0810",4);
	}
	else if ( 0 == strncmp (p_auth_tx->TLF01_details.message_type,"0820",4))
	{
		strncpy(p_auth_tx->TLF01_details.message_type,"0830",4);
	}
	...
	{
		nccup_log_message( 1,1,"Sending LOGON Response to host.",
								"perform_op_net_mngmnt_response",0 );
	   strcpy(vLogon_Resp_Indicator, LOGON_RESP_INDICATOR);  
	}
	else if (0 == strcmp(nmi_code, LOGON_NMI_LOGOFF))
	{
		nccup_log_message( 1,1,"Sending LOGOFF Response to host.",
								"perform_op_net_mngmnt_response",0 );
	}
	else if (0 == strcmp(nmi_code,LOGON_NMI_KEY_RESET_REQUEST))
	{
		nccup_log_message( 1,1,"Sending Key Reset Response to host.",
								"perform_op_net_mngmnt_response",0 );
	}
	else 
	{
		nccup_log_message( 1,1,"Sending ECHO Response to host.",
								"perform_op_net_mngmnt_response",0 );
	}
 ...
 /******************************************************************************
  *
  *  NAME:		   process_logon_Key_Reset_Transaction
  *
  *  DESCRIPTION:  This function store the PIK coming from CUP
  *
  *  INPUTS:	   None
  *
  *  OUTPUTS:	   None
  ...
  *
  ******************************************************************************/
  void  process_logon_Key_Reset_Transaction()
 {
 	char temp_len[4] = {0};
	...
	int  len = 0;
 	...
 	if (strlen(Auth_Tx.TLF01_details.voice_auth_text) > 0)
 	{
		...
		len = atoi(temp_len);
		memcpy(Feild_ident,Auth_Tx.TLF01_details.voice_auth_text+ CUP_DE48_LEN_SIZE, 2);
		memcpy(temp_data,Auth_Tx.TLF01_details.voice_auth_text+ CUP_DE48_LEN_SIZE + 2, len -2);
		
		bin_to_ascii (temp_data, buffer, strlen(temp_data));
		if(Feild_ident[0]=='N'&& Feild_ident[1]=='K')
		{ 
			// We have got new key , we will store this in rtdcwk feild of ncf01 tables
			nccup_log_message( 1,1,"Received  New Key from CUP.",
									"process_logon_Key_Reset_Transaction",0 );
			if( 0 == strncmp(AppName,"nccup3",6))
			{
				// NCCUP3 issuing module
				memcpy(Ncf01_I.retired_cwk,buffer,strlen(buffer));
				memcpy(static_Rtdcwk_Key,buffer,strlen(buffer));
				memcpy(Ncf01_I.kek,MAC,strlen(MAC));
				Ncf01_I.prev_src_station[0] = '0'; // Lets Desiable Key exchange flag
				Auth_Tx.TLF01_details.tx_key=AUTH_PIN_CHANGE_RESPONSE;
				memset(Auth_Tx.TLF01_details.product_codes[16].code,0x00,6);
				strncpy(Auth_Tx.TLF01_details.product_codes[16].code,"ISSLNK",6);
				update_ncf01(&Auth_Tx);
				sprintf(key_msg,"Key received from CUP is %s",Ncf01_I.retired_cwk);
				nccup_log_message( 1,1,key_msg,"process_logon_Key_Reset_Transaction",0 );
			}
			else
			{
				/* NCCUP Acquring module
				 nccup3 issuing link send key exchange request and CUP responded key exchange to nccup (Acquring module),
				 *  nccup module send recevied CUP keys nccup3 (issuing) module form CUP HOST */
				memcpy(Ncf01_Nccup3_I.retired_cwk,buffer,strlen(buffer));
				memcpy(static_Rtdcwk_Key,buffer,strlen(buffer));
				memcpy(Ncf01_Nccup3_I.kek,MAC,strlen(MAC));
				sprintf(key_msg,"Key received from CUP is %s",Ncf01_Nccup3_I.retired_cwk);
				nccup_log_message( 1,1,key_msg,"process_logon_Key_Reset_Transaction",0 );
				memset(Auth_Tx.TLF01_details.product_codes[16].code,0x00,6);
				// Send message to nccup3 to update its key as CUP had send a new key to nccup Acq module
				strncpy(Auth_Tx.TLF01_details.product_codes[16].code,"ACQLNK",6);
				nccup_send_cup_key_to_issuing_module( &Auth_Tx );
			}
		}
 	}
 }
 ...
 ******************************************************************************/
 INT process_incoming_message_ncf30_reply(pBYTE pCurrent)
 {
	...
	//Check if field 49 is PESO or Doller
	if((0 != strcmp(Auth_Tx.TLF01_details.currency_code,PESO))&&
		(0 != strcmp(Auth_Tx.TLF01_details.currency_code,USD)))
	{
	    ...
	    strcpy(Auth_Tx.TLF01_details.reversal_amount, ncf30.network_data);
	}
