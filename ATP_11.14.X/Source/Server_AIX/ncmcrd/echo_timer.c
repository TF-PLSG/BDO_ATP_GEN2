...
BYTE set_echo_timer(char* echo_key,WORD timer_length, pPTE_MSG *p_msg_set_timer,char* reply_q, BYTE flag )
{
		...
		pPTE_MSG temp_msg=NULL_PTR;
		 ...
		 {
            ncmcrd_log_message( 3, 2, "Insufficient Memory to build start timer message", "set_echo_timer", 0 ,"");
            return(false);
	     }
         ...
         else if(flag ==1)
         {
             ...
             if( ret_code < 0 )
		     {
                ...
                ncmcrd_log_message( 3, 3, Buffer, "set_echo_timer", 0 ,"");
...
BYTE clear_echo_timer(char* echo_key,pPTE_MSG *pte_clr_timer,char* reply_q,BYTE flag)
{
   ...
   if ( NULL_PTR == temp_msg )
   { 
      ncmcrd_log_message( 3, 2, "Insufficient Memory to build clear timer message", "clear_echo_timer", 0 ,"");
      return(false);
   }
     if( flag == 0)
     {
   ...
   else if(flag ==1)
   {
       ...
       {   /* Failed clearing timer data */
           ...
           sprintf( Buffer, "pteipc error sending to que timerdsA: %s",
                     ErrorMsg );
           ncmcrd_log_message( 3, 3,Buffer, "clear_echo_timer", 0 ,"");
           return(false);
       }
       return true;
   }
        ...
        else
         {
             ...
             else
             {
                ...
                if( ret_code < 0 )
                {
                   ...
                   ncmcrd_log_message( 3, 3,Buffer, "chain_clear_set_echo_timer", 0 ,"");
                   return(false);
                }
                else return true;
             }
...
BYTE chain_echo_host(char* echo_key, WORD timer_length,char* reply_q,pPTE_MSG pte_msg)
{
    ...
    if( timer_length == 0)
    {
       ...
       {  /* failed getting timer data */
          ...
          sprintf( Buffer, "pteipc error sending to que timerdsA: %s",
                    ErrorMsg );
          ncmcrd_log_message( 3, 3,Buffer, "chain_echo_host", 0 ,"");
          return(false);
       }  
       else return true;
    }
