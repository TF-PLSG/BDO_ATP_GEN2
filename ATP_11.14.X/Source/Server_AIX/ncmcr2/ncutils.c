                    ...
                    WORD     timer_length, pCHAR error_msg )
{
	...
	{
		trim_spaces_Only( p_auth_tx->TLF01_details.terminal_id );
	...
	{
		if(0 == strcmp(p_auth_tx->TLF01_details.handler_queue,"dcpisoA") ||
...
INT send_response_to_sbatch(pAUTH_TX p_auth_tx,BYTE msg_type,
        pCHAR    source_que, pCHAR error_msg)
{
	   pPTE_MSG   p_msg   = NULL_PTR;
	   INT        ret_val = NOT_OK;
   ...
   if ( OK == ret_val )
   {
      ...
      if ( (tempint >= MaxActiveTxns) && (MaxActiveTxns > 0) )
      {
         ...
         strcpy( errbuf,
		  ...
		  ncmcr2_log_message( 1, 1, errbuf , "increment_active_count" , 0 ,"");
      }
      else
      {
...
INT increment_timeout_count( pCHAR AppName, pCHAR errbuf, pAUTH_TX p_auth_tx )
{
   ...
   if ( OK == ret_val )
   {
      ...
      memset(errbuf,0x00,sizeof(errbuf)-1);
      sprintf(errbuf, "consecutive timeout count is %d",tempint);
      ncmcr2_log_message( 2, 2, errbuf , "increment_timeout_count" , 0 ,"");
      ...
      if ( MaxConsecTimeouts > 0 )
      {
         if ( tempint > MaxConsecTimeouts )
         {
            ...
            mask_card_number(masked_cardnbr, p_auth_tx->TLF01_details.card_num, strlen(p_auth_tx->TLF01_details.card_num));
            if(strlen(p_auth_tx->TLF01_details.orig_message) > 0)
            	memcpy(requestMti, p_auth_tx->TLF01_details.orig_message, 4);
            else
            	memcpy(requestMti, p_auth_tx->TLF01_details.message_type, 4);
            sprintf(errbuf,
            ...
            if(tempint == (MaxConsecTimeouts+1))
            	ncmcr2_log_message( 3, 2, errbuf , "increment_timeout_count" , 0 ,"");
			#else
			strcpy( errbuf,
                "Maximum consecutive timeouts reached.");
            ncmcr2_log_message( 1, 1, errbuf , "increment_timeout_count" , 0 ,"");
            #endif
         }
      }
...
INT update_host_totals( pAUTH_TX p_auth_tx, BYTE subtype_2, pCHAR network_type )
{
   ...
   if ( NULL_PTR == p_msg  )
   {
      ncmcr2_log_message( 2, 2,  "Insufficient Memory to build data server message" , "update_host_totals" , 0 ,"");
   }
   else
   {
      ...
      if ( OK != ret_code )
      {
         ...
         ncmcr2_log_message( 2, 3,  err_msg , "update_host_totals" , 0 ,p_auth_tx->TLF01_details.card_num);
      }
   }
   return( ret_code );
}
...
BOOLEAN ncmcr2_check_for_acq_refund_transaction(pAUTH_TX p_auth_tx)
{
...
BOOLEAN ncmcr2_check_for_acq_reversal_refund_transaction(pAUTH_TX p_auth_tx)
{
