 ...
 * All Rights Reserved.
 *
 * MODULE:      NCAMX_STATS.C
 *  
 * TITLE:       Amex Network Controller Statistics Processor
 *  
...
INT send_transaction_statistics( pPTE_MSG p_msg )
{
   ...
   if ( false == ret_code  )
   {
      natma2_log_message( 1, 2, err_msg, "send_transaction_statistics",1 );
   }
   return( ret_code );
}
...
INT reset_transaction_statistics( pPTE_MSG p_msg )
{
   ...
   if ( false == ret_code  )
   {
      natma2_log_message( 1, 2, err_msg, "reset_transaction_statistics",0 );
   }
   return( ret_code );
}
