...
* All Rights Reserved.
*
* MODULE:      NCAMX_VALIDATION.C
*  
* TITLE:       Amex Network Control Data Validation Functions
*  
...
* $Log:   N:\PVCS\PTE\Equitable\ncjcb\ncamx_validation.c  $
      ...
      Rev 1.3   Jan 26 2004 11:38:52   dirby
   Removed a check from yymm date validation to not discount
   years above 60.  EB still has cards they use that expire in 1999.
   
      Rev 1.2   Nov 21 2003 13:18:18   dirby
   Modifications to support Online Updates (1304/1314).
   SCR 1200
   
      Rev 1.1   Aug 27 2003 09:05:20   svelaga
 ...
 *  NAME:         validate_date_yymm
 *
 *  DESCRIPTION:  This function validates that a date in the format of MMYY
 *                has a valid year (numeric and greater than 0) and a valid
 *                month (numeric and ranging from 01 to 12).
 *
 *  INPUTS:       date - Date to be validated
...
INT validate_date_mmyy( pBYTE date, pCHAR err_buf )
{
   ...
   memcpy( temp_month,  date, 2 );
   memcpy( temp_year, date+2, 2 );
...
INT validate_date_time( pBYTE date_time )
{
   ...
   ret_val = validate_date_yyyymmdd( date,err_buf );
   if ( ret_val == true )
   {
      memcpy( time, date_time+8, 6 );
...
INT validate_host_msg()
{
   ...
   if ( ret_val == false )
      natma2_log_message( 1, 2, err_buf, "validate_host_msg",1 );
