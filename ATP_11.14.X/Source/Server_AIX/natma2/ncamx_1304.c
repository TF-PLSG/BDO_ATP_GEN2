 ...
 * $Log:   N:\PVCS\PTE\Equitable\ncjcb\ncamx_1304.c  $
      ...
      Rev 1.1   Dec 02 2003 15:27:18   dirby
   Corrected bugs resulting from unit testing.
   SCR 1200
   
      Rev 1.0   Nov 21 2003 13:27:10   dirby
   Initial revision.
 *
 ******************************************************************************/
...
INT process_file_update_request( pPTE_MSG p_msg )
{
   ...
   if((0== strncmp(Auth_Tx.TLF01_details.handler_queue,"ncposA",6)) || 
		 (0== strncmp(Auth_Tx.TLF01_details.handler_queue,"nposa2A",5)))
   {
...
INT send_response_to_GUI()
{
   ...
   if ( false == ret_val )
   {
	   natma2_log_message(1,3, err_msg, "send_response_to_GUI", 1);
   }
   return( ret_val );
