...
void 	process_1304_from_host();			/* ThoughtFocus Girija Y */
INT  	nposa2_insert_tlf01(pAUTH_TX p_auth_tx);
INT 	get_variable_auth_config_details();

void 	natma2_send_reversal_to_host_for_late_response( pCHAR );
...
void 	natma2_log_message( INT dest, INT severity, pCHAR mesg, pCHAR func_name, int details );
...
/*INT  	get_bdkkey01_with_wait( pBDKKEY01 pbdk01, pCHAR Buffer);*/
...
void 	process_decrypt_translate_pin_response( pPTE_MSG p_msg_in );
void 	process_decrypt_pin_response( pPTE_MSG p_msg_in );

void 	translate_pin( void );
void    decrypt_pin(void);
...
INT 	validate_date_mmyy( pBYTE date, pCHAR err_buf );
