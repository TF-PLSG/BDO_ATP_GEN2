...
CHAR cadencie_atm_que_name[] = "natma2A";  // ThoughtFocus Girija Y
CHAR pos_amex_que_name[]    = "ncposA";   // ThoughtFocus Girija Y
...
//ramya
CHAR  natma2_Erro_warning_Filename[256] = {0};
CHAR  natma2_module_error_warning_file_name[256] = {0};
CHAR  natma2_error_warning_file_path[256] = {0};
...
void ascendent_msg_handler( pPTE_MSG p_msg_in )
{
   ...
   switch( msgtype )
   {
      ...
      case MT_NSP_DECRYPT_TRANSLATE_PIN_RESPONSE:
          ...
          /* Response from the network security processor */
    	  process_decrypt_translate_pin_response( p_msg_in );
    	 break;
      ...
      case MT_SYSTEM_REQUEST:
               ...
               if ( false == flush_file(ppTraceFile_fp) )
               {
                  ...
                  sprintf( buffer,
                    "%s: Unable to flush trace file buffer. Turning Trace off.",
                     AppName );
                  natma2_log_message( 1, 1, buffer, "ascendent_msg_handler",0 );
         ...
         {
        	 natma2_read_TO_config_details_from_tf_ini();
        	 send_trace_response(p_msg_in);
        	 sprintf( buffer,
        	          "rvrsl_attmpts_value_fr_timeout_amex: %d, rvrsl_attmpts_value_fr_laterspns_amex %d",
					  rvrsl_attmpts_value_fr_timeout_amex, rvrsl_attmpts_value_fr_laterspns_amex );
        	 natma2_log_message( 1, 1, buffer, "ascendent_msg_handler" ,0);
         }
      break;
      ...
      default :
         ...
         natma2_log_message( 1, 2, buffer, "ascendent_msg_handler",0 );
      break;
   }  /* switch msg_type */
...
void MainProcessor()
{
   ...
   sprintf( buffer,
           "Starting the Network Controller Service: %s, version %s",
            ServiceName, Version );
   natma2_log_message( 2, 1,buffer, "MainProcessor", 0);
   PRINT2( "%s\n", buffer );
   natma2_get_error_warning_file_name_path();
...
#endif
   {
      natma2_log_message( 1, 3, "Failed to create XIPC queues.", "MainProcessor",0 );
      ...
      else
      {
         ...
         while( !EndProcessSignalled )
         {
            ...
            else if ( QUE_ER_TIMEOUT != ret_code ) 
            {
               ...
               natma2_log_message( 1, 3, buffer, "MainProcessor",0 );
            }
         }
      }
...
void EndProcess()
{
   ...
   if ( Tracing == TRACE_ON )
   {
      if ( false == close_trace_file( ppTraceFile_fp ))
      {
         ...
         natma2_log_message( 1, 1, Buffer, "startup" ,0);
      }
   }

   sprintf( Buffer, "Shutting down the %s Service, version %s",
            ServiceName, Version );
   natma2_log_message( 2, 1,Buffer, "EndProcess", 0);
   strcat( Buffer, "\n" );
   PRINT( Buffer );
}
...
INT startup()
{
         ...
         if (false == open_trace_file(AppName,DirPath,ppTraceFile_fp,TraceFile))
         {
            ...
            sprintf( err_buf,
                    "Unable to open trace file %s. Tracing is off",
                     TraceFile );
            natma2_log_message( 1, 1, err_buf, "startup",0 );
         }
         else
            natma2_log_message( 1, 1, "Tracing is turned ON", "startup" ,0);
      }
      else
      {
         Tracing = TRACE_OFF;
         ...
         natma2_log_message( 1, 1, "Tracing is turned off", "startup" ,0);
      }
   ...
   else
   {
      ...
      natma2_log_message( 1, 3, Buffer, "Startup" ,0);
   }
