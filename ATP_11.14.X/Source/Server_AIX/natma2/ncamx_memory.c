 ...
 * All Rights Reserved.
 *
 * MODULE:      NCAMX_MEMORY.C
 *  
 * TITLE:       Amex Network Controller Shared Memory Processor
 *  
 ...
 * $Log:   N:\PVCS\PTE\Equitable\ncjcb\ncamx_memory.c  $
      ...
      Rev 1.1   Aug 27 2003 09:05:12   svelaga
         ...
         if ( true == ret_val )
         {
            ...
            if ( 0 == strcmp(host_state, ONLINE) )
            {
               ...
               if ( false == set_echo_timer(err_buf) )
                  natma2_log_message( 1, 2, err_buf, "set_host_state" ,0);
            }
...
INT increment_active_count( pCHAR errbuf )
{
   ...
   if ( true == ret_val )
   {
      ...
      if ( (tempint >= MaxActiveTxns) && (MaxActiveTxns > 0) )
      {
         ...
         natma2_log_message( 1, 2, errbuf, "increment_active_count",0 );
      }
      else
      {
...
INT increment_timeout_count( pCHAR err_buf )
{
   ...
   if ( true == ret_val )
   {
	  ...
	  //multiple prompts on UI are displaying due to shared memory update delay.
...
int GetTimeoutCount()
{
	...
	ret_val = read_shared_mem_table( time_out_cnt, active_cnt,
	                                    host_state, err_buf );
	if(ret_val == true)
	{
