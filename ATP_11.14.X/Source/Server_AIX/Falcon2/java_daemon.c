...

{
	...
	(0 == strncmp(argv[1],"falcon",6)))
	{
		...
		if (PID > 0)
		{
			execl(java_buff, "java","-Xms256m","-Xmx512m", "-Dcom.ibm.CORBA.Debug.Output=/dev/null", javaAppName, javaServiceName,  "-jar" ,buffer,0);
		}
	}
	else
	{
...

{
   ...
   char javaAppName[125] = {0};
   char javaServiceName[125] = {0};
	...
	strcat(buffer,"/bin/falcon.jar");
   sprintf(javaAppName, "-Dservice.app.name=falcon");
   sprintf(javaServiceName, "-Dservice.service.name=%s", argv[1]);
	execl(java_buff, "java","-Xms256m","-Xmx512m", "-Dcom.ibm.CORBA.Debug.Output=/dev/null", javaAppName, javaServiceName, "-jar",
      buffer, 0);
