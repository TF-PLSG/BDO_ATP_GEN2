...
public class XcomManager {
	...
	public XcomManager(@Autowired Application app) {
		...
		jnaManager = JnaManager.getInstance(State.CURRENT.getAppName());
...
//		jnaManager.getLibFileVersion();
	...
	public synchronized String readFromXcom() {
		log.trace("Trying to read from Xcom");
		String message = jnaManager.sendRequestMessage(State.CURRENT.getServiceName());
		log.trace("Received from Xcom: {}", message.replaceAll("(\\r|\\n)", ""));
		return message;
	}
	...
	public void writeToXcom(String que_name, String nType, String nSubType1, 
			...
			jnaManager.sendRequestToService(que_name, nType, nSubType1, nSubType2, nAppDataType, p_data, nDataSize);
	}
