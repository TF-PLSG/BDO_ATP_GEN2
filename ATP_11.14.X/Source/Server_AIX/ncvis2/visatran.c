...
*
   ...
   ----------------
			...
			Rev 1.154  Dec 15, 2009 19:03:25   TF - Ajay
   Changed code to send reversal advice repeat as 0421 if advice reversal gets timeout 
   
	 Rev 1.153   Jul 30, 2009 14:23:25   TF - Ajay
   Change the code to not set flag "N" for manual keyed in txn of local processing 
   
	 
	   Rev 1.152   jun 15, 2009 17:23:25   TF - Ajay
   Change the code to  set timer for 0120 msg in 
   send_request_to_host() 
   Change the code to send the reversal as a 0420 if Original txn is 0120
   process_timeout_msg ()


		 Rev 1.151   May 04, 2009 16:26:25   TF - Ajay
   Change the code to not set timer for 0120 msg in 
   send_request_to_host() 
   
	  Rev 1.150   Mar 25, 2009 17:34:25   TF - Ajay
   Changed the msg type 0102 to 0400 for partial reversal 
     
	   Rev 1.149   Mar 25, 2009 17:34:25   TF - Ajay
   Changed the constant value of field 70 for logon and logoff msg 
   
	  Rev 1.148   Mar 12, 2008   dirby
...
CHAR  LOGON_NMI_LOGON[5] =          "0071";   /* Apr2009 VISA mandates, TF- Ajay */
...
BOOLEAN send_it_back_to_terminal(BYTE resp_code[3], pAUTH_TX p_auth_tx )
{
   ...
   if ( NULL_PTR == p_msg_term_handler_out )
   {
		...
		sprintf(Log_Bufr," Insufficient Memory to build response to txcntlTerm Id:%s ,Merch Id: %s RRN: %s ",
				p_auth_tx->TLF01_details.terminal_id,p_auth_tx->TLF01_details.merchant_id,p_auth_tx->TLF01_details.retrieval_ref_num);
		ncvis2_log_message( 3, 2, Log_Bufr , "send_it_back_to_terminal" , 0 ,p_auth_tx->TLF01_details.card_num);
		return(false);
   }
   ...
   if ( ret_code < 0 )
   {
		...
		ncvis2_log_message( 3, 2, Buffer , "send_it_back_to_terminal" , 0 ,p_auth_tx->TLF01_details.card_num);
...
INT incoming_terminal_request(pAUTH_TX p_auth_tx)
{
   ...
   if (false == get_visa_data (p_auth_tx) )
   {
      ...
      sprintf(Log_Bufr," Error trying to update Visa information Term Id:%s ,Merch Id: %s RRN: %s ",
    		  p_auth_tx->TLF01_details.terminal_id,p_auth_tx->TLF01_details.merchant_id,p_auth_tx->TLF01_details.retrieval_ref_num);
      ncvis2_log_message( 3, 2, Log_Bufr , "incoming_terminal_request" , 0 ,p_auth_tx->TLF01_details.card_num);
   ...
   switch ( temp_tx_key)
   {
      case AUTH_OFFLINE_SALE: 
		{	/* create tran type string and store it in the auth_tx_block */
            ...
            return(true); 
		}/* AUTH REFUND remove from here, as it has be sent to cadencie*/
      ...
      case AUTH_REVERSAL: 
		{	/* Create tran type string and store it in the auth_tx_block */
         ...
         {
            /* Partial Dispense */
			 /* It was  AUTH_ATM_CONFIRM as per Apr09 VISA mandates change into AUTH_REVERSAL, TF-Ajay*/
            p_auth_tx->TLF01_details.tx_key =AUTH_REVERSAL ;  
   ...
   if (false == verify_host_app_status(p_auth_tx))
   {
      ...
      ncvis2_log_message( 3, 2, "Host Inactive" , "incoming_terminal_request" , 0 ,"");
...
BOOLEAN process_logon_response(CHAR nmi_code[5], CHAR response_code[3])
{
   ...
   if (strcmp(LOGON_NMI_LOGON,nmi_code) == 0)
   {
           ...
           (response_code[0] == 0x00)  )
      {
         ...
         if ( ret_val == false )
         {
            ncvis2_log_message( 3, 3, "Unable to change host state" , "process_logon_response" , 0 ,"");
         }
         else
         {
            host_status_updated = true;
            ncvis2_log_message( 3, 1, "LOGON Request successful" , "process_logon_response" , 0 ,"");
         }
      }
      else
      {
         sprintf( ErrorMsg,
                 "Logon request declined. Response Code: %s",
                  response_code );
         ncvis2_log_message( 3, 2, ErrorMsg , "process_logon_response" , 0 ,"");
      }
   }
   else if (strcmp(LOGON_NMI_LOGOFF , nmi_code ) == 0)
   {
      ...
      {
         host_status_updated = true;
         ncvis2_log_message( 3, 1, "LOGOFF Request successful" , "process_logon_response" , 0 ,"");
         ...
         if ( echo_timer_length > 0 )
         {
            ...
            {
               ncvis2_log_message( 3,3, "Error clearing the echo_timer" , "process_logon_response" , 0 ,"");
            }
         }
      }
   }
   ...
   else if ( strcmp(LOGON_NMI_ECHO_TEST , nmi_code) == 0 )
   {
           ...
           (current_state[0] == cISSOFF_ACQON)  )
      {
         ...
         {
            host_status_updated = true;
            ncvis2_log_message( 3, 1, "Received response to echo. Going back Online", "process_logon_response" , 0 ,"");
         }
      }
...
/*
 *	ThoughtFocus Nirbhay : Cadencie changes
 *	This function sends 0302 msg from Cadencie to VISA (ncvisaA)
 *
 */
BYTE hhutil_transfer_302_msg_to_visa( pPTE_MSG p_msg )
{
   INT       len;
   BYTE      retval              = true;
   ...
   CHAR ncvisa_host_que_name  [MAX_QUE_NAME_SIZE+1] = "ncvis3A" ;
   ...
   if ( p_msg_out == NULL_PTR )
   {
      sprintf( strError,
              "Insufficient Memory to build ncvisa request message. Size: %d",
               buflen );
      ncvis2_log_message( 3, 3, strError, "hhutil_transfer_302_msg_to_visa" , 0 ,"");
      retval = false;
   }
   else
   {
      ptemsg_set_orig_comms_info( p_msg_out, originator_info );
      retCode = pteipc_send( p_msg_out, ncvisa_host_que_name );
      free (p_msg_out);
...
INT incoming_host_message(pPTE_MSG p_msg)
{
   ...
   pPTE_MSG			 p_msg_ins   = NULL_PTR;     // ThoughtFocus Girija Y
   AUTH_TX           auth_tx;
   LONG              ret_code;
   LONG              data_len;
   CHAR              Buffer[512]={0};
   ...
   if ( echo_timer_length > 0 )
   {
      ...
      if ( false == set_echo_timer(echo_lookup_key, echo_timer_length,
                                   ...
                                   SEND_TO_TIMERDS) )
      {
         ncvis2_log_message( 3, 3, "Error setting the echo_timer", "incoming_host_message" , 0 ,"");
      }
   }
   ...
   {
   	// Route 302 msg to VISA : ThoughtFocus nirbhay: cadencie changes.
	if(HOST_MESSAGE.msgtype_bcd[0] == 0x03 && HOST_MESSAGE.msgtype_bcd[1] == 0x02)
	{
   		hhutil_transfer_302_msg_to_visa(p_msg);// ThoughtFocus nirbhay: cadencie changes.
	}
      return(true);
   }
   ...
   memset(&auth_tx, 0, Auth_Tx_Size);
	...
	{
		sprintf(auth_tx.resp_data_len,"%4d", data_len);
   ...
   if(AUTH_INVALID == auth_tx.TLF01_details.tx_key)
   {
      ...
      strcat(Buffer,Log_Bufr);
      ncvis2_log_message( 3, 3, Buffer , "incoming_host_message" , 0 ,auth_tx.TLF01_details.card_num);
        ...
        ((Txn_Is_A_Reject == true) && (tran_type < 100)) )
   {
      ...
      if(0 == strcmp(auth_tx.TLF01_details.message_type, "0810"))
      {
		 ...
		 if (strcmp(LOGON_NMI_LOGON,nmi_code) == 0) /* Check whether the it is Logon Request or Logoff request */
		 {
			if ( (0 == strncmp(auth_tx.TLF01_details.response_code,"00",2)) || 
				(auth_tx.TLF01_details.response_code[0] == 0x00) ) /* Check whether Logon is successfull or Not - Girija Y */
			{
				...
				{
					strcpy( ErrorMsg, "Insufficient memory to build request message" );
					return(false);
			   ...
			   if ( ret_code < 0 )
			   {
				  ...
				  ncvis2_log_message( 3, 3, Buffer , "incoming_host_message" , 0 ,"");
      ...
      if((tran_type >= AUTH_SALE) && (tran_type <= AUTH_INVALID))
      {
              ...
              (current_state[0] == cISSOFF_ACQON) )
         {
            ...
            if (AUTH_ADMIN_ADVICE == auth_tx.TLF01_details.tx_key)
            {
               sprintf( Buffer,
				...
				sprintf(Log_Bufr," Term Id:%s ,Merch Id: %s RRN: %s ",
						auth_tx.TLF01_details.terminal_id,auth_tx.TLF01_details.merchant_id,auth_tx.TLF01_details.retrieval_ref_num);
				strcat(Buffer,Log_Bufr);
               ncvis2_log_message( 3, 1, Buffer , "incoming_host_message" , 0 ,auth_tx.TLF01_details.card_num);
               return(true);
            }
            ...
            if ( true == txn_is_blocked(&auth_tx,ErrorMsg) )
            {
               ...
               ncvis2_log_message( 3, 3, ErrorMsg , "incoming_host_message" , 0 ,auth_tx.TLF01_details.card_num);
               auth_tx.TLF01_details.tx_key =
                        match_request_to_response(auth_tx.TLF01_details.tx_key);
            ...
            else if ( true == is_txn_visa_electron( &auth_tx ) )
            {
               if ( false == validate_visa_electron( &auth_tx, ErrorMsg ) )
               {
                  ...
                  sprintf(Log_Bufr," Term Id:%s ,Merch Id: %s RRN: %s ",
                  		  auth_tx.TLF01_details.terminal_id,auth_tx.TLF01_details.merchant_id,auth_tx.TLF01_details.retrieval_ref_num);
                  strcat(ErrorMsg,Log_Bufr);
                  ncvis2_log_message( 3, 2, ErrorMsg , "incoming_host_message" , 0 ,auth_tx.TLF01_details.card_num);
                  auth_tx.TLF01_details.tx_key =
                        match_request_to_response(auth_tx.TLF01_details.tx_key);
                  (void)incoming_other_host_response(&auth_tx);
                  return( false );
      ...
      if ( NULL_PTR == p_msg_timer_out )
      {
         ...
         sprintf(Log_Bufr," Insufficient Memory to build clear timer message Term Id:%s ,Merch Id: %s RRN: %s ",
         		 auth_tx.TLF01_details.terminal_id,auth_tx.TLF01_details.merchant_id,auth_tx.TLF01_details.retrieval_ref_num);
         ncvis2_log_message( 3, 2, Log_Bufr , "incoming_host_message" , 0 ,auth_tx.TLF01_details.card_num);
         return(false);
      }
...
INT incoming_host_message_timer_reply(pPTE_MSG p_msg_in)
{
   ...
   else if (timer_reply_size != sizeof(buffer))
   {
      ...
      sprintf(Log_Bufr," No data returned with the get timer message Term Id:%s ,Merch Id: %s RRN: %s ",
      		 auth_tx.TLF01_details.terminal_id,auth_tx.TLF01_details.merchant_id,auth_tx.TLF01_details.retrieval_ref_num);
      ncvis2_log_message( 3, 2, Log_Bufr , "incoming_host_message_timer_reply" , 0 ,auth_tx.TLF01_details.card_num);
      ret_val = false;
   }
   else
   {
         ...
         else
         {
            ...
            if(strncmp(temp_auth_tx.TLF01_details.handler_queue, parvbh_que_name, 7)==0)
            {
													 ...
													 application_que_name, err_msg ))
			   {
				  ...
				  ncvis2_log_message( 1, 3, Log_Bufr , "incoming_host_message_timer_reply" , 0 ,temp_auth_tx.TLF01_details.card_num);
				  return( false );
			   }
            }
#endif
            /* Is this a repeat or a reversal caused by a time out? */
            else if(strncmp(temp_auth_tx.TLF01_details.handler_queue,"sbatch",6)==0)
            {
            ...
            else if ( (temp_auth_tx.TLF01_details.tx_key == AUTH_REVERSAL_REPEAT_RESPONSE) ||
...
INT process_timeout_msg(pPTE_MSG p_msg)
{
 	...
 	if(strcmp(lookup_key,echo_lookup_key) == 0)
   {
     ...
     /* Reset echo timer. */
      if(false == set_echo_timer(echo_lookup_key,echo_timer_length,
                                 ...
                                 SEND_TO_TIMERDS))
      {
         ncvis2_log_message( 3, 3, "Error setting the echo_timer" , "process_timeout_msg" , 0 ,"");
         ret_val = false;
      }
      else
         ret_val = true;
      ...
      if (false == perform_host_logon_request(ECHO_REQUEST, SOURCE_IS_OPERATOR,
                                             &auth_tx, ST2_ISSUER))
      {
         ncvis2_log_message( 3, 1, "Error executing host Echo Test", "process_timeout_msg" , 0 ,"");
      }
   }
   ...
   else
   {
      ...
      {
         ncvis2_log_message( 3, 2, "Error retrieving auth_tx from timer message", "process_timeout_msg" , 0 ,"");
         ret_val = false;
      }
      else
      {
         ...
         if ( MaxConsecTimeouts > 0 )
         {
            ...
            {
            	temp_tmouts_ctr = atoi(net_consec_tmouts_ctr);
            	//multiple prompts due to shm update delay, adding sleep
            	if(atoi(net_consec_tmouts_ctr) == MaxConsecTimeouts)
            	{
               ...
               sprintf(Log_Bufr," Maximum consecutive timeouts reached. Term Id:%s ,Merch Id: %s RRN: %s ",
               		   auth_tx.TLF01_details.terminal_id,auth_tx.TLF01_details.merchant_id,auth_tx.TLF01_details.retrieval_ref_num);
               ncvis2_log_message( 3, 2, Log_Bufr , "process_timeout_msg" , 0 ,auth_tx.TLF01_details.card_num);
               #else
               ...
               sprintf(Log_Bufr,
					   ...
					   masked_cardnbr);
               if(temp_tmouts_ctr == MaxConsecTimeouts)
            	   ncvis2_log_message( 3, 2, Log_Bufr , "process_timeout_msg" , 0 ,"");
               #endif
            }
            else
            {
         ...
         memset( Buffer, 0x00, sizeof(Buffer) );
         sprintf( Buffer,
                 "Txn to Visa Issuer Host(Vertexon) timed out.");

         memset(Log_Bufr,0x00,sizeof(Log_Bufr));
         sprintf(Log_Bufr," Term Id:%s ,Merch Id: %s RRN: %s ",
         		 auth_tx.TLF01_details.terminal_id,auth_tx.TLF01_details.merchant_id,auth_tx.TLF01_details.retrieval_ref_num);
         strcat(Buffer,Log_Bufr);
         ncvis2_log_message( 3, 1, Buffer , "process_timeout_msg" , 0 ,auth_tx.TLF01_details.card_num);
         ...
         else if ( auth_tx.advice_repeat_count[0] == 0x00 )
         {
            ...
            if ( AUTH_LOGON != auth_tx.TLF01_details.tx_key )
            {
               ...
               if ( AUTH_BALANCE_INQUIRY != temp_tx_key )
               {
				  ...
				  /* Send AUTH ADVICE REVERSAL as a 0420 msg TF- Ajay */
				  else if(AUTH_ADVICE == temp_tx_key)
				  {
         ...
         else
         {
     	   ...
     	   if(rvrsl_attmpts_value_fr_timeout_visa<=0)
     	   {
     		   rvrsl_attmpts_value_fr_timeout_visa=atoi(ncf01_i.advice_repeat_counter);
     	   }

     	   if(rvrsl_attmpts_value_fr_laterspns_visa <=0)
     	   {
     		   rvrsl_attmpts_value_fr_laterspns_visa=atoi(ncf01_i.advice_repeat_counter);
     	   }

       	   if(0 == strncmp(auth_tx.TLF01_details.response_text,"LR",2))
    	   {
    		  max_repeats=rvrsl_attmpts_value_fr_laterspns_visa;
    	   }
    	   else
    	   {
    		  max_repeats=rvrsl_attmpts_value_fr_timeout_visa;
    	   }
            ...
            if ( repeat_ctr < max_repeats )
            {
                ...
                */
         	   ...
         	   strcat(Buffer,Log_Bufr);
         	   ncvis2_log_message( 2, 1, Buffer , "process_timeout_msg" , 0 ,auth_tx.TLF01_details.card_num);
...
INT send_response_to_term_app(pAUTH_TX p_auth_tx)
{
   ...
   if ( ret_code < 0 )
   {
      ...
      sprintf(Log_Bufr," Term Id:%s ,Merch Id: %s RRN: %s ",
    		  p_auth_tx->TLF01_details.terminal_id,p_auth_tx->TLF01_details.merchant_id,p_auth_tx->TLF01_details.retrieval_ref_num);
      strcat(Buffer,Log_Bufr);
      ncvis2_log_message( 3, 3, Buffer , "send_response_to_term_app" , 0 ,p_auth_tx->TLF01_details.card_num);
   ...
   else
   {
	  ...
	  sprintf(Log_Bufr,"Response sent successfully to authorizer Term Id:%s ,Merch Id: %s RRN: %s ",
			  p_auth_tx->TLF01_details.terminal_id,p_auth_tx->TLF01_details.merchant_id,p_auth_tx->TLF01_details.retrieval_ref_num);
      ncvis2_log_message( 2, 1, Log_Bufr , "send_response_to_term_app" , 0 ,p_auth_tx->TLF01_details.card_num);
   }
   free(p_msg_term_handler_out);
   return(true);
...
INT alter_transaction_data(pAUTH_TX p_auth_tx, pAUTH_TX p_temp_auth_tx)
{
  ...
  if(p_auth_tx->TLF01_details.mcard_banknet[0] != 0x00)
   ...
   memset( &emv, 0x00, sizeof(EMV) );
	if(strcmp(p_temp_auth_tx->TLF01_details.handler_queue,"dcpisoA")== 0)	
	{  
		   ...
		   //emv_response_len = genutil_bcd_to_int( p_auth_tx->EMV_details.emv_block, 2 );
			...
			{	
			  ...
			  {
				memset(Log_Bufr,0x00,sizeof(Log_Bufr));
				sprintf(Log_Bufr," EMV response is > 240  Term Id:%s ,Merch Id: %s RRN: %s ",
						p_auth_tx->TLF01_details.terminal_id,p_auth_tx->TLF01_details.merchant_id,p_auth_tx->TLF01_details.retrieval_ref_num);
				 ncvis2_log_message( 3, 1, Log_Bufr, "alter_transaction_data" , 0 ,p_auth_tx->TLF01_details.card_num);
			  }
	...
	{
		emv_request_len  = p_temp_auth_tx->EMV_details.emv_block[0];
		emv_response_len =  p_auth_tx->EMV_details.emv_block[0];
		memcpy(p_temp_auth_tx->EMV_details.emv_block,
		p_auth_tx->EMV_details.emv_block,
		sizeof(p_auth_tx->EMV_details.emv_block));
	}
#if 0
	if( emv_request_len > 255)
		emv_request_len =0;
	if( emv_response_len > 255)
		emv_response_len =0;
...
INT send_request_to_host( pAUTH_TX p_auth_tx )
{
   ...
   CHAR     gmt_time[20]; INT buf_temp_len=0;
   ...
   if (GS_TXN_DROPPED == p_auth_tx->TLF01_details.general_status)
   {
      ...
      if(NULL_PTR == p_msg_term_hdlr)
      {
         ...
         sprintf(Log_Bufr," Insufficient Memory to build response to txcntl Term Id:%s ,Merch Id: %s RRN: %s ",
         					p_auth_tx->TLF01_details.terminal_id,p_auth_tx->TLF01_details.merchant_id,p_auth_tx->TLF01_details.retrieval_ref_num);
         ncvis2_log_message( 3, 2, Log_Bufr , "send_request_to_host" , 0 ,p_auth_tx->TLF01_details.card_num);
         return(false);
      } 
      ...
      if(ret_code < 0)
      {
         ...
         sprintf(Log_Bufr," Term Id:%s ,Merch Id: %s RRN: %s ",
         		p_auth_tx->TLF01_details.terminal_id,p_auth_tx->TLF01_details.merchant_id,p_auth_tx->TLF01_details.retrieval_ref_num);
         strcat(Buffer,Log_Bufr);
         ncvis2_log_message( 3, 3, Buffer , "send_request_to_host" , 0 ,p_auth_tx->TLF01_details.card_num);
   ...
   if ( p_auth_tx->TLF01_details.general_status == GS_TXN_PERF_REQ )
   {
      ...
      if (Build_Host_Request( p_auth_tx ))
      {
          ...
          */
		 /* For 0120 msg Response will not come from the cadencie. 
		    Need not to set timer for that. Otherwise it will generate Reversal. TF- Ajay */ 
		 /*  Again client want to send Reversal so setting timer for AUTH_ADVICE  TF-Ajay */
         ...
         if ( p_auth_tx->TLF01_details.tx_key != AUTH_ATM_CONFIRM)
         {
            ...
            {
               ncvis2_log_message( 3, 3, "Error chaining the timer request with clear & set the echo_timer" , "send_request_to_host" , 0 ,"");
            }
         }
         else
         {
            ...
            if ( echo_timer_length > 0 )
            {
               ...
               if ( false == set_echo_timer(echo_lookup_key, echo_timer_length,
                                   ...
                                   SEND_TO_TIMERDS) )
               {
                  ncvis2_log_message( 3, 3, "Error setting the echo_timer" , "send_request_to_host" , 0 ,"");
               }
            }
         }
#ifdef HCTS4
...
INT  verify_host_app_status(pAUTH_TX p_auth_tx)
{
   ...
   if ( (MaxConsecTimeouts > 0) && (ret_code == true) )
   {
      if (atoi(net_consec_tmouts_ctr) > MaxConsecTimeouts )
      {
         ...
         ncvis2_log_message( 3, 3, err_buf , "verify_host_app_status" , 0 ,p_auth_tx->TLF01_details.card_num);
      }
   }
      ...
      if ( MaxActiveTxns > 0 )
      {
         if( tempint >= MaxActiveTxns )
         {
            ...
            sprintf(Log_Bufr," Term Id:%s ,Merch Id: %s RRN: %s ",
            		p_auth_tx->TLF01_details.terminal_id,p_auth_tx->TLF01_details.merchant_id,p_auth_tx->TLF01_details.retrieval_ref_num);
            strcat(err_buf,Log_Bufr);
            ncvis2_log_message( 3, 2, err_buf , "verify_host_app_status" , 0 ,p_auth_tx->TLF01_details.card_num);
         }
      }
   ...
   switch (source)
	{
		case SOURCE_IS_FSVISA:
		{
         if ( 0 != strcmp( current_state, ONLINE ) )
			{
            /* Issue a LOFON request to the host. */
      ...
      case SOURCE_IS_OPERATOR:
      {
         ...
         if ( ret_val == true )
         {
            ...
            if ( ret_val == false )
            {
               ncvis2_log_message( 3, 3, "Unable to change host state" , "perform_host_logon_request" , 0 ,"");
            }
            else
               (void)send_network_status_to_monitor();
         }
...
INT send_network_status_to_monitor(void)
{
   ...
   if ( NULL_PTR == p_msg_ins )
   {
      ncvis2_log_message( 3, 2, "Insufficient Memory to create network query message" , "send_network_status_to_monitor" , 0 ,"");
      return( false );
   }
	...
	if (ConnectionId[0] != 0)
   {
      ...
      if ( 0 > ret_code )
      {
         ...
         ncvis2_log_message( 3, 3, Buff , "send_network_status_to_monitor" , 0 ,"");
         free( p_msg_ins );
         return( false );
      }
   }
 ...
 */
	...
	if (ConnectionId[0] != 0)
   {
      ...
      if ( 0 > ret_code )
      {
         pteipc_get_errormsg( ret_code, err_mesg );
         sprintf(Buff,"Failed transmit to que: %s. %s",applnk_que_name,err_mesg);
         ncvis2_log_message( 3, 2, Buff , "send_network_status_to_monitor" , 0 ,"");
         free( p_msg_ins );
         return( false );
      }
   }
                                   ...
                                   pBYTE    iss_state )
{
   ...
   if (0 == strcmp(request_type, LOGON_REQUEST))
   {
      ...
      if ( subtype2 == ST2_ACQUIRER )
      {
         if ( (state == cBOTH_ONLINE) || (state == cISSOFF_ACQON) )
         {
            ...
            ret_val = false;
            ncvis2_log_message( 3, 1, "Already online. Log ON request dropped." , "perform_op_net_mngmnt_request" , 0 ,"");
         }
         else
         {
            ...
            iss_state[0] = vSTAY;
            strcpy( vLogon_NMI_Code, LOGON_NMI_LOGON );
            ncvis2_log_message( 3, 1, "Sending LOGON Request to host." , "perform_op_net_mngmnt_request" , 0 ,"");
        }
      }
      else
      {
         if ( (state == cBOTH_ONLINE) || (state == cISSON_ACQOFF) )
         {
            ...
            ret_val =false;
            ncvis2_log_message( 1, 1, "Already online. Log ON request dropped." , "perform_op_net_mngmnt_request" , 0 ,"");
         }
         else
         {
            ...
            iss_state[0] = vSTAY;
            acq_state[0] = vSTAY;
            ncvis2_log_message( 1, 1, "Sending LOGON Request to host.", "perform_op_net_mngmnt_request" , 0 ,"");
         }
      }
   }
   else if (0 == strcmp(request_type, LOGOFF_REQUEST))
   {
      ...
      if ( subtype2 == ST2_ACQUIRER )
      {
         if ( (state == cBOTH_OFFLINE) || (state == cISSON_ACQOFF) )
         {
            ...
            ret_val = false;
            ncvis2_log_message( 1, 1,"Not online. Log OFF request dropped.", "perform_op_net_mngmnt_request" , 0 ,"");
         }
         else
         {
            ...
            iss_state[0] = vSTAY;
            ncvis2_log_message( 1, 1,"Sending LOGOFF Request to host.", "perform_op_net_mngmnt_request" , 0 ,"");
         }
      }
      else
      {
   ...
   else 
   {
      ...
      if ( vLogon_NMI_Code[0] != 0x00 )
      {
         ...
         if ( NULL_PTR == p_msg_set_timer )
         {
            ncvis2_log_message( 3, 2,"Insufficient Memory to build start timer message", "perform_op_net_mngmnt_request" , 0 ,"");
            memset( vLogon_NMI_Code, 0x00, sizeof(vLogon_NMI_Code) );
         }
         else
         {
            ...
            {
               memset( vLogon_NMI_Code, 0x00, sizeof(vLogon_NMI_Code) );
               ncvis2_log_message( 3, 3,"Error chaining the timer request with clear & set the echo_timer", "perform_op_net_mngmnt_request" , 0 ,"");
            }
         }
      }
   }
...
INT perform_op_net_mngmnt_response(char nmi_code[], pAUTH_TX p_auth_tx)
{
   ...
   {
      ncvis2_log_message( 1, 1,"Received LOGON request from host", "perform_op_net_mngmnt_response" , 0 ,"");
   ...
   {
      ncvis2_log_message( 1, 1,"Received LOGOFF request from host", "perform_op_net_mngmnt_response" , 0 ,"");
...
/* to support reversal advice repeat(0421) Dec 15,2009 TF-Ajay */
...
{     
   ...
   if ( ret_code < 0 )
   {
      ...
      sprintf(Log_Bufr," Term Id:%s ,Merch Id: %s RRN: %s ",
      		  p_auth_tx->TLF01_details.terminal_id,p_auth_tx->TLF01_details.merchant_id,p_auth_tx->TLF01_details.retrieval_ref_num);
      strcat(Buffer,Log_Bufr);
      ncvis2_log_message( 3, 3, Buffer , "perform_standin_processing" , 0 ,p_auth_tx->TLF01_details.card_num);
      free(p_msg_term_handler_out);
      return;
   }
   free(p_msg_term_handler_out);
...
INT get_visa_data (pAUTH_TX p_auth_tx)
{
   ...
   if ( AUTH_REVERSAL != p_auth_tx->TLF01_details.tx_key )
   {
      switch( p_auth_tx->TLF01_details.tx_key )
      {
         ...
         default:
			 //TF Phani .. Dont Change RRN keep the Original one..
...
INT process_incoming_message(pPTE_MSG p_msg)
{
      ...
      if( Tran_Flow == SWITCHED_IN )
      {
             ...
             (0 == strcmp(auth_tx.TLF01_details.response_code, "55"))  )
         {
        	 ...
        	 { // if the link offline mark it as standin, and send via sbatch
        		 if(true == ncvis2_check_for_acq_refund_transaction(&auth_tx))
        		 {
        		 strcpy(auth_tx.TLF01_details.product_codes[1].code,STANDIN);
        	 }
        		 else if(true == ncvis2_check_for_cadencie_acq_transaction(&auth_tx))
        		 {
            ...
            if ( false == update_database( &auth_tx ))
            {
               ...
               sprintf(Log_Bufr," Unable to update database tables. Term Id:%s ,Merch Id: %s RRN: %s ",
               		auth_tx.TLF01_details.terminal_id,auth_tx.TLF01_details.merchant_id,auth_tx.TLF01_details.retrieval_ref_num);
               ncvis2_log_message( 3, 3, Log_Bufr , "process_incoming_message" , 0 ,auth_tx.TLF01_details.card_num);
   ...
   if ( AUTH_REVERSAL == auth_tx.TLF01_details.tx_key )
   {
      ...
      sprintf( Buff,
              "Reversal going to Visa: %s. Card %s, Merchant %s",
               AppName,
      ...
      ncvis2_log_message( 3, 2, Buff , "process_incoming_message" , 0 ,auth_tx.TLF01_details.card_num);
   }
   else if(0 == strncmp(auth_tx.TLF01_details.handler_queue,"ncvis3",5))
   {
	   //Don't do PIN translation if tx is coming from VISA incoming
	   strcpy( auth_tx.TLF01_details.security_response_code, "00" );
	}
   else if ( true == hhutil_verify_pin_block(&auth_tx) && 
			true == verify_host_app_status(&auth_tx)/* TF Phani: Do Not perform PIN transalation, Since Host is offline, We may do STIP*/
...
INT process_auth_advices( pAUTH_TX p_auth_tx, INT status, NCF30 ncf30 )
{
   ...
   if ( false == ret_val )
   {
      ...
      sprintf(Log_Bufr," Unable to complete processing of an advice (1120). Term Id:%s ,Merch Id: %s RRN: %s ",
      			p_auth_tx->TLF01_details.terminal_id,p_auth_tx->TLF01_details.merchant_id,p_auth_tx->TLF01_details.retrieval_ref_num);
      ncvis2_log_message( 3, 3, Log_Bufr , "process_auth_advices" , 0 ,p_auth_tx->TLF01_details.card_num);
   }
   return( ret_val );
 }
...
BYTE process_auth_repeat_part1( pAUTH_TX p_auth_tx )
{
   ...
   if ( NULL_PTR == p_msg_ins  )
   {
      ...
      sprintf(Log_Bufr," Insufficient Memory for select ncf30 message Term Id:%s ,Merch Id: %s RRN: %s ",
      		  p_auth_tx->TLF01_details.terminal_id,p_auth_tx->TLF01_details.merchant_id,p_auth_tx->TLF01_details.retrieval_ref_num);
      ncvis2_log_message( 3, 3, Log_Bufr , "process_auth_repeats_part1" , 0 ,p_auth_tx->TLF01_details.card_num);
      ret_val = false;
   }
   else
   {
      ...
      if ( 0 > ret_code )
      {
         ...
         sprintf(Log_Bufr," Term Id:%s ,Merch Id: %s RRN: %s ",
         		p_auth_tx->TLF01_details.terminal_id,p_auth_tx->TLF01_details.merchant_id,p_auth_tx->TLF01_details.retrieval_ref_num);
         strcat(ErrorMsg,Log_Bufr);
         ncvis2_log_message( 3, 3, ErrorMsg , "process_auth_repeats_part1" , 0 ,p_auth_tx->TLF01_details.card_num);
         ret_val = false;
      }
   }
...
INT delete_orig_ncf30( pNCF30 pncf30, pAUTH_TX p_auth_tx )
{
   ...
   if ( NULL_PTR == p_msg  )
   {
      ...
      sprintf(Log_Bufr," Insufficient Memory to build NCF30 message Term Id:%s ,Merch Id: %s RRN: %s ",
      		  p_auth_tx->TLF01_details.terminal_id,p_auth_tx->TLF01_details.merchant_id,p_auth_tx->TLF01_details.retrieval_ref_num);
      ncvis2_log_message( 3, 3, Log_Bufr , "delete_orig_ncf30" , 0 ,p_auth_tx->TLF01_details.card_num);
      return( false );
   }
   ...
   if ( 0 > ret_code )
   {
      ...
      ncvis2_log_message( 3, 3, Buff , "delete_orig_ncf30" , 0 ,"");
      return( false );
   }
   return(true);
 }
...
INT recon_update( pAUTH_TX p_auth_tx, BYTE subtype_2, pCHAR network_type )
{
   ...
   if ( NULL_PTR == p_msg  )
   {
      ncvis2_log_message( 3, 2, "Insufficient Memory to build update NCF30 message" , "recon_update" , 0 ,"");
      return( false );
   }
   ...
   if ( 0 > ret_code )
   {
      ...
      ncvis2_log_message( 3, 3, Buff , "recon_update" , 0 ,"");
      return( false );
   }
   return( true );
}
...
INT update_database( pAUTH_TX p_auth_tx )
{
   ...
   if(NULL_PTR == p_msg)
   {
      ...
      sprintf(Log_Bufr," Insufficient Memory to build UPDATE ALL message Term Id:%s ,Merch Id: %s RRN: %s ",
      					p_auth_tx->TLF01_details.terminal_id,p_auth_tx->TLF01_details.merchant_id,p_auth_tx->TLF01_details.retrieval_ref_num);
      ncvis2_log_message( 3, 2, Log_Bufr , "update_database" , 0 ,p_auth_tx->TLF01_details.card_num);
      ret_val = false;
   }
   else
   {
      ...
      if(ret_code < 0)
	  {
         ...
         strcat(Buff,Log_Bufr);
         ncvis2_log_message( 3, 3, Buff , "update_database" , 0 ,p_auth_tx->TLF01_details.card_num);
         ret_val = false;
      }
   }
   return( ret_val );
...
INT send_transaction_statistics( pPTE_MSG p_msg )
{
   ...
   if ( false == ret_code  )
   {
      ncvis2_log_message( 3, 2, err_msg , "send_transaction_statistics" , 0 ,"");
   }
   return( ret_code );
}
...
INT reset_transaction_statistics( pPTE_MSG p_msg )
{
   ...
   if ( false == ret_code  )
   {
      ncvis2_log_message( 3, 2, err_msg , "reset_transaction_statistics" , 0 ,"");
   }
   return( ret_code );
}
...
INT process_0302_request( pPTE_MSG p_msg )
{
   ...
   if ( true == hhutil_parse_host_response_msg(&auth_tx) )
   {
      ...
      (void)move_iso_response_to_auth_struct( nmi_code, &auth_tx );
   	...
   	/* Route 0302 response (0312 message) from VISA to Cadencie*/ // ThoughtFocus nirbhay
	if(HOST_MESSAGE.msgtype_bcd[0] == 0x03 && HOST_MESSAGE.msgtype_bcd[1] == 0x12)
	{
		...
		//{
		//memmove(&ReqRsp_Buffer[VISA_HDR_SIZE], &ReqRsp_Buffer[0], buflen );
		//buflen += VISA_HDR_SIZE; // increase length for header.
		 //Build_Host_Header(&auth_tx);
		 if(false == hhutil_send_host_response(&auth_tx))
		 {
		    ...
		    sprintf(Log_Bufr," Error sending response to Visa host. Term Id:%s ,Merch Id: %s RRN: %s ",
		    					auth_tx.TLF01_details.terminal_id,auth_tx.TLF01_details.merchant_id,auth_tx.TLF01_details.retrieval_ref_num);
		    ncvis2_log_message( 3, 2, Log_Bufr , "process_0302_request" , 0 ,auth_tx.TLF01_details.card_num);
		    return false;
		 }
		//}
		return true;
      ...
      else
      {
         ...
         ncvis2_log_message( 3, 2, err_msg , "process_0302_request" , 0 ,"");
...
void issue_0302_response( pBYTE resp_code, pAUTH_TX p_auth_tx )
{
   ...
   if ( 0 != strcmp(resp_code, APPROVED) )
   {
      ...
      sprintf(Log_Bufr," Term Id:%s ,Merch Id: %s RRN: %s ",
      					...
      					p_auth_tx->TLF01_details.merchant_id,p_auth_tx->TLF01_details.retrieval_ref_num);
      strcat(Buffer,Log_Bufr);
      ncvis2_log_message( 3, 2, Buffer , "issue_0302_response" , 0 ,p_auth_tx->TLF01_details.card_num);
   }
   ...
   if ( NULL_PTR == p_msg_term )
   {
      ...
      ncvis2_log_message( 3, 2, errbuf , "issue_0302_response" , 0 ,p_auth_tx->TLF01_details.card_num);
   }
   else
   {
      ...
      if ( ret_code < 0 )
      {
         ...
         sprintf(Log_Bufr," Term Id:%s ,Merch Id: %s RRN: %s ",
         		p_auth_tx->TLF01_details.terminal_id,p_auth_tx->TLF01_details.merchant_id,p_auth_tx->TLF01_details.retrieval_ref_num);
         strcat(Buffer,Log_Bufr);
         ncvis2_log_message( 3, 3, Buffer , "issue_0302_response" , 0 ,p_auth_tx->TLF01_details.card_num);
      }
      free(p_msg_term);
   }
...
INT is_txn_reversal( BYTE tx_key )
{
   ...
   switch( tx_key )
   {
	  ...
	  /* Advice reversal repeat response Dec 15,2009 TF-Ajay */
 ...
 *  MODIFICATIONS:
 ******************************************************************************/
INT ncvis2_send_reversal_to_host_for_late_response( pAUTH_TX p_auth_tx )
{
	 pPTE_MSG	p_msg_set_timer     = NULL_PTR;
   pPTE_MSG p_msg_host_out = NULL_PTR;
   CHAR     gmt_time[20];
   ...
   if ( GS_TXN_PERF_REQ == p_auth_tx->TLF01_details.general_status )
   {
    	  ...
    	  memset(p_auth_tx->req_data_len,0x00,sizeof(p_auth_tx->req_data_len));
            ...
            ret_code = build_and_send_request_message(MT_OUTGOING,
			 ...
			 if ( 1 != ret_code )
			 {
				  ...
				  ncvis2_log_message( 3, 3, err_msg , "send_reversal_to_host_for_late_response" , 0 ,p_auth_tx->TLF01_details.card_num);
			 }
			 else
			 {
      ...
      else
      {
         ...
         memcpy( p_auth_tx->TLF01_details.response_text, "Unable to send request to host, tran table error",
                       sizeof(p_auth_tx->TLF01_details.response_text)-1 );
         ncvis2_log_message( 3, 2, Log_Bufr , "send_reversal_to_host_for_late_response" , 0 ,p_auth_tx->TLF01_details.card_num);
      }
   }
   return( ret_code );
