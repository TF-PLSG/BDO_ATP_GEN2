...
BOOLEAN mrads_Send_Request_To_Mracon_Service ( pAUTH_TX auth_tx );
...
void mrads_reset_stats( pTXN_COUNTS p_txn_cnts );
//BYTE mrads_send_transaction_statistics(pPTE_MSG p_msg );
...
CHAR  mrads_Erro_warning_Filename[256] = {0};
CHAR  mrads_module_error_warning_file_name[256] = {0};
CHAR  mrads_error_warning_file_path[256] = {0};
...
/*************************************************************************************/
void mrads_create_Error_Filename(  )
{
   ...
   /* Get system timestamp "YYYY-MM-DD-hh.mm.ss.jjj" */
    ...
    strcpy( mrads_Erro_warning_Filename, mrads_error_warning_file_path );
    strcat( mrads_Erro_warning_Filename, mrads_module_error_warning_file_name );
	strcat( mrads_Erro_warning_Filename, ".EW." );
    strncat(mrads_Erro_warning_Filename, system_date,   4 );  /* YYYY */
    strncat(mrads_Erro_warning_Filename, system_date+5, 2 );  /* MM   */
    strncat(mrads_Erro_warning_Filename, system_date+8, 2 );  /* DD   */
}
/*******************************************************
...
void mrads_get_error_warning_file_name_path(void )
{
   DWORD rc;
   CHAR  filename   [80] = {0};
   CHAR  tmpstr     [80] = {0};
   ...
   rc = GetPrivateProfileString(
								  ...
								  "DB_STATICS_PATH",       	/* points to key name             */
								   "",  					/* Default string                 */
								   mrads_error_warning_file_path,              	/* points to destination buffer   */
								   sizeof(mrads_error_warning_file_path)-1,   	 /* size of destination buffer     */
   ...
   rc = GetPrivateProfileString(
								  ...
								  "",  					/* Default string                 */
								  mrads_module_error_warning_file_name,              	/* points to destination buffer   */
								  sizeof(mrads_module_error_warning_file_name)-1,   	 /* size of destination buffer     */
								  filename                  /* points to ini filename         */
								);

   if((strlen(mrads_error_warning_file_path) > 0) &&
	  (strlen(mrads_module_error_warning_file_name)> 0))
   {
	   mrads_create_Error_Filename();
   }
}

/******************************************************************************
 *
 *  NAME:         mrads_log_message
 *
 ...
 *
 ******************************************************************************/
void mrads_log_message( INT dest, INT sev, pCHAR msg, pCHAR func, int details )
{
   INT  monitor_flag = 0;
   INT  eventlog_flag = 0;
   INT  msg_type = 0;
   ...
   mrads_Log_error_warning_to_File(text_message,sev,func,details);
   if(monitor_flag == 1)
   {
...
INT mrads_Log_error_warning_to_File(pCHAR Error_Buf,int sev,pCHAR func, int detail)
{
	INT   ret_val = 0;
	INT   len=0;
	INT   cardlen = 0;
    ...
    len = strlen(mrads_Erro_warning_Filename);
    path_len = strlen(mrads_error_warning_file_path);
    if( len == 0 ||
    	path_len==0 )
    {
    ...
    if ( 0 != strcmp(&mrads_Erro_warning_Filename[len-4], current_mmdd) )
    {
		/* Now create the new filename.*/
    	mrads_create_Error_Filename();
    }
	if((fp = fopen(mrads_Erro_warning_Filename,"a+b"))==NULL)
	{
		LogEvent(Error_Buf,INFO_MSG);
		return 0;
	}
...
pPTE_MSG process_insert_msg( pPTE_MSG p_msg_in )
{
   ...
   mrads_log_message( 2, 1,strTemp,"process_insert_msg", 0 );
   ...
   case MRA_AUTH_TBL_DATA :
			 ...
			 if(result_code != PTEMSG_OK)
			 {
				 ...
				 sprintf(strTemp, "Insert to MRADS Table Failed", reply_que);
				 mrads_log_message( 2, 1,strTemp,"process_insert_msg", 0 );
			 }
			 else
			 {
				 mrads_log_message( 2,1,"Successfully Inserted to MRADS Table","process_insert_msg", 0 );
			 }

			 if(1 == Mracon_Log_Enable_Flag)
			 {
				 mrads_Send_Request_To_Mracon_Service(auth_tx);
			 }
     ...
     case NFI01_DATA:
      ...
      /* case MRA01_DATA:*/
    	  ...
    	  /* memset(&db_recs.mra01,0x00,sizeof(db_recs.mra01));
    	   ...
    	   mrads_log_message( 2, 1,"MRA01 DATA","process_insert_msg", 0 );
    	   ...
    	   if(result_code == PTEMSG_OK)
    	   {
				 ...
				 if(result_code != PTEMSG_OK)
				 {
					 ...
					 mrads_log_message( 2, 3,strTemp,"process_insert_msg", 0 );
				 }
				 else
				 {
					 mrads_log_message( 2, 1,"Successfully Inserted to MRA01 Table","process_insert_msg", 0 );
				 }
    	   }
...
pPTE_MSG process_update_msg( pPTE_MSG p_msg_in )
{
   ...
   switch( app_data_type )
   {
      ...
      case BCH01_DATA:
         ...
         else if(MsgSubtype2 == ST2_DB_UPDATE_MOVE_DELETE )
         {
    		 if (DB_NullCheck_Flag==1)
    		 {
    			 CheckNULLTerminatedinBCH01 (&db_recs.bch01,MODE_UPDATE);
         ...
         else if(MsgSubtype2 == ST2_DB_UPDATE_BCH01_TO_CLOSE_BATCH)
         {
    		 if (DB_NullCheck_Flag==1)
    		 {
    			 CheckNULLTerminatedinBCH01 (&db_recs.bch01,MODE_UPDATE);
         ...
         else if(MsgSubtype2 == ST2_DB_UPDATE_BCH01_INSERT_BCH10)
		 {
			...
			//   printf("bch10_open_time: %s bch01_open_time: %s\n", bch10.open_time,db_recs.bch01.open_time);
			 ...
			 if (DB_NullCheck_Flag==1)
			 {
				 CheckNULLTerminatedinBCH10 (&bch10,MODE_INSERT);
			 }
...
BOOLEAN mrads_Send_Request_To_Mracon_Service ( pAUTH_TX auth_tx )
{
   ...
   if (p_msg_out == NULL)
   {
	   if(Mracon_Log_Enable_Flag == 1)
	   {
		  ...
		  mrads_log_message( 1, 3,strTemp,"mrads_Send_Request_To_Mracon_Service", 1 );
	   }
   }
   else
   {
      ...
      if (lReturnCode != 0)
      {
		  if(Mracon_Log_Enable_Flag == 1)
		  {
			 ...
			 mrads_log_message( 2, 3,strTemp,"mrads_Send_Request_To_Mracon_Service", 1 );
		  }

		  }
      else
      {
    	  mrads_log_message( 2, 1,"Txn posted to MRACON", "mrads_Send_Request_To_Mracon_Service", 0 );
      }
      free( p_msg_out );
   }
   return( retval );
...
/*      case ST1_DB_DELETE            : p_msg_out = process_delete_msg       (p_msg_in);
   ...
   if ( ds_timer_flag == DS_TIMER_ON )
   {
      ...
      if ( ret_val == false )
      {
         ...
         mrads_log_message( 2, 2, errbuf, "process_db_request", 0);
      }
   }
   return (p_msg_out);
}
   ...
   switch (ptemsg_get_msg_subtype1 (p_msg_in))
   {
      case ST1_LOG_REFEED_COMPLETE : 
         ...
         if (p_msg_trid [0] != '\0')
         {
            ...
            {
               dbcommon_rollback ();
               mrads_log_message( 1, 3, ErrorMsg, "process_logger_reply", 0 );
      ...
      default:
         ...
         mrads_log_message( 1, 2, ErrorMsg, "process_logger_reply", 0 );
         break;
   }
}
...
BOOLEAN perform_refeed ( pCHAR trid )
{
   ...
   if (trid [0] != '\0')
   {
      ...
      if (SendRefeedCmd (trid, ErrorMsg) == false)
      {
         ...
         mrads_log_message( 1, 3, ErrorMsg, "perform_refeed", 0 );
         return (false);
      }
...
BYTE validate_last_trid ( pPTE_MSG p_msg_in )
{
   ...
   if (result_code != PTEMSG_OK)
   {
      mrads_log_message( 1, 2, ErrorMsg, "validate_last_trid", 0 );
       /*
...
BYTE PinnacleMsgHandler ( pPTE_MSG p_msg_in )
{
   ...
   while ( (i <= num_messages) && (p_cur_msg != NULL_PTR) )
   {
      ...
      switch( ptemsg_get_msg_type( p_cur_msg ) )
      {
                                     ...
                                     if ( ST2_NO_VERSION != ptemsg_get_msg_subtype2(p_msg_in) )
                                     {
                                        ...
                                        sprintf( temp_str,
                                                "Pinged -> Data Server: %s, version %s",
                                                 ServiceName, Version );
                                       mrads_log_message( 1, 1, temp_str, "PinnacleMsgHandler", 0 );
                                       ...
                                       memset( temp_str,   0x00, sizeof(temp_str)   );
                                        ret_val = get_timing_parameters( &DB_Timing_Flag,
                                                                         ...
                                                                         &ForceReport,
                                                                          temp_str );
                                        mrads_log_message( 2, 1, temp_str, "PinnacleMsgHandler", 0);
                                        if ( DB_Timing_Flag == true )
                                        {
         ...
         /*case MT_GET_STATS 		: mrads_send_transaction_statistics(p_cur_msg);
         	 	 	 	 	 	  break;*/
...
/*         case MT_NEGATIVE_FILE  : p_tmp_msg = process_negative_file (p_cur_msg);
   ...
   if (result_code != PTEMSG_OK)
   {
      ...
      if (p_chn_out != NULL_PTR)
      {
		...
		{
			 mrads_log_message( 1, 2, ErrorMsg, "PinnacleMsgHandler", 0 );
			 return (PTEMSG_OK);
		}
      /*
       * Try to create an error response message to send back to the 
   ...
   {
      printf   ("%s\n", ErrorMsg);
      mrads_log_message( 2, 2, ErrorMsg, "PinnacleMsgHandler", 0);
   }*/
   ...
   if(DB_reconnect == 1)
   {
	   ...
	   if (dbcommon_connect ("equitable", "equitable", "equitable", "ORCL", ErrorMsg) != PTEMSG_OK)
	   {
		   ...
		   mrads_log_message(1, 3, ErrorMsg, "PinnacleMsgHandler",0);
			return (false);
	   }
	   else
	   {
		   mrads_log_message(1, 1, "DataBase Recoonected", "PinnacleMsgHandler",0);
	   }
...
BOOLEAN perform_startup ()
{
   ...
   {
      mrads_log_message( 1, 3, ErrorMsg, "perform_startup", 0 );
      return (false);
   }
   ...
   {     
      mrads_log_message( 1, 3, "Failed to create XIPC queues", "perform_startup", 0 );
   ...
   if ( false == ret_val )
   {
      mrads_log_message( 2, 2, Buffer, "perform_startup", 0);
   }
   else if ( ds_timer_flag == DS_TIMER_ON )
   {
      ...
      mrads_log_message( 2, 1, Buffer, "perform_startup", 0);
   }
...
void TrapFunction( INT signum )
{
   ...
   mrads_log_message( 2, 3, Buffer, "TrapFunction", 0);
}
...
void MainProcessor()
{
   ...
   GetAppName( AppName );
   mrads_get_error_warning_file_name_path();
  ...
  ret_val = get_dataserver_ini_db_error_statics_value( &db_error_statics_flag, &db_error_statics_value,
      ...
      MainProcessDone = 1;
	     sprintf( Buffer,
           "Shutting down the Service: %s, version %s",
            AppName, Version );
		  mrads_log_message( 2, 1, Buffer, "MainProcessor", 0);
			printf( "%s\n", Buffer );
   ...
   while ( !EndProcessSignalled )
   {
      ...
      else if( ipc_rcode != QUE_ER_TIMEOUT ) 
      {
         ...
         mrads_log_message( 1, 2, Buffer, "MainProcessor", 0 );
         ...
         if ( ++err_ctr > 10 )
         {
            ...
            sprintf( strError,
                    "SHUT DOWN %s. UNABLE TO ACCESS QUEUE!!!!",
                     ServiceName );
            mrads_log_message( 1, 3, strError, "MainProcessor", 0 );
            #ifdef WIN32
               Sleep (5000);
            #else
               usleep (5000000);
...
void EndProcess()
{
   ...
   sprintf( Buffer, "Shutting down the %s Service, version %s",
            ServiceName, Version );
   mrads_log_message( 2, 1, Buffer, "EndProcess", 0);
   strcat( Buffer, "\n" );
   PRINT( Buffer );
...
/******************************************************************************
 *
 *  NAME:         mrads_send_transaction_statistics
 *
...
/*BYTE mrads_send_transaction_statistics(pPTE_MSG p_msg )
{
   ...
   ret_code =  db_select_mra01(&db_recs.mra01,err_msg);*/
	...
	{*/
		//(void)mrads_reset_stats( &Txn_Counts );
                                   ...
                                   pCHAR appname, pCHAR errbuf )
{
   ...
   INT   ret_val = true;
   CHAR  mrads_error_warning_file_size[5] = {0};
   ...
   rc = GetPrivateProfileString(
							  ...
							  "500",  					/* Default string                 */
							  mrads_error_warning_file_size,              	/* points to destination buffer   */
							  sizeof(mrads_error_warning_file_size)-1,   	 /* size of destination buffer     */
							  filename                  /* points to ini filename         */
							);

	/* File size size conversion */
	Max_File_Size_Defined = atoi(mrads_error_warning_file_size);
