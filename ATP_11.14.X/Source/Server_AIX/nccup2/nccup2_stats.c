 ...
 * All Rights Reserved.
 *
 * MODULE:      nccup2_stats.c
 *  
 * TITLE:       CUP Network Controller Statistics Processor
 *  
...
INT send_transaction_statistics( pPTE_MSG p_msg )
{
   ...
   if ( false == ret_code  )
   {
      nccup2_log_message( 2, 2, err_msg, "send_transaction_statistics",0 );
   }
   return( ret_code );
}
...
INT reset_transaction_statistics( pPTE_MSG p_msg )
{
   ...
   if ( false == ret_code  )
   {
      nccup2_log_message( 2, 2, err_msg, "reset_transaction_statistics",0 );
   }
   return( ret_code );
}
