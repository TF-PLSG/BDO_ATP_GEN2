 ...
 * All Rights Reserved.
 *
 * MODULE:      nccup2_memory.c
 *  
 * TITLE:       CUP Network Controller Shared Memory Processor
 *  
         ...
         if ( true == ret_val )
         {
            ...
            if ( 0 == strcmp(host_state, ONLINE) )
            {
               ...
               if ( false == set_echo_timer(err_buf) )
                  nccup2_log_message( 1, 2, err_buf, "set_host_state",0 );
            }
...
INT increment_active_count( pCHAR errbuf )
{
   ...
   if ( true == ret_val )
   {
      ...
      if ( (tempint >= MaxActiveTxns) && (MaxActiveTxns > 0) )
      {
         ...
         nccup2_log_message( 1, 2, errbuf, "increment_active_count" ,0);
      }
      else
      {
