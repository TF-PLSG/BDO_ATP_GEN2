...
//praneeth    update_txn_stats( &Txn_Counts, (pAUTH_TX)NULL_PTR, RECEIVED );
   ...
   if ( false == reset_echo_timer(err_buf) )
      nccup2_log_message(2, 2, err_buf, "process_host_msg",0 );
   ...
   copy_to_req_resp_buffer( p_msg_in );
	...
	{ sprintf(err_buf,"Invalid length or Big msg recvd from Host:  %04d", buflen);
      nccup2_log_message( 1, 2, err_buf, "process_host_msg",0 );
	  return;
	}
...
///////////////////////////////////////////////////////////////////////////////
    ...
    if(AUTH_INVALID == Auth_Tx.TLF01_details.tx_key)
   {
      ...
      nccup2_log_message( 1, 3, Buffer, "process_request_from_host",1 );
      return(true);
   }
   ...
   if(0 == strcmp(Auth_Tx.TLF01_details.message_type, "0820"))
   {
	  ...
	  else if(0 == strncmp(NMI_Code,"101", 3))
   	   {
   	   		if(true == perform_host_logon_request(LOGON_NMI_KEY_RESET_REQUEST,SOURCE_IS_HOST,&Auth_Tx))
   	   		{
				Get_Key_from_ncf01_table();
				if(0!=strlen(static_Rtdcwk_Key))
				{
					perform_host_key_send();
					nccup2_log_message( 1, 1, "Sending Key to Host",
										"process_request_from_host",0 );
				}
				else
				{
			...
			{
				nccup2_log_message( 1, 3, "Host Key change request is failed",
									"process_request_from_host",0 );
			}
   	   }
	  return(true);
   ...
   else if((0 == strcmp(Auth_Tx.TLF01_details.message_type, "0830")) || 
   	        (0 == strcmp(Auth_Tx.TLF01_details.message_type, "0810")))
   {  
  	  strcpy( Auth_Tx.TLF01_details.handler_queue,application_que_name);
      ...
      (Auth_Tx.TLF01_details.tx_key <= AUTH_INVALID))
   {
      ...
      if ( true == txn_is_blocked(&Auth_Tx,Buffer) )
      {
    	 ...
    	 nccup2_log_message( 1, 2, Buffer, "process_request_from_host",1 );
...
void process_request_for_host( pPTE_MSG p_msg_in )
{
   ...
   if( 0 == strncmp(Auth_Tx.TLF01_details.handler_queue,"dcpiso",6) )
   {
		...
		{
			strcpy(Auth_Tx.TLF01_details.source_key, Ncf01_Nccup_A.cwk ) ;// Storing Source key
			strcpy(Auth_Tx.TLF01_details.dest_key, Ncf01_Nccup_I.retired_cwk ) ;// Storing Destination key
	   ...
	   if ( (!is_reversal()) && pin_block_exists() && true == verify_host_status(err_buf) )
		{
			...
			memcpy(Auth_Tx.TLF01_details.dest_pin_format, Ncf01_I.pin_type,1) ;// Destination pin format
			(void)translate_pin();
		}
		else
		{
   ...
   else if( 0 == strncmp(Auth_Tx.TLF01_details.handler_queue,"nccup3",5)) // Issuing transaction coming from nccup/nccup3
   {
		if ( (!is_reversal()) && pin_block_exists() && false == verify_host_status(err_buf))
		{
			//This is ATP STIP with Incoming transaction with Pin
			strcpy(Auth_Tx.TLF01_details.source_key, Ncf01_Nccup_I.retired_cwk ) ;// Storing Source key
			strcpy(Auth_Tx.TLF01_details.dest_key,  Ncf01_I.cwk ) ;// Storing Destination key
			memcpy(Auth_Tx.TLF01_details.source_pin_format, temp_source_pin_format,1)  ;// Source pin format
			memcpy(Auth_Tx.TLF01_details.dest_pin_format, temp_dest_pin_format,1) ;// Destination pin format
		    (void)translate_pin();
...
void process_request_for_host_after_security()
{
   ...
   switch ( tx_key )
   {
      ...
      case AUTH_REVERSAL_ADVICE:
         ...
         nccup2_log_message( 3, 2, Buff, "process_request_for_host_after_security",1 );
	  ...
	  case AUTH_OFFLINE_SALE:
	 		  ret_val = perform_standin_processing( CUP_APPROVED,
	 											    ...
	 											    err_buf );
	 	   if ( false == ret_val )
	 		  nccup2_log_message(2,2,err_buf,"process_request_for_host_after_security",0);
      ...
      default:
         ...
         sprintf( err_buf,
                 "Invalid txn. Cannot process. TX_KEY = %d",
                  tx_key );
         nccup2_log_message( 1, 2, err_buf,
...
void process_request_for_host_after_currency()
{
   ...
   CHAR err_buf[100]= {0};
	...
	if (true == nccup2_check_if_txn_is_upi_3ds2_iss_trn() &&
			Cup3ds2_Flag == 1)
	{
		   ret_val = perform_standin_processing( "01",GS_TXN_DROPPED,err_buf );
	}
	else
	{
...
/////////////////////////////////////
   ...
   if ( false == ret_val )
   {
	 ...
	 // {
		  // Copy Source key. This source key will be used for PIN validation during STIP

		  if( 0 == strncmp(Auth_Tx.TLF01_details.handler_queue,"dcpiso",5))
		  strcpy(Auth_Tx.TLF01_details.source_key, Ncf01_Nccup_A.cwk ) ;// Storing Source key
		  else
      ...
      if ( false == ret_val )
      {
         nccup2_log_message(2,2,err_buf,"process_request_for_host_after_currency",0);
      }
   }
   else
   {
...
void process_response_from_host()
{
   ...
   if ( false == decrement_active_count( err_buf ) )
      nccup2_log_message(1, 3, err_buf, "process_response_from_host",1 );
   ...
   if(Auth_Tx.TLF01_details.tx_key != AUTH_INVALID)
   {
      ...
      else
      {
         ...
         if ( false == clear_request_timer(lookup_key) )
         {
            ...
            nccup2_log_message(1, 2, err_buf, "process_response_from_host",0 );
         }
      }
   }
   return;
      ...
      if( ptemsg_get_result_code( p_msg_in ) == PTEMSG_NOT_FOUND )
      {
         ...
         sprintf( err_buf,
                 "Unable to retrieve timer data for lookup key %s",
                  lookup_key );
         nccup2_log_message( 1, 3, err_buf, "process_clear_timer_reply",0);
      }
      ...
      else if ( timer_reply_size != Auth_Tx_Size )
      {
         sprintf( err_buf,
                 "Did not receive auth_tx data with timer reply. Key = %s",
                  lookup_key );
         nccup2_log_message( 1, 2, err_buf, "process_clear_timer_reply",0 );
      }
      else
      {
         ...
         if ( data_len != Auth_Tx_Size )
         {
            sprintf( err_buf,
                 "Did not receive chained host response data with timer reply. Key = %s",
                  lookup_key );
            nccup2_log_message( 2, 2, err_buf, "process_clear_timer_reply",0 );
         }
         ...
         else
         {
            ...
            else if ( false == send_msg_to_authorizer(MT_AUTH_RESPONSE) )//MT for RT approved, MT for RTdecline for txcntl
            {
               sprintf( err_buf,
...
/*
	...
	if ( false == ret_val )
   {
    	nccup2_log_message( 1, 2, err_msg, "process_1304_from_host",0 );
   }*/
...
INT delete_orig_ncf30( NCF30 ncf30, pAUTH_TX p_auth_tx )
{
   ...
   if ( NULL_PTR == p_msg  )
   {
	   nccup2_log_message( 1, 2, "Insufficient Memory to build NCF30 message",
			   	   	   	   	   	   "delete_orig_ncf30",1 );
      return( false );
   }
   ...
   if ( 0 > ret_code )
   {
      ...
      nccup2_log_message( 1, 3, Buff,"delete_orig_ncf30",1 );
      return( false );
   }
   return(true);
 }
 ...
 BOOLEAN process_logon_response(CHAR nmi_code[4],pAUTH_TX p_auth_tx)
 {
	...
	if (strcmp(LOGON_NMI_LOGON ,p_auth_tx->function_code) == 0)
	{
	   if ( strcmp( p_auth_tx->TLF01_details.response_code, "00" ) )
	   {
		  ...
		  nccup2_log_message( 1,1,"LOGON Request successful.","process_logon_response",0 );
	   }
	   else
	   {
		   nccup2_log_message( 1,1,"LOGON Request DENIED.","process_logon_response",0 );
	   }
	}
	...
	else if (strcmp(LOGON_NMI_LOGOFF,p_auth_tx->function_code) == 0)
	{
	   if ( strcmp( p_auth_tx->TLF01_details.response_code, "00" ) )
	   {
		  ...
		  update_host_status = true;
		  nccup2_log_message( 1,1,"LOGOFF Request successful.","process_logon_response",0 );
	   }
	   else
	   {
		   nccup2_log_message( 1,1,"LOGOFF Request DENIED.","process_logon_response",0 );
	   }
	}
	...
	else if (strcmp(LOGON_NMI_ECHO_TEST,p_auth_tx->function_code) == 0)
	{
	   if ( strcmp( p_auth_tx->TLF01_details.response_code, "00" ) )
	   {
		  strcpy(current_state, "1");
		  update_host_status = true;
		  nccup2_log_message( 1,1,"ECHO Request successful.","process_logon_response",0 );
	   }
	   else
	   {
		   nccup2_log_message( 1,1,"ECHO Request DENIED.","process_logon_response",0 );
	   }
	}
	else if (strcmp(LOGON_NMI_KEY_RESET_REQUEST,p_auth_tx->function_code) == 0)
	{
	   if ( strcmp( p_auth_tx->TLF01_details.response_code, "00" ) )
	   {
		   nccup2_log_message( 1,1,"Key Exchange Success successful.","process_logon_response",0 );
	   }
	   else
	   {
		   nccup2_log_message( 1,1,"Key Exchange Request DENIED.","process_logon_response",0 );
	   }
	   return true;
	}
 ...
 ******************************************************************************/
 INT process_incoming_message(pPTE_MSG p_msg)
 {
		  ...
		  (Auth_Tx.TLF01_details.tx_key > AUTH_INVALID))  /* a response */
	   {
		 ...
		 /* Call function for TLF logginglogging
		  ...
		  if((0 == strcmp(Auth_Tx.TLF01_details.response_code, "00")) || (0 == strcmp(Auth_Tx.TLF01_details.response_code, "55")))
		  {
			  ...
			  {
				  nccup2_log_message( 1,3,"Unable to update database tables.",
				 ...
				 {
					 nccup2_log_message( 1,3,"Unable to send response to host.",
					 				  		 "process_incoming_message",1 );
				 }
			  }
		  ...
		  else
		  {
			  ...
			  */
			  if(false == incoming_other_host_response_db_reply())
			  {
				  nccup2_log_message( 1,3,"Unable to send response to host system.",
				 					 		"process_incoming_message",1 );
			  }
		  }
		  return(true);
	...
	*/
	   ...
	   {
		   nccup2_log_message( 1,3,"Failed to process debit transaction.",
		   				 			"process_incoming_message",1 );
		   perform_error_processing( &Auth_Tx );
		   strcpy(Auth_Tx.TLF01_details.response_code,"91");
		   Auth_Tx.TLF01_details.tx_key =
 ...
 ******************************************************************************/
 BOOLEAN send_it_back_to_terminal(BYTE resp_code[3], pAUTH_TX p_auth_tx )
 {
	...
	if ( NULL_PTR == p_msg_term_handler_out )
	 {
		nccup2_log_message( 1,3,"Insufficient Memory to build response to txcntl.",
	...
	if ( ret_code < 0 )
	{
		 ...
		 nccup2_log_message( 1,3,Buffer,"send_it_back_to_terminal",1 );
		 free(p_msg_term_handler_out);
		 return(false);
	}
	else
	{
		 nccup2_log_message( 1,1,"Response sent successfully to terminal handler",
				 	 	 	 	 "send_it_back_to_terminal",1 );
	}
 ...
 ******************************************************************************/
 BOOLEAN hhutil_process_debit_transaction(pAUTH_TX p_auth_tx)
 {
	...
	if ( NULL_PTR == p_msg )
	 {
		nccup2_log_message( 1,3,"Insufficient Memory to build pin translation message",
	 ...
	 if( ret_code < 0 )
	 {
		...
		nccup2_log_message( 1,3,Buffer,"hhutil_process_debit_transaction",1 );
		free( p_msg );
		return(false);
	 }
 ...
 INT incoming_terminal_request(pAUTH_TX p_auth_tx)
 {
	...
	if (false == verify_host_app_status(p_auth_tx))
	{
	   /* build response_for_terminal */
		nccup2_log_message( 1,1,"Host Inactive",
								"incoming_terminal_request",0 );
		p_auth_tx->TLF01_details.tx_key = match_request_to_response(p_auth_tx->TLF01_details.tx_key);
 
		if (0 == strcmp(current_state,"2"))  /* check if we are error state */
	   {
			nccup2_log_message( 1,2,"Host Inactive: current_state!=0",
									"incoming_terminal_request",0 );
	...
	switch (source)
	 {
		 case SOURCE_IS_FSVISA:
		 {
			 ...
			 {
				 nccup2_log_message( 1,1,"Sending logon request to host:",
		 ...
		 case SOURCE_IS_STATE:
		 {
			 ...
			 {
				 nccup2_log_message( 1,1,"Sending logon ECHO request to host:",
				 				 			"perform_host_logon_request",0 );
				 p_auth_tx->TLF01_details.tx_key = AUTH_LOGON;
	   ...
	   case SOURCE_IS_OPERATOR:
	   {
		 ...
		 case SOURCE_IS_HOST:
		 {
			 ...
			 if (strcmp(LOGON_NMI_START , request_type)==0)
			 {
				 ...
				 {
					 nccup2_log_message( 1,1,"Received logon request from host:",
					 				 		 "perform_host_logon_request",0 );
					 p_auth_tx->TLF01_details.tx_key = AUTH_LOGON_RESPONSE;
					 strcpy(vLogon_NMI_Code, LOGON_NMI_START);
					 // Logon Only if Key exchange is successful between nccup2 and Cadencie
				 ...
				 {
					 nccup2_log_message( 1,1,"Received logon request from host, Already Online:",
					 					 		"perform_host_logon_request",0 );
					 p_auth_tx->TLF01_details.tx_key = AUTH_LOGON_RESPONSE;
					 strcpy(vLogon_NMI_Code, LOGON_NMI_START);
					 perform_op_net_mngmnt_response(request_type, p_auth_tx);
			 ...
			 {
				 nccup2_log_message( 1,1,"Received logoff request from host:",
				 					 		"perform_host_logon_request",0 );
				p_auth_tx->TLF01_details.tx_key = AUTH_LOGON_RESPONSE;
			 ...
			 {
				 nccup2_log_message( 1,1,"Received UnionPay starts date switch request from host:",
										 "perform_host_logon_request",0 );
				 p_auth_tx->TLF01_details.tx_key = AUTH_LOGON_RESPONSE;
			 ...
			 {
				 nccup2_log_message( 1,1,"Received UnionPay ends date switch request from host:",
				 						  "perform_host_logon_request",0 );
				 p_auth_tx->TLF01_details.tx_key = AUTH_LOGON_RESPONSE;
		     ...
		     else if (strcmp(LOGON_NMI_KEY_RESET_REQUEST , request_type)==0)
			 {
		    	 nccup2_log_message( 1,1,"Received Key request reset:",
		    	 				 		 "perform_host_logon_request",0 );
		    	 p_auth_tx->TLF01_details.tx_key = AUTH_LOGON_RESPONSE;
				 strcpy(vLogon_NMI_Code, LOGON_NMI_CUP_KEY_RESET_REQUEST_INDICATOR);
				 perform_op_net_mngmnt_response(request_type, p_auth_tx);
			 }
			 else
			 {
				 if (0 == atoi(current_state))
				 {
					 //nccup2_log_message( 1,1,"Received network request from host:",
					 //		    	 		  "perform_host_logon_request",0 );
					 p_auth_tx->TLF01_details.tx_key = AUTH_LOGON_RESPONSE;
					 strcpy(vLogon_NMI_Code, LOGON_NMI_ECHO_TEST);
 ...
 ******************************************************************************/
 INT  verify_host_app_status(pAUTH_TX p_auth_tx)
 {
	...
	{
		nccup2_log_message( 1,3,"Host app not in state 0, cannot process tran",
	...
	if (2 == atoi(current_state))  
	{  
		nccup2_log_message( 1,2,"Maximum consecutive timeouts reached",
										"verify_host_app_status",1 );
	...
	{
		nccup2_log_message( 1,2,"Maximum active transactions reached",
								"verify_host_app_status",1 );
 	    return(false);
	}
 ...
 INT perform_op_net_mngmnt_request( char request_type[], pAUTH_TX p_auth_tx )
 {
	...
	{
		nccup2_log_message( 1,1,"Sending LOGOFF Request to host",
								"perform_op_net_mngmnt_request",0 );
 ...
 ******************************************************************************/
 int perform_op_net_mngmnt_response(char nmi_code[], pAUTH_TX p_auth_tx)
 {
	...
	{
		nccup2_log_message( 1,1,"Sending LOGON Response to host",
	...
	{
		nccup2_log_message( 1,1,"Sending LOGOFF Response to host",
								"perform_op_net_mngmnt_response",0 );
	}
	else if (0 == strcmp(nmi_code,LOGON_NMI_KEY_RESET_REQUEST))
	{
		nccup2_log_message( 1,1,"Sending Key Reset request Response to host",
								"perform_op_net_mngmnt_response",0 );
	}
	else 
	{
		nccup2_log_message( 1,1,"Sending ECHO Response to host",
								"perform_op_net_mngmnt_response",0 );
	}
