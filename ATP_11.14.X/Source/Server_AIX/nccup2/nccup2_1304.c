...
INT process_file_update_request( pPTE_MSG p_msg )
{
   ...
   if ( ret_val == false )
   {
      strcpy( Auth_Tx.accptor_bus_code, CUP_FIELD_EDIT_ERROR );
      issue_1304_response( CUP_FORMAT_ERROR, err_msg );
   }
   else
   {
      ...
      if ( false == ret_val )
      {
         ...
         issue_1304_response( CUP_HOST_UNAVAILABLE, err_msg );
      }
      else
      {
...
INT send_response_to_GUI()
{
   ...
   if ( false == ret_val )
   {
	   nccup2_log_message( 1, 3, err_msg, "send_response_to_GUI",0 );
   }
   return( ret_val );
