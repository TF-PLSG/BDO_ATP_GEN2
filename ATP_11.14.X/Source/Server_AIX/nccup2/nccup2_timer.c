      ...
      if ( timer_reply_size != Auth_Tx_Size )
	   {
         sprintf( err_buf,
                 "Error retrieving auth_tx from timer message. Lookup key = %s",
                  lookup_key );
         nccup2_log_message( 1, 2, err_buf, "process_timeout_msg",0 );
	   }
	   else
      {
         ...
         strcpy( Auth_Tx.TLF01_details.response_code, ISO_TO );
         ...
         strcpy( err_buf,"Txn to CUP Issuer Host(Vertexon) timed out" );
         nccup2_log_message( 1, 1, err_buf, "process_timeout_msg",1 );
            	   ...
            	   if(0 ==strncmp(Auth_Tx.TLF01_details.handler_queue,"sbatch",6))
            	     {
            	   			...
            	   			sprintf(Log_Bufr," timeout  Advice_Sbatch Transaction with MSG_TYPE:%s STAN:% Term Id:%s ,Merch Id: %s RRN: %s ",
            	   			...
            	   			nccup2_log_message(2, 1, Log_Bufr,
         ...
         if ( false == increment_timeout_count(err_buf) )
         {
             ...
             */
        	 ...
        	 if(GetTimeoutCount() == (MaxConsecTimeouts+1))
        		 nccup2_log_message( 3, 2, err_buf, "process_timeout_msg",0 );
        	 #else
			 nccup2_log_message( 1, 2, err_buf, "process_timeout_msg",1 );
			 #endif
         ...
         else
         {
            ...
            if ( is_reversal() )
            {
               /* This is a reversal. Resend it to the host. */
         	   if(rvrsl_attmpts_value_fr_timeout_cup<=0)
         	   {
         		   rvrsl_attmpts_value_fr_timeout_cup=atoi(Ncf01_I.advice_repeat_counter);
         	   }

         	   if(rvrsl_attmpts_value_fr_laterspns_cup <=0)
         	   {
         		   rvrsl_attmpts_value_fr_laterspns_cup=atoi(Ncf01_I.advice_repeat_counter);
         	   }

         	   if(0 == strncmp(Auth_Tx.TLF01_details.response_text,"LR",2))
         	   {
         		  max_repeats=rvrsl_attmpts_value_fr_laterspns_cup;
         	   }
         	   else
         	   {
         		  max_repeats=rvrsl_attmpts_value_fr_timeout_cup;
         	   }
...
INT send_timeout_response( pCHAR err_buf )
{
   ...
   ret_val = perform_standin_processing( ISO_TO, GS_TXN_TIMEDOUT, err_buf );
   if (ret_val == true )
...
void transform_0200_into_0420()
{
   ...
   strcpy( Auth_Tx.TLF01_details.response_code, CUP_TIMEOUT );
...
INT set_timer( pCHAR lookup_key, INT timer_length, INT type_of_timer )
{
   ...
   if ( false == ret_val )
      nccup2_log_message( 2, 2, err_buf, "set_timer",0 );
...
INT clear_timer( pCHAR lookup_key )
{
   ...
   if ( false == ret_val )
      nccup2_log_message( 2, 2, err_buf, "clear_timer",0 );
...
INT clear_request_timer( pCHAR lookup_key )
{
   ...
   else
   {
      ...
      p_chain_msg = ptemsg_chain_new_message( p_timer_msg,
                                              MT_DB_REQUEST, //timer reply
                                              ST1_DB_ECHO,	//Approve, decline, timeout
                                              ST2_NONE,
                                              timerds_que_name,
...
INT set_echo_timer( pCHAR err_buf )
{
   ...
   if ( Echo_Timer_Flag == CUP_ECHO_TIMER_IS_CLEARED )
   {
      echo_timer_seconds = atoi( Ncf01_I.echo_interval );
      if ( echo_timer_seconds > 0 )
      {
         ...
         if ( ret_val == true )
         {
            if( 0 == strcmp( current_state, ONLINE ) )
            {
               ...
               if (false == set_timer(echo_timer_key,echo_timer_seconds,ECHO_TIMER))
               {
                  ...
                  Echo_Timer_Flag = CUP_ECHO_TIMER_IS_SET;
            }
...
INT clear_echo_timer( pCHAR err_buf )
{
   ...
   if ( Echo_Timer_Flag == CUP_ECHO_TIMER_IS_SET )
   {
      echo_timer_seconds = atoi( Ncf01_I.echo_interval );
      if ( echo_timer_seconds > 0 )
      {
   ...
   switch( tx_key )
   {
      ...
      case AUTH_ATM_CONFIRM_RESPONSE:
      {
         ...
         memcpy( key_last_six, &Auth_Tx.TLF01_details.card_num[pan_len - 6], 6);
            ...
            /* For POS Reversals, use Merchant ID (TID doesn't go to CUP. */
  	  ...
  	  case AUTH_ADVICE_REPEAT_RESPONSE:
      {
         ...
         strcpy( temp_mid, Auth_Tx.TLF01_details.terminal_id);
         trim_trailing_spaces( temp_mid );
