...
extern CHAR  nccup2_Erro_warning_Filename[256] ;
extern CHAR  nccup2_module_error_warning_file_name[256];
extern CHAR  nccup2_error_warning_file_path[256];
UINT Max_File_Size_Defined = 0 ;
/*Flags*/
extern INT extendedbit;
extern INT Txn_Is_A_Reject;

INT rvrsl_attmpts_value_fr_timeout_cup=0;
INT rvrsl_attmpts_value_fr_laterspns_cup=0;
INT Cup3ds2_Flag=0;
       ...
       ((send_flag == SEND_IF_NOT_REVERSAL) && (false == is_reversal() )) )
   {
      ...
      if ( true == build_response_msg() )
      {
		 ...
		 {	
			...
			sprintf (err_buf, "Sending CUP Host response to Cadencie, message type :%s", 
					Auth_Tx.TLF01_details.message_type) ;
			nccup2_log_message( 2, 1, err_buf, "send_response_to_host",1 );
		 }      
         if ( false == send_message_to_host(err_buf,false) )//praneeth added false as parameter
            nccup2_log_message(1, 2, err_buf, "send_response_to_host",1 );
      }
...
void send_request_to_host( pCHAR timer_resp_flag )
{
   ...
   if ( true == ret_val )
   {
		...
		/*Send whatever msg coming from CUP HOST. Make sure we have valid msg from HOST*/
		 if(0 == strcmp(Auth_Tx.TLF01_details.dispensation.origin,"RO"))
		 {
		 	if(0 == strncmp(Auth_Tx.TLF01_details.handler_queue,"nccup",5))
		 	{
				...
				{	buflen = buf_temp_len;
					...
					sprintf (mesbuf, "Sending Message coming from CUP Host to Cadencie, message type :%s", 
					Auth_Tx.TLF01_details.message_type) ;
					strcat( err_buf,mesbuf);
					nccup2_log_message( 2,1, err_buf, "send_request_to_host",0 );
				}
   ...
   if ( false == ret_val )
   {
       ...
       */
      nccup2_log_message( 1, 2, err_buf, "send_request_to_host",0 );
      ret_val = perform_standin_processing( CUP_SYSTEM_ERROR,
                                            GS_TXN_DROPPED,
                                            err_buf );
      if ( false == ret_val )
         nccup2_log_message( 2, 2, err_buf, "send_request_to_host",0 );
   }
...
INT send_msg_to_authorizer( BYTE msgtype )
{
   ...
   if ( ret_val == false )
   {
      ...
      nccup2_log_message( 1, 2, Buffer, "send_msg_to_authorizer",1 );
   }
...
INT determine_tx_key( )
{
   ...
   if ( ret_val == false )
   {
      ...
      nccup2_log_message( 1, 2, err_buf, "determine_tx_key",1 );
...
struct trandef *Find_Tran_Table( pAUTH_TX p_auth_tx )
{
   ...
   nccup2_log_message( 2, 2, Buffer, "FIND_TRAN_TABLE",0 );
...
struct bitfield_data *Find_Bitfield_Table( enum bitfields bitfield )
{
   ...
   sprintf(temp_str, "This bitfield not found in BFData, bitfield: %d",
                      bitfield);
   nccup2_log_message( 2, 2, temp_str, "Find_Bitfield_Table",0 );
   return NULL;
...
INT build_host_msg( pCHAR err_buf )
{
   ...
   if(0 == strncmp(Auth_Tx.TLF01_details.message_type, "0800",4))
   {
		ReqRsp_Buffer[5] = CUP_MSG_HEADER_FLAG_KEY_SEND;
   }
   else
   {
   		ReqRsp_Buffer[5] = CUP_MSG_HEADER_FLAG_PROD_VER;
   }

   // We need to do this only for internal testing.
  ...
  /* Destination ID*/
   ...
   {
	   //strncpy(temp_destination_id,"00010344   ",DEST_ID_LEN);
	   strncpy(temp_destination_id,"27240608   ",CUP_DEST_ID_LEN);
   }
   ...
   if(source_id_len == 0)
   {
	   strncpy(temp_source_id,"00010344   ",CUP_SOURCE_ID_LEN);
   }
  ...
  memcpy( &ReqRsp_Buffer[21],temp_source_id,CUP_SOURCE_ID_LEN);
   /* Destination ID*/
  // memcpy( &ReqRsp_Buffer[10],"27240608   ",DEST_ID_LEN);

   /*Source ID */
  // memcpy( &ReqRsp_Buffer[21],"00010344   ",DEST_ID_LEN);
   ...
   if((0 == strncmp(Auth_Tx.TLF01_details.processing_code,"01",2)) &&
   	...
   	{
   		/* We Need to change the Message type from 0200 to 0100 as we are using same key for 
   		Message type 200 with Processing code 01 
   		Message type 0100 with Processing code 01
   ...
   if ((0 == strncmp(Auth_Tx.TLF01_details.handler_queue, "nceqit", 6) ||
		...
		0 == strncmp(Auth_Tx.TLF01_details.processing_code, "01", 2) &&
	    0 == strncmp(Auth_Tx.TLF01_details.message_type, "0200", 4))
   {
	   /*For Cash Adv coming from BDO ATM(IST), change msg type from 2 to 1*/
	   ReqRsp_Buffer[51]= msg_chg[0];
   }
...
INT build_host_structure( pCHAR err_buf )
{
    ...
    /* SWITCHED OUT Transactions */
      if (temp_message_type[1] == '1') 
      {
         if(temp_message_type[2] == '0') 
         {
			...
			(void)build_request_field_117(  &result_code, err_buf );
   ...
   if ( (result_code == 1) &&
   	  ...
   	  (false == verify_host_status( err_buf )) )
   	{
    	ret_val = false;
   	}
...
INT parse_host_msg( pCHAR err_buf )
{
   ...
   if (0x2E == ReqRsp_Buffer[4])
   {
    ...
    else if (0 == strncmp( ReqRsp_Buffer, "0000", 4))
	{
	   ...
	   nccup2_log_message( 1, 3, ErrorMsg, "parse_host_msg",0 );
...
INT populate_auth_tx( pCHAR err_buf )
{
   ...
   if ( TranFlow == SWITCHED_IN )
   {
	  ...
	  {
		 /* Move fields necessary for 0420 POS reversal RESPONSE messages. */
		  (void)move_field_2(  &result_code, err_buf );
		  (void)move_field_3(  &result_code, err_buf );
		  (void)move_field_4(  &result_code, err_buf );
   ...
   else  /* SWITCHED OUT Transactions */
   {
      ...
      if (Auth_Tx.TLF01_details.message_type[1] == '1') 
      {
         if(Auth_Tx.TLF01_details.message_type[2] == '1')
         {
      	...
      	{
	  	 		/* Move fields necessary for 0130 request messages. */
	  		(void)move_field_2(  &result_code, err_buf );
	        (void)move_field_3(  &result_code, err_buf );
	      	(void)move_field_4(  &result_code, err_buf );
	        (void)move_field_5(  &result_code, err_buf );
...
INT build_response_msg()
{
   ...
   /* Log any errors. */
   if ( ret_val == false )
      nccup2_log_message( 1, 2, err_buf, "build_response_message",1 );
...
INT build_request_msg( pCHAR err_buf )
{
   ...
   if ( true == (ret_val = build_host_structure(err_buf)) )
   {
	  // TF Phani ..change the tx_key .. Changeing to VOID for VOICE reversals.
	  if(((Auth_Tx.TLF01_details.tx_key == AUTH_REVERSAL_ADVICE) ||
		 ...
		 (strcmp(Auth_Tx.TLF01_details.response_code, ISO_TO) != 0) 	&&
		 (strcmp(Auth_Tx.TLF01_details.response_code, CUP_TIMEOUT) != 0)
		)
	  { 
			Auth_Tx.TLF01_details.tx_key = AUTH_VOID_SALE;
			memcpy(HOST_MESSAGE.proccode,"200000",CUP_DE3_SIZE );
	  } 
	  ...
	  if ((Auth_Tx.TLF01_details.tx_key == AUTH_REVERSAL_ADVICE ||
		  ...
		  (0 == strncmp(Auth_Tx.TLF01_details.handler_queue, "nceqit", 6) ||
		   0 == strncmp(Auth_Tx.TLF01_details.handler_queue, "neqit2", 6)))
	  {
		  strncpy(Auth_Tx.TLF01_details.acquirer_id, "nccup2", 6);
		  strncpy(Auth_Tx.TLF01_details.authorizing_host_queue, "nccup2A", 7);
	  }
 ...
 *
 ******************************************************************************/
void nccup2_log_message( INT dest, INT sev, pCHAR msg, pCHAR func, int details )
{
   INT  monitor_flag;
   INT  eventlog_flag;
   INT  msg_type;
   ...
   nccup2_Log_error_warning_to_File(text_message,sev,func,details);
...
INT send_network_status_to_monitor()
{
   ...
   if (ConnectionId[0] != 0)
   {
      if ( NetworkType == CUP_ACQUIRER  ||  NetworkType == CUP_BOTH )
      {
         ...
         if ( ret_code == false )
         {
            nccup2_log_message( 2, 2, err_msg, "send_network_status_to_monitor",0 );
         }
      }
   ...
   {
      nccup2_log_message( 2, 2, "Unable to get connection info for Monitor.",
...
INT send_message_to_host( pCHAR err_buf,INT mgt ) //praneeth added bool mgt as parameter
{
   ...
   ret_val = build_and_send_request_message( MT_OUTGOING,
                                             ...
                                             buffer );
   /* Debug info */
		 /*strcpy(message, "Messsage send to TPDU ID:"); 
		 strcat( message, host_tpdu_id );
     nccup2_log_message( 2, 1, message, "send_message_to_host" );*/
   ...
   else
   {
      ...
      if ( Auth_Tx.TLF01_details.tx_key < AUTH_INVALID )
      {
         ...
         if ( false == increment_active_count(err_buf) )
            nccup2_log_message(1, 2, err_buf, "send_message_to_host",1 );
      }

      /* Cycle the echo timer because traffic went out over the line. */
      if ( false == reset_echo_timer(err_buf) )
         nccup2_log_message(2, 2, err_buf, "send_message_to_host",0 );
   }
   return( ret_val );
}
   ...
   if ( ret_val == true )
   {
      ...
      if ( MaxConsecTimeouts > 0 )
      {
         if (atoi(net_consec_tmouts_ctr) > MaxConsecTimeouts )
         {
            ...
            nccup2_log_message( 2, 2, err_buf, "verify_host_status",1 );

            ret_val = false;
            if ( false == set_host_state(OFFLINE,err_buf) )
               nccup2_log_message( 2, 3, err_buf, "verify_host_status",1 );
         }
      }
      ...
      if ( (MaxActiveTxns > 0) && (ret_val == true) )
      {
         if( atoi(active_txns_ctr) >= MaxActiveTxns )
         {
            ...
            nccup2_log_message( 1, 2, err_buf, "verify_host_status",1 );
         }
      }
   }
   return( ret_val );
      ...
      if ( rc < 0 )
      {
         ...
         sprintf( err_buf,
                 "Error logging entry to Txn Trace File = %s",
                  TraceFile );
         nccup2_log_message( 2, 1, err_buf, "process_host_msg",0 );
         if ( rc == -1 )
            strcpy( err_buf,
...
BYTE hhutil_process_host_request(pAUTH_TX p_auth_tx)
{
        ...
        (AUTH_ADVICE          == p_auth_tx->TLF01_details.tx_key)  )
   {
      if ( AUTH_REVERSAL_ADVICE == p_auth_tx->TLF01_details.tx_key )
      {
         ...
         nccup2_log_message( 2, 1, Buff, "hhutil_process_host_request",1 );
      }
      else
      {
      ...
      {
    	  nccup2_log_message( 1, 3, "Insufficient Memory for select ncf30 message",
      ...
      if ( 0 > ret_code )
      {
         ...
         nccup2_log_message( 1, 3, Buff,"hhutil_process_host_request",1 );
         return( false );
      }
      return( true );
   }
   ...
   {
	   nccup2_log_message( 1, 3, "Unable to send the request to the authorizer application",
			   	   	   	   	   	   "hhutil_process_host_request",1 );
       return(false);
   }
   return(true);
...
BOOLEAN hhutil_transfer_ctl_to_auth_app ( pAUTH_TX p_auth_tx)
{
   ...
   if ( NULL_PTR == p_msg_host_out )
	{
	   nccup2_log_message( 1, 3, "Insufficient Memory to build host message",
	...
	if( ret_code < 0 )
	{
	   ...
	   nccup2_log_message( 1,3,Buffer,"hhutil_transfer_ctl_to_auth_app",1 );
	   free( p_msg_host_out );
	   return(false);
	}
...
BOOLEAN process_logon_request(CHAR nmi_code[4], pAUTH_TX p_auth_tx)
{
   ...
   if ( 0 != strcmp(LOGON_NMI_ECHO_TEST, p_auth_tx->function_code) )
   {
      ...
      nccup2_log_message( 1,1,info_buffer,"process_logon_request",1 );
   }
...
INT incoming_other_host_response(pAUTH_TX p_auth_tx)
{
...

   {
      ...
      if ( false == ret_code )
      {
    	  nccup2_log_message( 1,2,"Unable to delete reversal's original txn in NCF30",
...
INT incoming_other_host_response_db_reply( )
{
   ...
   if (true == build_host_msg(err_buf))
   {
	  		...
	  		)
		{
			...
			{	buflen = buf_temp_len;
				memcpy(ReqRsp_Buffer,Auth_Tx.resp_buffer,buflen);
				nccup2_log_message( 1,1,"Sending CUP HOST response to Cadencie",
				    			  	  	 "incoming_other_host_response",1 );
   ...
   {
	   nccup2_log_message( 1,3,"Error building response for the host",
	   				    		"incoming_other_host_response",1 );
      return(false);
   }
   ...
   if (AUTH_REVERSAL_ADVICE_RESPONSE == Auth_Tx.TLF01_details.tx_key)
   {
      ...
      strcpy(Auth_Tx.TLF01_details.currency_code, PESO);
   }
...
BYTE  get_tran_type_2(BYTE tx_key) 
{
	...
	{
		// Currently BDO HOST is not supporting 0120 and 0120 message , we are sending them as OFFLINE Sale
	...
	/* Check for a Key reset transaction response 0810 message type*/
	else if ( ('8' == HOST_MESSAGE.msgtype [1]) && ('1' == HOST_MESSAGE.msgtype [2]))
	{
		return( AUTH_PIN_CHANGE );
	}
	...
	else
	{
		...
		nccup2_log_message( 1,2,Errorbuf,"get_tran_type_2",1 );
	}
}
...
INT insert_tlf01(pAUTH_TX p_auth_tx)
{
   ...
   if(NULL_PTR == p_msg_ins)
   {
	   nccup2_log_message( 1,3,"Insufficient Memory to build insert TLF01 message",
			   	   	   	   	   "insert_tlf01",1 );
	   return(false);
   }
   ...
   if(ret_code < 0)
   {
      ...
      nccup2_log_message( 1,3,Buff,"insert_tlf01",1 );
      free(p_msg_ins);
      return(false);
   }
   free(p_msg_ins);
...
INT hhutil_send_net_mngmnt_request(pAUTH_TX p_auth_tx, CHAR cup_response_code[4])
{
   ...
   {
	   nccup2_log_message( 1,3,"Invalid bit field: building network management request msg.",
			   	   	   	   	    "hhutil_send_net_mngmnt_request",1 );
	   return(false);
   }
   ...
   free( p_msg_host_out );
 	...
 	if( ret_code < 0 )
	{
		...
		nccup2_log_message( 1,3,Buffer,"hhutil_send_net_mngmnt_request",1 );
		return(false);
	}
   else
   {
...
INT hhutil_send_net_mngmnt_response(pAUTH_TX p_auth_tx)
{
   ...
   if ( false == build_host_msg(ErrorMsg) )
   {
	   nccup2_log_message( 1,3,"Invalid bit field: building network management response msg.",
	   	   			   	   	   	   	"hhutil_send_net_mngmnt_response",0 );
	   return(false);
   }
   ...
   free (p_msg_host_out);
 	...
 	if( ret_code < 0 )
	{
		...
		nccup2_log_message( 1,3,Buffer,"hhutil_send_net_mngmnt_response",0 );
		free(p_msg_host_out);
		return(false);
	}
   else
...
void update_ncf01( pAUTH_TX p_auth_tx )
{
   ...
   if ( NULL_PTR == p_msg  )
   {
	   nccup2_log_message( 1,3,"Insufficient Memory to build insert NCF01 message",
			   	   	   	   	   "update_ncf01",0 );
       ret_code = false;
...
BOOLEAN hhutil_send_host_response(pAUTH_TX p_auth_tx)
{
   ...
   (void)set_originator_info( p_auth_tx, p_msg_out );
	...
	if( ret_code < 0 )
	{
	   ...
	   sprintf( Buffer, "error on pteipc_send to que %s: %s",
               p_auth_tx->TLF01_details.originator_queue, ErrorMsg );
	   nccup2_log_message( 1,3,Buffer,"hhutil_send_host_response",1 );
	   free( p_msg_out );
	   return(false);
	}
   else
   {
	  ...
	  nccup2_log_message( 2,3,Buffer,"hhutil_send_host_response",1 );
      /* Update transaction statistics */
...
INT nccup2_check_emv_capability_device(void)
{
...
void nccup2_get_error_warning_file_name_path(void )
{
   DWORD rc;
   CHAR  filename   [80] = {0};
   CHAR  tmpstr     [80] = {0};
   CHAR  nccup2_error_warning_file_size[5] = {0};
   ...
   rc = GetPrivateProfileString(
								  ...
								  "DB_STATICS_PATH",       	/* points to key name             */
								   "",  					/* Default string                 */
								   nccup2_error_warning_file_path,              	/* points to destination buffer   */
								   sizeof(nccup2_error_warning_file_path)-1,   	 /* size of destination buffer     */
   ...
   rc = GetPrivateProfileString(
								  ...
								  "",  					/* Default string                 */
								  nccup2_module_error_warning_file_name,              	/* points to destination buffer   */
								  sizeof(nccup2_module_error_warning_file_name)-1,   	 /* size of destination buffer     */
								  filename                  /* points to ini filename         */
								);
	   ...
	   rc = GetPrivateProfileString(
								  ...
								  "500",  					/* Default string                 */
								  nccup2_error_warning_file_size,              	/* points to destination buffer   */
								  sizeof(nccup2_error_warning_file_size)-1,   	 /* size of destination buffer     */
								  filename                  /* points to ini filename         */
								);

	/* File size size conversion */
	Max_File_Size_Defined = atoi(nccup2_error_warning_file_size);
   ...
   if((strlen(nccup2_error_warning_file_path) > 0) &&
	  (strlen(nccup2_module_error_warning_file_name)> 0))
   {
	   nccup2_create_Error_Filename();
   }
}

INT nccup2_Log_error_warning_to_File(pCHAR Error_Buf,int sev, pCHAR func,int detail)
{
	INT   ret_val = 0;
	INT   len=0;
	INT   cardlen = 0;
    ...
    len = strlen(nccup2_Erro_warning_Filename);
    path_len = strlen(nccup2_error_warning_file_path);
    if( len == 0 ||
    	path_len==0 )
    {
    ...
    if ( 0 != strcmp(&nccup2_Erro_warning_Filename[len-4], current_mmdd) )
    {
		/* Now create the new filename.*/
    	nccup2_create_Error_Filename();
    }
	if((fp = fopen(nccup2_Erro_warning_Filename,"a+b"))==NULL)
	{
		LogEvent(Error_Buf,INFO_MSG);
		return 0;
	}
...
void nccup2_create_Error_Filename( void )
{
   ...
   /* Get system timestamp "YYYY-MM-DD-hh.mm.ss.jjj" */
    ...
    strcpy( nccup2_Erro_warning_Filename, nccup2_error_warning_file_path );
    strcat( nccup2_Erro_warning_Filename, nccup2_module_error_warning_file_name );
	strcat( nccup2_Erro_warning_Filename, ".EW." );
    strncat(nccup2_Erro_warning_Filename, system_date,   4 );  /* YYYY */
    strncat(nccup2_Erro_warning_Filename, system_date+5, 2 );  /* MM   */
    strncat(nccup2_Erro_warning_Filename, system_date+8, 2 );  /* DD   */
}
void nccup2_read_stip_flag_for_cup3ds2_inc_transactions( void )
{
	   DWORD rc;
	   CHAR  filename   [80] = {0};
	   CHAR  tmpstr     [80] = {0};
	   CHAR  atp_stip_flag   [2]  = {0};
	   ...
	   rc = GetPrivateProfileString(  "UPI_3DS2_MODE",   /* points to section name         */
									  "ATP_STIP_FLAG",       				/* points to key name             */
									   "0",  						/* Default string                 */
									   atp_stip_flag,              		/* points to destination buffer   */
									   sizeof(atp_stip_flag)-1,   	 	/* size of destination buffer     */
									   filename                  	/* points to ini filename         */
							 	   );
		if(rc != 1)
		{
			nccup2_log_message(2, 2, "UPI_3DS2_MODE not read", "nccup2_read_stip_flag_for_cup3ds2_inc_transactions", 0);
		}
		Cup3ds2_Flag = atoi(atp_stip_flag);

	   memset( atp_stip_flag, 0x00, sizeof(atp_stip_flag) );
}

BOOLEAN nccup2_Check_If_CUP_MOTO_TRN(void)
{
	if((0 == strncmp(Auth_Tx.industry_code,CUP_MOTO_INDUSTRY_CODE,INDUSTRY_CODE_CODE))&&
	   ((true == nccup2_is_POS_Acq_transaction())||
		(true == nccup2_is_voice_transaction()))&&
...
BOOLEAN nccup2_is_voice_transaction(void)
{
...
BOOLEAN nccup2_is_POS_Acq_transaction(void)
{
...
BOOLEAN nccup2_Check_If_CUP_RPS_TRN(void)
{
...
BOOLEAN nccup2_Check_If_CUP_BDO_PAY(void)
{
...
void nccup2_read_TO_config_details_from_tf_ini()
{
	   DWORD rc;
	   CHAR  filename   [80] = {0};
	   ...
	   rc = GetPrivateProfileString(  "REVERSL_ATTEMPTS_FOR_TIMEOUT_TRANSACTION",           /* points to section name         */
									  "REVERSL_ATTEMPTS_FOR_TIMEOUT_TRANSACTION_UPI",       	/* points to key name             */
									   "1",  												/* Default string                 */
	   ...
	   rvrsl_attmpts_value_fr_timeout_cup=atoi(readvalue);

	   if(rc == 0)
	   {
		   nccup2_log_message( 2, 1, "REVERSL_ATTEMPTS_FOR_TIMEOUT_TRANSACTION_UPI Value not read from tf.ini","nccup2_read_tf_ini",0);
	   }

	   readvalue[0]='\0';
	   rc = GetPrivateProfileString(  "REVERSL_ATTEMPTS_FOR_LATERESPONSE_TRANSACTION",              /* points to section name         */
									  "REVERSL_ATTEMPTS_FOR_LATERESPONSE_TRANSACTION_CUP",       	/* points to key name             */
									   "1",  														/* Default string                 */
									   readvalue,              										/* points to destination buffer   */
									   sizeof(readvalue),   	 									/* size of destination buffer     */
									   filename                  									/* points to ini filename         */
	   ...
	   rvrsl_attmpts_value_fr_laterspns_cup=atoi(readvalue);

	   if(rc == 0)
	   {
		   nccup2_log_message( 2, 1, "REVERSL_ATTEMPTS_FOR_LATERESPONSE_TRANSACTION_CUP Value not read from tf.ini","nccup2_read_tf_ini",0);
	   }
 ...
 *  MODIFICATIONS:
 ******************************************************************************/
INT nccup2_send_reversal_to_host_for_late_response()
{
	   INT   ret_val = true;
	   INT   seconds,buf_temp_len=0;
	   CHAR  err_buf[100]  ="";
...
INT nccup2_check_if_trx_is_bdo_pos_acq_trx( )
{
	 if((0 == strncmp(Auth_Tx.TLF01_details.handler_queue,
...
INT nccup2_is_valid_BDO_pos_acq_trn( )
{
	if(0 == strncmp(Auth_Tx.TLF01_details.product_codes[4].code, "ACQUR", 5))
	{
		return (true);
	}
	else
...
INT nccup2_check_if_txn_is_upi_3ds2_iss_trn( )
{
	if(0 == strncmp(Auth_Tx.TLF01_details.handler_queue, "nccup", 5) &&
...
INT nccup2_check_if_PEM_is_MobilePOS_NFC_Tap2Phone_trx_with_pin( )
{
	 if(0 == strncmp(Auth_Tx.TLF01_details.pos_entry_mode,
...
INT nccup2_check_if_PEM_is_MobilePOS_NFC_Tap2Phone_trx_without_pin( )
{
	if(0 == strncmp(Auth_Tx.TLF01_details.pos_entry_mode,
...
INT nccup2_check_if_trx_qualify_for_MobilePOS_NFC_Tap2Phone_trx ( )
{
...
INT nccup2_check_if_trx_is_upi_MobilePOS_NFC_Tap2Phone_trx_with_pin(  )
{
	if((true == nccup2_check_if_PEM_is_MobilePOS_NFC_Tap2Phone_trx_with_pin()) &&
	   (true == nccup2_check_if_trx_is_bdo_pos_acq_trx())&&
	   (true == nccup2_check_if_trx_qualify_for_MobilePOS_NFC_Tap2Phone_trx()))
	{
		return (true);
	}
	else
...
INT nccup2_check_if_trx_is_upi_MobilePOS_NFC_Tap2Phone_trx_without_pin(    )
{
	if((true == nccup2_check_if_PEM_is_MobilePOS_NFC_Tap2Phone_trx_without_pin()) &&
	   (true == nccup2_check_if_trx_is_bdo_pos_acq_trx())&&
	   (true == nccup2_check_if_trx_qualify_for_MobilePOS_NFC_Tap2Phone_trx()))
	{
		return (true);
...
INT nccup2_check_if_PEM_is_Swiftpass_BDO_Pay_credit_trx_with_pin( )
{
	 if(0 == strncmp(Auth_Tx.TLF01_details.pos_entry_mode,
...
INT nccup2_check_if_PEM_is_Swiftpass_BDO_Pay_credit_trx_without_pin( )
{
	if(0 == strncmp(Auth_Tx.TLF01_details.pos_entry_mode,
...
INT nccup2_check_if_trx_qualify_for_Swiftpass_BDO_Pay_credit_trx ( )
{
	if((0 == strncmp(Auth_Tx.TLF01_details.message_type,
			 	 	 POS_ACQ_SALE_MESSAGE_TYPE,
					 ISO_MESSAGE_TYPE_LEN))||
...
INT nccup2_check_if_trx_is_upi_Swiftpass_BDO_Pay_credit_trx_with_pin(  )
{
	if((true == nccup2_check_if_PEM_is_Swiftpass_BDO_Pay_credit_trx_with_pin()) &&
	   (true == nccup2_check_if_trx_is_bdo_pos_acq_trx())&&
	   (true == nccup2_check_if_trx_qualify_for_Swiftpass_BDO_Pay_credit_trx()))
	{
		return (true);
	}
	else
...
INT nccup2_check_if_trx_is_upi_Swiftpass_BDO_Pay_credit_trx_without_pin()
{
	if((true == nccup2_check_if_PEM_is_Swiftpass_BDO_Pay_credit_trx_without_pin()) &&
	   (true == nccup2_check_if_trx_is_bdo_pos_acq_trx())&&
	   (true == nccup2_check_if_trx_qualify_for_Swiftpass_BDO_Pay_credit_trx()))
	{
		return (true);
...
INT nccup2_check_if_trx_ecommerce_entry_mode_from_swiftpass_BDOPay_trx ( )
{
	if((0 == strncmp(Auth_Tx.TLF01_details.message_type,
			 	 	 POS_ACQ_SALE_MESSAGE_TYPE,
					 ISO_MESSAGE_TYPE_LEN))||
...
INT nccup2_check_if_trx_ecommerce_POS_entry_mode_from_swiftpass_BDOPay_trx( )
{
	if((0 == strncmp(Auth_Tx.TLF01_details.pos_entry_mode,
...
INT nccup2_check_if_trx_is_upi_ecommerce_swiftpass_BDOPay_trx( )
{
	if((true == nccup2_check_if_trx_ecommerce_entry_mode_from_swiftpass_BDOPay_trx()) &&
	   (true == nccup2_check_if_trx_is_bdo_pos_acq_trx())&&
	   (true == nccup2_check_if_trx_ecommerce_POS_entry_mode_from_swiftpass_BDOPay_trx()))
	{
		return (true);
	}
	else
...
INT nccup2_check_if_sbatch_qualify_for_MobilePOS_NFC_Tap2Phone_trx_with_pin(  )
{
	if ((true == nccup2_is_sbatch_trx()) &&
		(true == nccup2_is_valid_BDO_pos_acq_trn()) &&
		(true == nccup2_check_if_PEM_is_MobilePOS_NFC_Tap2Phone_trx_with_pin()))
	{
...
INT nccup2_check_if_sbatch_qualify_for_MobilePOS_NFC_Tap2Phone_trx_without_pin(  )
{
	if ((true == nccup2_is_sbatch_trx()) &&
		(true == nccup2_is_valid_BDO_pos_acq_trn()) &&
		(true == nccup2_check_if_PEM_is_MobilePOS_NFC_Tap2Phone_trx_without_pin()))
	{
...
INT nccup2_check_if_sbatch_qualify_for_Swiftpass_BDO_Pay_credit_trx_with_pin(  )
{
	if ((true == nccup2_is_sbatch_trx()) &&
		(true == nccup2_is_valid_BDO_pos_acq_trn()) &&
		(true == nccup2_check_if_PEM_is_Swiftpass_BDO_Pay_credit_trx_with_pin()))
	{
...
INT nccup2_check_if_sbatch_qualify_for_Swiftpass_BDO_Pay_credit_trx_without_pin(  )
{
	if ((true == nccup2_is_sbatch_trx()) &&
		(true == nccup2_is_valid_BDO_pos_acq_trn()) &&
		(true == nccup2_check_if_PEM_is_Swiftpass_BDO_Pay_credit_trx_without_pin()))
	{
...
INT nccup2_check_if_sbatch_qualify_for_ecommerce_swiftpass_BDOPay_trx( )
{
	if((true == nccup2_is_sbatch_trx()) &&
	   (true == nccup2_is_valid_BDO_pos_acq_trn())&&
	   (true == nccup2_check_if_trx_ecommerce_POS_entry_mode_from_swiftpass_BDOPay_trx()))
