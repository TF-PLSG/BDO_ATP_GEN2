   ...
   while ( (i <= num_messages) && (p_cur_msg != NULL_PTR) )
   {
      ...
      switch( ptemsg_get_msg_type( p_cur_msg ) )
      {
         case MT_DB_REQUEST     : p_tmp_msg = process_db_request (p_cur_msg);  
                                  break;                                       
         case MT_TIMER_REQUEST  :

        	 	 	 	 	 	 if (ptemsg_get_msg_subtype1 (p_msg_in) == ST1_TIMER_START)
        	 	 	 	 	 	 {
        	 	 	 	 	 		 //timerds_log_message( 2, 1, "START_TIMER ", "PinnacleMsgHandler",0 );
        	 	 	 	 	 		 sprintf(buffer, "START_TIMER : %s", lkpkey);
        	 	 	 	 	 		 timerds_log_message( 2, 1, buffer, "PinnacleMsgHandler",0 );
        	 	 	 	 	 	 }
        	 	 	 	 	 	 else if (ptemsg_get_msg_subtype1 (p_msg_in) == ST1_TIMER_CLEAR )
        	 	 	 	 	 	 {
        	 	 	 	 	 		 //timerds_log_message( 2, 1, "CLEAR_TIMER", "PinnacleMsgHandler",0 );
        	 	 	 	 	 		 sprintf(buffer, "CLEAR_TIMER : %s", lkpkey);
        	 	 	 	 	 		 timerds_log_message( 2, 1, buffer, "PinnacleMsgHandler",0 );
        	 	 	 	 	 	 }
