...
INT send_transaction_statistics( pPTE_MSG p_msg )
{
   ...
   if ( false == ret_code  )
   {
      dcpos_log_message( 1, 1, err_msg,"send_transaction_statistics",0 );
   }
   return( ret_code );
}
...
INT reset_transaction_statistics( pPTE_MSG p_msg )
{
   ...
   if ( false == ret_code  )
   {
      dcpos_log_message( 1, 1, err_msg,"reset_transaction_statistics",0 );
   }
   return( ret_code );
}
      ...
      if ( rc < 0 )
      {
         ...
         sprintf( err_buf,
                 "Error logging entry to Txn Trace File = %s",
                  TraceFile );
         dcpos_log_message( 1, 1, err_buf,"write_to_txn_file",0 );
         if ( rc == -1 )
            strcpy( err_buf,
         ...
         if ( false == flush_file(ppTraceFile_fp) )
         {
            ...
            sprintf( err_buf,
                    "%s: Unable to flush trace file buffer. Turning Trace off.",
                     ServiceName );
            dcpos_log_message( 1, 2, err_buf,"write_to_txn_file",0 );
