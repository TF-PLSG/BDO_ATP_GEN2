 ...
 * All Rights Reserved.
 *
 * MODULE:      FGRD_MEMORY.C
 *  
 * TITLE:       Equitable Fraudguard Shared Memory Processor
 *
 ...
 * REVISION HISTORY:
 *
 * $Log:   N:\POS\PVCS6.6\EPICPORTZ\PTE\Equitable\fguard\fgrd_memory.c  $
      ...
      if ( 0 != strcmp(host_state, prev_host_state) )
      {
         ...
         if ( true == ret_val )
         {
            ...
            if ( 0 == strcmp(host_state, ONLINE) )
            {
               ...
               //   log_message( 3, 2, err_buf, "set_host_state" );
            }
...
INT increment_active_count( pCHAR errbuf )
{
   ...
   if ( true == ret_val )
   {
      ...
      if ( (tempint >= MaxActiveTxns) && (MaxActiveTxns > 0) )
      {
         ...
         log_message( 1, 2, errbuf, "increment_active_count" );
      }
      else
      {
...
XINT ReadMemTable( pCHAR TableName, pBYTE tmouts, pBYTE txns, pBYTE state )
{
   ...
   memset(&fg_tbl, 0x00, sizeof(SHARED_MEM));
   Ret = MemRead(Rt,CurrentPosition +1,sizeof(SHARED_MEM),&fg_tbl,MEM_WAIT);

   strcpy( tmouts, fg_tbl.net_consec_tmouts_ctr);
   strcpy( txns,   fg_tbl.active_txns_ctr      );
   strcpy( state,  fg_tbl.host_state           );
...
LONG WriteMemTable( pCHAR TableName, pBYTE tmouts, pBYTE txns, pBYTE state )
{
   ...
   memset( &fg_tbl, 0x00, sizeof(SHARED_MEM) );
   strcpy( fg_tbl.net_consec_tmouts_ctr, tmouts );
   strcpy( fg_tbl.active_txns_ctr,       txns   );
   strcpy( fg_tbl.host_state,            state  );

   Ret = MemWrite(Rt, Current_Offset + 1, sizeof(fg_tbl), &fg_tbl, MEM_WAIT);
