...
*
   ...
   ----------------
	   ...
	   Rev 1.156   Oct 08 2009   TF-Ajay 
    Updated version to 4.4.1.56
	To compare string length was decreased, to copy CVV result code 
	
	  Rev 1.155   Oct 08 2009   TF-Ajay 
    Updated version to 4.4.1.55
    Added code to copy the CVV result code. If it is validate by IST(Eqit host)	in the field 44.5
	IST sends in format <CVV ResultCode> followed by 4 spaces
	First byte ATP need to send to VISA host in 44.5
		
		Rev 1.154   Oct 07 2009   TF-Ajay 
    Updated version to 4.4.1.54
    Added code to copy the CVV result code. If it is validate by IST(Eqit host)
	in the field 44.5
		
	  Rev 1.155   Sep 14 2009   TF-Ajay 
    Updated version to 4.4.1.53
    Added code to copy the CVV result code. If it is validate by IST(Eqit host)
	in the field 44.5 
	
	  Rev 1.154   Jul 14 2009   TF-Phani 
    Updated version to 4.4.1.52
    Added code  CVV1/CVV2 not passed to visa
	
	   Rev 1.153   Jul 13 2009   TF-Ajay 
    Updated version to 4.4.1.51
    Added code to copy the STIP txn status from ncf30 
		
	  Rev 1.152 Jun 11 2009 Ajay  TF
	Updated version to 4.4.1.50
	Field 104 length stores in the data itself to extract in ncvis2
	move_iso_response_to_auth_struct()
	
	  Rev 1.151 Jun 10 2009 Ajay  TF
	Updated version to 4.4.1.49
	Field 104 length was not calculating properly 
	move_iso_response_to_auth_struct()
	
	  Rev 1.150 May 20 2009 Ajay  TF
	Updated version to 4.4.1.48
	ncvisa was crashing while copy the data filed 104 from host structure to auth structure 

		Rev 1.149 Apr 14 2009 Ajay  TF
	Updated version to 4.4.1.47
	
	  Rev 1.149 Apr 09 2009 Girija Y  TF
	Updated version to 4.4.1.46

	Rev 1.148 Apr 09 2009 Girija Y TF
	Updated version to 4.4.1.46

	Rev 1.147 Mar 31 2009 Girija Y TF
	Updated version to 4.4.1.45

    Rev 1.146   Mar 30, 2009  Girija Y TF Bangalore
    Updated version to 4.4.1.44
	Change Request for sending SIGN ON and OFF to Host with DE70 = "078" & "079".

	Rev 1.145   Mar 25, 2009  Girija Y TF Bangalore
      Updated version to 4.4.1.43
	Apr 09 VISA mandate changes


	Rev 1.144   Jan 21, 2009  Girija Y TF Bangalore
      Updated version to 4.4.1.42
	ATP EQIT INTERFACES

	Rev 1.143   Jan 16, 2009  Girija Y TF Bangalore
   ...
   Rev 1.140   Feb 4, 2008   dirby
      Updated version to 4.4.1.32
   SCR 25045
...
extern CHAR  SIGNON_ADVICE_RETRIEVAL[];
extern CHAR  SIGNOFF_ADVICE_RETRIEVAL[];

extern CHAR        applnk_que_name[];
extern char visa_scheme_timeout_rc [3];
extern char visa_scheme_fai_rc [3];
...
CHAR  ncvisa_Error_warning_Filename[256] = {0};
CHAR  ncvisa_module_error_warning_file_name[256] = {0};
CHAR  ncvisa_error_warning_file_path[256] = {0};
...
CHAR eci_05_success_value;
CHAR eci_06_success_value;
CHAR eci_05_failure_value;
CHAR eci_06_failure_value;
void Get_CAVV_Result_Code_Values_FromNetworkINI();
/*****************************************************************************
...
void PinnacleMsgHandler( pPTE_MSG p_msg_in )
{
   ...
   switch( ptemsg_get_msg_type(p_cur_msg) )
   {
      case MT_AUTH_REQUEST: 
          ...
          {
             ncvisa_log_message( 3, 2, "Error processing incoming terminal request", "PinnacleMsgHandler", 0 ,"");
      ...
      case MT_AUTH_RESPONSE: 
          ...
          if (false == process_incoming_message(p_msg_in)) 
          {
             ncvisa_log_message( 3, 2, "Error processing incoming response for host", "PinnacleMsgHandler", 0 ,"");
             pCurrent = ptemsg_get_pte_msg_data_data(ptemsg_get_pte_msg_data(p_msg_in));
             memset( &auth_tx, 0,        Auth_Tx_Size );
             memcpy( &auth_tx, pCurrent, Auth_Tx_Size );
             perform_error_processing( &auth_tx );
      ...
      case MT_TIMER_TIMEOUT: 
          ...
          {
             ncvisa_log_message( 3, 2,"Error processing timeout message", "PinnacleMsgHandler", 0 ,"");
          }
          break;
         ...
         switch( subtype1 )
         {
            case ST1_LOGON:
               ...
               if ( subtype2 == ST2_ISSUER )
               {
                  ...
                  if ( ret_val == false )
                  {
                     ncvisa_log_message( 3, 3,"Unable to change host state", "PinnacleMsgHandler", 0 ,"");
                  }
                  else
			...
			/* CR to send SIGN ON and OFF request to Host, Girija Y TF, March 30 2009 */

			case ST1_SIGNON_ADVICE:

				if( subtype2 == ST2_NONE )
				{
					memset (&auth_tx, 0, Auth_Tx_Size);
                    (void)perform_host_logon_request( SIGNON_ADVICE_RETRIEVAL,
                                                      SOURCE_IS_OPERATOR,
                                                      &auth_tx,
                                                      subtype2 );
               }
			...
			case ST1_SIGNOFF_ADVICE:

				if( subtype2 == ST2_NONE )
				{
					memset (&auth_tx, 0, Auth_Tx_Size);
                    (void)perform_host_logon_request( SIGNOFF_ADVICE_RETRIEVAL,
                                                      SOURCE_IS_OPERATOR,
                                                      &auth_tx,
                                                      subtype2 );
               }
            ...
            case ST1_ECHOTEST:
               ...
               memset (&auth_tx, 0, Auth_Tx_Size);
               //HCTS1-D
               ncvisa_log_message( 1, 1, "Sending ECHO TEST Request to host." , "PinnacleMsgHandler" , 0 ,"");
            ...
            case ST1_NETWORK_QUERY:
               ...
               if (ConnectionId[0] != 0)
               {
                  ...
                  if ( 0 > ret_code )
                  {
                     ...
                     ncvisa_log_message( 3, 3,Buff, "PinnacleMsgHandler", 0 ,"");
                     free( p_msg_ins );
                  }
               }
               free( p_msg_ins );
      ...
      case MT_SYSTEM_REQUEST: 
               ...
               if ( false == flush_file(ppTraceFile_fp) )
               {
                  ...
                  sprintf( Buff,
                    "%s: Unable to flush trace file buffer. Turning Trace off.",
                     ServiceName );
                  ncvisa_log_message( 3, 1, Buff , "PinnacleMsgHandler" , 0 ,"");
         ...
         else if(ST1_LOAD_VISA3DS2_RECORDS == ptemsg_get_msg_subtype1(p_msg_in))
         {
        	 Get_CAVV_Result_Code_Values_FromNetworkINI();
        	 sprintf( Buff, "Reloaded the DE44.13 CAVV reasult code values: %s, version %s",
        	                      ServiceName, Version );
        	 ncvisa_log_message( 3, 1, Buff , "PinnacleMsgHandler" , 0 ,"");
         ...
         {
        	 ncvisa_read_TO_rc_from_tf_ini();
        	 send_trace_response(p_msg_in);
        	 sprintf( Buff,
        	          "visa_scheme_timeout_rc: %s",
					  visa_scheme_timeout_rc);
        	 ncvisa_log_message( 1, 1, Buff, "PinnacleMsgHandler" ,0,"");
         }
         else if(ST1_LOAD_RC_FOR_FAI == ptemsg_get_msg_subtype1(p_msg_in))
         {
        	 ncvisa_read_fai_rc_from_tf_ini();
        	 send_trace_response(p_msg_in);
        	 sprintf( Buff,
        	          "visa_scheme_fai_rc: %s",
					  visa_scheme_fai_rc);
        	 ncvisa_log_message( 1, 1, Buff, "PinnacleMsgHandler" ,0,"");
         }
         break;
      ...
      case MT_TIMER_REPLY:
         ...
         else
         {
            ...
            ncvisa_log_message( 3, 2, err_mesg , "PinnacleMsgHandler" , 0 ,auth_tx.TLF01_details.card_num);
         }
         break;
      ...
      case MT_NSP_TRANSLATE_PIN_RESPONSE:
         ...
         {
            ncvisa_log_message( 3, 2, "Error processing incoming security request" , "PinnacleMsgHandler" , 0 ,"");
         }
      break;
      ...
      default :
         ncvisa_log_message( 3, 2, "Unknown message type received" , "PinnacleMsgHandler" , 0 ,"");
         break;
	}  /* switch msg_type */
...
void Get_CAVV_Result_Code_Values_FromNetworkINI()
{
	   ...
	   GetPrivateProfileString(
	    "CAVV_RESUL_CODES",	            /*  points to section name  */
	    "ECI_05_SUCCESS_VALUE",           /*  points to key name  */
	    "0",                      /*  points to default string  */
	    tmpstr,                         /*  points to destination buffer  */
	    sizeof(tmpstr) - 1,             /*  size of destination buffer  */
	    filename                        /*  points to initialization filename  */
	   ...
	   eci_05_success_value=tmpstr[0];

	   GetPrivateProfileString(
	    "CAVV_RESUL_CODES",	            /*  points to section name  */
	    "ECI_06_SUCCESS_VALUE",           /*  points to key name  */
	    "0",                      /*  points to default string  */
	    tmpstr,                         /*  points to destination buffer  */
	    sizeof(tmpstr) - 1,             /*  size of destination buffer  */
	    filename                        /*  points to initialization filename  */
	   ...
	   eci_06_success_value=tmpstr[0];

	   GetPrivateProfileString(
	    "CAVV_RESUL_CODES",	            /*  points to section name  */
	    "ECI_05_FAILURE_VALUE",           /*  points to key name  */
	    "0",                      /*  points to default string  */
	    tmpstr,                         /*  points to destination buffer  */
	    sizeof(tmpstr) - 1,             /*  size of destination buffer  */
	    filename                        /*  points to initialization filename  */
	   ...
	   eci_05_failure_value=tmpstr[0];

	   GetPrivateProfileString(
	    "CAVV_RESUL_CODES",	            /*  points to section name  */
	    "ECI_06_FAILURE_VALUE",           /*  points to key name  */
	    "0",                      /*  points to default string  */
	    tmpstr,                         /*  points to destination buffer  */
	    sizeof(tmpstr) - 1,             /*  size of destination buffer  */
	    filename                        /*  points to initialization filename  */
...
void MainProcessor()
{
   ...
   sprintf( Buffer,
           "Starting the Network Controller Service: %s, version %s",
            ServiceName, Version );
   ncvisa_get_error_warning_file_name_path();
   ncvisa_log_message( 2, 1, Buffer , "MainProcessor" , 0 ,"");
   strcat( Buffer, "\n" );
   PRINT( Buffer );
	...
	if( !pteipc_init_multiple_instance_app( cur_app_name, ServiceName, Buffer) )
	{
      ...
      ncvisa_log_message( 3, 3, "Failed to create XIPC queues.", "MainProcessor", 0 ,"");
		/* pteipc_shutdown_single_instance_app(); */
		pteipc_shutdown_multiple_instance_app();
		MainProcessDone = 1;
		return;
	}
   ncvisa_log_message( 3, 1, "Started the Visa Host Handler Service", "MainProcessor", 0 ,"");
   ...
   if (strlen(AppName) > (sizeof(ncf01_i.primary_key.network_id)-2) )
	{
      ...
      ncvisa_log_message( 3, 2, Buffer, "MainProcessor", 0 ,"");
      /* pteipc_shutdown_single_instance_app(); */
		pteipc_shutdown_multiple_instance_app();
		MainProcessDone = 1;
		return;
   ...
   */
   ...
   else if ( 0 == ret_code )
   {
      ...
      if ( 0 != ret_code)
      {
        ...
        ncvisa_log_message( 3, 2, "Unable to write to shared memory", "MainProcessor", 0 ,"");
      }
   }
   else
   {
         /* Error creating the table.   */
         ncvisa_log_message( 3, 2, "Unable to create shared memory", "MainProcessor", 0 ,"");
   }
      ...
      if (false == open_trace_file(AppName,DirPath,ppTraceFile_fp,TraceFile))
      {
         ...
         sprintf( ErrorMsg,
                 "Unable to open trace file %s. Tracing is off",
                  TraceFile );
         ncvisa_log_message( 3, 1, ErrorMsg, "MainProcessor", 0 ,"");
      }
      else
      {
    	  ncvisa_log_message( 3, 1, "Tracing is turned ON", "MainProcessor", 0 ,"");
      }
   }
   else
   {
      ...
      ncvisa_log_message( 3, 1, "Tracing is turned off", "MainProcessor", 0 ,"");
   }
...
void EndProcess()
{
   ...
   if ( Tracing == TRACE_ON )
   {
      if ( false == close_trace_file( ppTraceFile_fp ))
      {
         ...
         ncvisa_log_message( 3,2, Buffer, "EndProcess", 0 ,"");
      }
   }

   sprintf( Buffer, "Shutting down the %s Service, version %s",
            ServiceName, Version );
   ncvisa_log_message( 2,1, Buffer, "EndProcess", 0 ,"");
   strcat( Buffer, "\n" );
   PRINT( Buffer );
}
