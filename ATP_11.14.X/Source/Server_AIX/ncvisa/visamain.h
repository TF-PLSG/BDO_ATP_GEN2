...
*
   ...
   Rev 1.50    Feb 6, 2008   dirby
      Added Card Issuer Reference Data (issref) field to the ISO message
      structure.  This is field 116 for MC transaction responses thru Visa.
      Added response code definitions 14 and 61.
   SCR 25045
...
struct iso_msg_rec
   {
        ...
        BYTE pos_condcd    [1];			   
        BYTE pin_captcd    [1];			   
    	BYTE tran_fee      [9];	 /* Apr 2009 Mandates, Ajay TF, March 25, 2009 */
        BYTE acq_inst_len  [1];
        BYTE acq_inst_id   [6];
        BYTE fwd_inst_len  [1];			
        BYTE fwd_inst_id   [7];
        BYTE pan_ext_len   [2];		/* DE 34 */
        BYTE pan_ext       [255];
        BYTE track2_len    [1];
        BYTE track2        [37];
        BYTE rrn           [12];
...
#define  ACCOUNT_VER_RESP  "85"
#define  ACNT_VERIFY_DCLN  "82"
