...
//praneeth    update_txn_stats( &Txn_Counts, (pAUTH_TX)NULL_PTR, RECEIVED );
   ...
   if ( false == reset_echo_timer(err_buf) )
      nciftr_log_message(1, 2, err_buf, "process_host_msg",0 );
  ...
  (void)get_originator_info( p_msg_in, Auth_Tx.TLF01_details.originator_queue,
   ...
   else
   {
       ...
       *Copy response msg from cadice to req buffer- TF Abhishek*/
		...
		{	// Drop the TPDU 5 bytes, AS ATP is adding TPDU
			buflen = buflen -5 ;
			sprintf(Auth_Tx.resp_data_len,"%04d", buflen);
			memcpy(Auth_Tx.resp_buffer,&ReqRsp_Buffer[5],buflen);
		}
...
///////////////////////////////////////////////////////////////////////////////
    ...
    if(AUTH_INVALID == Auth_Tx.TLF01_details.tx_key)
   {
      ...
      nciftr_log_message(1,3, Buffer, "process_request_from_host", 1);
      return(true);
   }
   ...
   /*
		We need to discuss on this on the business Process.
   ...
   */
   ...
   {
      if (0 != strncmp(Auth_Tx.TLF01_details.currency_code, PESO_CURRENCY_CODE,3))
      {
         if (0 != strncmp(Auth_Tx.settlement_curr_code, USA_CURRENCY_CODE, 3))
         {
            ...
            nciftr_log_message(1,2, "Invalid currency code", "process_request_from_host", 1);
      ...
      (Auth_Tx.TLF01_details.tx_key <= AUTH_INVALID))
   {
      ...
      {
    	  nciftr_log_message(1,2, Buffer, "process_request_from_host", 1);
...
void process_request_for_host_after_security()
{
   ...
   switch ( tx_key )
   {
      ...
      case AUTH_REVERSAL_ADVICE:
         ...
         sprintf( Buff,
                 "Reversal going to BDO HOST: %s.Merchant %s",
                  AppName,
                  Auth_Tx.TLF01_details.merchant_id );
         nciftr_log_message( 3, 2, Buff, "process_request_for_host_after_security" ,1);
      ...
      default:
         ...
         sprintf( err_buf,
                 "Invalid txn. Cannot process. TX_KEY = %d",
                  tx_key );
         nciftr_log_message( 1, 2, err_buf,
         ...
         ret_val = perform_standin_processing( INVALID_TXN,
                                               ...
                                               err_buf );
         if ( false == ret_val )
            nciftr_log_message(1,2,err_buf,"process_request_for_host_after_security",1);
...
/////////////////////////////////////
   ...
   if ( false == ret_val )
   {
      nciftr_log_message( 1, 3, err_buf, "process_request_for_host_after_currency",0 );
	  /*TF Praneeth revert back tx_key  while stip transaction */
	  if ((Auth_Tx.TLF01_details.voice_auth_text[0] == AUTH_AUTHORIZATION_OVERRIDE) || (Auth_Tx.TLF01_details.voice_auth_text[0] == AUTH_CASH_ADVANCE_AUTHORIZATION_OVERRIDE))
	  {
		  Auth_Tx.TLF01_details.tx_key = Auth_Tx.TLF01_details.voice_auth_text[0];
      ...
      ret_val = perform_standin_processing( "01",
                                            ...
                                            err_buf );
      if ( false == ret_val )
         nciftr_log_message(1,2,err_buf,"process_request_for_host_after_currency",0);
   }
   else
   {
...
void process_response_from_host()
{
   ...
   if ( false == decrement_active_count( err_buf ) )
      nciftr_log_message(1, 3, err_buf, "process_response_from_host",1 );
   ...
   {
      /* Handle Logon protocol seperatly
		 We have sended 200 message with wrong card number as a logon request.
		 if we received a respons for that message we assume host is online.
		 Stan , Tid  data are harc coded for this */
		if ( (0 == strncmp(Auth_Tx.TLF01_details.sys_trace_audit_num,trmp_stand ,6))&&
			 (0 == strncmp(Auth_Tx.TLF01_details.terminal_id,DUMMY_TID ,8)))
		{
			update_txn_stats( &Txn_Counts, (pAUTH_TX)NULL_PTR, MGT );//praneeth added line with MGT
         	process_net_management_from_host(LOGON);
		 	sprintf( err_buf, "received log on response from HOST");
            nciftr_log_message(5, 1, err_buf, "process_response_from_host",1 );
     	 }
      else
      {
         /* Update transaction statistics. */
         ...
         if ( false == clear_request_timer(lookup_key) )
         {
            ...
            nciftr_log_message(1, 2, err_buf, "process_response_from_host",1 );
         }
      }
   }
   return;
      ...
      if( ptemsg_get_result_code( p_msg_in ) == PTEMSG_NOT_FOUND )
      {
         ...
         sprintf( err_buf,
                 "Unable to retrieve timer data for lookup key %s",
                  lookup_key );
         nciftr_log_message( 1, 3, err_buf, "process_clear_timer_reply",0 );
      }
      ...
      else if ( timer_reply_size != Auth_Tx_Size )
      {
         sprintf( err_buf,
                 "Did not receive auth_tx data with timer reply. Key = %s",
                  lookup_key );
         nciftr_log_message( 1, 2, err_buf, "process_clear_timer_reply",0 );
      }
      else
      {
		...
		if ( (0 == strncmp(Auth_Tx.TLF01_details.sys_trace_audit_num,trmp_stand ,6))&&
			 (0 == strncmp(Auth_Tx.TLF01_details.terminal_id,DUMMY_TID ,8)))
			{
			 	 update_txn_stats( &Txn_Counts, (pAUTH_TX)NULL_PTR, MGT );//praneeth added line with MGT
				 process_net_management_from_host(LOGON);
				 sprintf( err_buf, " received Logon response form HOST");
                 nciftr_log_message( 1, 2, err_buf, "process_clear_timer_reply",0 );
			 }
...
void copy_resp_into_request( pAUTH_TX host_auth_tx )
{
   ...
   if ( Auth_Tx.TLF01_details.visa_validation_code[0] != 0x00 )
   {
	...
	/*We do not send RRN in request transaction, we shall keep the Cadicen RRN*/
	strcpy( Auth_Tx.TLF01_details.retrieval_ref_num,
				 host_auth_tx->TLF01_details.retrieval_ref_num );
   ...
   */
	...
	// Stroing feild 63 respons ecoming for MP transaction
	memcpy(Auth_Tx.future_use_1,host_auth_tx->future_use_1,strlen(host_auth_tx->future_use_1));
...
void process_1304_from_host()
{
	...
	nciftr_log_message(1,3, err_msg, "process_1304_from_host", 1);
...
INT delete_orig_ncf30( NCF30 ncf30, pAUTH_TX p_auth_tx )
{
   ...
   if ( NULL_PTR == p_msg  )
   {
      nciftr_log_message(1,2, "Insufficient Memory to build NCF30 message", "delete_orig_ncf30", 1);
      return( false );
   }
   ...
   if ( 0 > ret_code )
   {
      ...
      nciftr_log_message(1,3, Buff, "delete_orig_ncf30", 1);
      return( false );
   }
   return(true);
 }
 ...
 BOOLEAN process_logon_response(CHAR nmi_code[4],pAUTH_TX p_auth_tx)
 {
	...
	if (strcmp(LOGON_NMI_LOGON ,p_auth_tx->function_code) == 0)
	{
	   if ( strcmp( p_auth_tx->TLF01_details.response_code, "00" ) )
	   {
		  ...
		  nciftr_log_message(1,1, "LOGON Request successful.", "process_logon_response", 0);
	   }
	   else
	   {
		  nciftr_log_message(1,3, "LOGON Request DENIED.", "process_logon_response", 0);
	   }
	}
	...
	else if (strcmp(LOGON_NMI_LOGOFF,p_auth_tx->function_code) == 0)
	{
	   if ( strcmp( p_auth_tx->TLF01_details.response_code, "00" ) )
	   {
		  ...
		  update_host_status = true;
		  nciftr_log_message(1,1, "LOGOFF Request successful.", "process_logon_response", 0);
	   }
	   else
	   {
		  nciftr_log_message(1,3, "LOGOFF Request DENIED.", "process_logon_response", 0);
	   }
	}
	...
	else if (strcmp(LOGON_NMI_ECHO_TEST,p_auth_tx->function_code) == 0)
	{
	   if ( strcmp( p_auth_tx->TLF01_details.response_code, "00" ) )
	   {
		  strcpy(current_state, "1");
		  update_host_status = true;
		  nciftr_log_message(1,1, "ECHO Request successful.", "process_logon_response", 0);
	   }
	   else
	   {
		 nciftr_log_message(1,3, "ECHO Request DENIED.", "process_logon_response", 0);
	   }
	}
	...
	{
	    nciftr_log_message(1,3, "Insufficient Memory to build insert TLF01 message", "process_logon_response", 0);
		return(false);
	}
	else
	{
 ...
 ******************************************************************************/
 INT process_incoming_message(pPTE_MSG p_msg)
 {
		  ...
		  (Auth_Tx.TLF01_details.tx_key > AUTH_INVALID))  /* a response */
	   {
		  ...
		  if((0 == strcmp(Auth_Tx.TLF01_details.response_code, "00")) || (0 == strcmp(Auth_Tx.TLF01_details.response_code, "55")))
		  {
			  if ( false == update_database( ))
			  {
				  ...
				  nciftr_log_message(1,3, "Unable to update database tables", "process_incoming_message", 0);
				 /* Unable to complete a transaction.  Inform host; log to TLF01. */
				 if(false == incoming_other_host_response_db_reply( ) )
				 {
					 nciftr_log_message(1,3, "Unable to send response to host", "process_incoming_message", 0);
				 }
			  }
	...
	if (true == hhutil_verify_pin_block(&Auth_Tx))
	{
	   ...
	   {
		 nciftr_log_message(1,2, "Failed to process debit transaction", "process_incoming_message", 0);
		 perform_error_processing( &Auth_Tx );
		 strcpy(Auth_Tx.TLF01_details.response_code,"91");
		 Auth_Tx.TLF01_details.tx_key =
						match_request_to_response(Auth_Tx.TLF01_details.tx_key);
 ...
 ******************************************************************************/
 BOOLEAN send_it_back_to_terminal(BYTE resp_code[3], pAUTH_TX p_auth_tx )
 {
	...
	if ( NULL_PTR == p_msg_term_handler_out )
	 {
	    nciftr_log_message(1,2, "Insufficient Memory to build response to txcntl", "send_it_back_to_terminal", 0);
		return(false);
	 }
	...
	if ( ret_code < 0 )
	{
	   ...
	   nciftr_log_message(1,3, Buffer, "send_it_back_to_terminal", 0);
	   free(p_msg_term_handler_out);
	   return(false);
	}
	else
	{
	    nciftr_log_message(1,3, "Response sent successfully to terminal handler\n", "send_it_back_to_terminal", 0);
	}
 ...
 ******************************************************************************/
 BOOLEAN hhutil_process_debit_transaction(pAUTH_TX p_auth_tx)
 {
	...
	if ( NULL_PTR == p_msg )
	 {
	     nciftr_log_message(1,2, "Insufficient Memory to build pin translation message", "hhutil_process_debit_transaction", 0);
		 return(false);
	 }
	 ...
	 if( ret_code < 0 )
	 {
	   ...
	   nciftr_log_message(1,3, Buffer, "hhutil_process_debit_transaction", 0);
	   free( p_msg );
	   return(false);
	 }
 ...
 ******************************************************************************/
 INT incoming_terminal_request(pAUTH_TX p_auth_tx)
 {
	...
	{
	    nciftr_log_message(1,3, "Host Inactive", "incoming_terminal_request", 0);
	   p_auth_tx->TLF01_details.tx_key = match_request_to_response(p_auth_tx->TLF01_details.tx_key);
 
		if (0 == strcmp(current_state,"2"))  /* check if we are error state */
	   {		 
			nciftr_log_message(1,3, "Host Inactive: current_state!=0", "incoming_terminal_request", 0);
	...
	switch (source)
	 {
		 case SOURCE_IS_FSVISA:
		 {
			 ...
			 {
			  nciftr_log_message(1,3, "Sending logon request to host:", "perform_host_logon_request", 0);
	   ...
	   case SOURCE_IS_OPERATOR:
	   {
		 ...
		 case SOURCE_IS_HOST:
		 {
			 ...
			 if (strcmp(LOGON_NMI_START , request_type)==0)
			 {
				 if (0 == atoi(current_state))
				 {
					 nciftr_log_message(1,3, "Received logon request to host:", "perform_host_logon_request", 0);
				 ...
				 {
				 	 nciftr_log_message(1,3, "Received logon request to host,Already Online:", "perform_host_logon_request", 0);
					 p_auth_tx->TLF01_details.tx_key = AUTH_LOGON_RESPONSE;
					 strcpy(vLogon_NMI_Code, LOGON_NMI_START);
					 perform_op_net_mngmnt_response(request_type, p_auth_tx);
				 }
			 ...
			 {
			  		nciftr_log_message(1,3, "Received logoff request from host:", "perform_host_logon_request", 0);
				  	p_auth_tx->TLF01_details.tx_key = AUTH_LOGON_RESPONSE;
			 ...
			 {
			  		nciftr_log_message(1,3, "Received UnionPay starts date switch request from host:", "perform_host_logon_request", 0);
				  	p_auth_tx->TLF01_details.tx_key = AUTH_LOGON_RESPONSE;
			 ...
			 else
			 {
				 if (0 == atoi(current_state))
				 {
					// nciftr_log_message(1,3, "Received network request from host:", "perform_host_logon_request", 0);
					 p_auth_tx->TLF01_details.tx_key = AUTH_LOGON_RESPONSE;
					 strcpy(vLogon_NMI_Code, LOGON_NMI_ECHO_TEST);
 ...
 ******************************************************************************/
 INT  verify_host_app_status(pAUTH_TX p_auth_tx)
 {
	...
	{
	   nciftr_log_message(1,2, "Host app not in state 0, cannot process tran", "verify_host_app_status", 0);
	   return(false);
	}
 
	if (2 == atoi(current_state))  
	{  
	    nciftr_log_message(1,2, "Maximum consecutive timeouts reached", "verify_host_app_status", 0);
		strcpy(current_state,"2"); 
		return(false);
	}
	if (atoi(active_txns_ctr) >= atoi(ncf01.max_active_txns))
	{
	   nciftr_log_message(1,2, "Maximum active transactions reached", "verify_host_app_status", 0);
 ...
 INT perform_op_net_mngmnt_request( char request_type[], pAUTH_TX p_auth_tx )
 {
	...
	{
	   nciftr_log_message(1,1, "Sending LOGON Request to host.", "perform_op_net_mngmnt_request", 0);
	   strcpy(vLogon_NMI_Code, LOGON_NMI_LOGON);
	}
	else if (0 == strcmp(request_type, LOGOFF_REQUEST))
	{
	   nciftr_log_message(1,1, "Sending LOGOFF Request to host.", "perform_op_net_mngmnt_request", 0);
 ...
 ******************************************************************************/
 int perform_op_net_mngmnt_response(char nmi_code[], pAUTH_TX p_auth_tx)
 {
	...
	{
		nciftr_log_message(1,1, "Sending ECHO Response to host", "perform_op_net_mngmnt_request", 0);
	}
  ...
  void  process_logon_Key_Reset_Transaction()
 {
	...
	char Feild_ident[3]="";
 	...
 	if (strlen(Auth_Tx.TLF01_details.voice_auth_text) > 0)
 	{
 		...
 		memcpy(temp_len,Auth_Tx.TLF01_details.voice_auth_text, DE48_LEN_SIZE);
		len = atoi(temp_len);
		memcpy(Feild_ident,Auth_Tx.TLF01_details.voice_auth_text+ DE48_LEN_SIZE, 2);
		memcpy(temp_data,Auth_Tx.TLF01_details.voice_auth_text+ DE48_LEN_SIZE + 2, len -2);
