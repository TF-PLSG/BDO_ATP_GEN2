 ...
 * All Rights Reserved.
 *
 * MODULE:      nciftr_timer.c
 *  
 * TITLE:       CUP Network Controller Timer Processor
 *  
...
extern INT LR_reversal_attempts_value;
extern INT TO_reversal_attempts_value;
/******************************************************************************
 *
...
void process_timeout_msg( pPTE_MSG p_msg )
{
	...
	CHAR		err_buf[200]						= {0};
	CHAR        echo_timer_key[LOOKUP_KEY_SIZE]		= {0};
	CHAR        echo_req_timer_key[LOOKUP_KEY_SIZE]	= {0};
	BYTE        temp_tx_key							= 0;
    LONG        timer_reply_size 					= 0;
	INT         repeat_ctr							= 0;
	INT         max_repeats							= 0;
	CHAR        advice_repeat_count[3]				= {0};
	#ifdef SME3
	CHAR		err_buf2[100]						= {0};
   ...
   else if (0 == strcmp(lookup_key, echo_req_timer_key) )
   {
   ...
   else
	{
		if ( timer_reply_size != Auth_Tx_Size )
		{
			...
			lookup_key );
			nciftr_log_message( 1, 2, err_buf, "process_timeout_msg",0 );
		}
		else
		{
				 ...
				 (0 == strncmp(Auth_Tx.TLF01_details.terminal_id,DUMMY_TID ,8)))
			{
				...
				process_net_management_from_host(LOGOFF);
				nciftr_log_message( 1, 2, "Log On request timed out", "process_timeout_msg",0 );
			}

			/* Only send timeout response if txn has not been repeated. */
			memset( err_buf, 0x00, sizeof(err_buf) );
			strncpy(advice_repeat_count,Auth_Tx.advice_repeat_count,2);
			repeat_ctr = atoi( advice_repeat_count );
			if ( repeat_ctr == 0 )
			{
			...
			/* Increment number of consecutive timeouts.It returns false if max timeouts is reached. */
			/* if ( false == increment_timeout_count(err_buf) )
			{
			// Either max timeouts has been reached,or there is a problem with shared memory. Either way, log the problem and discontinue processing.

			nciftr_log_message( 1, 1, err_buf, "process_timeout_msg",0 );
			...
			if ( is_reversal() )
			{
	         	   ...
	         	   if(LR_reversal_attempts_value<=0)
	         	   {
	         		  LR_reversal_attempts_value=atoi(Ncf01_I.advice_repeat_counter);
	         	   }

	         	   if(TO_reversal_attempts_value <=0)
	         	   {
	         		  TO_reversal_attempts_value=atoi(Ncf01_I.advice_repeat_counter);
	         	   }

	           	   if(0 == strncmp(Auth_Tx.TLF01_details.response_text,"LR",2))
				   {
					  max_repeats=LR_reversal_attempts_value;
				   }
				   else
				   {
					  max_repeats=TO_reversal_attempts_value;
				   }
				...
				nciftr_log_message( 1, 1, "Reversal got timed out dropping the transaction",
					"process_timeout_msg",1 );
			}
			else
			{
				...
				*/
				...
				{
					nciftr_log_message( 1, 1, "Txn to NCIFTR Issuer Host(Vertexon) timed out", "process_timeout_msg",1 );
					(void)transform_0200_into_0420();
					(void)send_request_to_host( TIMER_REVERSAL );
					/* Increment time out count only for Sale transaction, If TO reversal's gets timed out again ATP will drop
					* the TO reversal*/
					if ( false == increment_timeout_count(err_buf) )
					{
						...
						*/
			        	...
			        	if(GetTimeoutCount() == (MaxConsecTimeouts+1))
			        		nciftr_log_message( 3, 2, err_buf , "process_timeout_msg",0 );
						#else
						nciftr_log_message( 1, 1, "Incrementing time out count failed" , "process_timeout_msg",0 );
						#endif
						(void)send_network_status_to_monitor();
					}
				}
...
INT set_timer( pCHAR lookup_key, INT timer_length, INT type_of_timer )
{
   ...
   if ( false == ret_val )
      nciftr_log_message( 1, 2, err_buf, "set_timer",0 );
...
INT clear_timer( pCHAR lookup_key )
{
   ...
   if ( false == ret_val )
      nciftr_log_message( 1, 2, err_buf, "clear_timer" ,0);
...
void create_request_timer_key( pCHAR timer_key )
{
   ...
   switch( tx_key )
   {
      ...
      case AUTH_VOID_SALE_RESPONSE:
    	  //strcpy(temp_str, "VOID :");
    	  strcpy( tid, Auth_Tx.TLF01_details.terminal_id );
         pan_len = strlen( Auth_Tx.TLF01_details.retrieval_ref_num);
         memcpy( key_last_six, &Auth_Tx.TLF01_details.retrieval_ref_num[pan_len - 6], 6);
   ...
   /*if(pan_len > 0)
   {
	   strcat (temp_str,timer_key);
	   TxUtils_Send_Msg_To_Operator (1, temp_str, 1, INFO_ERROR,
								     "create_request_timer_key", 4, INFO_ERROR, NULL_PTR,
							         NULL_PTR, NULL_PTR);
   }*/
   return;
}
