...
* All Rights Reserved.
*
* MODULE:      NCEQ_VALIDATION.C
*  
* TITLE:       Equitable Network Control Data Validation Functions
*  
...
* $Log:   N:\PVCS\PTE\Equitable\nceqit_ncmlnk_iso_format\nceq_validation.c  $
      ...
      Rev 1.6   Jul 14 2004 14:32:12   dirby
   Added 0201/0211 and 0202/0212 to the list of supported message
   types.  This is for reloads.
   SCR 1287
   
      Rev 1.5   Sep 29 2003 13:51:40   dirby
   Added support for Fee Collection and Funds Disbursement for
   Acquirers.
   SCR 1083
   
      Rev 1.4   Aug 26 2003 16:22:56   svelaga
   Administrative text message logic fixed.
   Version bumped to 4.2.0.11
   
      Rev 1.3   Aug 25 2003 13:48:00   svelaga
   Fixed
   ...
   3. Backoffice adjustment and Representment are distinguised based on Field 25 ( pos condition code ) instead of processing code.
      ...
      Rev 1.2   Aug 06 2003 10:38:14   svelaga
   Version 4.2.0.9:  Support for more POSCC added.
   
      Rev 1.1   Jul 25 2003 17:33:02   svelaga
   ...
   DE18,DE19 etc...
      ...
      Rev 1.0   Aug 06 2001 09:48:40   dirby
   Initial revision.
*
****************************************************************************/
#include <stdio.h>
...
INT validate_host_msg()
{
    ...
    *    TID and MID
    */
   /* No need to check these values for administrative text message */
   ...
   else if ( 0 == strlen( Auth_Tx.TLF01_details.card_num ) )
   {
      sprintf( err_buf,
   ...
   else if ( (0 == strlen( Auth_Tx.TLF01_details.pos_entry_mode ) ) && (Auth_Tx.TLF01_details.tx_key != AUTH_REPRESENTMENT))
   {
		sprintf( err_buf,
   ...
   {
      /* Also check currency code for 0200 txns. */
   ...
   if ( ret_val == false )
      nceqit_log_message( 1, 2, err_buf, "validate_host_msg",0 );
