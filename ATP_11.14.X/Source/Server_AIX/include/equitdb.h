...
{
	BYTE  rule_id           	    [11];
	BYTE  rule_bin_high				[20];
	BYTE  rule_bin_low				[20];
	BYTE  rulename       		    [31];
	BYTE  status     			    [3];
	BYTE  rule_processing     		[3];
	BYTE  single_amtlimit			[13];
	BYTE  acc_amtlimit				[13];
	BYTE  acc_amtlimit_counter				[13];
	BYTE  acc_transactionlimit 		[13];
	BYTE  acc_transactionlimit_counter 		[13];
	BYTE  response_code				[4];
	struct
	{
      BYTE exception_cardnum        [20];
	}cardnum_blocking[50];
	BYTE  future_use_1         		[13];
	BYTE  future_use_2         		[13];
	BYTE  future_use_3         		[13];
	BYTE  future_use_4         		[13];
	BYTE  future_use_5         		[13];
	BYTE  future_use_6         		[13];
	BYTE  future_use_7         		[13];
	BYTE  future_use_8         		[13];
	BYTE  future_use_9        		[13];
	BYTE  future_use_10       		[13];
...
}PTSN01_ARCHIVE,*pPTSN01_ARCHIVE;
...
typedef struct
{
    ...
    BYTE bin_type							[2];
    BYTE card_family						[4];
    BYTE bin_description					[31];
    BYTE response_text						[101];
...
typedef struct
{
	...
	char future_field5				[61];
	char purge_date                 [25];
}FTLF01_ARCHIVE, *pFTLF01_ARCHIVE;
...
/* merchant group control file */
typedef struct
{
   struct
   {
      BYTE  merchant_group_id                      [16];
   }primary_key;

    BYTE  effective_date                           [9];
...
typedef struct
{
	...
	BYTE minmajpurchamt				[13];
	BYTE maxmajpurchamt				[13];
	BYTE mininstamt					[13];
	BYTE maxinstamt					[13];
	BYTE minmajpurchlmt				[11];
	BYTE minpenaltyamt				[16];
	BYTE maxpenaltyamt				[16];
	BYTE penaltyper					[8];
	BYTE penaltyday					[4];
	BYTE minamtofaging				[19];
	BYTE mntowrtof					[3];
	BYTE BPIminamt					[13];
	BYTE brokenperdinst				[2];
	BYTE minfreepurchamt			[13];
	BYTE errorunauthamt				[16];
	BYTE fixedmpfee					[16];
	BYTE mpfeepercnt				[7];
	BYTE earlyrepayfeeper			[9];
	BYTE effective_date				[9];
	BYTE maxtrnperday				[3];
	BYTE maxamtperday				[16];
	BYTE authoverlimit				[8];
	struct
	{
	      BYTE  response_code		[3];
	      BYTE message              [3];
...
{
   BYTE  num_returned					[5];
   CR01 CR01_record             [GUI_MAX_LIST_SIZE];

} CR01_GUI_LIST, *pCR01_GUI_LIST;
/* fleet card file*/
typedef struct
{
   struct
...
typedef struct
{
   ...
   BYTE  UPI_card_std_len          [20];
   BYTE  cardholder_name           [27];
...
typedef struct
{
   ...
   {
     	BYTE  merchant_group_id   [16];
   }merchant_group_info[GUI_MAX_LIST_SIZE];

}MGF01_GUI_LIST, *pMGF01_GUI_LIST;
...
typedef struct
{
   ...
   {
     	BYTE  merchant_group_id           [16];
   }merchant_group_info[SERVICE_LIST_SIZE];

}MGF01_GUI_DROPLIST, *pMGF01_GUI_DROPLIST;
...
typedef struct
{
   ...
   {
     	BYTE  product_code           [3];
   }mppara_info[GUI_MAX_LIST_SIZE];

}MPPARA01_GUI_LIST, *pMPPARA01_GUI_LIST;
...
{
   BYTE  num_returned     [5];
   BIN01 bin_range_list   [SELECT_SERVICE_BIN01_LIST_SIZE];
...
{
	BYTE   num_returned    [5];
	MCC01  mcc01_pk 	[GUI_MAX_LIST_SIZE];
}MCC01_GUI_LIST, *pMCC01_GUI_LIST;
typedef struct
{
	BYTE   num_returned    [5];
	MCC01  mcc01_pk 	[200];
}MCC01_LIST, *pMCC01_LIST;
...
{
   BYTE  num_returned             [5];
   TPOS01 tpos01_structure          [GUI_MAX_LIST_SIZE1];

}TPOS01_GUI_LIST, *pTPOS01_GUI_LIST;

typedef struct
{
	BYTE num_returned	[5];
	FCLF01 fclf01_list	[GUI_MAX_LIST_SIZE];
}FCLF01_GUI_LIST, *pFCLF01_GUI_LIST;

typedef struct
{
	BYTE num_returned	[5];
	FRULE01 frule01_list	[GUI_MAX_LIST_SIZE];
}FRULE01_GUI_LIST, *pFRULE01_GUI_LIST;
...
typedef struct
{
	...
	BYTE date_yyyymmdd_start[9];
	BYTE date_yyyymmdd_end[9];
	BYTE time_hhmmss_start[9];
	BYTE time_hhmmss_end[9];
	BYTE merchant_id_min[16];
	BYTE merchant_id_max[16];
	BYTE terminal_id_min[9];
	BYTE terminal_id_max[9];
...
typedef struct
   {
      ...
      BYTE currency_code                      [4];
      BYTE system_date                        [9];
   }ARCHIVE_TLF01_LIST_STRUCTURE, *pARCHIVE_TLF01_LIST_STRUCTURE;

typedef struct
{
   BYTE  num_returned             [5];
   ARCHIVE_TLF01_LIST_STRUCTURE  tlf01_list_structure[SCH_MAX_LIST_SIZE];

}ARCHIVE_TLF01_LIST, *pARCHIVE_TLF01_LIST;

typedef struct
{
   BYTE  num_returned             [5];
   ARCHIVE_TLF01 tlf01_structure          [ARCH_GUI_MAX_LIST_SIZE];

}ARCHIVE_TLF01_GUI_LIST, *pARCHIVE_TLF01_GUI_LIST;
...
typedef struct
{
   ...
   BYTE system_date_min[9];
   BYTE system_date_max[9];
}ARCHIVE_TLF01_LIMITS, *pARCHIVE_TLF01_LIMITS;
...
typedef struct
{
	BYTE	merchant_id				[16];
	BYTE	card_num				[20];
	BYTE	date_yyyymmdd			[11];
	BYTE 	visa_tran_id			[33];
	BYTE	system_date				[11];
	BYTE	total_amount			[13];
	BYTE 	currency_code			[4];
	BYTE 	transaction_id			[21];
