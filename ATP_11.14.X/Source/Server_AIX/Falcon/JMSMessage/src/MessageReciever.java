 ...
 * 
 * @author 100454
 * In setup() need to configure the OutputQ Based on environment
 * 
 * This class will recieve the response from the output Queue from WAS 
 * and call the C using JNI
...
public class MessageReciever {
	...
	public MessageReciever(){
		...
		try {   
			...
			if(falconHandler.getLibFileVersion()== currentVersion){
				if(MsgSenderThread.logsCreate == true && logsenalble == true){
					logger.info("The lib file version:"+currentVersion+" is matching with the java version,Continue with initialize.");
				}
				...
				if(isInitialize == null || isInitialize.equals("null")){
					...
					if(MsgSenderThread.logsCreate == true && logsenalble == true){
						logger.info("The lib file version:"+falconHandler.getLibFileVersion()+" is greater than to the java version:"+currentVersion+",So not going to initialize.");
					}
					//Fine Call
					System.exit(-1);
				}else{
					if(MsgSenderThread.logsCreate == true && logsenalble == true){
						logger.info("The lib file version:"+falconHandler.getLibFileVersion()+" is greater than to the java version:"+currentVersion+",Still based on config going to initialize.");
					}
				}
				
			}else if(falconHandler.getLibFileVersion() < currentVersion) {
				if(MsgSenderThread.logsCreate == true && logsenalble == true){
					logger.info("The lib file version:"+falconHandler.getLibFileVersion()+" is less than to the java version:"+currentVersion+",So not going to initialize.");
				}
				//Fine Call
				System.exit(-1);
			}
	...
	public void readMessage() {
		...
		if(isConnAvailable){
			...
			try {
				...
				if(!isanyMessages){
					/*if(MsgSenderThread.logsCreate == true && logsenalble == true){
						logger.info("There are no messages inside JMS Queue=========");
					}*/
				}
	...
	private void getInitialContext() throws NamingException, ConnectException {
		...
		properties.put(Context.PROVIDER_URL, "iiop://" + prop.getProperty("serverHost") + ":"
				+ prop.getProperty("serverPort"));
		properties.put(Context.INITIAL_CONTEXT_FACTORY,
				prop.getProperty("contextFactory"));
				logger.info("SERVER IP: and SERVER PORT " +prop.getProperty("serverHost") + ":"
				+ prop.getProperty("serverPort"));
		try {
	...
	private boolean setup() {
		...
		try {
			...
			logger.info("The Connection with the server is established!!!");
			return true;
		} catch (JMSException jmse) {
			if(MsgSenderThread.logsCreate == true && logsenalble == true){
