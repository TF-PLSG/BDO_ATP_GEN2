#include<stdlib.h>
#include<stdio.h>
#include "tx_sha256_encryption.h"
...
void GetOnlyNumber( char * Supplemental_data_input, char *Supplemental_data_output)
{
    int i;
	int j = 0;
	int k=0;
	...
	int len_value=0;
	char data[2]={0};
	char Alpha_supplimental_data[100]={0};


    for (i = 0; Supplemental_data_input[i]; i++)
    {

        if (Supplemental_data_input[i] >= '0' && Supplemental_data_input[i] <= '9')
        {
            Supplemental_data_output[j] = Supplemental_data_input[i];
            j++;
        }
		else
		{
			
			Alpha_supplimental_data[k] =(Supplemental_data_input[i]-'A')+'0'; 
			
			k++;
		}
    }

	Len=strlen(Supplemental_data_output);
	if(Len<16)
	{
		len_value=16-Len;
		strncat(Supplemental_data_output,Alpha_supplimental_data,len_value);
		j=j+len_value;
		
	}
    Supplemental_data_output[j] = '\0';	

	
}	

void ExtractOnlyNumber( char * Supplemental_data_input, char *Supplemental_data_output)
{
    int i;
	int j = 0;
	int k=0;
