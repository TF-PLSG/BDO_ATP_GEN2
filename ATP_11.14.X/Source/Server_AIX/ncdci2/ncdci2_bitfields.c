...
/******************************************************************************
 *
 *  NAME:         ncdci2_build_request_field_2
 *
 ...
 *                ncdci2_ncdci2_Host_Message-  DCI host specific structure (Global)
 *
 *  RETURN VALUE:   NONE
 *
...
void ncdci2_build_request_field_2( pINT result_flag, pCHAR err_buf )
{
	...
	memset(ncdci2_HOST_MESSAGE.pan, '0', DINERS_MAX_DE2_SIZE );

	if(  true == ncdci2_is_issuing_transaction() )
	{
		len = strlen( Auth_Tx.TLF01_details.card_num );
		if( (len > 0) && (len <=DINERS_MAX_DE2_SIZE))
		{
			sprintf(ncdci2_HOST_MESSAGE.pan_len, "%02d", len	);
			memcpy( ncdci2_HOST_MESSAGE.pan,
					Auth_Tx.TLF01_details.card_num,
					len );
		}
	}

	else if ( ncdci2_isnum(Auth_Tx.TLF01_details.card_num) )
	{
		len = strlen( Auth_Tx.TLF01_details.card_num );
		if( (len > 0) && (len <= DINERS_MAX_DE2_SIZE))
		{
...
/******************************************************************************
 *
 *  NAME:         ncdci2_build_request_field_3
 *
 ...
 *  OUTPUTS:      err_buf      - Contains text to describe a reason for failure
 *                result_flag  - Set to 1 if an error occurs
 *                ncdci2_Host_Message- DCI  host specific structure (Global)
 *
 *  RTRN VALUE:   NONE
 *
 *  AUTHOR:       Abhishek Verma
...
void ncdci2_build_request_field_3( pINT result_flag, pCHAR err_buf )
{
	if(  true == ncdci2_is_issuing_transaction() )
	{
		...
		{
			memcpy( ncdci2_HOST_MESSAGE.proccode,
	...
	else if ( *result_flag == 0 )
	{
		if( AUTH_BALANCE_INQUIRY == Auth_Tx.TLF01_details.tx_key  )
		{
			if(  true == ncdci2_is_IST_transaction() )
			{
				...
				memcpy( ncdci2_HOST_MESSAGE.proccode + DINERS_DE3_PROC_CODE_FIRST_2_DIGIT_LEN,
					    Auth_Tx.TLF01_details.processing_code + DINERS_DE3_PROC_CODE_FIRST_2_DIGIT_LEN ,
					    (DINERS_DE3_SIZE -DINERS_DE3_PROC_CODE_FIRST_2_DIGIT_LEN));
			}
			else
			{
				memcpy( ncdci2_HOST_MESSAGE.proccode,
		...
		else if(true == ncdci2_is_reversal())
		{
			memcpy( ncdci2_HOST_MESSAGE.proccode,"000000",
					DINERS_DE3_SIZE );
		}
		...
		{
			memcpy( ncdci2_HOST_MESSAGE.proccode,"000000",DINERS_DE3_SIZE );
		}
		else if( strncmp(Auth_Tx.TLF01_details.processing_code,"11",2) == 0)
		{
			memcpy( ncdci2_HOST_MESSAGE.proccode,"000000",DINERS_DE3_SIZE);
		}
		else if ( ncdci2_isnum(Auth_Tx.TLF01_details.processing_code) )
		{
			memcpy( ncdci2_HOST_MESSAGE.proccode,
					Auth_Tx.TLF01_details.processing_code,
					DINERS_DE3_SIZE );
	...
	/* In order to show proceesing code in transaction lookup page( OCT-2017 Mandated changes) */
		memcpy( Auth_Tx.TLF01_details.product_codes[17].amount,ncdci2_HOST_MESSAGE.proccode,DINERS_DE3_SIZE );
	}
}

/******************************************************************************
 *
 *  NAME:         ncdci2_build_request_field_4
 *
...
void ncdci2_build_request_field_4( pINT result_flag, pCHAR err_buf )
{
   if ( *result_flag == 0 )
   {
		   ...
		   (Auth_Tx.TLF01_details.tx_key == AUTH_VOID_PRE_AUTHORIZATION))
      {
		  ...
		  {
			  memcpy( ncdci2_HOST_MESSAGE.amount,
      ...
      {
    	  if ( ncdci2_isnum(Auth_Tx.TLF01_details.total_amount) )
    	  {
    		  if((true == ncdci2_is_sbatch_transaction() ||
					Auth_Tx.TLF01_details.message_type[1] == '4') &&
					Auth_Tx.TLF01_details.product_codes[13].code[0] != '\0')
    		  {
				  memcpy( ncdci2_HOST_MESSAGE.amount,
						  Auth_Tx.TLF01_details.orig_amount,
						  DINERS_DE4_SIZE);
      ...
      else if(0 == strlen(Auth_Tx.TLF01_details.total_amount))
      {
    	 ...
    	 if(false== ncdci2_check_if_valid_zero_amount_transaction())
    	 {
			   *result_flag = 1;
			   sprintf( err_buf,
...
/******************************************************************************
 *
 *  NAME:         ncdci2_build_request_field_6
 *
 ...
 *
 ******************************************************************************/
void ncdci2_build_request_field_6( pINT result_flag, pCHAR err_buf )
{
	if(  true == ncdci2_is_issuing_transaction() )
	{
		if(strlen(Auth_Tx.ch_billing_amt)> 0)
		{
			memcpy(ncdci2_HOST_MESSAGE.ch_bill_amt,Auth_Tx.ch_billing_amt,DINERS_DE6_SIZE );
		}
	}
	else  if ( *result_flag == 0 )
	{
...
/******************************************************************************
 *
 *  NAME:         ncdci2_build_request_field_7
 *
...
void ncdci2_build_request_field_7( pINT result_flag, pCHAR err_buf )
{
   ...
   if ( *result_flag == 0 )
   {
	  ...
	  {
			memcpy( ncdci2_HOST_MESSAGE.gmt_time,
					Auth_Tx.orig_local_time_date,
					DINERS_DE7_SIZE );
	  }
      if ( strlen(Auth_Tx.TLF01_details.transmission_timestamp) > 0 )
	  {
		  memcpy( ncdci2_HOST_MESSAGE.gmt_time,
      ...
      else if ( 0 == strlen(Auth_Tx.TLF01_details.transmission_timestamp) )
	  {
    	  ...
    	  ncdci2_get_date( time_date, date_str );
    	  ncdci2_get_time( time_date, time_str );
...
/******************************************************************************
 *
 *  NAME:         ncdci2_build_request_field_11
 *
...
void ncdci2_build_request_field_11( pINT result_flag, pCHAR err_buf )
{
	if(  true == ncdci2_is_issuing_transaction() )
	{
		...
		{
			memcpy(ncdci2_HOST_MESSAGE.stan,
	...
	else if ( *result_flag == 0 )
	{
		...
		else if(  Auth_Tx.TLF01_details.message_type[1] == '4' )
		{
			...
			memcpy( Auth_Tx.TLF01_details.sys_trace_audit_num,
					ncdci2_HOST_MESSAGE.stan,
					DINERS_DE11_SIZE );
		}
		else if ( ncdci2_isnum(Auth_Tx.TLF01_details.sys_trace_audit_num) )
		{
				memcpy( ncdci2_HOST_MESSAGE.stan,
						Auth_Tx.TLF01_details.sys_trace_audit_num, 
						DINERS_DE11_SIZE );
		}
		else if ( ncdci2_isnum(Auth_Tx.TLF01_details.orig_sys_trace_num) )
		{
			memcpy( ncdci2_HOST_MESSAGE.stan,
					Auth_Tx.TLF01_details.orig_sys_trace_num, 
					DINERS_DE11_SIZE );
...
/******************************************************************************
 *
 *  NAME:         ncdci2_build_request_field_12
 *
...
void ncdci2_build_request_field_12( pINT result_flag, pCHAR err_buf )
{
	...
	if(  true == ncdci2_is_issuing_transaction() )
	{
		...
		{
			memcpy(ncdci2_HOST_MESSAGE.time,
				...
				DINERS_MESSAGE_TYPE_SIZE))
		{
			 if (DINERS_DE7_SIZE == strlen(Auth_Tx.TLF01_details.transmission_timestamp))
			 {
				 ...
				 memcpy( ncdci2_HOST_MESSAGE.time,
						 ...
						 DINERS_YY_LEN);
				  /* Store hhmmss in transmission_timestamp after storing DDMM*/
				 memcpy( ncdci2_HOST_MESSAGE.time + DINERS_YY_LEN,
						 Auth_Tx.TLF01_details.transmission_timestamp,
						 DINERS_DE7_SIZE);
			 ...
			 else
			 {
				 ...
				 ncdci2_get_time( time_date, time_str );
				  ...
				  memcpy( ncdci2_HOST_MESSAGE.time,
						  ...
						  DINERS_YYMMDD_LEN);
				  /* Store hhmmss in transmission_timestamp after storing DDMM*/
				  memcpy( ncdci2_HOST_MESSAGE.time + DINERS_YYMMDD_LEN,
						  time_str,
						  DINERS_HHMMSS_LEN);
				  if(true != ncdci2_is_reversal() )
				  {
					  memcpy( Auth_Tx.TLF01_details.product_codes[12].amount ,
							  ncdci2_HOST_MESSAGE.time,DINERS_DE12_SIZE);
				  }
			 }
		}
	}
			...
			Auth_Tx.TLF01_details.message_type[1] == '4')
	{
		...
		{
			memcpy( ncdci2_HOST_MESSAGE.time,&Auth_Tx.TLF01_details.date_yyyymmdd[2],2);
			memcpy( ncdci2_HOST_MESSAGE.time+2,
							 Auth_Tx.orig_local_time_date,
									  10);
			memcpy( Auth_Tx.TLF01_details.product_codes[12].amount ,
										  ncdci2_HOST_MESSAGE.time,DINERS_DE12_SIZE);
			 ...
			 (0 == strncmp(Auth_Tx.TLF01_details.handler_queue,"neqit2",6)))
	 {
		if(strlen(Auth_Tx.TLF01_details.product_codes[12].amount) == 0)
		{
			...
			{
				memcpy( ncdci2_HOST_MESSAGE.time,Auth_Tx.orig_local_time_date,  DINERS_DE12_SIZE);
				memcpy( Auth_Tx.TLF01_details.product_codes[12].amount ,
							  ncdci2_HOST_MESSAGE.time,DINERS_DE12_SIZE);
			}
		}
		else if(strlen(Auth_Tx.TLF01_details.product_codes[12].amount) < 12 )
		{
			memcpy( ncdci2_HOST_MESSAGE.time,&Auth_Tx.TLF01_details.date_yyyymmdd[2],2);
			memcpy( ncdci2_HOST_MESSAGE.time+2,
					Auth_Tx.TLF01_details.product_codes[12].amount, 10);
			memcpy( Auth_Tx.TLF01_details.product_codes[12].amount ,
					ncdci2_HOST_MESSAGE.time,DINERS_DE12_SIZE);
		}
		else if (strlen(Auth_Tx.TLF01_details.product_codes[12].amount) == 12)
...
/******************************************************************************
 *
 *  NAME:         ncdci2_build_request_field_13
 *
...
void ncdci2_build_request_field_13( pINT result_flag, pCHAR err_buf )
{
	...
	if( true == ncdci2_is_voice_transaction() ||
	    true == ncdci2_is_POS_Acq_transaction())
	{

	}

	else if(  true == ncdci2_is_issuing_transaction() )
	{
		if(strlen(Auth_Tx.TLF01_details.product_codes[15].code)> 0)
		{
			 memcpy( ncdci2_HOST_MESSAGE.date,
	...
	/*else if ( *result_flag == 0 )
	{
		...
		{
			ptetime_get_timestamp( time_date );
			ncdci2_get_time( time_date, Auth_Tx.TLF01_details.date_yyyymmdd );
		}
		memcpy( ncdci2_HOST_MESSAGE.date,
...
/******************************************************************************
 *
 *  NAME:         ncdci2_build_request_field_14
 *
...
void ncdci2_build_request_field_14( pINT result_flag, pCHAR err_buf )
{
    if(  true == ncdci2_is_issuing_transaction() )
	{
		...
		{
			memcpy( ncdci2_HOST_MESSAGE.date_expd,
...
/******************************************************************************
 *
 *  NAME:         ncdci2_build_request_field_22
 *
...
void ncdci2_build_request_field_22( pINT result_flag, pCHAR err_buf )
{
	...
	memset (ncdci2_HOST_MESSAGE.posentry,0x30,DINERS_DE22_SIZE);

	if(  true == ncdci2_is_issuing_transaction() )
	{
		if( strlen(Auth_Tx.TLF01_details.product_codes[16].amount) > 0)
		{
			memcpy( ncdci2_HOST_MESSAGE.posentry,
	...
	else if ( *result_flag == 0 )
	{
		...
		else if ( false == ncdci2_isnum(Auth_Tx.TLF01_details.pos_entry_mode) )
		{
			*result_flag = 1;
			sprintf( err_buf,
		...
		{
			ncdci2_build_pos_data_from_iso_pos_entry_mode();
			memcpy( ncdci2_HOST_MESSAGE.posentry,
					Auth_Tx.TLF01_details.product_codes[16].amount,
					DINERS_DE22_SIZE);
		}
...
/******************************************************************************
 *
 *  NAME:         ncdci2_build_request_field_23
 *
 ...
 *
 ******************************************************************************/
void ncdci2_build_request_field_23( pINT result_flag, pCHAR err_buf )
{
   INT	   itemp = 0;
   CHAR    temp_str[4]={0};
   CHAR    temp_str1[4]={0};
   ...
   if(  true == ncdci2_is_issuing_transaction() )
   {
	   ...
	   {
		   memcpy( ncdci2_HOST_MESSAGE.card_seq_num,
   ...
   else if ( *result_flag == 0 )
   {
		...
		else if ( true == ncdci2_is_sbatch_transaction())
		{
		    if (strlen(Auth_Tx.TLF01_details.product_codes[11].code)> 0)
			{
				memcpy( temp_str,
						Auth_Tx.TLF01_details.product_codes[11].code,
						DINERS_DE23_SIZE);
				itemp = atoi( temp_str );
				sprintf( temp_str1, "%03d", itemp );
				memcpy( ncdci2_HOST_MESSAGE.card_seq_num,
...
/******************************************************************************
 *
 *  NAME:         ncdci2_build_request_field_24
 *
 ...
 *
 ******************************************************************************/
void ncdci2_build_request_field_24( pINT result_flag, pCHAR err_buf )
{
	INT  len = 0;
	BYTE temp_len[3] = {0};
   ...
   if(  true == ncdci2_is_issuing_transaction() )
   {
	   ...
	   {
		   memcpy( ncdci2_HOST_MESSAGE.net_manag_info_code,
   ...
   else if ( *result_flag == 0 )
   {
	   ...
	   {
		   if(strlen(Auth_Tx.TLF01_details.product_codes[0].code) > 0 )
			   memcpy( ncdci2_HOST_MESSAGE.net_manag_info_code,Auth_Tx.TLF01_details.product_codes[0].code,DINERS_DE24_SIZE);
	   }
	   else if(strlen(Auth_Tx.function_code)>0)
		{
			memcpy( ncdci2_HOST_MESSAGE.net_manag_info_code,
					Auth_Tx.function_code,
					DINERS_DE24_SIZE);
		}
		else if(true == ncdci2_Auth_transactio())
		{
			memcpy( ncdci2_HOST_MESSAGE.net_manag_info_code,
					DINERS_DE24_FUNCION_CODE_MTI_1100_100,
					DINERS_DE24_SIZE);
		}
		else if(true == ncdci2_Auth_Advice_transaction())
		{
			memcpy( ncdci2_HOST_MESSAGE.net_manag_info_code,
...
/******************************************************************************
 *
 *  NAME:         ncdci2_build_request_field_25
 *
 ...
 *  OUTPUTS:      err_buf - Contains text to describe a reason for failure
 *                result_flag  - Set to 1 if an error occurs
 *                ncdci2_Host_Message- DCI  hos specific structure (Global)
 *
 *  RTRN VALUE:   NONE
 *
 *  AUTHOR:     Abhishek Verma
   ...
   if(  true == ncdci2_is_issuing_transaction() )
	{
		...
		{
			memcpy( ncdci2_HOST_MESSAGE.pos_condcd,
	...
	else if ( *result_flag == 0 )
	{
		...
		else if ( strlen(Auth_Tx.TLF01_details.product_codes[5].code) > 0)
		{
			memcpy( ncdci2_HOST_MESSAGE.pos_condcd,
					Auth_Tx.TLF01_details.product_codes[5].code,
					DINERS_DE25_SIZE);
...
/******************************************************************************
 *
 *  NAME:         ncdci2_build_request_field_26
 *
   ...
   if(  true == ncdci2_is_issuing_transaction() )
	{
		...
		{
			memcpy( ncdci2_HOST_MESSAGE.accpt_bus_cd,
...
/******************************************************************************
 *
 *  NAME:         ncdci2_build_request_field_30
 *
 ...
 *
 ******************************************************************************/
void ncdci2_build_request_field_30( pINT result_flag, pCHAR err_buf )
{
   LONG	  est_amt = 0.0;
   BYTE   estmatd_amount[13] = {0};
	...
	if(  true == ncdci2_is_issuing_transaction() )
	{
		...
		{
			memcpy( ncdci2_HOST_MESSAGE.Original_amount,
...
/******************************************************************************
 *
 *  NAME:         ncdci2_build_request_field_32
 *
...
void ncdci2_build_request_field_32( pINT result_flag, pCHAR err_buf )
{
	...
	if(  true == ncdci2_is_issuing_transaction() )
	{
		if( strlen(Auth_Tx.acq_inst_id_cd)> 0 )
		{
			...
			memcpy( ncdci2_HOST_MESSAGE.acq_inst_id,
	...
	if(len > 0)
	{
		...
		memcpy( ncdci2_HOST_MESSAGE.acq_inst_len,
				temp_len,
		...
		if ( Auth_Tx.TLF01_details.acquiring_id[0] == 0x00 )
		{
			memcpy( Auth_Tx.TLF01_details.acquiring_id,
					ncdci2_HOST_MESSAGE.acq_inst_id,
					len );
		}
	}
}
...
/******************************************************************************
 *
 *  NAME:         ncdci2_build_request_field_33
 *
...
void ncdci2_build_request_field_33( pINT result_flag, pCHAR err_buf )
{
	...
	if(  true == ncdci2_is_issuing_transaction() )
	{
		if ( strlen(Auth_Tx.forwarding_inst_id_cd) > 0 )
		{
			...
			sprintf( temp_len, "%02d", len );
			memcpy( ncdci2_HOST_MESSAGE.fwd_inst_len,
					temp_len,
					DINERS_DE33_LEN_SIZE );

			memcpy( ncdci2_HOST_MESSAGE.fwd_inst_id,
...
/******************************************************************************
 *
 *  NAME:         ncdci2_build_request_field_35
 *
...
void ncdci2_build_request_field_35( pINT result_flag, pCHAR err_buf )
{
	...
	if(  true == ncdci2_is_issuing_transaction() )
	{
		if(strlen(Auth_Tx.TLF01_details.track2) > 0 )
		{
			...
			sprintf( temp_len, "%02d", len );
			memcpy( ncdci2_HOST_MESSAGE.track2_len,
					temp_len,
					DINERS_DE35_LEN_SIZE);

			memcpy( ncdci2_HOST_MESSAGE.track2,
...
/******************************************************************************
 *
 *  NAME:         ncdci2_build_request_field_37
 *
...
void ncdci2_build_request_field_37( pINT result_flag, pCHAR err_buf )
{
		...
		Auth_Tx.TLF01_details.tx_key == AUTH_VOID_PRE_AUTHORIZATION)
	{
		...
		{
			memcpy( ncdci2_HOST_MESSAGE.rrn,
...
/******************************************************************************
 *
 *  NAME:         ncdci2_build_request_field_38
 *
...
void ncdci2_build_request_field_38( pINT result_flag, pCHAR err_buf )
{
	if(  true == ncdci2_is_issuing_transaction() )
	{
		...
		{
			memcpy(	ncdci2_HOST_MESSAGE.authcode,
	...
	else if ( *result_flag == 0 )
	{
		if (true == ncdci2_is_reversal())
		{
			...
			else if(true == ncdci2_is_voice_transaction())
			{
...
/******************************************************************************
 *
 *  NAME:         ncdci2_build_request_field_39
 *
...
void ncdci2_build_request_field_39( pINT result_flag, pCHAR err_buf )
{
   ...
   if(  true == ncdci2_is_issuing_transaction() )
	{	
		if(strlen( Auth_Tx.TLF01_details.response_code)> 0)
		{
			...
			memcpy(	ncdci2_HOST_MESSAGE.responsecd,
					dci_response_code,
					DINERS_DE39_SIZE );
		}
		else if(0 == strncmp(Auth_Tx.TLF01_details.message_type,
   ...
   else if(0 == strncmp(Auth_Tx.TLF01_details.handler_queue, "sbatch",5) &&
...

   {
	   ...
	   {
		   memcpy(	ncdci2_HOST_MESSAGE.responsecd,
				   	   DINERS_RESPONSE_APPROVED_ISSUER,
					   DINERS_DE39_SIZE );
	   }
...
/******************************************************************************
 *
 *  NAME:         ncdci2_build_request_field_40
 *
 ...
 *
 ******************************************************************************/
void ncdci2_build_request_field_40( pINT result_flag, pCHAR err_buf )
{
	CHAR temp_str[8] = {0};
	...
	if(  true == ncdci2_is_issuing_transaction() )
	{
		...
		{
			memcpy( ncdci2_HOST_MESSAGE.service_code,
...
/******************************************************************************
 *
 *  NAME:         ncdci2_build_request_field_41
 *
 *  DESCRIPTION:  This function will move the terminal Id
 *                to the Host Message structure according to DCI  host
 *                formatting specifications.
...
void ncdci2_build_request_field_41( pINT result_flag, pCHAR err_buf )
{
	...
	if(  true == ncdci2_is_issuing_transaction() )
	{
		...
		{
			memcpy( ncdci2_HOST_MESSAGE.termid,
	...
	else if ( *result_flag == 0 )
	{
		if (strlen(Auth_Tx.TLF01_details.terminal_id) > 0)
		{
			...
			memcpy( ncdci2_HOST_MESSAGE.termid,temp_str, DINERS_DE41_SIZE);
		}
	}
}
...
/******************************************************************************
 *
 *  NAME:         ncdci2_build_request_field_42
 *
...
void ncdci2_build_request_field_42( pINT result_flag, pCHAR err_buf )
{
   ...
   if(  true == ncdci2_is_issuing_transaction() )
   {
   		...
   		{
   			memcpy( ncdci2_HOST_MESSAGE.cardaccid,
   					Auth_Tx.TLF01_details.merchant_id,
					DINERS_DE42_SIZE);
   		}
   }

   else if(  true == ncdci2_is_IST_transaction() )
   {
   		if(strlen(Auth_Tx.TLF01_details.merchant_id)>0)
   		{
   			memset(ncdci2_HOST_MESSAGE.cardaccid,0x20,DINERS_DE42_SIZE);
   ...
   else if ( *result_flag == 0 )
   {
		...
		{
			memset (ncdci2_HOST_MESSAGE.cardaccid,0x20,DINERS_DE42_SIZE);
			memcpy( ncdci2_HOST_MESSAGE.cardaccid,Auth_Tx.TLF01_details.merchant_id,DINERS_DE42_SIZE);
		}
    }
}
...
/******************************************************************************
 *
 *  NAME:         ncdci2_build_request_field_43
 *
 ...
 *  OUTPUTS:      err_buf - Contains text to describe a reason for failure
 *                result_flag  - Set to 1 if an error occurs
 *                ncdci2_Host_Message- Card Acceptor Name/Location specific structure (Global)
 *
 *  RTRN VALUE:   NONE
 *
 *  AUTHOR:       Abhishek Verma
...
void ncdci2_build_request_field_43( pINT result_flag, pCHAR err_buf )
{
	...
	*/
	...
	if(  true == ncdci2_is_issuing_transaction() )
	{
		...
		{
			memset (ncdci2_HOST_MESSAGE.cardaccname,0x20,DINERS_MAX_DE43_SIZE);

			len = strlen( Auth_Tx.int_net_facility_data);
			sprintf(temp_len, "%02d", len);
			memcpy ( ncdci2_HOST_MESSAGE.cardaccname_len,
					 temp_len,
					 DINERS_DE43_LEN_SIZE);

			memcpy ( ncdci2_HOST_MESSAGE.cardaccname,
	...
	else if( *result_flag == 0)
	{
		...
		else if( 0 == strncmp(Auth_Tx.TLF01_details.handler_queue,"nceq",4) ||
				0 == strncmp(Auth_Tx.TLF01_details.handler_queue,"neqit",4)
				)
		{
			/* Filling the dummy  data as it is going to Cadencie in order to supprot
			 * nceqit to ncdci2 flow */
			/*Copying name*/
			len = 10;
			memcpy( ncdci2_HOST_MESSAGE.cardaccname,
					"BDO ATM   ",
					len);
			F43_len = len +1 ;
			ncdci2_HOST_MESSAGE.cardaccname[F43_len]=0x5c;
			F43_len = F43_len + 1;

			/*Copying street name*/
			len = 10 ;
			memcpy( ncdci2_HOST_MESSAGE.cardaccname + F43_len,
					"BDO       ",
					len);
			F43_len = F43_len + len;
			ncdci2_HOST_MESSAGE.cardaccname[F43_len]=0x5c;
			...
			len = 5 ;
			memcpy( ncdci2_HOST_MESSAGE.cardaccname + DINERS_DE43_POSTAL_CODE_POSTION,
					"00011",
					len);

			len = 3;
			memcpy( ncdci2_HOST_MESSAGE.cardaccname + DINERS_DE43_REGION_POSTION,
					"001",
					len);

			memcpy( ncdci2_HOST_MESSAGE.cardaccname + DINERS_DE43_COUNTRY_CODE_POSTION,
		...
		else if( 0 == strncmp(Auth_Tx.TLF01_details.handler_queue,"sbat",4) )
		{
			/* Filling the dummy  data as it is going to Cadencie in order to supprot
			 * nceqit to ncdci2 flow */
			/*Copying name*/
		...
		//memset( ncdci2_HOST_MESSAGE.cardaccname + F43_len,0x5c,1);
		ncdci2_HOST_MESSAGE.cardaccname[F43_len]=0x5c;
		...
		//memset( ncdci2_HOST_MESSAGE.cardaccname + city_last_position,0x5c,1);
		ncdci2_HOST_MESSAGE.cardaccname[city_last_position]=0x5c;
...
/******************************************************************************
 *
 *  NAME:         ncdci2_build_request_field_44
 *
...
void ncdci2_build_request_field_44( pINT result_flag, pCHAR err_buf )
{
	...
	if(  true == ncdci2_is_issuing_transaction() )
	{
		if((strlen(Auth_Tx.acct_id_1)> 0 ))
		{
			...
			sprintf(temp_len, "%02d", len);
			memcpy( ncdci2_HOST_MESSAGE.addl_rsp_len,temp_len,DINERS_DE44_LEN_SIZE);

			memcpy( ncdci2_HOST_MESSAGE.addlrspdata,Auth_Tx.acct_id_1+2,len);
		}
	}
	...
	else if ( *result_flag == 0 )
	{
		...
		{
			len = strlen(Auth_Tx.TLF01_details.product_codes[18].amount);
			sprintf(temp_len, "%02d", len);
			memcpy( ncdci2_HOST_MESSAGE.addl_rsp_len,temp_len,DINERS_DE44_LEN_SIZE);

			memcpy( ncdci2_HOST_MESSAGE.addlrspdata,Auth_Tx.TLF01_details.product_codes[18].amount,len);
		}
	}
}
...
/******************************************************************************
 *
 *  NAME:         ncdci2_build_request_field_45
 *
...
void ncdci2_build_request_field_45( pINT result_flag, pCHAR err_buf )
{
   ...
   if(  true == ncdci2_is_issuing_transaction() )
   {
		if(strlen(Auth_Tx.TLF01_details.track1)>0)
		{
			...
			memcpy( ncdci2_HOST_MESSAGE.track1_len,
					temp_len ,
					DINERS_DE45_LEN_SIZE);

			memcpy( ncdci2_HOST_MESSAGE.track1,
...
/******************************************************************************
 *
 *  NAME:         ncdci2_build_request_field_46
 *
 ...
 *
 ******************************************************************************/
void ncdci2_build_request_field_46( pINT result_flag, pCHAR err_buf )
{
	INT   len = 0;
	CHAR  temp_len[4] = {0};
	...
	if(  true == ncdci2_is_issuing_transaction() )
	{
		if( strlen(Auth_Tx.TLF01_details.tax_amount ) > 0)
		{
			...
			memcpy( ncdci2_HOST_MESSAGE.surcharge_fee_len,
					temp_len,
					DINERS_DE46_LEN_SIZE);

			memcpy(	ncdci2_HOST_MESSAGE.surcharge_fee,
...
/******************************************************************************
 *
 *  NAME:         ncdci2_build_request_field_48
 *
...
void ncdci2_build_request_field_48( pINT result_flag, pCHAR err_buf )
{
	...
	if(  true == ncdci2_is_issuing_transaction() )
	{
		if( strlen(Auth_Tx.TLF01_details.voice_auth_text) > 0)
		{
			...
			memcpy( ncdci2_HOST_MESSAGE.addl_data_len,
					temp_len,
					DINERS_DE48_LEN_SIZE);

			memcpy(	ncdci2_HOST_MESSAGE.addl_data,
...
/******************************************************************************
 *
 *  NAME:         ncdci2_build_request_field_49
 *
...
void ncdci2_build_request_field_49( pINT result_flag, pCHAR err_buf )
{
	if(  true == ncdci2_is_issuing_transaction() )
	{
		...
		{
			memcpy( ncdci2_HOST_MESSAGE.tran_curr_cd,
	...
	{
			memcpy( ncdci2_HOST_MESSAGE.tran_curr_cd,
					Auth_Tx.TLF01_details.product_codes[13].code,
					DINERS_DE49_SIZE);
	}
	else if ( *result_flag == 0 )
	{
		...
		{
		   	if ( false == ncdci2_isalphanum(Auth_Tx.TLF01_details.currency_code) )
			{
				 *result_flag = 1;
				 sprintf( err_buf,
...
/******************************************************************************
 *
 *  NAME:         ncdci2_build_request_field_51
 *
 ...
 *
 ******************************************************************************/
void ncdci2_build_request_field_51 ( pINT result_flag, pCHAR err_buf )
{
	if(  true == ncdci2_is_issuing_transaction() )
	{
		if (Auth_Tx.ch_billing_curr_code [0] != '\0')
		{
			memcpy( ncdci2_HOST_MESSAGE.card_curr_cd,
...
/******************************************************************************
 *
 *  NAME:         ncdci2_build_request_field_52
 *
...
void ncdci2_build_request_field_52( pINT result_flag, pCHAR err_buf )
{
   ...
   if(  true == ncdci2_is_issuing_transaction() )
   {
	   if ( true == ncdci2_pin_block_exists() )
	   {
		    memset( ncdci2_HOST_MESSAGE.pinblock, 0x00, DINERS_DE52_SIZE );
		    ncdci2_ascii_to_bin_with_length( Auth_Tx.TLF01_details.pin_block,
											 16,
											 &tempstr[0], sizeof(tempstr) );
			memcpy( ncdci2_HOST_MESSAGE.pinblock,
					tempstr,
					DINERS_DE52_SIZE);
	   }
   }
...
/******************************************************************************
 *
 *  NAME:         ncdci2_build_request_field_54
 *
 ...
 *
 ******************************************************************************/
void ncdci2_build_request_field_54 ( pINT result_flag, pCHAR err_buf )
{
	INT   len     = 0;
	BYTE temp_len[4] = {0};
	...
	if(  true == ncdci2_is_issuing_transaction() )
	{
		...
		if(len > 0)
		{
			...
			memcpy( ncdci2_HOST_MESSAGE.AddlAmts_len,
					temp_len,
					DINERS_DE54_LEN_SIZE);

			memcpy( ncdci2_HOST_MESSAGE.AddlAmts,
					Auth_Tx.TLF01_details.mcard_banknet ,
					len);
		}
	}
...
/******************************************************************************
 *
 *  NAME:         ncdci2_build_request_field_55
 *
...
void ncdci2_build_request_field_55 ( pINT result_flag, pCHAR err_buf )
{
	INT  len = 0;
	INT  i = 0;
	INT  emv_len = 0;
	...
	//ncdci2_HOST_MESSAGE.IccData_len
	if(  true == ncdci2_is_issuing_transaction() )// Issuing tx, Coming from ncdci
	{
		// Response TAG should populate
		if(strlen(Auth_Tx.EMV_details.future_field1) > 0)
		{
			memcpy( ncdci2_HOST_MESSAGE.IccData_len,Auth_Tx.EMV_details.future_field1,3);
			strncpy(temp_ascii_len,ncdci2_HOST_MESSAGE.IccData_len,3);
			len = atoi(temp_ascii_len);
			memcpy( ncdci2_HOST_MESSAGE.IccData,Auth_Tx.EMV_details.future_field1+3,len);

		}
		else
		{
			memcpy(temp_ascii_len,Auth_Tx.EMV_details.emv_block,3); // First 3 bytes length of ncdci EMV data
			memcpy( ncdci2_HOST_MESSAGE.IccData_len,temp_ascii_len,3);
			len = atoi(temp_ascii_len);
			memcpy( ncdci2_HOST_MESSAGE.IccData,Auth_Tx.EMV_details.emv_block+3,len);
		}
	...
	else if(true == ncdci2_is_IST_transaction())
	{
		...
		/* Checking the header length and the actual data length to be sent to DCI from IST.
		...
		{
			ncdci2_log_message( 2, 3, "DE55, Header length and actual data length mis match", "ncdci2_build_request_field_55",1 );
		}
		if(emv_len == 0)
		{
			return ;
		}
		sprintf(temp_ascii_len, "%03d", emv_len);
		memcpy( ncdci2_HOST_MESSAGE.IccData_len,temp_ascii_len,3);
		memcpy( ncdci2_HOST_MESSAGE.IccData,&Auth_Tx.EMV_details.emv_block[4],emv_len);
	}
	else if(true == ncdci2_is_sbatch_transaction())
	{
		if(strncmp(Auth_Tx.TLF01_details.issuer_id, "ncdci", 5) == 0)
	{
			memcpy(temp_ascii_len,Auth_Tx.EMV_details.emv_block,3); // First 3 bytes length of ncdci EMV data
		memcpy( ncdci2_HOST_MESSAGE.IccData_len,temp_ascii_len,3);
		len = atoi(temp_ascii_len);
		...
		else
		{
			...
			{
				sprintf (ncdci2_HOST_MESSAGE.IccData_len,"%03d",emv_len);
				// EMV data
				memcpy( ncdci2_HOST_MESSAGE.IccData,Auth_Tx.EMV_details.emv_block+2,emv_len);
			}
...
/******************************************************************************
 *
 *  NAME:         ncdci2_build_request_field_56
 *
...
void ncdci2_build_request_field_56 ( pINT result_flag, pCHAR err_buf )
{
	...
	if(  true == ncdci2_is_issuing_transaction() )
	{
		if ( strlen(Auth_Tx.TLF01_details.orig_message)> 0)
		{
			...
			if(0 == strncmp(Auth_Tx.TLF01_details.orig_message,"0100",4))
			{
				strcat (orig_data, "1100");
			}
			else if(0 == strncmp(Auth_Tx.TLF01_details.orig_message,"04",2))
			{
				strcat (orig_data, "1420");
			}
			else
			{
			...
			memcpy( ncdci2_HOST_MESSAGE.orig_data_len,
					temp_len,
					DINERS_DE56_LEN_SIZE);

			memcpy (ncdci2_HOST_MESSAGE.orig_data,
					orig_data,
					len);
		}
	}
...
/******************************************************************************
 *
 *  NAME:         ncdci2_build_request_field_58
 *
 ...
 *
 ******************************************************************************/
void ncdci2_build_request_field_58( pINT result_flag, pCHAR err_buf )
{
	INT  len = 0;
	CHAR temp_len[3] = {0};
	...
	if(  true == ncdci2_is_issuing_transaction() )
	{
		...
		if( len > 0)
		{
			memcpy( ncdci2_HOST_MESSAGE.Auth_agent_IIC,
					Auth_Tx.TLF01_details.product_codes[6].amount,
					DINERS_DE58_SIZE);
			sprintf( temp_len, "%02d", len );
			memcpy( ncdci2_HOST_MESSAGE.Auth_agent_IIC_len,
					temp_len,
					DINERS_DE32_LEN_SIZE );
		}
	}
...
/******************************************************************************
 *
 *  NAME:         ncdci2_build_request_field_59
 *
 ...
 *
 ******************************************************************************/
void ncdci2_build_request_field_59( pINT result_flag, pCHAR err_buf )
{
	INT   len     = 0;
	BYTE  temp_len[4] = {0};
    CHAR  tempstr[20] = {0};
   ...
   if(  true == ncdci2_is_issuing_transaction() )
	   ...
	   {
		   len = strlen();
		   memcpy(ncdci2_HOST_MESSAGE.NetworkID_Transport_data,
				   tempstr,
				   DINERS_DE59_LEN_SIZE);
		   len = strlen(ncdci2_HOST_MESSAGE.NetworkID_Transport_data);
		   sprintf(temp_len, "%03d", len);
		   memcpy(ncdci2_HOST_MESSAGE.NetworkID_Transport_data_len,
				   tempstr,
		   		   DINERS_DE59_LEN_SIZE);
...
/******************************************************************************
 *
 *  NAME:         ncdci2_build_request_field_62
 *
...
void ncdci2_build_request_field_62 ( pINT result_flag, pCHAR err_buf )
{
	...
	if(  true == ncdci2_is_issuing_transaction() )
	{
		...
		{
			sprintf( temp_len, "%03d", len );
			memcpy (ncdci2_HOST_MESSAGE.addition_auth_data_len,temp_len, DINERS_DE62_LEN_SIZE);
			memcpy (ncdci2_HOST_MESSAGE.addition_auth_data, Auth_Tx.TLF01_details.product_codes[19].amount,len);
			  ...
			  Auth_Tx.TLF01_details.tx_key == AUTH_CARD_VERIFICATION )
	{
		...
		{
			memcpy (ncdci2_HOST_MESSAGE.addition_auth_data, DINERS_DE62_TAG05_VALUE_FOR_INC_PRE_AUTH,6);
		}
		else
		{
			memcpy (ncdci2_HOST_MESSAGE.addition_auth_data, DINERS_DE62_TAG05_VALUE_FOR_PRE_AUTH,6);
	...
	else if(true == ncdci2_check_if_trx_is_dci_MobilePOS_NFC_Tap2Phone_trx_with_pin())
	{
		sprintf( temp_len, "%03d", DINERS_DE62_TAG01_VALUE_FOR_TAP_ON_MOBILR_LEN );
		memcpy (ncdci2_HOST_MESSAGE.addition_auth_data_len,temp_len, DINERS_DE62_LEN_SIZE);
		memcpy (ncdci2_HOST_MESSAGE.addition_auth_data, DINERS_DE62_TAG01_VALUE_FOR_TAP_ON_MOBILR,
	...
	else if(true == ncdci2_check_if_trx_is_dci_MobilePOS_NFC_Tap2Phone_trx_without_pin())
	{
		sprintf( temp_len, "%03d", DINERS_DE62_TAG01_VALUE_FOR_TAP_ON_MOBILR_LEN );
		memcpy (ncdci2_HOST_MESSAGE.addition_auth_data_len,temp_len, DINERS_DE62_LEN_SIZE);
		memcpy (ncdci2_HOST_MESSAGE.addition_auth_data, DINERS_DE62_TAG01_VALUE_FOR_TAP_ON_MOBILR,
...
/*	else if(true == ncdci2_is_POS_Acq_transaction())
	{
		if(true == ncdci2_check_RPS_Transaction())
		{
			sprintf( temp_len, "%03d", DINERS_DE62_TAG05_VALUE_FOR_RPS_LEN );
			memcpy (ncdci2_HOST_MESSAGE.addition_auth_data_len,temp_len, DINERS_DE62_LEN_SIZE);
			memcpy (ncdci2_HOST_MESSAGE.addition_auth_data, DINERS_DE62_TAG05_VALUE_FOR_RPS,DINERS_DE62_TAG05_VALUE_FOR_RPS_LEN);
...
/******************************************************************************
 *
 *  NAME:         ncdci2_build_request_field_64
 *
 ...
 *
 ******************************************************************************/
void ncdci2_build_request_field_64 ( pINT result_flag, pCHAR err_buf )
{
	BYTE temp_len[4] = {0};
	INT len = 0;
	...
	if(  true == ncdci2_is_issuing_transaction() )
	{
		...
		{
			memcpy( ncdci2_HOST_MESSAGE.mess_auth_code,
...
/******************************************************************************
 *
 *  NAME:         ncdci2_build_request_field_72
 *
 ...
 *
 ******************************************************************************/
void ncdci2_build_request_field_72 ( pINT result_flag, pCHAR err_buf )
{
	BYTE temp_len[4] = {0};
	INT len = 0;
 ...
 *
 ******************************************************************************/
void ncdci2_build_request_field_92( pINT result_flag, pCHAR err_buf )
{
	if(  true == ncdci2_is_issuing_transaction() )
	{
		/*Find the length*/
		...
		{
			memcpy( ncdci2_HOST_MESSAGE.ctry_code,
	...
	else if ( *result_flag == 0 )
	{
		...
		{
			if ( ncdci2_isnum(Ncf01_I.country_code) )
			{
				memcpy( ncdci2_HOST_MESSAGE.ctry_code,
			...
			else
			{
				*result_flag = 1;
				sprintf( err_buf,
				 ...
				 true == ncdci2_is_IST_transaction()||
				 true == ncdci2_is_sbatch_transaction())
		{
			memcpy( ncdci2_HOST_MESSAGE.ctry_code,
					DINERS_PESO_CURRENCY_CODE,
					DINERS_DE92_SIZE );
					extendedbit=true;
		}
		else
...
/******************************************************************************
 *
 *  NAME:         ncdci2_build_request_field_93
 *
 ...
 *
 ******************************************************************************/
void ncdci2_build_request_field_93 ( pINT result_flag, pCHAR err_buf )
{
	INT  len = 0;
	BYTE temp_len[3] = {0};
	...
	if(  true == ncdci2_is_issuing_transaction() )
	{
		...
		if(strlen(Auth_Tx.TLF01_details.product_codes[5].amount)> 0)
		{
			...
			memcpy( ncdci2_HOST_MESSAGE.TranDestination_ICC_len,
					temp_len,
					DINERS_DE94_LEN_SIZE);

			/*Store data*/
			memcpy( ncdci2_HOST_MESSAGE.TranDestination_ICC,
...
/******************************************************************************
 *
 *  NAME:         ncdci2_build_request_field_94
 *
 ...
 *
 ******************************************************************************/
void ncdci2_build_request_field_94 ( pINT result_flag, pCHAR err_buf )
{
	INT  len = 0;
	BYTE temp_len[3] = {0};
	...
	if(  true == ncdci2_is_issuing_transaction() )
	{
		...
		if(strlen(Auth_Tx.TLF01_details.product_codes[4].amount)> 0)
		{
			...
			memcpy( ncdci2_HOST_MESSAGE.TranOriginator_ICC_len,
					temp_len,
					DINERS_DE94_LEN_SIZE);

			/*Store data*/
			memcpy( ncdci2_HOST_MESSAGE.TranOriginator_ICC,
...
/******************************************************************************
 *
 *  NAME:         ncdci2_build_request_field_96
 *
 ...
 *
 ******************************************************************************/
void ncdci2_build_request_field_96 ( pINT result_flag, pCHAR err_buf )
{
	INT  len = 0;
	BYTE temp_len[4] = {0};
	...
	if(  true == ncdci2_is_issuing_transaction() )
	{
		 if (keyblock_enable_flag_for_DCI == KEYBLOCK_ENABLE)
		 {
			...
			{
				if(strlen(Ncf01_I_key_exchange_Keyblock.iss_rtdcwk_keyblock)> 0)
				{
					len = strlen(Ncf01_I_key_exchange_Keyblock.iss_rtdcwk_keyblock);
					sprintf(temp_len, "%03d", len);
					memcpy( ncdci2_HOST_MESSAGE.Key_Managment_data_len,
							temp_len, DINERS_DE96_LEN_SIZE);

					/*Store data*/
					memcpy( ncdci2_HOST_MESSAGE.Key_Managment_data,
							Ncf01_I_key_exchange_Keyblock.iss_rtdcwk_keyblock, len);
					extendedbit=true;
				}
			}
		 }
		 else
		 {
			 ...
			 if(0 == strncmp(Auth_Tx.function_code,
									...
									DINERS_NETWORK_FUNCTION_CODE_LEN))
			 {
				if(strlen(Ncf01_I_key_exchange.retired_cwk)> 0)
				{
					...
					/*Store data*/
					memcpy( ncdci2_HOST_MESSAGE.Key_Managment_data,
							Ncf01_I_key_exchange.retired_cwk, len);
					extendedbit=true;
				}
			}
		 }
	...
	else if( *result_flag == 0 )
	{
		if(strlen(Ncf01_I.receiving_inst_id_code)> 0)
		{
			memcpy(ncdci2_HOST_MESSAGE.recv_ins_id_code,
					Ncf01_I.receiving_inst_id_code, strlen(Ncf01_I.receiving_inst_id_code));
			len = strlen(Ncf01_I.receiving_inst_id_code);
			sprintf(temp_len, "%02d", len);
			memcpy( ncdci2_HOST_MESSAGE.recv_ins_id_code_len, temp_len, DINERS_DE100_SIZE_LEN);
			extendedbit=true;
		}
		else
		{
			strcpy( err_buf, "Receiving Institution Identification Code is Empty (field 100)");
		}
	}
 }
...
/******************************************************************************
 *
 *  NAME:         ncdci2_build_request_field_100
 *
 ...
 *
 ******************************************************************************/
void ncdci2_build_request_field_100 ( pINT result_flag, pCHAR err_buf )
{
	INT  len = 0;
	BYTE temp_len[3] = {0};
	...
	{
		if(strlen(Ncf01_I.acquirer_inst_id_code)> 0)
		{
			memcpy(ncdci2_HOST_MESSAGE.recv_ins_id_code,
...
/******************************************************************************
 *
 *  NAME:         ncdci2_build_request_field_101
 *
 ...
 *
 ******************************************************************************/
void ncdci2_build_request_field_101 ( pINT result_flag, pCHAR err_buf )
{
    INT len = 0;
	BYTE temp_len[3] = {0};
	...
	if(  true == ncdci2_is_issuing_transaction() )
	{
		/* Need to identify the filed and build
		if( strlen(Auth_Tx.TLF01_details.resp_source) > 0)
		{

			len = atoi(Auth_Tx.TLF01_details.resp_source);
			sprintf(temp_len, "%02d", len);
			memcpy( ncdci2_HOST_MESSAGE.networ_ref_id_len,
					temp_len,
					DINERS_DE125_SIZE_LEN);

			memcpy( ncdci2_HOST_MESSAGE.networ_ref_id,
...
/******************************************************************************
 *
 *  NAME:         ncdci2_build_request_field_106
 *
...
void ncdci2_build_request_field_106 ( pINT result_flag, pCHAR err_buf )
{
	...
	if(  true == ncdci2_is_issuing_transaction() )
	{
		if(strlen(Auth_Tx.TLF01_details.product_codes[13].amount)> 0)
		{
			...
			sprintf(temp_len, "%03d", len);
			memcpy( ncdci2_HOST_MESSAGE.tran_data_len, temp_len, DINERS_DE106_SIZE_LEN);

					/*Store data*/
			memcpy(ncdci2_HOST_MESSAGE.tran_data,
	...
	else if (*result_flag == 0)
	{
		...
		{
			strcpy( err_buf, " Transactional Data Code is Empty (field 100)");
		}
...
/******************************************************************************
 *
 *  NAME:         ncdci2_build_request_field_122
 *
...
void ncdci2_build_request_field_122 ( pINT result_flag, pCHAR err_buf )
{
	...
	if(  true == ncdci2_is_issuing_transaction() )
	{
		...
		{
			len = atoi(Auth_Tx.TLF01_details.dcf01_retired_cwk);
			if (len < DINERS_MAX_DE122_SIZE)
			{
				sprintf(temp_len, "%03d", len);
				memcpy( ncdci2_HOST_MESSAGE.Add_authentication_data_len, temp_len, DINERS_DE122_SIZE_LEN);
				memcpy(ncdci2_HOST_MESSAGE.Add_authentication_data, Auth_Tx.future_use_4, len );
			}
	...
	else if (*result_flag == 0)
	{
		if (strlen(Auth_Tx.TLF01_details.dcf01_retired_cwk) > 0)
		{
				...
				if (len < DINERS_MAX_DE122_SIZE)
				{
					sprintf(temp_len, "%03d", len);
					memcpy( ncdci2_HOST_MESSAGE.Add_authentication_data_len, temp_len, DINERS_DE122_SIZE_LEN);
					memcpy(ncdci2_HOST_MESSAGE.Add_authentication_data, Auth_Tx.TLF01_details.dcf01_retired_cwk, len );
				}
		}
...
/******************************************************************************
 *
 *  NAME:         ncdci2_build_request_field_123
 *
 ...
 *
 ******************************************************************************/
void ncdci2_build_request_field_123 ( pINT result_flag, pCHAR err_buf )
{
    INT len = 0;
	BYTE temp_len[4] = {0};
	...
	else if(ncdci2_is_reversal())
	{
		...
		{
			memset(ncdci2_HOST_MESSAGE.networ_ref_id_len, 0x00, DINERS_DE123_SIZE_LEN);
			memset(ncdci2_HOST_MESSAGE.networ_ref_id, 0x00,
					sizeof(ncdci2_HOST_MESSAGE.networ_ref_id)-1);
		}
	}
}
...
/******************************************************************************
 *
 *  NAME:         ncdci2_build_request_field_124
 *
 ...
 *
 ******************************************************************************/
void ncdci2_build_request_field_124 ( pINT result_flag, pCHAR err_buf )
{
    INT len = 0;
	CHAR  temp_len[4] = {0};
	...
	if(  true == ncdci2_is_issuing_transaction() )
	{
		if(strlen (Auth_Tx.future_use_5) > 0)
		{
			...
			sprintf(temp_len, "%03d", len);
			memcpy( ncdci2_HOST_MESSAGE.transaction_qualifer_len,
					temp_len,
					DINERS_DE125_SIZE_LEN);

			memcpy( ncdci2_HOST_MESSAGE.transaction_qualifer,
...
/******************************************************************************
 *
 *  NAME:         ncdci2_build_request_field_125
 *
 ...
 *
 ******************************************************************************/
void ncdci2_build_request_field_125 ( pINT result_flag, pCHAR err_buf )
{
    INT len = 0;
	CHAR  temp_len[4] = {0};
	...
	if(  true == ncdci2_is_issuing_transaction() )
	{
		if(strlen(Auth_Tx.TLF01_details.vehicle_number)>0)
		{
			...
			sprintf(temp_len, "%03d", len);
			memcpy( ncdci2_HOST_MESSAGE.replacement_pin_len,
					temp_len,
					DINERS_DE125_SIZE_LEN);

			memcpy( ncdci2_HOST_MESSAGE.replacement_pin,
 ...
 *  DATA ELEMENT FORMAT :n, 19 LLVAR
 *
 *  INPUTS:       ncdci2_Host_Message- DCI  host specific structure (Global)
...
void ncdci2_move_field_2( pINT result_flag, pCHAR err_buf )
{
	...
	memcpy( Auth_Tx.TLF01_details.card_num_len,	ncdci2_HOST_MESSAGE.pan_len,
			DINERS_DE2_LEN_SIZE );
	...
	{
		memcpy( Auth_Tx.TLF01_details.card_num,ncdci2_HOST_MESSAGE.pan, len );
		if (false == ncdci2_isnum(Auth_Tx.TLF01_details.card_num))
		{
...
void ncdci2_move_field_3( pINT result_flag, pCHAR err_buf )
{
	if ( *result_flag == 0 )
	{
		if(ncdci2_HOST_MESSAGE.proccode[0] != 0x00 )
		{
			memcpy( Auth_Tx.TLF01_details.processing_code,
					ncdci2_HOST_MESSAGE.proccode,
					DINERS_DE3_SIZE );

			if (false == ncdci2_isnum(Auth_Tx.TLF01_details.processing_code))
			{
				sprintf( err_buf,
...
void ncdci2_move_field_4( pINT result_flag, pCHAR err_buf )
{
   ...
   if ( *result_flag == 0 )
   {
	    if(ncdci2_HOST_MESSAGE.amount[0] != 0x00 )
   		{
   		    memcpy( Auth_Tx.TLF01_details.total_amount,
        	       ncdci2_HOST_MESSAGE.amount,
            	    DINERS_DE4_SIZE );
			  
		  	if (false ==(ret_val = ncdci2_isnum(Auth_Tx.TLF01_details.total_amount)))
      	  	{
         			*result_flag = 1;
         		  	sprintf( err_buf,
 ...
 *
 ******************************************************************************/
void ncdci2_move_field_6( pINT result_flag, pCHAR err_buf )
{
	INT   len     = 0 ;

	if ( *result_flag == 0 )
	{
		if(ncdci2_HOST_MESSAGE.ch_bill_amt[0] != 0x00 )
		{
			memcpy( Auth_Tx.ch_billing_amt,
					ncdci2_HOST_MESSAGE.ch_bill_amt,
					DINERS_DE6_SIZE );

			 if (false == ncdci2_isnum(Auth_Tx.ch_billing_amt))
			 {
				*result_flag = 1;
				sprintf( err_buf,
...
void ncdci2_move_field_7( pINT result_flag, pCHAR err_buf )
{
   ...
   if ( *result_flag == 0 )
   {
	    if(ncdci2_HOST_MESSAGE.gmt_time[0] != 0x00 )
   		{
	       	 memcpy( Auth_Tx.TLF01_details.transmission_timestamp,
        	    	 ncdci2_HOST_MESSAGE.gmt_time,
            		 DINERS_DE7_SIZE );
			 
			 ret_val = ncdci2_isnum(Auth_Tx.TLF01_details.transmission_timestamp);
         	 if ( ret_val == false )
         	 {
            	*result_flag = 1;
           		 sprintf( err_buf,
         	...
         	else if ( false == ncdci2_validate_date_time(Auth_Tx.TLF01_details.transmission_timestamp))
         	{
            	ret_val = false;
            	*result_flag = 1;
            	sprintf( err_buf,
...
void ncdci2_move_field_11( pINT result_flag, pCHAR err_buf )
{
   ...
   if ( *result_flag == 0 )
   {
	    if(ncdci2_HOST_MESSAGE.stan[0] != 0x00 )
   		{
   			memcpy( Auth_Tx.TLF01_details.sys_trace_audit_num,
            	    ncdci2_HOST_MESSAGE.stan,
            	    DINERS_DE11_SIZE );
			ret_val = ncdci2_isnum(Auth_Tx.TLF01_details.sys_trace_audit_num);
      		if ( ret_val == false )
      		{
        		*result_flag = 1;
         	 	sprintf( err_buf,
...
void ncdci2_move_field_12( pINT result_flag, pCHAR err_buf )
{
   ...
   if ( *result_flag == 0 )
   {
	  if(ncdci2_HOST_MESSAGE.time[0] != 0x00 )
   	  {
		  /* store Local Transaction Date And Time */
  			...
  			memcpy( Auth_Tx.TLF01_details.date_yyyymmdd,time,DINERS_YYYYMMDD_LEN);
  			memcpy( Auth_Tx.TLF01_details.time_hhmmss,
  			      	time + DINERS_HHMMSS_START_POS,DINERS_HHMMSS_LEN);
   	  		ret_val = ncdci2_isnum(time);
      		if ( ret_val == false )
   			{
       			*result_flag = 1;
       			sprintf( err_buf,
...
void ncdci2_move_field_13( pINT result_flag, pCHAR err_buf )
{
   ...
   if ( *result_flag == 0 )
   {
	   if(ncdci2_HOST_MESSAGE.date[0] != 0x00 )
		{
		  memcpy( Auth_Tx.TLF01_details.product_codes[15].code,
				  ncdci2_HOST_MESSAGE.date,DINERS_DE13_SIZE);
		}
   	}
}
...
void ncdci2_move_field_14( pINT result_flag, pCHAR err_buf )
{
   ...
   if ( *result_flag == 0 )
   {
	  if(ncdci2_HOST_MESSAGE.date_expd[0] != 0x00 )
	  {
		  memcpy( Auth_Tx.TLF01_details.exp_date,
				  ncdci2_HOST_MESSAGE.date_expd,
				  DINERS_DE14_SIZE);

		   ret_val = ncdci2_isnum(Auth_Tx.TLF01_details.exp_date);
		   if ( ret_val == false )
      	   {
		        *result_flag = 1;
        		 sprintf( err_buf,
...
void ncdci2_move_field_22( pINT result_flag, pCHAR err_buf )
{
   ...
   if ( *result_flag == 0 )
   {
	    if(ncdci2_HOST_MESSAGE.posentry[0] != 0x00 )
   		{
   			memcpy (Auth_Tx.TLF01_details.product_codes[16].amount,
   					ncdci2_HOST_MESSAGE.posentry, DINERS_DE22_SIZE);
   			memcpy (temp_posentry,ncdci2_HOST_MESSAGE.posentry,
   			   		DINERS_DE22_SIZE);
			ret_val = ncdci2_isalphanum(temp_posentry);
	      	if ( ret_val == false )
    		{
			     *result_flag = 1;
			      sprintf( err_buf,
                	 	  "Invalid POS Entry Mode (field 22): %s,",
						  ncdci2_HOST_MESSAGE.posentry );
      		}
	      	else
	      	{
	      		ncdci2_populate_iso_pos_entry_mode();
	      	}
		}
   }
}
 ...
 *
 ******************************************************************************/
void ncdci2_move_field_23( pINT result_flag, pCHAR err_buf )
{
   INT   ret_val = true;
   INT   len     = 0 ;
   ...
   if ( *result_flag == 0 )
   {
	    if(ncdci2_HOST_MESSAGE.card_seq_num[0] != 0x00 )
   	    {
   	    	memcpy( Auth_Tx.EMV_details.pan_sequence_number ,
				    ncdci2_HOST_MESSAGE.card_seq_num,
				    DINERS_DE23_SIZE );

			ret_val = ncdci2_isnum(Auth_Tx.EMV_details.pan_sequence_number);
		    if ( ret_val == false )
	    	{
			      *result_flag = 1;
			      sprintf( err_buf,
 ...
 *
 ******************************************************************************/
void ncdci2_move_field_24( pINT result_flag, pCHAR err_buf )
{
   ...
   if ( *result_flag == 0 )
   {
	    if(ncdci2_HOST_MESSAGE.net_manag_info_code[0] != 0x00 )
		{
			memcpy( Auth_Tx.function_code,
					ncdci2_HOST_MESSAGE.net_manag_info_code,
					DINERS_DE24_SIZE);
 ...
 *  DATA ELEMENT FORMAT :n 4
 *
 *  INPUTS:       ncdci2_Host_Message- DCI  host  specific specific structure (Global)
 *                result_flag  - If 1, an error occurred else where
 *
 *  OUTPUTS:      err_buf     - Contains text to describe a reason for failure
 *                Auth_Tx     - (Global)Structure populated from Host Message
...
void ncdci2_move_field_25( pINT result_flag, pCHAR err_buf )
{
   ...
   if ( *result_flag == 0 )
   {
	  if(ncdci2_HOST_MESSAGE.pos_condcd[0] != 0x00 )
   	  {
			memcpy( Auth_Tx.TLF01_details.product_codes[5].code,
				    ncdci2_HOST_MESSAGE.pos_condcd,
				    DINERS_DE25_SIZE);

		    ret_val = ncdci2_isnum(Auth_Tx.TLF01_details.product_codes[5].code);
			if ( ret_val == false )
      		{
			   *result_flag = 1;
			    sprintf( err_buf,
 ...
 *  DATA ELEMENT FORMAT :n 4
 *
 *  INPUTS:       ncdci2_Host_Message- Equitable specific structure (Global)
 *                result_flag  - If 1, an error occurred else where
 *
 *  OUTPUTS:      err_buf     - Contains text to describe a reason for failure
 *                Auth_Tx     - (Global)Structure populated from Host Message
...
void ncdci2_move_field_26( pINT result_flag, pCHAR err_buf )
{
   ...
   if ( *result_flag == 0 )
   {
	   if(ncdci2_HOST_MESSAGE.accpt_bus_cd[0] != 0x00 )
   	   {
	   	   memcpy( Auth_Tx.TLF01_details.category_code,
				   ncdci2_HOST_MESSAGE.accpt_bus_cd,
				   DINERS_DE26_SIZE );
				
			ret_val = ncdci2_isnum(Auth_Tx.TLF01_details.category_code);
			if ( ret_val == false )
			{
				*result_flag = 1;
				sprintf( err_buf,
 ...
 *
 ******************************************************************************/
void ncdci2_move_field_30( pINT result_flag, pCHAR err_buf )
{
   INT   ret_val = true;
   INT   len     = 0;
   ...
   if ( *result_flag == 0 )
   {
	   if(ncdci2_HOST_MESSAGE.Original_amount[0] != 0x00 )
   	   {
   		   // We need to find a place where to store De 30 in TLF01
	   	   memcpy( Auth_Tx.TLF01_details.mcard_banknet,
				   ncdci2_HOST_MESSAGE.Original_amount ,
				   DINERS_DE30_SIZE );

			ret_val = ncdci2_isalphanum(Auth_Tx.TLF01_details.mcard_banknet);
			if ( ret_val == false )
			{
				*result_flag = 1;
				sprintf( err_buf,
 ...
 *  DATA ELEMENT FORMAT :n 11 LLVAR
 *
 *  INPUTS:       ncdci2_Host_Message- Transaction Fee Amount specific structure (Global)
 *                result_flag  - If 1, an error occurred else where
 *
 *  OUTPUTS:      err_buf     - Contains text to describe a reason for failure
 *                Auth_Tx     - (Global)Structure populated from Host Message
...
void ncdci2_move_field_32( pINT result_flag, pCHAR err_buf )
{
	...
	if ( *result_flag == 0 )
	{
		if(ncdci2_HOST_MESSAGE.acq_inst_len[0] != 0x00 )
		{
			/* First get the length. */
			memcpy( len_str,ncdci2_HOST_MESSAGE.acq_inst_len, DINERS_DE32_LEN_SIZE);
				
			len = atoi( len_str );
			if ( len <= DINERS_MAX_DE32_SIZE)
			{
				/* Now copy the Id into Auth_Tx and check it. */
				memcpy( Auth_Tx.acq_inst_id_len,ncdci2_HOST_MESSAGE.acq_inst_len, DINERS_DE32_LEN_SIZE);
				memcpy( Auth_Tx.acq_inst_id_cd,ncdci2_HOST_MESSAGE.acq_inst_id, len );

				if (false == ncdci2_isnum(Auth_Tx.acq_inst_id_cd))
				{
					*result_flag = 1;
					sprintf( err_buf,
...
void ncdci2_move_field_33( pINT result_flag, pCHAR err_buf )
{
   ...
   if ( *result_flag == 0 )
   {
	  if(ncdci2_HOST_MESSAGE.fwd_inst_len[0] != 0x00 )
   	  {
	      /* First get the length. */
	      memcpy( len_str,ncdci2_HOST_MESSAGE.fwd_inst_len, DINERS_DE33_LEN_SIZE);
	      len = atoi( len_str );
	      if ( len <= DINERS_MAX_DE33_SIZE )
	      {
	         /* Now copy the Id into Auth_Tx and check it. */
			 memcpy( Auth_Tx.forwarding_inst_id_len,ncdci2_HOST_MESSAGE.fwd_inst_len,DINERS_DE33_LEN_SIZE);
	         memcpy( Auth_Tx.forwarding_inst_id_cd,ncdci2_HOST_MESSAGE.fwd_inst_id, len );
	         if (false == ncdci2_isnum(Auth_Tx.forwarding_inst_id_cd))
	         {
	            *result_flag = 1;
	            sprintf( err_buf,
	                    "Non-alphanumeric acquiring inst. Id (field 33): %s,",
						ncdci2_HOST_MESSAGE.fwd_inst_id );
	         }
	      }
		  else
		  {
...
void ncdci2_move_field_35( pINT result_flag, pCHAR err_buf )
{
	...
	if ( *result_flag == 0 )
	{
		if(ncdci2_HOST_MESSAGE.track2[0] != 0x00 )
		{
			/* First get the length. */
			memcpy( len_str,ncdci2_HOST_MESSAGE.track2_len, DINERS_DE35_LEN_SIZE);
			len = atoi( len_str );
			if ( len <= DINERS_MAX_DE35_SIZE )
			{
				memcpy( Auth_Tx.TLF01_details.track2 ,ncdci2_HOST_MESSAGE.track2, len);
				if(0 == strlen(Auth_Tx.TLF01_details.service_code) )
				{
					// Let us get the service code.
					if((true == ncdci2_get_service_code(  service_code,  error_msg )))
					{
...
void ncdci2_move_field_37( pINT result_flag, pCHAR err_buf )
{
   ...
   if ( *result_flag == 0 )
   {
	  if(ncdci2_HOST_MESSAGE.rrn[0] != 0x00 )
   	  {
		  memcpy( Auth_Tx.TLF01_details.retrieval_ref_num,
			 	  ncdci2_HOST_MESSAGE.rrn, DINERS_DE37_SIZE );
			  
		  ret_val = ncdci2_isalphanum_space(Auth_Tx.TLF01_details.retrieval_ref_num);
...
void ncdci2_move_field_38( pINT result_flag, pCHAR err_buf )
{
   ...
   if ( *result_flag == 0 )
   {
	  if(ncdci2_HOST_MESSAGE.authcode[0] != 0x00 )
      {
    	  memcpy( temp_auth_id,ncdci2_HOST_MESSAGE.authcode,
    			  DINERS_DE38_SIZE );
			
	  		ret_val = ncdci2_isalphanum(temp_auth_id);
			if ( ret_val == true )
	  		{
				memcpy( Auth_Tx.TLF01_details.auth_number,
					    ncdci2_HOST_MESSAGE.authcode, DINERS_DE38_SIZE );
   			}
      	}
   	}
}
...
void ncdci2_move_field_39( pINT result_flag, pCHAR err_buf )
{
	...
	if (0 == strncmp(Auth_Tx.TLF01_details.handler_queue,"sbatch", 6))
	{
		...
		if (0 == strncmp(Auth_Tx.TLF01_details.response_code, "19", 2)) {
			...
			ncdci2_log_message(2, 1, Log_Bufr, "build_host_structure",
					1);
		} else {
	...
	if ( *result_flag == 0 )
	{
		if(ncdci2_HOST_MESSAGE.responsecd[0] != 0x00 )
		{
			//Store Action code so that we can show in transaction Lookup
			memcpy( Auth_Tx.TLF01_details.product_codes[12].code,
					ncdci2_HOST_MESSAGE.responsecd, DINERS_DE39_SIZE );
			memcpy( dci_response_code,ncdci2_HOST_MESSAGE.responsecd, DINERS_DE39_SIZE );
			ret_val = ncdci2_isnum(dci_response_code);
			if ( ret_val == false )
			{
				   sprintf( err_buf,
							"Non-numeric response code (field 39) : %s,",
							 ncdci2_HOST_MESSAGE.responsecd);
 ...
 *
 ******************************************************************************/
void ncdci2_move_field_40( pINT result_flag, pCHAR err_buf )
{
   INT   ret_val = true;
   INT   len     = 0;
   ...
   if ( *result_flag == 0 )
   {
	    if(ncdci2_HOST_MESSAGE.service_code[0] != 0x00 )
   		{
   			if(0 == strlen(Auth_Tx.TLF01_details.cvc_data))
   			{
   				memcpy( Auth_Tx.TLF01_details.cvc_data,
   						ncdci2_HOST_MESSAGE.service_code,
						DINERS_DE40_SIZE );
   				ret_val = ncdci2_isnum(Auth_Tx.TLF01_details.cvc_data);
				if ( ret_val == false )
				{
					   sprintf( err_buf,
								"Non-numeric CCV2 (field 40) : %s,",
								ncdci2_HOST_MESSAGE.service_code);
				}
   			}
 		}
...
void ncdci2_move_field_41( pINT result_flag, pCHAR err_buf )
{
   ...
   if ( *result_flag == 0 )
   {
	    if(ncdci2_HOST_MESSAGE.termid[0] != 0x00 )
   		{
   		    memcpy( Auth_Tx.TLF01_details.terminal_id,
              		ncdci2_HOST_MESSAGE.termid, DINERS_DE41_SIZE );
 		}
   }
}
...
void ncdci2_move_field_42( pINT result_flag, pCHAR err_buf )
{
   ...
   if ( *result_flag == 0 )
   {
	    if(ncdci2_HOST_MESSAGE.cardaccid[0] != 0x00 )
   		{
	   		memcpy( Auth_Tx.TLF01_details.merchant_id,
             		ncdci2_HOST_MESSAGE.cardaccid, DINERS_DE42_SIZE );
		}
   	}
}
...
void ncdci2_move_field_43( pINT result_flag, pCHAR err_buf )
{
   ...
   if ( *result_flag == 0 )
   {
	    if(ncdci2_HOST_MESSAGE.cardaccname_len[0] != 0x00 )
   		{
			...
			*/
            ...
            /* Store Name/ street /city*/
   			memcpy( temp_len,
   					ncdci2_HOST_MESSAGE.cardaccname_len,
					DINERS_DE43_LEN_SIZE);

   			memcpy( temp_cardaccname,
   					ncdci2_HOST_MESSAGE.cardaccname,
			...
			memcpy( Auth_Tx.int_net_facility_data,
   			...
   			ncdci2_populate_DE43_name_street_city(& temp_cardaccname);
         	/*Store postal code*/
         	memcpy( Auth_Tx.TLF01_details.product_codes[14].amount,
					ncdci2_HOST_MESSAGE.cardaccname + DINERS_DE43_POSTAL_CODE_POSTION,
					DINERS_DE43_POSTAL_CODE_LEN );

         	/*store region*/
         	memcpy( Auth_Tx.TLF01_details.product_codes[14].code,
				    ncdci2_HOST_MESSAGE.cardaccname + DINERS_DE43_REGION_POSTION,
					DINERS_DE43_REGION_LEN );

         	/*store country code*/
            memcpy( Auth_Tx.country_code,
					ncdci2_HOST_MESSAGE.cardaccname + DINERS_DE43_COUNTRY_CODE_POSTION,
...
void ncdci2_move_field_44( pINT result_flag, pCHAR err_buf )
{
   ...
   if ( *result_flag == 0 )
   {  
	    if(ncdci2_HOST_MESSAGE.addlrspdata[0] != 0x00 )
		{
	    	/* 1st find the length */
			memcpy( temp_len,
					ncdci2_HOST_MESSAGE.addl_rsp_len,
			...
			if(len <= DINERS_MAX_DE44_SIZE) // validate data length
			{
				...
				memcpy( Auth_Tx.acct_id_1 ,
						ncdci2_HOST_MESSAGE.addlrspdata,
	    ...
	    strncpy(Auth_Tx.TLF01_details.operator_id, &ncdci2_HOST_MESSAGE.addlrspdata[5], 2);
   	}
}

/******************************************************************************
 *
 *  NAME:         ncdci2_move_field_45
 *
 ...
 *
 ******************************************************************************/
void ncdci2_move_field_45( pINT result_flag, pCHAR err_buf )
{
   INT   ret_val = true;
   INT   len     = 0;
   BYTE temp_len[3] = {0};
   ...
   if ( *result_flag == 0 )
   {  
	    if(ncdci2_HOST_MESSAGE.track1[0] != 0x00 )
		{
	    	/* 1st find the length */
			memcpy(temp_len,ncdci2_HOST_MESSAGE.track1_len,DINERS_DE45_LEN_SIZE);
			len = atoi (temp_len);
			
			if(len <= DINERS_MAX_DE45_SIZE) // validate data length
			{
				memcpy( Auth_Tx.TLF01_details.track1 ,ncdci2_HOST_MESSAGE.track1,len);
				if(0 == strlen(Auth_Tx.TLF01_details.service_code) )
				{
					// Let us get the service code.
					if((true == ncdci2_get_service_code(  service_code,  error_msg )))
 ...
 *
 ******************************************************************************/
void ncdci2_move_field_46( pINT result_flag, pCHAR err_buf )
{
   INT   ret_val = true;
   INT   len     = 0;
   BYTE  temp_len[4] = {0};
   ...
   if ( *result_flag == 0 )
   {
	    if(ncdci2_HOST_MESSAGE.surcharge_fee_len[0] != 0x00 )
		{
			/* 1st find the length */
			memcpy(temp_len,ncdci2_HOST_MESSAGE.surcharge_fee_len,DINERS_DE46_LEN_SIZE);
			len = atoi(temp_len);

			if( len <= DINERS_MAX_DE48_SIZE) // validate data length
			{
				memcpy( Auth_Tx.TLF01_details.tax_amount ,
						ncdci2_HOST_MESSAGE.surcharge_fee,
						len);
				ret_val = ncdci2_isalphanum(Auth_Tx.TLF01_details.tax_amount);
				if ( ret_val == false )
				{
					  *result_flag = 1;
					  sprintf( err_buf,
 ...
 *
 ******************************************************************************/
void ncdci2_move_field_48( pINT result_flag, pCHAR err_buf )
{
   INT   len     = 0;
   BYTE  temp_len[4] = {0};
   ...
   if ( *result_flag == 0 )
   {
	   if(ncdci2_HOST_MESSAGE.addl_data_len[0] != 0x00 )
	   {
			/* 1st find the length */
			memcpy( temp_len,
					ncdci2_HOST_MESSAGE.addl_data_len,
					DINERS_DE48_LEN_SIZE);
			len = atoi(temp_len);

			if( len <= DINERS_MAX_DE48_SIZE) // validate data length
			{
				if(strlen(ncdci2_HOST_MESSAGE.addl_data) < (sizeof(Auth_Tx.TLF01_details.voice_auth_text) -1))
				{
					/* Store additional response data into host message */
					memcpy( Auth_Tx.TLF01_details.voice_auth_text,
							ncdci2_HOST_MESSAGE.addl_data,len);
				}
				else
				{
					/* Store additional response data into host message */
					memcpy( Auth_Tx.TLF01_details.voice_auth_text ,
							ncdci2_HOST_MESSAGE.addl_data,
...
void ncdci2_move_field_49( pINT result_flag, pCHAR err_buf )
{
   ...
   if ( *result_flag == 0 )
   {
	    if(ncdci2_HOST_MESSAGE.tran_curr_cd[0] != 0x00 )
   		{
	   		memcpy( Auth_Tx.TLF01_details.currency_code,
            	    ncdci2_HOST_MESSAGE.tran_curr_cd,
            	    DINERS_DE49_SIZE );
      	}
   	}
}
 ...
 *
 ******************************************************************************/
void ncdci2_move_field_51( pINT result_flag, pCHAR err_buf )
{
   INT   len     = 0;

   if ( *result_flag == 0 )
   {
	    if(ncdci2_HOST_MESSAGE.card_curr_cd[0] != 0x00 )
   		{
   			memcpy( Auth_Tx.ch_billing_curr_code, 
					ncdci2_HOST_MESSAGE.card_curr_cd,
				    DINERS_DE51_SIZE);
   		}
   }
}
...
void ncdci2_move_field_52( pINT result_flag, pCHAR err_buf )
{
   ...
   if ( *result_flag == 0 )
   {
	    if(ncdci2_HOST_MESSAGE.pinblock[0] != 0x00 &&ncdci2_HOST_MESSAGE.pinblock[1] != 0x00  )
   		{
			memcpy( Auth_Tx.TLF01_details.pin_block, 
					ncdci2_HOST_MESSAGE.pinblock,
				    DINERS_DE52_SIZE);
   		}
   }
}
 ...
 *
 ******************************************************************************/
void ncdci2_move_field_54( pINT result_flag, pCHAR err_buf )
{
   INT   len     = 0;
   BYTE temp_len[4] = {0};
   ...
   if ( *result_flag == 0 )
   {
		memcpy(temp_len,ncdci2_HOST_MESSAGE.AddlAmts_len,DINERS_DE54_LEN_SIZE);
		len = atoi(temp_len);

		if( len <= DINERS_MAX_DE54_SIZE)
		{
			if(strlen(ncdci2_HOST_MESSAGE.AddlAmts) < (sizeof(Auth_Tx.TLF01_details.mcard_banknet) -1))
			{
				memcpy( Auth_Tx.TLF01_details.mcard_banknet ,
						ncdci2_HOST_MESSAGE.AddlAmts,len);// Store data after length
			}
			else
			{
				memcpy( Auth_Tx.TLF01_details.mcard_banknet  ,
						ncdci2_HOST_MESSAGE.AddlAmts,
...
void ncdci2_move_field_55( pINT result_flag, pCHAR err_buf )
{
	...
	if ( *result_flag == 0 )
	{
		strncpy(temp_emv_len,ncdci2_HOST_MESSAGE.IccData_len,3);
		emvLen = atoi(temp_emv_len);
		if(emvLen > 0 && emvLen < sizeof (ncdci2_HOST_MESSAGE.IccData))
		{
			// Request Msg
			if(Auth_Tx.TLF01_details.tx_key < 100)
			{
				//Copy raw data into EMV RAW
				memcpy(Auth_Tx.EMV_details.emv_block,ncdci2_HOST_MESSAGE.IccData_len,emvLen+3); // First 3 byte ACii len + EMV data in TLV format

				memset(ICCdata, 0, sizeof(ICCdata));
				memcpy(ICCdata,ncdci2_HOST_MESSAGE.IccData, emvLen);
			...
			else
			{
				...
				if(emvLen >0 && emvLen < 80)
				{
					...
					ncdci2_asc_to_bcd (ascii_len, 4, bcd_len);
					memcpy(Auth_Tx.EMV_details.future_field1,bcd_len,2); // convert ascii length to BCD, both nceqit and dcpiso expects length in bcd
					// TAG len includes First byte length and follwed by IAD daata 8 byte ARPC and 2 byte RC
					memcpy(&Auth_Tx.EMV_details.future_field1[2],&ncdci2_HOST_MESSAGE.IccData,emvLen);
					...
					if ( emvLen > (sizeof(Auth_Tx.EMV_details.future_field1)-3) )
					{
						...
						memcpy( Auth_Tx.EMV_details.future_field2,
								ncdci2_HOST_MESSAGE.IccData + (sizeof(Auth_Tx.EMV_details.future_field1)-3),
...
void ncdci2_move_field_56( pINT result_flag, pCHAR err_buf )
{
   ...
   if ( *result_flag == 0 )
   {
		 ...
		 +-------------------------------------------------+
		 */
	     if(ncdci2_HOST_MESSAGE.orig_data_len[0] != 0x00 )
   		 {
   			len = memcpy( temp_len ,ncdci2_HOST_MESSAGE.orig_data_len,
   						  DINERS_DE56_LEN_SIZE);
   			len = atoi(temp_len);
   			memcpy (tempstr ,ncdci2_HOST_MESSAGE.orig_data,len);
			...
			memcpy( Auth_Tx.TLF01_details.orig_amount,
...
*
******************************************************************************/
void ncdci2_move_field_58( pINT result_flag, pCHAR err_buf )
{
	INT   ret_val = true;
	INT   len     = 0;
	BYTE  temp_len[3] = {0};
	...
	if ( *result_flag == 0 )
	{
		if(ncdci2_HOST_MESSAGE.Auth_agent_IIC_len[0] != 0x00 )
		{
			memcpy( temp_len,ncdci2_HOST_MESSAGE.Auth_agent_IIC_len,
					DINERS_DE58_SIZE_LEN);
			len=atoi(temp_len);
			if(len <=DINERS_DE58_MAX_SIZE_LEN)
			{
				memcpy( Auth_Tx.TLF01_details.product_codes[6].amount,
						ncdci2_HOST_MESSAGE.Auth_agent_IIC,
						DINERS_DE58_SIZE);
			}
			else
			{
 ...
 *
 ******************************************************************************/
void ncdci2_move_field_59( pINT result_flag, pCHAR err_buf )
{
   INT   len     = 0;
   BYTE  temp_len[4] = {0};
   ...
   if ( *result_flag == 0 )
   {
	   if(ncdci2_HOST_MESSAGE.NetworkID_Transport_data_len[0] != 0x00 )
	   {
		   // Need to see a sample data
		   memcpy(temp_len,
				   ncdci2_HOST_MESSAGE.NetworkID_Transport_data_len,
				   DINERS_DE59_LEN_SIZE);
		   len = atoi(temp_len);
		   if(len <=DINERS_MAX_DE59_SIZE)
		   {
			   //Need to find out a field that we will using to store this data
			   //memcpy(temp_len,ncdci2_HOST_MESSAGE.NetworkID_Transport_data,len);
		   }
		   else
		   {
			   *result_flag = 1;
...
void ncdci2_move_field_62( pINT result_flag, pCHAR err_buf )
{
   ...
   if ( *result_flag == 0 )
   {
	   if(ncdci2_HOST_MESSAGE.addition_auth_data_len[0] != 0x00 )
	   {
		   memcpy(temp_len,ncdci2_HOST_MESSAGE.addition_auth_data_len,DINERS_DE62_LEN_SIZE);
		   len = atoi(temp_len);
 ...
 *
 ******************************************************************************/
void ncdci2_move_field_64( pINT result_flag, pCHAR err_buf )
{
   INT   len     = 0;

   if ( *result_flag == 0 )
   {
	    if(ncdci2_HOST_MESSAGE.mess_auth_code[0] != 0x00 )
   		{
			memcpy( Auth_Tx.TLF01_details.product_codes[0].amount,
					ncdci2_HOST_MESSAGE.mess_auth_code,
					DINERS_DE64_SIZE);
		}
   	}
}
 ...
 *
 ******************************************************************************/
void ncdci2_move_field_72( pINT result_flag, pCHAR err_buf )
{
   INT   len     = 0;
   BYTE temp_len[4] = {0};
   ...
   if ( *result_flag == 0 )
   {
	   if(ncdci2_HOST_MESSAGE.data_record_len[0] != 0x00 )
	   {
		   memcpy(temp_len,ncdci2_HOST_MESSAGE.data_record_len,DINERS_DE72_LEN_SIZE);
		   len = atoi(temp_len);
 ...
 *
 ******************************************************************************/
void ncdci2_move_field_92( pINT result_flag, pCHAR err_buf )
{
   INT   ret_val = true;
   INT   len     = 0;
   ...
   if ( *result_flag == 0 )
   {
	   if(ncdci2_HOST_MESSAGE.ctry_code[0] != 0x00 )
   	   {
	   		memcpy( Auth_Tx.country_code,
	   				ncdci2_HOST_MESSAGE.ctry_code,
            	    DINERS_DE92_SIZE );

			ret_val = ncdci2_isnum(Auth_Tx.country_code);
      		if ( ret_val == false )
      		{
		      	*result_flag = 1;
        		 sprintf( err_buf,
 ...
 *
 ******************************************************************************/
void ncdci2_move_field_93( pINT result_flag, pCHAR err_buf )
{
   INT   ret_val = true;
   INT   len     = 0;
   BYTE temp_len[3] = {0};
   ...
   if ( *result_flag == 0 )
   {
	    if(ncdci2_HOST_MESSAGE.TranDestination_ICC_len[0] != 0x00 )
   		{
   			memcpy(temp_len,ncdci2_HOST_MESSAGE.TranDestination_ICC_len,DINERS_DE93_LEN_SIZE);
   			len = atoi(temp_len);
   			if(len <=DINERS_MAX_DE93_SIZE)
   			{
				 ...
				 memcpy( Auth_Tx.TLF01_details.product_codes[5].amount,
						 ncdci2_HOST_MESSAGE.TranDestination_ICC,
						 len);
			}
   			else
   			{
 ...
 *
 ******************************************************************************/
void ncdci2_move_field_94( pINT result_flag, pCHAR err_buf )
{
   INT   ret_val = true;
   INT   len     = 0;
   BYTE temp_len[3] = {0};
   ...
   if ( *result_flag == 0 )
   {
	    if(ncdci2_HOST_MESSAGE.TranOriginator_ICC_len[0] != 0x00 )
   		{
   			memcpy(temp_len,ncdci2_HOST_MESSAGE.TranOriginator_ICC_len,DINERS_DE94_LEN_SIZE);
   			len = atoi(temp_len);
   			if(len <=DINERS_MAX_DE94_SIZE)
   			{
				 ...
				 memcpy( Auth_Tx.TLF01_details.product_codes[4].amount,
						ncdci2_HOST_MESSAGE.TranOriginator_ICC,
						 len);
			}
   			else
   			{
 ...
 *
 ******************************************************************************/
void ncdci2_move_field_96( pINT result_flag, pCHAR err_buf )
{
   INT   ret_val = true;
   INT   len     = 0;
   BYTE temp_len[4] = {0};
   ...
   if ( *result_flag == 0 )
   {
	    if( 0 == strncmp(NMI_Code,
	    				 DINERS_NMI_KEYTEST,
						 DINERS_NETWORK_FUNCTION_CODE_LEN))
	    {
			if(ncdci2_HOST_MESSAGE.Key_Managment_data_len[0] != 0x00 )
			{
				memcpy(temp_len,ncdci2_HOST_MESSAGE.Key_Managment_data_len,DINERS_DE96_LEN_SIZE);
				len = atoi(temp_len);
				if(len <=DINERS_MAX_DE96_SIZE)
				{
					memcpy(Auth_Tx.TLF01_details.source_key,
							ncdci2_HOST_MESSAGE.Key_Managment_data,
							DINERS_KEY_LEN);
					if (keyblock_enable_flag_for_DCI == KEYBLOCK_ENABLE)
					{
						 memcpy( Ncf01_I_key_exchange_Keyblock.iss_rtdcwk_keyblock,
								 ncdci2_HOST_MESSAGE.Key_Managment_data,len);

						 memcpy( Ncf01_I_key_exchange_Keyblock.iss_prev_src_station,
								 ncdci2_HOST_MESSAGE.Key_Managment_data + DINERS_KEY_LEN,
								 DINERS_KEY_CHECK_LEN);
					}
					else
					{
						memcpy( Ncf01_I_key_exchange.retired_cwk,ncdci2_HOST_MESSAGE.Key_Managment_data,len);
						memcpy( Ncf01_I_key_exchange.prev_src_station,
								ncdci2_HOST_MESSAGE.Key_Managment_data + DINERS_KEY_LEN,
								DINERS_KEY_CHECK_LEN);
					}
				}
 ...
 *
 ******************************************************************************/
void ncdci2_move_field_100( pINT result_flag, pCHAR err_buf )
{
   INT   ret_val = true;
   INT   len     = 0;
   BYTE  temp_len[3] = {0};
   ...
   if ( *result_flag == 0 )
   {
	    if(ncdci2_HOST_MESSAGE.recv_ins_id_code_len[0] != 0x00 )
   		{
   			memcpy(temp_len,ncdci2_HOST_MESSAGE.recv_ins_id_code_len,DINERS_DE100_SIZE_LEN);
   			len=atoi(temp_len);
			if(len <=DINERS_MAX_DE100_SIZE)
			{
				 /*Store data in  Auth_Tx.TLF01_details.product_codes[3].amount */
	  			 memcpy( Auth_Tx.TLF01_details.product_codes[3].amount,
				 		ncdci2_HOST_MESSAGE.recv_ins_id_code,
				 		 len);
			}
			else
			{
 ...
 *
 ******************************************************************************/
void ncdci2_move_field_101( pINT result_flag, pCHAR err_buf )
{
   INT   len     = 0;
   BYTE  temp_len[3] = {0};
   ...
   if ( *result_flag == 0 )
   {
	    if(ncdci2_HOST_MESSAGE.file_name_len[0] != 0x00 )
		{
			memcpy(temp_len,ncdci2_HOST_MESSAGE.file_name_len,DINERS_DE101_SIZE_LEN);
			len=atoi(temp_len);
			if(len <=DINERS_MAX_DE101_SIZE)
			{

				 /* Need to identify where to store this data */
				// memcpy( Auth_Tx.TLF01_details.product_codes[3].amount,ncdci2_HOST_MESSAGE.file_name_len,len);
			}
			else
			{
				*result_flag = 1;
...
void ncdci2_move_field_106( pINT result_flag, pCHAR err_buf )
{
   ...
   if ( *result_flag == 0 )
   {
	    if(ncdci2_HOST_MESSAGE.tran_data_len[0] != 0x00 )
		{
			// We are not stroing in TLF01 , we need to prepare seperate table to store them

			memcpy(temp_len,ncdci2_HOST_MESSAGE.tran_data_len,DINERS_DE106_SIZE_LEN);
			len=atoi(temp_len);
			if(len <=DINERS_MAX_DE106_SIZE)
			{
				 /* Need to identify where to store this data */
				 //memcpy( Auth_Tx.TLF01_details.product_codes[14].quantity,ncdci2_HOST_MESSAGE.tran_data_len,DINERS_DE106_SIZE_LEN);
				 //Store Tag 67 here
	  			 memcpy( Auth_Tx.TLF01_details.product_codes[13].amount,
				 		ncdci2_HOST_MESSAGE.tran_data,
				 		 len);
			}
			else
			{
...
void ncdci2_move_field_122( pINT result_flag, pCHAR err_buf )
{
   ...
   if ( *result_flag == 0 )
     {
	    if(ncdci2_HOST_MESSAGE.Add_authentication_data_len[0] != 0x00 )
  		{
  			memcpy(temp_len,ncdci2_HOST_MESSAGE.Add_authentication_data_len,DINERS_DE122_SIZE_LEN);
  			len=atoi(temp_len);
  			if(len <=DINERS_MAX_DE122_SIZE)
  			{
 ...
 *
 ******************************************************************************/
void ncdci2_move_field_123( pINT result_flag, pCHAR err_buf )
{
    INT   len     = 0;
    CHAR  temp_len[4] = {0};
    ...
    if ( *result_flag == 0 )
	{
    	if(ncdci2_HOST_MESSAGE.networ_ref_id_len[0] != 0x00 )
		{
			memcpy(temp_len,ncdci2_HOST_MESSAGE.networ_ref_id_len,DINERS_DE123_SIZE_LEN);
			len=atoi(temp_len);
			if(len <=DINERS_MAX_DE123_SIZE)
			{
				memcpy( Auth_Tx.TLF01_details.resp_source,
						ncdci2_HOST_MESSAGE.networ_ref_id,len);
			}
			else
			{
				*result_flag = 1;
 ...
 *
 ******************************************************************************/
void ncdci2_move_field_124( pINT result_flag, pCHAR err_buf )
{
	INT   len     = 0;
	CHAR  temp_len[4] = {0};
	...
	if ( *result_flag == 0 )
	{
		if(ncdci2_HOST_MESSAGE.transaction_qualifer_len[0] != 0x00 )
		{
			memcpy(temp_len,ncdci2_HOST_MESSAGE.transaction_qualifer_len,DINERS_DE124_SIZE_LEN);
			len=atoi(temp_len);
			if(len <=DINERS_MAX_DE124_SIZE)
			{
				memcpy(	Auth_Tx.future_use_5,
						ncdci2_HOST_MESSAGE.transaction_qualifer,
 ...
 *
 ******************************************************************************/
void ncdci2_move_field_125( pINT result_flag, pCHAR err_buf )
{
	INT   len     = 0;
	CHAR  temp_len[4] = {0};
	...
	if ( *result_flag == 0 )
	{
		if(ncdci2_HOST_MESSAGE.replacement_pin_len[0] != 0x00 )
		{
			memcpy(temp_len,ncdci2_HOST_MESSAGE.replacement_pin_len,DINERS_DE125_SIZE_LEN);
			len=atoi(temp_len);
			if(len <=DINERS_MAX_DE125_SIZE)
			{
				/* Store additional response data into host message ,
				 *  In Build function we will calculate the length from this filed*/
				 memcpy( Auth_Tx.TLF01_details.vehicle_number,ncdci2_HOST_MESSAGE.replacement_pin,len);
			}
			else
			{
				*result_flag = 1;
