/******************************************************************************
    ...
    Copyright (c) 2004, ThoughtFocus, Inc.
    All Rights Reserved.
 
    TITLE:  Diners Host Handler Driver
...
extern CHAR    ncdci_fileupdate_link_que_name[];
...
extern CHAR  ncdci2_Erro_warning_Filename[256] ;
extern CHAR  ncdci2_module_error_warning_file_name[256];
extern CHAR  ncdci2_error_warning_file_path[256];
...
INT rvrsl_attmpts_value_fr_timeout_dci=0;
INT rvrsl_attmpts_value_fr_laterspns_dci=0;
...
void ncdci2_create_rrn( pCHAR rrn )
{
   ...
   ncdci2_calc_julian_day( date_str, julian_day );
...
/******************************************************************************
 *
 *  NAME:         ncdci2_copy_to_req_resp_buffer
 *
...
/******************************************************************************
 *
 *  NAME:         ncdci2_determine_txn_flow
 *
 ...
 *
 ******************************************************************************/
void ncdci2_send_response_to_host( INT send_flag )
{
   CHAR  err_buf[100] = {0};
   BYTE  temp_tx_key = {0};
   INT buf_temp_len = 0;
   ...
   if(true == ncdci2_check_for_cadencieacq_transaction())
   {
	  ...
	  if ( Auth_Tx.TLF01_details.tx_key < 100 )
 	  ...
 	  ncdci2_send_message_to_host(err_buf,false);
 	  return;
   }
   if ( (send_flag == DINERS_SEND_UNCONDITIONALLY) ||
       ((send_flag == DINERS_SEND_IF_NOT_REVERSAL) && (false == ncdci2_is_reversal() )) )
   {
      ...
      if ( true == ncdci2_build_response_msg() )
      {
		 ...
		 {	
			...
			sprintf (err_buf, "Sending Diners Host response to Cadencie, message type :%s",
					Auth_Tx.TLF01_details.message_type) ;
			ncdci2_log_message( 2, 1, err_buf, "ncdci2_send_response_to_host",1 );
		 }
         if ( false == ncdci2_send_message_to_host(err_buf,false) )//praneeth added false as parameter
            ncdci2_log_message( 1, 2, err_buf, "ncdci2_send_response_to_host",1 );
      }
   ...
   else if(DINERS_SEND_IF_KEY_TRAN_FAILED == send_flag )
   {
	   /* Make sure the tx_key is for a response. */
		 ...
		 if ( true == ncdci2_build_response_msg() )
		 {
			 if ( false == ncdci2_send_message_to_host(err_buf,false) )
			 {
				  ncdci2_log_message( 1, 2, err_buf, "ncdci2_send_response_to_host",1 );
			 }
		 }
		 ...
		 {
			 sprintf (err_buf, "Diners Key test request failed message type :%s",
								Auth_Tx.TLF01_details.message_type) ;
			 ncdci2_log_message( 2, 1, err_buf, "ncdci2_send_response_to_host",0 );
		 }
...
/******************************************************************************
 *
 *  NAME:         ncdci2_sendfile_response_to_host
 *
 *  DESCRIPTION:  This function sends a response to the host.  It gets the
 *                response code and other information from Auth_Tx.
 *
 ...
 *
 ******************************************************************************/
void ncdci2_sendfile_response_to_host( INT send_flag )
{
   CHAR  err_buf[100] = {0};
   BYTE  temp_tx_key = {0};
   INT buf_temp_len = 0;

   ncdci2_build_response_msg();
   ...
   if(buf_temp_len < DINERS_COMM_BUF_SZE && buf_temp_len > 0)
   {
			...
			sprintf (err_buf, "Sending file response coming Diners Host response to Cadencie, message type :%s",
					Auth_Tx.TLF01_details.message_type) ;
			ncdci2_log_message( 2, 1, err_buf, "ncdci2_sendfile_response_to_host",1 );
			if ( false == ncdci2_send_message_to_host(err_buf,false))
			{
				 ncdci2_log_message( 1, 2, err_buf, "ncdci2_sendfile_response_to_host",1 );
			}
			(void)ncdci2_insert_db_record( TLF01_DATA );
   }
   else
...
/******************************************************************************
 *
 *  NAME:         ncdci2_sendfile_request_to_host_1324
 *
 ...
 *
 ******************************************************************************/
void ncdci2_sendfile_request_to_host_1324( pCHAR timer_resp_flag )
{
	INT   ret_val;
	...
	(void)ncdci2_create_request_timer_key( lookup_key );
	seconds = atoi(Ncf01_I.request_timer);
	strcpy( TimerAppData2, timer_resp_flag );
	ret_val = ncdci2_set_timer( lookup_key, seconds, DINERS_REQUEST_TIMER );
	if ( true == ret_val )
	{
		...
		if(buf_temp_len < DINERS_COMM_BUF_SZE && buf_temp_len > 0)
		{
			...
			ncdci2_log_message( 2,1, err_buf, "ncdci2_sendfile_request_to_host_1324",1 );
			ret_val = ncdci2_send_message_to_host( err_buf,false );//praneeth added false as parameter
		}
		else
		{
			sprintf (mesbuf, "Failed Sending file Message coming from DCI Cadence, message type :%s",
					 	 	 Auth_Tx.TLF01_details.message_type) ;
			strcat( err_buf,mesbuf);
			ncdci2_log_message( 2,1, err_buf, "ncdci2_sendfile_request_to_host_1324",1 );
		}
	...
	if ( false == ret_val )
	{
		...
		*/
		ncdci2_log_message( 2, 2, err_buf, "ncdci2_sendfile_request_to_host_1324",1 );
		ret_val = ncdci2_perform_standin_processing( DINERS_SYSTEM_ERROR,
													GS_TXN_DROPPED,
													err_buf );
		if ( false == ret_val )
		{
...
/******************************************************************************
 *
 *  NAME:         ncdci2_send_request_to_host
 *
 *  DESCRIPTION:  This function sends a request to the host.  It gets the
 *                information it needs from Auth_Tx and Ncf01.
 *
...
void ncdci2_send_request_to_host( pCHAR timer_resp_flag )
{
	...
	ret_val = ncdci2_build_request_msg( err_buf );
	if ( true == ret_val )
	{
		ptetime_get_strtime( Auth_Tx.TLF01_details.host_start_time );
		/*Send whatever msg coming from Diners HOST. Make sure we have valid msg from HOST*/
		if(0 == strcmp(Auth_Tx.TLF01_details.dispensation.origin,"RO"))
		{
			if( true == ncdci2_is_issuing_transaction())
			{
				...
				if( AUTH_CASH_ADVANCE == Auth_Tx.TLF01_details.tx_key ||
				    AUTH_BALANCE_INQUIRY == Auth_Tx.TLF01_details.tx_key ||
				   (true == ncdci2_pin_block_exists ()))
				{
					sprintf (mesbuf, "Sending Message to Cadencie after PIN transalation, message type :%s",
									 Auth_Tx.TLF01_details.message_type) ;
					strcat( err_buf,mesbuf);
					ncdci2_log_message( 2,1, err_buf, "ncdci2_send_request_to_host",1 );
					// Do nothing, we have to build and send our request message
				}
				else if(buf_temp_len < DINERS_COMM_BUF_SZE && buf_temp_len > 0)
				{
					buflen = buf_temp_len;
					memcpy(ReqRsp_Buffer,Auth_Tx.req_buffer,buflen);
					sprintf (mesbuf, "Sending Message coming from Diners Host to Cadencie, message type :%s",
							 Auth_Tx.TLF01_details.message_type) ;
					strcat( err_buf,mesbuf);
					ncdci2_log_message( 2,1, err_buf, "ncdci2_send_request_to_host",1 );
				}
	...
	if ( false == ret_val )
	{
		/* Since the txn request did not go to the host, we need to initiate a response.*/
		ncdci2_log_message( 2, 2, err_buf, "ncdci2_send_request_to_host",1 );
		ret_val = ncdci2_perform_standin_processing( DINERS_SYSTEM_ERROR,
													 GS_TXN_DROPPED,err_buf );
		if ( false == ret_val )
		{
			ncdci2_log_message( 2, 2, err_buf, "ncdci2_send_request_to_host",1 );
		}
...
/******************************************************************************
 *
 *  NAME:         ncdci2_send_file_request_to_host
 *
 *  DESCRIPTION:  This function sends a request to the host.  It gets the
 *                information it needs from Auth_Tx and Ncf01.
 *
 ...
 *
 ******************************************************************************/
void ncdci2_send_file_request_to_host( pCHAR timer_resp_flag )
{
	CHAR  err_buf[100]  = {0};
	CHAR  lookup_key[DINERS_LOOKUP_KEY_SIZE] = {0};
	CHAR  mesbuf[100] = {0};
	...
	if(buf_temp_len < DINERS_COMM_BUF_SZE && buf_temp_len > 0)
	{
		buflen = buf_temp_len;
		memcpy(ReqRsp_Buffer,Auth_Tx.resp_buffer,buflen);
		sprintf (mesbuf, "Sending Message coming from Cadecnie Host to DCI , message type :%s",
				 Auth_Tx.TLF01_details.message_type) ;
		strcat( err_buf,mesbuf);
		ncdci2_log_message( 2,1, err_buf, "ncdci2_send_request_to_host",1 );
	}
	ret_val = ncdci2_build_and_send_request_message( MT_INCOMING_GUI,
	                                      	  ST1_NONE,
											  ST2_NONE,
											  ncdci_fileupdate_link_que_name,
											  (pBYTE)&Auth_Tx,
											  sizeof(Auth_Tx),
											  0,
											  NULL_STR,
 ...
 *
 ******************************************************************************/
INT ncdci2_send_response_to_GUI()
{
	CHAR  err_buf[100]  = {0};
	CHAR  lookup_key[DINERS_LOOKUP_KEY_SIZE] = {0};
	CHAR  mesbuf[100] = {0};
	...
	if(buf_temp_len < DINERS_COMM_BUF_SZE && buf_temp_len > 0)
	{
		buflen = buf_temp_len;
		memcpy(ReqRsp_Buffer,Auth_Tx.resp_buffer,buflen);
		sprintf (mesbuf, "Sending File response coming from Cadencie TO DCI, message type :%s",
				 Auth_Tx.TLF01_details.message_type) ;
		strcat( err_buf,mesbuf);
		ncdci2_log_message( 2,1, err_buf, "ncdci2_send_response_to_GUI",1 );
		ret_val = ncdci2_build_and_send_request_message( MT_INCOMING_GUI,
			                                      	  ST1_NONE,
													  ST2_NONE,
...
} /* ncdci2_send_response_to_GUI */
/******************************************************************************
 *
 *  NAME:         ncdci2_send_msg_to_authorizer
 *
...
INT ncdci2_send_msg_to_authorizer( BYTE msgtype )
{
   ...
   INT   ret_val = false;
	...
	{
		ret_val = ncdci2_build_and_send_request_message( msgtype,
														 0,
														 0,
		...
		if ( ret_val == false )
		{
			...
			ncdci2_log_message( 1, 3, Buffer, "ncdci2_send_msg_to_authorizer",1 );
		}
	}
	else
#endif
...
/******************************************************************************
 *
 *  NAME:         ncdci2_build_and_send_request_message
 *
 ...
 *
 ******************************************************************************/
INT ncdci2_build_and_send_request_message( BYTE     msgtype,
                                    BYTE     subtype1,
                                    BYTE     subtype2, 
                                    pBYTE    que_name, 
                                    pBYTE    buffer, 
                                    ...
                                    pCHAR    err_msg )
{
   ...
   else
   {
      ...
      else
      {
         ...
         ret_code = ncdci2_send_msg( p_msg, que_name, err_msg );
         free(p_msg);
      }
   }
   return( ret_code );
...
INT ncdci2_init_Auth_Tx(void)
{
   ...
   ret_val = ncdci2_determine_tx_key();
...
INT ncdci2_determine_tx_key( )
{
   ...
   if ( ret_val == false )
   {
      ...
      {
         sprintf( err_buf,
              "Unsupported txn from DINERS ATP: msgtype = %s, processing code = %s",
               msgtype, Auth_Tx.TLF01_details.processing_code );
      }
      ncdci2_log_message( 2, 2, err_buf, "ncdci2_determine_tx_key",1 );
...
/******************************************************************************
 *
 *  NAME:         ncdci2_set_tx_key_to_response
 *
...
/*****************************************************************************
  ...
  Function:    ncdci2_vBCDMove 

  Description: This function will move data from the ncdci2_HOST_MESSAGE structure to 
  ...
  Inputs:
     ...
     ncdci2_HOST_MESSAGE (global)  - structure that contains data to go into the 
...
void ncdci2_vBCDMove(enum bitfields fieldid)
{
   ...
   /*---------------------------------------------------*/
   pBF = ncdci2_Find_Bitfield_Table(fieldid);
...
INT ncdci2_vEBCDICMove( enum bitfields fieldid )
{
   ...
   if (pBF != NULL)
   {
   	   if(fieldid == 55)
		{
			memcpy(temp_bcd_len,ncdci2_HOST_MESSAGE.IccData_len,pBF->bd_fieldlen);
			// convert EBCD to ascii
			for( i=0; i<pBF->bd_fieldlen; i++ )
		    {
		        ncdci2_convert_to_ascii( temp_bcd_len, temp_ascii_len,pBF->bd_fieldlen );
	   ...
	   {
			movelen  = ncdci2_get_variable_length( pBF->p_bd_pdata, pBF->bd_fieldlen );
			movelen += pBF->bd_fieldlen;

			ncdci2_convert_to_ebcdic( pBF->p_bd_pdata, &ReqRsp_Buffer[reqidx], movelen );
			reqidx += movelen;
	   }
   }
   else
...
} /* end ncdci2_vEBCDICMove */



/******************************************************************************
 *
 *  NAME:         ncdci2_EBCDICMove
 *
...
} /* end ncdci2_EBCDICMove */


/******************************************************************************
 *
 *  NAME:         ncdci2_RspEBCDICvMove
 *
 ...
 *                fields in the ncdci2_HOST_MESSAGE data structure.  The length is
 *                also copied.
 *
...
INT ncdci2_RspEBCDICvMove (enum bitfields fieldid)
{
   ...
   if (pBF != NULL)
   {
      ...
      /* Determine number of bytes to move. */
      ncdci2_convert_to_ascii( &ReqRsp_Buffer[reqidx], temp_string_len, FieldLen );
      movelen = atoi(temp_string_len) + FieldLen;
		...
		else if ( (reqidx + movelen) <= buflen )
       {
         ...
         ncdci2_convert_to_ascii( &ReqRsp_Buffer[reqidx], pBF->p_bd_pdata, movelen );
         reqidx += movelen;
      }
      else
      {
...
} /* end ncdci2_RspEBCDICvMove */


/******************************************************************************
 *
 *  NAME:         ncdci2_RspEBCDICMove
 *
 *  DESCRIPTION:  This function converts a fixed length string from EBCDIC,
 *                into ASCII.  It copies it from ReqRsp_Buffer into where ever
 *                the parameter 'fieldid' is pointing, usually into specific
 *                fields in the ncdci2_HOST_MESSAGE data structure.
 *
 *  INPUTS:       fieldid - Field Id to denote the destination of the data
 *
 *  OUTPUTS:      None
...
/*****************************************************************************
  ...
  Description: This function will move data from the raw ISO message to 
               the ncdci2_HOST_MESSAGE structure.  
  Author: 
      Abhishek Verma
  Inputs:
      fieldid                - the bit field of the data to be moved
      ReqRsp_Buffer (global) - the buffer that contains the ISO formatted message
  Outputs:
     ncdci2_HOST_MESSAGE (global)  - structure for the outgoing ISO message
  Return values:
      None
  Modified by:
      Abhishek Verma      
...
} /* end ncdci2_GenericMove */


/*****************************************************************************

  Function:   ncdci2_vDataMove
...
/*****************************************************************************
  ...
  Function:   ncdci2_RspMovevData  

  Description: This function will move data from the the raw ISO message 
               to the ncdci2_HOST_MESSAGE structure.  Note: it moves the number 
...
} /* endncdci2_RspMovevData */


/***********************************************
   ncdci2_RspMoveGeneric
************************************************/
...
/******************************************************************************
 *
 *  NAME:         ncdci2_Move_IfThere
 *
 *  DESCRIPTION:  This function returns a value based on whether or not there
 *                is data in the specifiedncdci2_HOST_MESSAGE field.  If there is
...
INT ncdci2_Move_IfThere( enum bitfields fieldid, BYTE p_trantype )
{
   ...
   return( false );
} /* end ncdci2_Move_IfThere */


/******************************************************************************
 *
 *  NAME:         ncdci2_Move_IfThereWithSpaces
 *
 *  DESCRIPTION:  This function returns a value based on whether or not there
 *                is data in the specifiedncdci2_HOST_MESSAGE field.  If there is
 *                data in it, TRUE is returned, else FALSE is returned.
...
INT ncdci2_Move_IfThereWithSpaces( enum bitfields fieldid, BYTE p_trantype )
{
   ...
   return( false );
} /* end ncdci2_Move_IfThereWithSpaces */
...
/******************************************************************************
 *
 *  NAME:         ncdci2_get_variable_length
 *
...
struct trandef *ncdci2_Find_Tran_Table( pAUTH_TX p_auth_tx )
{
   ...
   ncdci2_log_message( 2, 2, Buffer, "FIND_TRAN_TABLE",0 );

   return NULL;
} /* end ncdci2_Find_Tran_Table */   


/******************************************************************************
 *
 *  NAME:         ncdci2_Find_Bitfield_Table
 *
...
struct bitfield_data *ncdci2_Find_Bitfield_Table( enum bitfields bitfield )
{
   ...
   sprintf(temp_str, "This bitfield not found in BFData, bitfield: %d",
                      bitfield);
   ncdci2_log_message( 2, 2, temp_str, "ncdci2_Find_Bitfield_Table",0 );
   return NULL;

} /* end ncdci2_Find_Bitfield_Table */
...
/******************************************************************************
 *
 *  NAME:         ncdci2_build_host_msg
 *
 *  DESCRIPTION:  This procedure copies the information from the ncdci2_HOST_MESSAGE
 ...
 *               ncdci2_HOST_MESSAGE - (Global)Transaction data destined for the host
 *
...
INT ncdci2_build_host_msg( pCHAR err_buf )
{
	...
	{
		if(Auth_Tx.TLF01_details.tx_key == AUTH_ADVICE_REPEAT)
		{
			Auth_Tx.TLF01_details.tx_key = AUTH_ADVICE;
		}
	}
	pTranDef = ncdci2_Find_Tran_Table( &Auth_Tx );
	...
	for (bfidx=0; bfidx < BitFieldSize; bfidx++)
	{
		...
		if (pTranDef->td_bitfields[bfidx] != bf_done)
		{
			...
			/* find pointer to bitfield info for this bitfield...*/
			/*---------------------------------------------------*/
			pBF = ncdci2_Find_Bitfield_Table(pTranDef->td_bitfields[bfidx]);
...
/******************************************************************************
 *
 *  NAME:         ncdci2_build_host_structure
 *
...
INT ncdci2_build_host_structure( pCHAR err_buf )
{
	...
	memcpy(temp_message_type, Auth_Tx.TLF01_details.message_type,4);
	  ...
	  if(true == ncdci2_check_for_cadencieacq_transaction() &&
		...
		0 ==strncmp( Auth_Tx.TLF01_details.response_code, "01",2)))
	  {
		  temp_message_type[2] = '1';
	  }
	 if (Auth_Tx.TLF01_details.tx_key == AUTH_VOID_PRE_AUTHORIZATION)
	 {
		 strncpy(temp_message_type, "1420",4);
	 }
	...
	if ( TranFlow == DINERS_SWITCHED_OUT )
	{
		...
		if (temp_message_type[1] == '1')
		{
			if(temp_message_type[2] == '0')
			{
				...
				ncdci2_build_request_field_2(  &result_code, err_buf );
				ncdci2_build_request_field_3(  &result_code, err_buf );
				ncdci2_build_request_field_4(  &result_code, err_buf );
				ncdci2_build_request_field_6( &result_code, err_buf );
				ncdci2_build_request_field_7(  &result_code, err_buf );
				ncdci2_build_request_field_11( &result_code, err_buf );
				ncdci2_build_request_field_12( &result_code, err_buf );
				ncdci2_build_request_field_13( &result_code, err_buf );
				ncdci2_build_request_field_14( &result_code, err_buf );
				ncdci2_build_request_field_22( &result_code, err_buf );
				ncdci2_build_request_field_23( &result_code, err_buf );
				ncdci2_build_request_field_24( &result_code, err_buf );
				ncdci2_build_request_field_26( &result_code, err_buf );
				ncdci2_build_request_field_32( &result_code, err_buf );
				ncdci2_build_request_field_33( &result_code, err_buf );
				ncdci2_build_request_field_35( &result_code, err_buf );
				ncdci2_build_request_field_37( &result_code, err_buf );
				ncdci2_build_request_field_38( &result_code, err_buf );
				ncdci2_build_request_field_40( &result_code, err_buf );
				ncdci2_build_request_field_41( &result_code, err_buf );
				ncdci2_build_request_field_42( &result_code, err_buf );
				ncdci2_build_request_field_43( &result_code, err_buf );
				ncdci2_build_request_field_44( &result_code, err_buf );
				ncdci2_build_request_field_45( &result_code, err_buf );
				ncdci2_build_request_field_48( &result_code, err_buf );
				ncdci2_build_request_field_49( &result_code, err_buf );
				ncdci2_build_request_field_51( &result_code, err_buf );
				ncdci2_build_request_field_52( &result_code, err_buf );
				ncdci2_build_request_field_55( &result_code, err_buf );
				ncdci2_build_request_field_56( &result_code, err_buf );
				ncdci2_build_request_field_58( &result_code, err_buf );
				ncdci2_build_request_field_59( &result_code, err_buf );
				ncdci2_build_request_field_62( &result_code, err_buf );
				ncdci2_build_request_field_64( &result_code, err_buf );
				ncdci2_build_request_field_92( &result_code, err_buf );
				ncdci2_build_request_field_93( &result_code, err_buf );
				ncdci2_build_request_field_100(&result_code, err_buf );
				ncdci2_build_request_field_122(&result_code, err_buf );
				ncdci2_build_request_field_123( &result_code, err_buf );
				ncdci2_build_request_field_125(&result_code, err_buf );
			}
			...
			else if(temp_message_type[2] == '2')
			{
				...
				ncdci2_build_request_field_23( &result_code, err_buf );
				ncdci2_build_request_field_24( &result_code, err_buf );
				ncdci2_build_request_field_25( &result_code, err_buf );
				ncdci2_build_request_field_26( &result_code, err_buf );
				ncdci2_build_request_field_30( &result_code, err_buf );
				ncdci2_build_request_field_32( &result_code, err_buf );
				ncdci2_build_request_field_33( &result_code, err_buf );
				ncdci2_build_request_field_35( &result_code, err_buf );
				ncdci2_build_request_field_38( &result_code, err_buf );
				ncdci2_build_request_field_39( &result_code, err_buf );
				ncdci2_build_request_field_40( &result_code, err_buf );
				ncdci2_build_request_field_41( &result_code, err_buf );
				ncdci2_build_request_field_42( &result_code, err_buf );
				ncdci2_build_request_field_43( &result_code, err_buf );
				...
				ncdci2_build_request_field_49( &result_code, err_buf );
				ncdci2_build_request_field_51( &result_code, err_buf );
				ncdci2_build_request_field_54( &result_code, err_buf );
				ncdci2_build_request_field_55( &result_code, err_buf );
				ncdci2_build_request_field_56( &result_code, err_buf );
				ncdci2_build_request_field_58( &result_code, err_buf );
				ncdci2_build_request_field_62( &result_code, err_buf );
				...
				ncdci2_build_request_field_93( &result_code, err_buf );
				ncdci2_build_request_field_100(&result_code, err_buf );
				ncdci2_build_request_field_106(&result_code, err_buf );
				ncdci2_build_request_field_122(&result_code, err_buf );
				ncdci2_build_request_field_124(&result_code, err_buf );
			}
			else
			{
				/* Unsupported message type.  Log an error. */
				ret_val = false;
				sprintf( err_buf,"Unsupported message type (%s).",
						Auth_Tx.TLF01_details.message_type );
		...
		else if (temp_message_type[1] == '2')
		{
			if(temp_message_type[2] == '0')
			{
				/* Build fields necessary for 0200 REQUEST from POS messages. */
				ncdci2_build_request_field_2(  &result_code, err_buf );
				ncdci2_build_request_field_3(  &result_code, err_buf );
				ncdci2_build_request_field_4(  &result_code, err_buf );
				ncdci2_build_request_field_6( &result_code, err_buf );
		...
		else if(temp_message_type[1] == '3')
		{
			   ...
			   temp_message_type[2] == '1')
			{
				...
				ncdci2_build_request_field_56( &result_code, err_buf );
				ncdci2_build_request_field_64( &result_code, err_buf );
				ncdci2_build_request_field_72( &result_code, err_buf );
				ncdci2_build_request_field_92( &result_code, err_buf );
				ncdci2_build_request_field_93( &result_code, err_buf );
				ncdci2_build_request_field_94( &result_code, err_buf );
				ncdci2_build_request_field_100( &result_code, err_buf );
				ncdci2_build_request_field_101( &result_code, err_buf );
			}
			else if(temp_message_type[2] == '2' ||
	...
	else if ( TranFlow == DINERS_SWITCHED_IN )
	{
		...
		if (temp_message_type[1] == '1')
		{
			if(temp_message_type[2] == '1')
			{
				...
				ncdci2_build_request_field_43( &result_code, err_buf );
				ncdci2_build_request_field_44( &result_code, err_buf );
				ncdci2_build_request_field_46( &result_code, err_buf );
				ncdci2_build_request_field_48( &result_code, err_buf );
				ncdci2_build_request_field_49( &result_code, err_buf );
				ncdci2_build_request_field_51( &result_code, err_buf );
				ncdci2_build_request_field_54( &result_code, err_buf );
		...
		if ( result_code == 1 || ret_val == false )
		{
			if(true == ncdci2_is_approved_by_Cadencie)
			{
				ret_val = true;
			}
			else
...
/******************************************************************************
 *
 *  NAME:         ncdci2_parse_host_msg
 *
 *  DESCRIPTION:  This procedure copies information from a string sent by the
 *                host.  It copies it into the ncdci2_HOST_MESSAGE structure.  The
...
INT ncdci2_parse_host_msg( pCHAR err_buf )
{
	...
	/* Initialize the ncdci2_HOST_MESSAGE structure. */
	memset(&ncdci2_HOST_MESSAGE, 0x00, sizeof (ncdci2_HOST_MESSAGE));
	...
	memcpy(ncdci2_HOST_MESSAGE.msgtype, &ReqRsp_Buffer[reqidx], DINERS_MESSAGE_TYPE_SIZE);
	...
	for( bfidx = 2; bfidx <= BitFieldSize; bfidx++ )
	{
			 ...
			 BITTAB[bfidx].bit_mask )
		{
			...
			pBF = ncdci2_Find_Bitfield_Table(bfidx);
			...
			if ( NULL == pBF )
			{
				...
				strncat( err_buf,ncdci2_HOST_MESSAGE.msgtype, 4 );
			...
			if ( false == (ret_val = (pBF->bd_BFRsp)(pBF->bd_fieldid)) )
			{
				if ( true == invalid_msg_format )
				{
					...
					strcat( err_buf, " MsgType = " );
					strncat( err_buf,ncdci2_HOST_MESSAGE.msgtype, 4 );
					ncdci2_log_message( 2, 2, err_buf, "ncdci2_parse_host_msg",0 );
					break;
				}
				else
				{
					sprintf( err_buf,
							...
							bfidx,ncdci2_HOST_MESSAGE.msgtype );
					ncdci2_log_message( 2, 2, err_buf, "ncdci2_parse_host_msg",0 );
				}
			}
		}
	...
	return( ret_val );
} /* ncdci2_parse_host_msg */  

/******************************************************************************
 *
 *  NAME:         ncdci2_populate_auth_tx
 *
 *  DESCRIPTION:  This function transfers information from the ncdci2_HOST_MESSAGE
 *                structure that contains data from the host into Auth_Tx.
 *
 *  INPUTS:      ncdci2_HOST_MESSAGE - (Global)Data from host
 *
 *  OUTPUTS:      err_buf - Textual message in case of a problem
...
INT ncdci2_populate_auth_tx( pCHAR err_buf )
{
	...
	if ( TranFlow == DINERS_SWITCHED_IN )
	{
						   ...
						   DINERS_MESSAGE_TYPE_SIZE))
		{
			...
			ncdci2_move_field_2(  &result_code, err_buf );
			ncdci2_move_field_3(  &result_code, err_buf );
			ncdci2_move_field_4(  &result_code, err_buf );
			ncdci2_move_field_6(  &result_code, err_buf );
			ncdci2_move_field_7(  &result_code, err_buf );
			ncdci2_move_field_11( &result_code, err_buf );
			ncdci2_move_field_12( &result_code, err_buf );
			ncdci2_move_field_13( &result_code, err_buf );
			ncdci2_move_field_14( &result_code, err_buf );
			ncdci2_move_field_22( &result_code, err_buf );
			ncdci2_move_field_23( &result_code, err_buf );
			ncdci2_move_field_24( &result_code, err_buf );
			ncdci2_move_field_26( &result_code, err_buf );
			ncdci2_move_field_32( &result_code, err_buf );
			ncdci2_move_field_33( &result_code, err_buf );
			ncdci2_move_field_35( &result_code, err_buf );
			ncdci2_move_field_37( &result_code, err_buf );
			ncdci2_move_field_40( &result_code, err_buf );
			ncdci2_move_field_41( &result_code, err_buf );
			ncdci2_move_field_42( &result_code, err_buf );
			ncdci2_move_field_43( &result_code, err_buf );
			ncdci2_move_field_44( &result_code, err_buf );
			ncdci2_move_field_45( &result_code, err_buf );
			ncdci2_move_field_46( &result_code, err_buf );
			ncdci2_move_field_48( &result_code, err_buf );
			ncdci2_move_field_49( &result_code, err_buf );
			ncdci2_move_field_51( &result_code, err_buf );
			ncdci2_move_field_52( &result_code, err_buf );
			ncdci2_move_field_54( &result_code, err_buf );
			ncdci2_move_field_55( &result_code, err_buf );
			ncdci2_move_field_58( &result_code, err_buf );
			ncdci2_move_field_59( &result_code, err_buf );
			ncdci2_move_field_62( &result_code, err_buf );
			ncdci2_move_field_64( &result_code, err_buf );
			ncdci2_move_field_92( &result_code, err_buf );
			ncdci2_move_field_93( &result_code, err_buf );
			ncdci2_move_field_100(&result_code, err_buf );
			ncdci2_move_field_122(&result_code, err_buf );
			ncdci2_move_field_123(&result_code, err_buf );
			ncdci2_move_field_124(&result_code, err_buf );
			ncdci2_move_field_125(&result_code, err_buf );
		}
								...
								DINERS_MESSAGE_TYPE_SIZE))
		{
			...
			ncdci2_move_field_23( &result_code, err_buf );
			ncdci2_move_field_24( &result_code, err_buf );
			ncdci2_move_field_25( &result_code, err_buf );
			ncdci2_move_field_26( &result_code, err_buf );
			ncdci2_move_field_30( &result_code, err_buf );
			ncdci2_move_field_32( &result_code, err_buf );
			ncdci2_move_field_33( &result_code, err_buf );
			ncdci2_move_field_35( &result_code, err_buf );
			ncdci2_move_field_37( &result_code, err_buf );
			ncdci2_move_field_38( &result_code, err_buf );
			ncdci2_move_field_39( &result_code, err_buf );
			ncdci2_move_field_40( &result_code, err_buf );
			ncdci2_move_field_41( &result_code, err_buf );
			ncdci2_move_field_42( &result_code, err_buf );
			ncdci2_move_field_43( &result_code, err_buf );
			...
			ncdci2_move_field_51( &result_code, err_buf );
			ncdci2_move_field_55( &result_code, err_buf );
			ncdci2_move_field_56( &result_code, err_buf );
			ncdci2_move_field_58( &result_code, err_buf );
			ncdci2_move_field_62( &result_code, err_buf );
			ncdci2_move_field_64( &result_code, err_buf );
			ncdci2_move_field_92( &result_code, err_buf );
							    ...
							    DINERS_MESSAGE_TYPE_SIZE ))
		{
			...
			ncdci2_move_field_64( &result_code, err_buf );
			ncdci2_move_field_93( &result_code, err_buf );
			ncdci2_move_field_94( &result_code, err_buf );
			ncdci2_move_field_96( &result_code, err_buf );
			ncdci2_move_field_100(&result_code, err_buf );
		}
		else if ( 0 == strncmp( Auth_Tx.TLF01_details.message_type,
								...
								DINERS_FILE_TRN_MESSAGE_TYPE_LEN ))
		{
			...
			ncdci2_move_field_33( &result_code, err_buf );
			ncdci2_move_field_37( &result_code, err_buf );
			ncdci2_move_field_72( &result_code, err_buf );
			ncdci2_move_field_92( &result_code, err_buf );
			ncdci2_move_field_93( &result_code, err_buf );
			ncdci2_move_field_94( &result_code, err_buf );
			ncdci2_move_field_100( &result_code, err_buf );
			ncdci2_move_field_101(&result_code, err_buf );
		}
	...
	else  /* SWITCHED OUT Transactions */
	{
								...
								DINERS_MESSAGE_TYPE_SIZE ))
		{
			...
			ncdci2_move_field_100(&result_code, err_buf );
								...
								DINERS_MESSAGE_TYPE_SIZE ))
		{
			/* Move fields of 1304 FILE UPDATE REQUEST messages. */
			ncdci2_move_field_2( &result_code, err_buf );
			ncdci2_move_field_3( &result_code, err_buf );
			ncdci2_move_field_4( &result_code, err_buf );
			ncdci2_move_field_7( &result_code, err_buf );
...
/******************************************************************************
 *
 *  NAME:         ncdci2_remove_track2_end_sentinel
 *
...
void ncdci2_remove_track2_end_sentinel( pBYTE track2 )
{
   ...
   ncdci2_trim_trailing_spaces( track2 );
...
/******************************************************************************
 *
 *  NAME:         ncdci2_build_response_msg
 *
 ...
 *               ncdci2_HOST_MESSAGE, then another to take the data from HOST_
 ...
 *
 ******************************************************************************/
INT ncdci2_build_response_msg()
{

   CHAR  err_buf[100] = {0};
   INT   ret_val=true;

   if ( true == (ret_val = ncdci2_build_host_structure(err_buf)) )
   {
      ret_val = ncdci2_build_host_msg( err_buf );
   }

   /* Log any errors. */
   if ( ret_val == false )
      ncdci2_log_message( 2, 2, err_buf, "build_response_message",1 );
...
INT ncdci2_build_request_msg( pCHAR err_buf )
{
	...
	if ( true == (ret_val = ncdci2_build_host_structure(err_buf)) )
	{
		// TF Phani ..change the tx_key .. Changing to VOID for VOICE reversals.
			...
			(true == ncdci2_is_voice_transaction()))
		{
			...
			memcpy( ncdci2_HOST_MESSAGE.proccode,"200000",DINERS_DE3_SIZE );
	...
	{
		ret_val = false;
		ncdci2_log_message( 2, 2, err_buf, "ncdci2_build_request_msg", 1 );
	}
 ...
 *
 ******************************************************************************/
void ncdci2_log_message( INT dest, INT sev, pCHAR msg, pCHAR func, int details )
{
	BYTE severity = 0;
	BYTE log_type[20] = {0};
	...
	ncdci2_Log_error_warning_to_File(text_message,func,sev,details);
	if(monitor_flag == 1)
    {
	   if(details == 1)
	   {
...
/******************************************************************************
 *
 *  NAME:         ncdci2_send_network_status_to_monitor
 *
 ...
 *
 ******************************************************************************/
INT ncdci2_send_network_status_to_monitor()
{
   ...
   if (ConnectionId[0] != 0)
   {
      if ( NetworkType == DINERS_ACQUIRER  ||  NetworkType == DINERS_BOTH )
      {
         ...
         ret_code = ncdci2_build_and_send_request_message( MT_SYS_MONITOR,
                                                    ST1_NETWORK_UPDATE,
                                                    ST2_NONE, 
                                                    applnk_que_name, 
                                                    (pBYTE)&ncf01, 
         ...
         if ( ret_code == false )
         {
            ncdci2_log_message( 2, 2, err_msg,
            					"ncdci2_send_network_status_to_monitor",0 );
         }
      }
   ...
   {
      ncdci2_log_message( 2, 2, "Unable to get connection info for Monitor.",
                  	  	  "ncdci2_send_network_status_to_monitor",0 );
   }
   return( ret_code );
...
/******************************************************************************
 *
 *  NAME:         ncdci2_send_message_to_host
 *
 ...
 *
 ******************************************************************************/
INT ncdci2_send_message_to_host( pCHAR err_buf,INT mgt ) //praneeth added bool mgt as parameter
{
   ...
   ret_val = ncdci2_build_and_send_request_message( MT_OUTGOING,
                                             ST1_NONE,
                                             ST2_NONE,
                                             dialog_que_name,
                                             ReqRsp_Buffer,
   ...
   else
   {
      ...
      {
         /* This is a request. */
         if ( false == ncdci2_increment_active_count(err_buf) )
            ncdci2_log_message( 1, 2, err_buf, "ncdci2_send_message_to_host",0 );
      }

      /* Cycle the echo timer because traffic went out over the line. */
      if ( false == ncdci2_reset_echo_timer(err_buf) )
         ncdci2_log_message(2, 2, err_buf, "ncdci2_send_message_to_host",0 );
   }
   return( ret_val );
}
...
INT ncdci2_pin_block_exists()
{
	...
	else if (ncdci2_blank_string(Auth_Tx.TLF01_details.pin_block,
...
/******************************************************************************
 *
 *  NAME:         ncdci2_verify_host_status
 *
...
INT  ncdci2_verify_host_status( pCHAR err_buf )
{
	...
	{
		strcpy( err_buf,"Host is not Online." );
		ret_val = false;
	}

	if ( ret_val == true )
	{
		/* Check number of consecutive time outs. If too many, issue a warning.*/
		if ( MaxConsecTimeouts > 0 )
		{
			if (atoi(net_consec_tmouts_ctr) > MaxConsecTimeouts )
			{
				strcpy( err_buf,"Maximum consecutive timeouts reached.");
				ncdci2_log_message( 1, 3, err_buf, "ncdci2_verify_host_status",0 );
				ret_val = false;
		...
		/* Check number of active transactions.If too many, issue a warning.*/
		if ( (MaxActiveTxns > 0) && (ret_val == true) )
		{
			if( atoi(active_txns_ctr) >= MaxActiveTxns )
			{
				strcpy( err_buf,"Maximum active txns reached. Continuing to process." );
				ncdci2_log_message( 1, 2, err_buf, "ncdci2_verify_host_status",0 );
			}
		}
	}
...
/******************************************************************************
 *
 *  NAME:         ncdci2_perform_error_processing
 *
...
void ncdci2_perform_error_processing( pCHAR resp_code )   
{
   ...
   (void)ncdci2_send_msg_to_authorizer( MT_AUTH_RESPONSE );
   return;
}

/******************************************************************************
 *
 *  NAME:         ncdci2_perform_standin_processing
 *
 ...
 *
 ******************************************************************************/
INT ncdci2_perform_standin_processing( pCHAR rsp_code, BYTE gen_status, pCHAR err_buf )
{
	CHAR  time_date[25] = {0};
	INT   ret_val = 0;
	...
	if ( 0 == strcmp(rsp_code, DINERS_ISO_APPROVED) )
	{
		...
		if ( Auth_Tx.TLF01_details.auth_number[0] == 0x00 )
		{
			...
			{
				strcpy( Auth_Tx.TLF01_details.auth_number,Auth_Tx.TLF01_details.time_hhmmss );
			}
			else
			{
				/* Get local time. */
				ptetime_get_timestamp( time_date );
				ncdci2_get_time( time_date, Auth_Tx.TLF01_details.auth_number );
			}
		}
	}
	...
	ret_val = ncdci2_send_msg_to_authorizer( MT_AUTH_RESPONSE );
	if ( false == ret_val )
	{
	sprintf( err_buf,
...
/*****************************************************************************
  ...
  Function:   ncdci2_ascii_to_bin_with_length
...
/******************************************************************************
 *
 *  NAME:         ncdci2_trim_trailing_spaces
 *
 ...
 *
 ******************************************************************************/
void ncdci2_trim_trailing_spaces( pBYTE p_string )
{
   INT   i = 0;
   INT   len = 0;
...
void ncdci2_write_to_txn_file()
{
   ...
   if ( Tracing == DINERS_TRACE_ON )
   {
      ...
      if ( rc < 0 )
      {
         ...
         ncdci2_log_message( 2, 1, err_buf, "ncdci2_process_host_msg",0 );
 ...
 *  MODIFICATIONS:
 ******************************************************************************/
void ncdci2_Rj_with_lead_zeros( pCHAR str, INT FieldLen )
{
   int i = 0;
   int dif = 0;
   int strlength = 0;
...
/*****************************************************************************
  ...
  Function:    ncdci2_hhutil_process_host_request
...
BYTE ncdci2_hhutil_process_host_request(pAUTH_TX p_auth_tx)
{
   ...
   /*PRINT("ncdci2_hhutil_process_host_request: RECEIVED DIALOG MESSAGE\n"); */
   ...
   if(true == ncdci2_check_for_cadencieacq_transaction())
   {
	   sprintf(Buff, "RECEIVED DIALOG MESSAGE: %s request from vertexon",
			   	   	   	   p_auth_tx->TLF01_details.message_type);
	   ncdci2_log_message( 2, 1, Buff, "ncdci2_hhutil_process_host_request",1 );

	   if(false == ncdci2_transfer_response_msg_to_dci())
	   {
		   ncdci2_log_message( 1, 3, "Unable to send the request to the ncdci application",
				   	   	   	   	   "ncdci2_hhutil_process_host_request",1 );
		   return(false);
	   }
	   memset(Buff, 0x00, sizeof(Buff));
	   sprintf(Buff, "ncdci2 able to send the %s request to the ncdci application",
			   	   	   	   	   p_auth_tx->TLF01_details.message_type);
	   ncdci2_log_message( 2, 1, Buff, "ncdci2_hhutil_process_host_request",1 );
   }
        ...
        (AUTH_ADVICE          == p_auth_tx->TLF01_details.tx_key)  )
   {
      if ( AUTH_REVERSAL_ADVICE == p_auth_tx->TLF01_details.tx_key )
      {
          ...
          */
         memset( Buff, 0x00, sizeof(Buff) );
         strcpy( Buff,"Reversal from Diners: " );
         ncdci2_log_message( 2, 1, Buff, "ncdci2_hhutil_process_host_request",1 );
      }
      else
      {
      ...
      {
    	  ncdci2_log_message( 1, 3, "Insufficient Memory for select ncf30 message",
    			  	  	  	  	  "ncdci2_hhutil_process_host_request",1 );
          return( false );
      }
      ...
      if ( 0 > ret_code )
      {
         ...
         ncdci2_log_message( 1, 3, Buff, "ncdci2_hhutil_process_host_request",1 );
         return( false );
      }
      return( true );
   }
   else  if (false == ncdci2_hhutil_transfer_ctl_to_auth_app(p_auth_tx))
   {
	   ncdci2_log_message( 1, 3, "Unable to send the request to the authorizer application",
			   	   	   	   	   "ncdci2_hhutil_process_host_request",1 );
	   return(false);
   }
   return(true);
...
/*****************************************************************************
  ...
  Function:    ncdci2_hhutil_transfer_ctl_to_auth_app
...
BOOLEAN ncdci2_hhutil_transfer_ctl_to_auth_app ( pAUTH_TX p_auth_tx)
{
   ...
   if ( NULL_PTR == p_msg_host_out )
	{
	   ncdci2_log_message( 1, 3, "Insufficient Memory to build host message",
	   			   	   	   	   	   "ncdci2_hhutil_transfer_ctl_to_auth_app",1 );
      return(false);
	}
	...
	if( ret_code < 0 )
	{
      ...
      ncdci2_log_message( 1, 3, Buffer,"ncdci2_hhutil_transfer_ctl_to_auth_app",1 );
      free( p_msg_host_out );
      return(false);
	}
...
INT ncdci2_init_txn( pAUTH_TX p_auth_tx )
{
   ...
   if(true == ncdci2_check_for_cadencieacq_transaction())
   {
	   // dont alter the request recievd from cadencie
   }
   else
   {
   ...
   {
	  strcpy(p_auth_tx->TLF01_details.message_type,DINERS_NETWORK_REQ_MESSAGE);
   }
   }
...
/*****************************************************************************
  ...
  Function:    ncdci2_process_logon_request
...
BOOLEAN ncdci2_process_logon_request(CHAR nmi_code[4], pAUTH_TX p_auth_tx)
{
   ...
   if ( 0 != strcmp(LOGON_NMI_ECHO_TEST, p_auth_tx->function_code) )
   {
      ...
      ncdci2_log_message( 1, 1, info_buffer,"ncdci2_process_logon_request",1 );
   }

   /* Prepare the response and sent it. */
   strcpy( p_auth_tx->TLF01_details.response_code, "00" );
   p_auth_tx->TLF01_details.tx_key =
                  ncdci2_match_request_to_response( p_auth_tx->TLF01_details.tx_key );
   (void)ncdci2_incoming_other_host_response( p_auth_tx );
   return(true);
}
...
/*****************************************************************************
  ...
  Function:    ncdci2_match_request_to_response
...
}; /* ncdci2_match_request_to_response */
...
/*****************************************************************************
  ...
  Function:    ncdci2_incoming_other_host_response

  Description: This function will process an incoming response from another host
               that needs to go to the Diners host.
  Author: 
      Abhishek Verma
  Inputs:
...
INT ncdci2_incoming_other_host_response(pAUTH_TX p_auth_tx)
{
   ...
   PRINT("ncdci2_incoming_other_host_response\n");
...

   {
      ...
      ret_code = ncdci2_delete_orig_ncf30( ncf30, p_auth_tx );
      if ( false == ret_code )
      {
    	  ncdci2_log_message( 2, 2, "Unable to delete reversal's original txn in NCF30",
    			  	  	  	  "ncdci2_incoming_other_host_response",1 );
      }
   }

   
   /* Continue to send response to host. */
   return( ncdci2_incoming_other_host_response_db_reply( ) );
}
 ...
 *                   6010, 6011, 6012, 6051
 *
 *                NOTE:  This function assumes a txn switched in from Diners
 *
   ...
   if ( CC[0] != 0x00 )
   {
      ...
      while( BLOCKED_MCC[i][0] != 'd' )
      {
         if ( 0 == strcmp(p_auth_tx->TLF01_details.category_code,BLOCKED_MCC[i]) )
         {
            ...
            sprintf( error_msg,
                    "Blocked txn from Diners: MCC=%s, %s, Stan=%s, RRN=%s",
...
/******************************************************************************
 *
 *  NAME:         ncdci2_get_originator_info
 *
...
/******************************************************************************
 *
 *  NAME:         ncdci2_incoming_other_host_response_db_reply
 *
 ...
 *                up where 'ncdci2_incoming_other_host_response' left off.  You come
...
INT ncdci2_incoming_other_host_response_db_reply( )
{
   ...
   ncdci2_build_host_structure(err_buf);
   
   if (true == ncdci2_build_host_msg(err_buf))
   {
	  		...
	  		)
		{
			...
			{	buflen = buf_temp_len;
				memcpy(ReqRsp_Buffer,Auth_Tx.resp_buffer,buflen);
				ncdci2_log_message( 2, 1, "Sending Diners HOST response to Cadencie",
				    			  	  	  "ncdci2_incoming_other_host_response_db_reply",1 );
			}
		 }
      		ncdci2_hhutil_send_host_response(&Auth_Tx);
     }
   else
   {
	   ncdci2_log_message( 2, 3, "Error building response for the host",
	   				    	"ncdci2_incoming_other_host_response_db_reply",1 );
      return(false);
   }
   ...
   if (AUTH_REVERSAL_ADVICE_RESPONSE == Auth_Tx.TLF01_details.tx_key)
   {
      ...
      ncdci2_Rj_with_lead_zeros(Auth_Tx.TLF01_details.total_amount, 12);
...
/*****************************************************************************
  ...
  Function:    ncdci2_get_tran_type_2

  Description: This function will get the transaction type using the ncdci2_HOST_MESSAGE
               structure
  Author: 
      Abhishek Verma
  Inputs:
      tx_key - the current transaction type
     ncdci2_HOST_MESSAGE (global) - the request buffer containing the message from the host
  Outputs:
      None
  Return values:
...
BYTE  ncdci2_get_tran_type_2(BYTE tx_key) 
{
	...
	if (('1' ==ncdci2_HOST_MESSAGE.msgtype [1] ) && ('0' ==ncdci2_HOST_MESSAGE.msgtype [2]))
	{
		if ( 0 == strncmp( ncdci2_HOST_MESSAGE.proccode,
		...
		else if ( (0 == strncmp( ncdci2_HOST_MESSAGE.proccode,
								 DINERS_DE3_PROC_CODE_ISO_AUTH_BAL_INQ_VAL,
								 DINERS_DE3_PROC_CODE_FIRST_2_DIGIT_LEN ) ) ||
				  (0 == strncmp( ncdci2_HOST_MESSAGE.proccode,
	...
	else if ( ('1' ==ncdci2_HOST_MESSAGE.msgtype [1]) && ('1' ==ncdci2_HOST_MESSAGE.msgtype [2]) )
	{
	...
	else if (('2' == ncdci2_HOST_MESSAGE.msgtype [1] ) && ('0' ==ncdci2_HOST_MESSAGE.msgtype [2]))
	{
		if ( 0 == strncmp(  ncdci2_HOST_MESSAGE.proccode,
	...
	else if ( 0 == strncmp( ncdci2_HOST_MESSAGE.msgtype,
							...
							DINERS_MESSAGE_TYPE_SIZE))
	{
		return( AUTH_ADVICE_RESPONSE );
							...
							DINERS_MESSAGE_TYPE_SIZE)) ||
			  (0 == strncmp(ncdci2_HOST_MESSAGE.msgtype,
	...
	else if ( ('8' ==ncdci2_HOST_MESSAGE.msgtype [1]) && ('0' ==ncdci2_HOST_MESSAGE.msgtype [2]))
	{
		return( AUTH_PIN_CHANGE );
	}
	else if ( ('8' ==ncdci2_HOST_MESSAGE.msgtype [1]) && ('1' ==ncdci2_HOST_MESSAGE.msgtype [2]))
	{
		return( AUTH_LOGON );
	}
	...
	else
	{
		...
		ncdci2_HOST_MESSAGE.msgtype ,ncdci2_HOST_MESSAGE.proccode);
		ncdci2_log_message( 2, 3, Errorbuf, "ncdci_insert_tlf01",1 );
		return(AUTH_INVALID);
	}
}
...
INT ncdci2_insert_tlf01(pAUTH_TX p_auth_tx)
{
   ...
   if(ret_code < 0)
   {
      ...
      ncdci2_log_message( 1, 3, Buff, "ncdci2_insert_tlf01",1 );
      free(p_msg_ins);
      return(false);
   }
   free(p_msg_ins);
...
/*****************************************************************************
  ...
  Function:    ncdci2_hhutil_send_net_mngmnt_request
  ...
  Modified by:
******************************************************************************/
INT ncdci2_hhutil_send_net_mngmnt_request(pAUTH_TX p_auth_tx, CHAR Diners_response_code[4])
{
   pPTE_MSG	 p_msg_host_out        = NULL_PTR;
   CHAR	    ErrorMsg[100] = {0};
   CHAR	    Buffer[256] = {0};
   ...
   ncdci2_build_host_structure(ErrorMsg);
   if ( false == ncdci2_build_host_msg(ErrorMsg) )
   {
	   ncdci2_log_message( 2, 3, "Invalid bit field: building network management request msg",
			   	   	   	   	   "ncdci2_hhutil_send_net_mngmnt_request",1 );
       return(false);
   }
   ...
   p_msg_host_out = ptemsg_build_msg( MT_OUTGOING, 
	...
	(void)ncdci2_set_originator_info( p_auth_tx, p_msg_host_out );
   ...
   return(true);
} /* ncdci2_hhutil_send_net_mngmnt_request */


/*****************************************************************************

  Function:    ncdci2_hhutil_send_net_mngmnt_response
...
INT ncdci2_hhutil_send_net_mngmnt_response(pAUTH_TX p_auth_tx)
{
   ...
   if ( false == ncdci2_build_host_msg(ErrorMsg) )
   {
	   ncdci2_log_message( 2, 3, "Invalid bit field: building network management response msg",
	         	   			   	  "ncdci2_hhutil_send_net_mngmnt_response",1 );
       return(false);
   }
   ...
   free(p_msg_host_out);
   return(true);
} /* ncdci2_hhutil_send_net_mngmnt_response */


/******************************************************************************
 *
 *  NAME:         ncdci2_update_ncf01
 *
...
void ncdci2_update_ncf01( pAUTH_TX p_auth_tx )
{
   ...
   if ( NULL_PTR == p_msg  )
   {
	   ncdci2_log_message( 1, 3, "Insufficient Memory to build insert NCF01 message",
	         	   	         	 "ncdci2_update_ncf01",1 );
       ret_code = false;
   ...
   else
   {
      ...
      ret_code = ncdci2_send_msg( p_msg, netds_que_name, p_auth_tx );
      free( p_msg );
   }
}
...
void ncdci2_update_ncf01_keyblock( pAUTH_TX p_auth_tx )
{
   pPTE_MSG        p_msg = NULL_PTR;
   BYTE            buffer1[sizeof(AUTH_TX)+sizeof(NCF01_KEYBLOCK01_SIZE)] = {0};
   BOOLEAN         ret_code = false;
...
/*****************************************************************************
  ...
  Function:    ncdci2_hhutil_send_host_response
...
BOOLEAN ncdci2_hhutil_send_host_response(pAUTH_TX p_auth_tx)
{
   ...
   if(true == ncdci2_check_for_cadencieacq_transaction())
   {
	    ...
	    sprintf(Buffer, "Sending  host Response %s to Vertexon",
	    					p_auth_tx->TLF01_details.message_type);
	    ncdci2_log_message( 2, 1, Buffer, "ncdci2_hhutil_send_host_response",1 );
	    memset(Buffer, 0x00, sizeof(Buffer));
   ...
   if ( NULL_PTR == p_msg_out )
	{
	   ncdci2_log_message( 1, 3, "Insufficient Memory to build host message",
	   	         	   	         	"ncdci2_hhutil_send_host_response",1 );
       return(false);
	}
   (void)ncdci2_set_originator_info( p_auth_tx, p_msg_out );
...
{
	if( 0 == strncmp(Auth_Tx.TLF01_details.handler_queue,"sbat",
...
BOOLEAN ncdci2_is_IST_transaction(void)
{
...
{
	if(  0 == strncmp(Auth_Tx.TLF01_details.handler_queue,pos_handler_module_name,DINERS_POS_HANDLER_MODULE_COMP_LEN) )
	{
		return true;
	}
	else
...
{
	if(  0 == strncmp(Auth_Tx.TLF01_details.handler_queue,voice_handler_que_name,DINERS_VOICE_MODULE_COMP_LEN) ||
...
INT ncdci2_check_emv_capability_device(void)
{
...
INT ncdci2_check_Asiapay_Ecommerce_transaction (void)
{
...
INT ncdci2_check_RPS_Transaction(void)
{
...
INT ncdci2_check_Asiapay_Recurring (void)
{
	if( (0 == strncmp(Auth_Tx.TLF01_details.pos_condition_code,
					  DINERS_DE25_RECURRING_TRANSACTION,
					  DINERS_DE25_RECURRING_TRANSACTION_INDICATOR_LEN ))&&
...
{
	if(true == ncdci2_is_Magstrip_pos_transaction())
	{
		ncdci2_build_Magstrip_pos_data_from_iso_pos_entry_mode();
		return;
	}
	else if(true == ncdci2_is_Chip_pos_transaction())
	{
		ncdci2_build_Chip_pos_data_from_iso_pos_entry_mode();
		return;
	}
	else if(true == ncdci2_is_Fallback_pos_transaction())
	{
		ncdci2_build_Fallback_pos_data_from_iso_pos_entry_mode();
		return;
	}
	else if(true == ncdci2_is_Contactless_Mastripe_pos_transaction())
	{
		ncdci2_build_Contactless_Mastripe_pos_data_from_iso_pos_entry_mode();
		return;
	}
	else if(true == ncdci2_is_Contactless_Chip_pos_transaction())
	{
		ncdci2_build_Contactless_Chip_pos_data_from_iso_pos_entry_mode();
		return;
	}
	else if(true == ncdci2_is_voice_transaction())
	{
		ncdci2_build_voice_transaction_pos_data_from_iso_pos_entry_mode();
	}
#if BDOR_68_REQ_5_DCI /*Ecommerce Entry Mode From Swiftpass BDOPay Transactions*/
	else if(true == ncdci2_check_if_trx_is_dci_ecommerce_swiftpass_BDOPay_trx ())
	{
		ncdci2_build_ecommerce_swiftpass_BDOPay_trx_pos_data_from_iso_pos_entry_mode();
    }
#endif
	else if(true == ncdci2_is_Manual_pos_transaction())
	{
		ncdci2_build_Manual_pos_data_from_iso_pos_entry_mode();
		return;
	}
	...
	{
		ncdci2_build_MobilePOS_NFC_Tap2Phone_trx_with_pin_pos_data_from_iso_pos_entry_mode();
	}
	...
	{
		ncdci2_build_MobilePOS_NFC_Tap2Phone_trx_without_pin_pos_data_from_iso_pos_entry_mode();
	}
#endif
#if BDOR_68_REQ_3_DCI
	/*Swiftpass BDO Pay Credit Transactions With Pin*/
	else if(true == ncdci2_check_if_trx_is_dci_Swiftpass_BDO_Pay_credit_trx_with_pin())
	{
		ncdci2_build_Swiftpass_BDO_Pay_credit_trx_with_pin_pos_data_from_iso_pos_entry_mode();
	}
	else if(true == ncdci2_check_if_trx_is_dci_Swiftpass_BDO_Pay_credit_trx_without_pin())
	{
		ncdci2_build_Swiftpass_BDO_Pay_credit_trx_without_pin_pos_data_from_iso_pos_entry_mode();
	}
#endif
	else
	{
		/*its unknow pos data*/
		ncdci2_build_unknow_pos_data_from_iso_pos_entry_mode();
	}
...
{
	if( true == ncdci2_is_Card_present_pos_transaction() )
	{
		if(true == ncdci2_is_Magstrip_pos_transaction())
		{
			ncdci2_build_Magstrip_pos_data_in_iso_pos_entry_mode_format_from_pos_data();
		}
		else if(true == ncdci2_is_Chip_pos_transaction())
		{
			ncdci2_build_Chip_pos_data_in_iso_pos_entry_mode_format_from_pos_data();
		}
		else if(true == ncdci2_is_Fallback_pos_transaction())
		{
			ncdci2_build_Fallback_pos_data_in_iso_pos_entry_mode_format_from_pos_data();
		}
		else if(true == ncdci2_is_Contactless_Mastripe_pos_transaction())
		{
			ncdci2_build_Contactless_Mastripe_pos_data_in_iso_pos_entry_mode_format_from_pos_data();
		}
		else if(true == ncdci2_is_Contactless_Chip_pos_transaction())
		{
			ncdci2_build_Contactless_Chip_pos_data_in_iso_pos_entry_mode_format_from_pos_data();
		}
		else if(true == ncdci2_is_Manual_pos_transaction())
		{
			ncdci2_build_Manual_pos_data_in_iso_pos_entry_mode_format_from_pos_data();
		}
		else
		{
			//its unknow pos data
			ncdci2_build_unknow_pos_data_in_iso_pos_entry_mode_format_from_pos_data();
		}
	}

	else if(true ==  ncdci2_is_Card_present_pos_transaction())
	{
		if(true == ncdci2_is_Manual_pos_transaction())
		{
			ncdci2_build_Manual_pos_data_in_iso_pos_entry_mode_format_from_pos_data();
		}
		else
	...
	if(true == ncdci2_is_pin_transaction())
	{
...
{
	if(true == ncdci2_check_emv_capability_device ())
	{
	...
	else if(true == ncdci2_pin_block_exists())
	{
		strncpy(Auth_Tx.TLF01_details.product_codes[16].amount,
...
void ncdci2_build_Manual_pos_data_from_iso_pos_entry_mode()
{
	if(true == ncdci2_check_emv_capability_device ())
	{
		if(true == ncdci2_check_Asiapay_Ecommerce_transaction())
		{
			strncpy(Auth_Tx.TLF01_details.product_codes[16].amount,
					DCI_POS_DATA_DC_PRESENT_ASIAPAY_ECOM,
					DINERS_DE22_SIZE);
		}
		else if (true == ncdci2_check_RPS_Transaction())
		{
			strncpy(Auth_Tx.TLF01_details.product_codes[16].amount,
					DCI_POS_DATA_DC_PRESENT_RPS,
					DINERS_DE22_SIZE);
		}
		else if (true == ncdci2_check_Asiapay_Recurring())
		{
			strncpy(Auth_Tx.TLF01_details.product_codes[16].amount,
					DCI_POS_DATA_DC_PRESENT_ASIAPAY_RPS	,
					DINERS_DE22_SIZE);
		}
		else if (true == ncdci2_check_BDO_PAY())
		{
			strncpy(Auth_Tx.TLF01_details.product_codes[16].amount,
	...
	else if(true == ncdci2_check_Asiapay_Ecommerce_transaction())
	{
		strncpy(Auth_Tx.TLF01_details.product_codes[16].amount,
...
void ncdci2_get_error_warning_file_name_path(void )
{
   ...
   CHAR  filename   [80] = {0};
   CHAR  tmpstr     [80] = {0};
   CHAR  ncdci2_error_warning_file_size[5] = {0};
   ...
   rc = GetPrivateProfileString(
								  ...
								  "DB_STATICS_PATH",       	/* points to key name             */
								   "",  					/* Default string                 */
								   ncdci2_error_warning_file_path,              	/* points to destination buffer   */
								   sizeof(ncdci2_error_warning_file_path)-1,   	 /* size of destination buffer     */
   ...
   rc = GetPrivateProfileString(
								  ...
								  "",  					/* Default string                 */
								  ncdci2_module_error_warning_file_name,              	/* points to destination buffer   */
								  sizeof(ncdci2_module_error_warning_file_name)-1,   	 /* size of destination buffer     */
								  filename                  /* points to ini filename         */
								);
   ...
   rc = GetPrivateProfileString(
							  ...
							  "500",  					/* Default string                 */
							  ncdci2_error_warning_file_size,              	/* points to destination buffer   */
							  sizeof(ncdci2_error_warning_file_size)-1,   	 /* size of destination buffer     */
							  filename                  /* points to ini filename         */
							);

	/* File size size conversion */
	Max_File_Size_Defined = atoi(ncdci2_error_warning_file_size);
   ...
   if((strlen(ncdci2_error_warning_file_path) > 0) &&
	  (strlen(ncdci2_module_error_warning_file_name)> 0))
   {
	   ncdci2_create_Error_Filename();
   }
}

INT ncdci2_Log_error_warning_to_File(pCHAR Error_Buf,pCHAR func,int sev, int detail)
{
	INT   ret_val = 0;
	INT   len=0;
	INT   cardlen = 0;
    ...
    len = strlen(ncdci2_Erro_warning_Filename);
    path_len = strlen(ncdci2_error_warning_file_path);
    if( len == 0 ||
    	path_len==0 )
    {
    ...
    if ( 0 != strcmp(&ncdci2_Erro_warning_Filename[len-4], current_mmdd) )
    {
		/* Now create the new filename.*/
    	ncdci2_create_Error_Filename();
    }
	if((fp = fopen(ncdci2_Erro_warning_Filename,"a+b"))==NULL)
	{
		LogEvent(Error_Buf,INFO_MSG);
		return 0;
	}
...
void ncdci2_create_Error_Filename(  )
{
   ...
   /* Get system timestamp "YYYY-MM-DD-hh.mm.ss.jjj" */
    ...
    strcpy( ncdci2_Erro_warning_Filename, ncdci2_error_warning_file_path );
    strcat( ncdci2_Erro_warning_Filename, ncdci2_module_error_warning_file_name );
	strcat( ncdci2_Erro_warning_Filename, ".EW." );
    strncat(ncdci2_Erro_warning_Filename, system_date,   4 );  /* YYYY */
    strncat(ncdci2_Erro_warning_Filename, system_date+5, 2 );  /* MM   */
    strncat(ncdci2_Erro_warning_Filename, system_date+8, 2 );  /* DD   */
}
...
void ncdci2_read_TO_config_details_from_tf_ini()
{
	   DWORD rc;
	   CHAR  filename   [80] = {0};
	   ...
	   rc = GetPrivateProfileString(  "REVERSL_ATTEMPTS_FOR_TIMEOUT_TRANSACTION",           /* points to section name         */
									  "REVERSL_ATTEMPTS_FOR_TIMEOUT_TRANSACTION_DCI",       	/* points to key name             */
									   "1",  												/* Default string                 */
	   ...
	   rvrsl_attmpts_value_fr_timeout_dci=atoi(readvalue);

	   if(rc == 0)
	   {
		   ncdci2_log_message( 2, 1, "REVERSL_ATTEMPTS_FOR_TIMEOUT_TRANSACTION_DCI Value not read from tf.ini","ncdci2_read_tf_ini",0);
	   }

	   readvalue[0]='\0';
	   rc = GetPrivateProfileString(  "REVERSL_ATTEMPTS_FOR_LATERESPONSE_TRANSACTION",              /* points to section name         */
									  "REVERSL_ATTEMPTS_FOR_LATERESPONSE_TRANSACTION_DCI",       	/* points to key name             */
									   "1",  														/* Default string                 */
									   readvalue,              										/* points to destination buffer   */
									   sizeof(readvalue),   	 									/* size of destination buffer     */
									   filename                  									/* points to ini filename         */
	   ...
	   rvrsl_attmpts_value_fr_laterspns_dci=atoi(readvalue);

	   if(rc == 0)
	   {
		   ncdci2_log_message( 2, 1, "REVERSL_ATTEMPTS_FOR_LATERESPONSE_TRANSACTION_DCI Value not read from tf.ini","ncdci2_read_tf_ini",0);
	   }
 ...
 *  MODIFICATIONS:
 ******************************************************************************/
INT ncdci2_send_reversal_to_host_for_late_response()
{
	   INT   ret_val = true;
	   INT   seconds,buf_temp_len=0;
	   CHAR  err_buf[100]  ="";
	   CHAR  lookup_key[DINERS_LOOKUP_KEY_SIZE]="";
	   ...
	   if ( true == ret_val )
	   {
	      ...
	      (void)ncdci2_create_request_timer_key( lookup_key );
	      seconds = atoi(Ncf01_I.request_timer);
	      ret_val = ncdci2_set_timer( lookup_key, seconds, DINERS_TIMER_REVERSAL );
	      if ( true == ret_val )
	      {
	         /* Send the transaction request to the host. */
	         ret_val = ncdci2_send_message_to_host( err_buf,false );//praneeth added false as parameter
...
{
	if((true == ncdci2_check_if_PEM_is_MobilePOS_NFC_Tap2Phone_trx_with_pin()) &&
	   (true == ncdci2_check_if_trx_is_bdo_pos_acq_trx())&&
	   (true == ncdci2_check_if_trx_qualify_for_MobilePOS_NFC_Tap2Phone_trx()))
	{
		return (true);
	}
	else
...
{
	if((true == ncdci2_check_if_PEM_is_MobilePOS_NFC_Tap2Phone_trx_without_pin()) &&
	   (true == ncdci2_check_if_trx_is_bdo_pos_acq_trx())&&
	   (true == ncdci2_check_if_trx_qualify_for_MobilePOS_NFC_Tap2Phone_trx()))
	{
		return (true);
...
{
	if((true == ncdci2_check_if_PEM_is_Swiftpass_BDO_Pay_credit_trx_with_pin()) &&
	   (true == ncdci2_check_if_trx_is_bdo_pos_acq_trx())&&
	   (true == ncdci2_check_if_trx_qualify_for_MobilePOS_NFC_Tap2Phone_trx()))
	{
		return (true);
...
{
	if((true == ncdci2_check_if_PEM_is_Swiftpass_BDO_Pay_credit_trx_without_pin()) &&
	   (true == ncdci2_check_if_trx_is_bdo_pos_acq_trx())&&
	   (true == ncdci2_check_if_trx_qualify_for_MobilePOS_NFC_Tap2Phone_trx()))
	{
		return (true);
...
{
	if((true == ncdci2_check_if_trx_ecommerce_entry_mode_from_swiftpass_BDOPay_trx()) &&
	   (true == ncdci2_check_if_trx_is_bdo_pos_acq_trx())&&
	   (true == ncdci2_check_if_trx_ecommerce_POS_entry_mode_from_swiftpass_BDOPay_trx()))
	{
		return (true);
	}
	else
