...
* All Rights Reserved.
*
* MODULE:      ncdci2_validation.c
*  
* TITLE:      Diners Network Control Data Validation Functions
*  
...
/******************************************************************************
 *
 *  NAME:         ncdci2_validate_date_yymm
 *
 ...
 *
 ******************************************************************************/
INT ncdci2_validate_date_yymm( pBYTE date, pCHAR err_buf )
{
   ...
   if ( true != ncdci2_isnum(temp_year) )
      sprintf( err_buf, "Invalid year - not numeric: %s", temp_year );
   else if ( true != ncdci2_isnum(temp_month) )
...
INT ncdci2_validate_date_yyyymmdd( pBYTE date_str, pCHAR err_buf )
{
   ...
   if ( (true == ncdci2_isnum(date_str)) && (8 == strlen(date_str)) )
   {
      /* Validate month */
...
INT ncdci2_validate_time( pBYTE time_str )
{
   ...
   if ( (true == ncdci2_isnum(time_str)) && (6 == strlen(time_str)) )
   {
      /* Validate hour */
...
/******************************************************************************
 *
 *  NAME:         ncdci2_validate_date_time
 *
...
INT ncdci2_validate_date_time( pBYTE date_time )
{
   ...
   ret_val = ncdci2_validate_date_mmdd( date );
   if ( ret_val == true )
   {
      memcpy( time, date_time+4, 6 );
      ret_val = ncdci2_validate_time( time );
   }
   return( ret_val );
}
/******************************************************************************
 *
 *  NAME:         ncdci2_validate_YY_date_time
 *
 *  DESCRIPTION:  This function validates that a date in the format of MMDD
 *                has a valid day (numeric and ranging from 01 to 31) and a
 *                valid month (numeric and ranging from 01 to 12).  It also
 ...
 *
 ******************************************************************************/
INT ncdci2_validate_YY_date_time( pBYTE date_time )
{
   INT   ret_val = false;
   CHAR  err_buf[100]={0};
   BYTE  date[9] = {0};
...
/******************************************************************************
 *
 *  NAME:         ncdci2_validate_date_mmdd
 *
 *  DESCRIPTION:  This function verifies a date to be valid.  The date must
...
INT ncdci2_validate_date_mmdd( pBYTE date_str )
{
   ...
   /* Validate date is numeric. */
   if ( (true == ncdci2_isnum(date_str)) && (4 == strlen(date_str)) )
   {
      /* Validate month */
...
INT ncdci2_validate_host_msg()
{
   ...
   if ( ret_val == false )
      ncdci2_log_message( 2, 2, err_buf, "ncdci2_validate_host_msg",0 );
 ...
 *                is reported.
 *
 *  INPUTS:      ncdci2_HOST_MESSAGE.msgtype - (GLOBAL) Message type from host
 *
 *  OUTPUTS:      msgtype - Message type fromncdci2_HOST_MESSAGE
...
INT ncdci2_validate_msgtype( pCHAR msgtype, pCHAR err_buf )
{
	...
	do
	{
		memcpy( msgtype,ncdci2_HOST_MESSAGE.msgtype, 4 );
...
/******************************************************************************
 *
 *  NAME:         ncdci2_check_end_of_year_rollover
 *
...
void ncdci2_check_end_of_year_rollover( pBYTE txn_mmdd, pBYTE ATP_date_yyyymmdd )
{
   ...
   ncdci2_get_date( time_date, date_yyyymmdd );
