...
* REVISION HISTORY:
*
* $Log:   N:\POS\PVCS6.6\EPICPORTZ\PTE\Equitable\new_dcpiso\iso_utilities.c  $
   
      Rev 1.36   Mar 09 2006 09:17:40   dirby
   Modified to process CVV2 data when it is all blanks - offus only.
   SCR 19670
   
      Rev 1.35   Feb 24 2006 16:36:10   dirby
   Modified to look for invalid CVV2 data from terminal and to look
   past the presence indicator and response type (bytes 1 and 2)
   when getting the CVV2 value.  This value can be 3 chars or 4.
   If 3, there is one space in front of it for padding.
   SCR 19670
   
      Rev 1.34   Jun 22 2005 13:34:50   chuang
   Version 4.4.1.6. Fix 16168: crash due to invalid EMV data(field 55).
   
      Rev 1.33   May 26 2005 16:17:10   dirby
   Added 0340 as a valid message type.
   SCR 15507
   
      Rev 1.32   May 26 2005 13:53:02   dirby
   Added an EMV TC Upload response message type.  Also modified
   field 60 parsing to get original stan from 0320 messages.
   SCR 15371
   
      Rev 1.31   Mar 02 2005 16:27:12   dirby
   Added bitfield 23 for the card sequence number.
   SCR 14460
   
      Rev 1.30   Dec 09 2004 14:26:42   dirby
   Modified to populate the length of e-commerce data from DE63.
   SCR 12475
   
      Rev 1.29   Nov 12 2004 17:13:48   dirby
   Modified the parsing of DE48 to get either a 3-digit CVV2 or a 4-digit CSC2.
   SCR 12483
   
      Rev 1.28   Sep 20 2004 15:29:38   dirby
   Added field 4 (balance amount) to reload response, reload repeat
   response, and reload confirm response.
   SCR 12503
   
      Rev 1.27   Jul 22 2004 11:17:36   mbysani
   added the reload repeat and reload confirm
   
      Rev 1.26   Jul 21 2004 10:39:42   mbysani
   added the void_reload response 
   
      Rev 1.25   Jul 13 2004 17:06:08   mbysani
   Added new functionality for AUTO INIT and RELOAD transaction s
   
      Rev 1.24   Jul 09 2004 11:59:22   mbysani
   Added support for EMV chip data coming in field 55
   
      Rev 1.23   Aug 29 2003 11:18:16   svelaga
   Added support for security control information.
   field 53. Version number updated: 4.2.0.4
   
      Rev 1.22   Aug 26 2003 13:58:48   mbysani
   added support for VbV transactions 
   
      Rev 1.21   Feb 11 2003 14:11:02   dirby
   Modified to look in table Id '59' for deferred payment info in
   batch uploads.  In so doing, I reworked this function to use
   a switch statement instead of doing lots and lots of string compares.
   SCR 1019
   
      Rev 1.20   Sep 06 2002 13:40:56   dirby
   Modified Table ID '57' to be length 3.  Apparently this is the
   table EB is using for Deferred Transaction information.
   SCR 888
   
      Rev 1.19   Aug 22 2002 17:01:38   dirby
   Corrected some parsing issues with field 63 for deferred txns
   when there is odometer data.  Apparently, odometer data is
   optional, but when it exists, it is in the same field as deferred
   terms information.
   SCR 255
   
      Rev 1.18   Aug 21 2002 15:35:54   dirby
   Added a response message type for deferred responses.
   Added parsing for online deferred transactions, field 63.
   Added code to build field 63 for deferred transaction responses.
   SCR 255
   
      Rev 1.17   Jun 19 2002 13:45:10   dirby
   Changed Quasi-Cash response to be 0110 with processing code 11nnnn.
   This is for EFT Master transactions.
   SCR 792
   
      Rev 1.16   Oct 24 2001 16:47:32   dirby
   Modified to be able to parse Merchant Category Code (field 18)
   from a device record.  This is possible for EFT transactions.   SCR 520
   
      Rev 1.15   Oct 08 2001 10:39:52   dirby
   Modified to issue a response message type of 0110 if original
   message type is 0100.  For example a cash advance should
   be 0210 response, but if txn came from EFT, it should be 0110.
   SCR 507
   
      Rev 1.14   Oct 01 2001 13:16:50   dirby
   Modified function 'isoutil_parse_adddata_48' to initialize a
   local variable before using it.  It was putting garbage in cvc_data.  SCR 473
   
      Rev 1.13   Jul 10 2001 10:30:38   dirby
   Modified function isoutil_RspIgnore to return a BOOLEAN value.
   
      Rev 1.12   Jun 18 2001 10:09:44   SYARLAGA
   Modified   bf_18sicnumber
   
      Rev 1.11   Jun 15 2001 15:11:44   SYARLAGA
   Modified isoutil_parse_adddata_48. And    bf_18sicnumber.
   
      Rev 1.10   Jun 01 2001 10:48:50   SYARLAGA
   Added bit fields 18 and 43.Added parse function for field 48.
   
      Rev 1.9   Aug 01 2000 11:50:24   dirby
   Corrected parsing of Field 63 for ticket number and odometer
   
      Rev 1.8   Jul 18 2000 09:10:40   dirby
   Modified the ticket number in field 63 to move up to 15 characters.
   
      Rev 1.7   Apr 18 2000 14:41:06   gdaniels
   ticket number field 63
   
      Rev 1.6   Feb 04 2000 09:42:22   sdasgupt
   Modified the field63 parsing routine - fn was returning false if field63 length was not 16 - removed the if
   
      Rev 1.5   Jan 07 2000 17:45:58   sdasgupt
    
   
      Rev 1.4   Dec 27 1999 17:21:08   sdasgupt
    
   
      Rev 1.3   Dec 17 1999 18:03:26   sdasgupt
    
   
      Rev 1.2   Nov 30 1999 17:01:40   sdasgupt
   Updates
   
      Rev 1.1   Nov 22 1999 17:52:00   sdasgupt
    
   
      Rev 1.0   Nov 17 1999 13:48:38   sdasgupt
   Initial release
...
/* P3 decleration*/
#define POS_ENVT_VAR_POSITION 0
#define VISA_TRAN_ID_LEN  15
/************************************************
...
static  struct bitfield_data BFData[] = {
   ...
   {
   bf_64priv64,                /* field */
   ISO_MESSAGE.priv64,        /* data to move */
   8,                            /* # of bytes in iso8583 field */
   isoutil_Move_IfThere,         /* conditional */
   isoutil_GenericMove,          /* move routine */
   isoutil_RspMoveGeneric        /* response routine */
...
#define N_BFData ( sizeof(BFData) / sizeof (struct bitfield_data) )
   ...
   static  struct trandef TRANTABLE[] = {
      ...
      ********************************************/
	  ...
	  {
	  /********************************************
		   PREAUTH V O I D   S A L E   R E S P O N S E
		********************************************/
...
extern void isoutil_build_terminal_response ( INT trantype)
{
   ...
   {
      dcpiso_log_message( 1, 3, "Response transaction message type is not found",
    		  	  	  	  "isoutil_build_terminal_response",1 );
	   return;
   }
   ...
   while ((field_len = genutil_bcd_to_int (p_uchar, 2)) > 0)
   {
      ...
      switch( i_table )
      {
         case 12: /* Ticket Number */
				...
				if( 0 == dcpiso_check_If_VISA_Transaction() &&
					...
					0 == strncmp(auth_tx.TLF01_details.pos_condition_code,"08",2))
				{
					...
					if(move_len > 15)
					{
						if((move_len-15) > 12)
						{
							memcpy(auth_tx.TLF01_details.product_codes[14].amount, p_char+15, 12);// 14 Amount
							/*for RPS txn visa u need 35 bytes for building DE104 in visa, copy remeianing 8 bytes into
							 *  emv table under auth tx */
							memcpy(auth_tx.EMV_details.serial_number,p_char+27,8);
						}
						else
						{
							memcpy(auth_tx.TLF01_details.product_codes[14].amount, p_char+15, (move_len-15));
						}
         ...
         case 71:
        	 ...
        	 move_len = 20;
        	 p_uchar += VISA_TRAN_ID_LEN;
             memcpy( auth_tx.TLF01_details.visa_tran_id,
                     p_char, VISA_TRAN_ID_LEN);
             p_uchar += 20;
             	 break;
...
/* Check if this is A MP transaction or not*/
  if(2 == strlen(data_array) )
  {
	...
	if(term_value >=2 && term_value <= 99)
	{
    	...
    	//sprintf(auth_tx.TLF01_details.deferred_term_length, "%02d", strlen(term_data) );
		// We have to store term value to find intrest details.
		memcpy(auth_tx.TLF01_details.deferred_term_length,term_data,strlen(term_data));
	}
	else
	{
		// Clear MP purchase ID TYPE details
		memset(auth_tx.TLF01_details.product_codes[17].code,0x00,6);
		memset(auth_tx.TLF01_details.product_codes[17].amount,0x00,12);
		memset(auth_tx.TLF01_details.product_codes[18].amount,0x00 ,12);
		memset(auth_tx.TLF01_details.product_codes[18].code,0x00 ,12);
	}
  }
   return;
...
extern void isoutil_parse_adddata_48()
{
   ...
   /* Collect POS environment field*/
   if( 0 == strncmp(auth_tx.TLF01_details.pos_condition_code,"08",2) &&
	   0 == strncmp(auth_tx.TLF01_details.pos_entry_mode,"0012",4))
   {
	   memset(auth_tx.TLF01_details.product_codes[7].quantity ,0x00,
			  sizeof(auth_tx.TLF01_details.product_codes[7].quantity));

	   auth_tx.TLF01_details.product_codes[7].quantity[0] = temp[POS_ENVT_VAR_POSITION];

	   if(f48_len <=1 )
	   {
		   /* If length is less than or equal to 1, CVV wont be present. hence
   ...
   if ( cvc2_exists == true )
   {
      ...
      if ( cvc2_len == 3 )
         memcpy( auth_tx.TLF01_details.cvc_data, &temp[num_spaces], cvc2_len );
      else if ( temp[2] == ' ' )
      {
         ...
         else
         {
            ...
            auth_tx.TLF01_details.product_codes[17].quantity[0] = temp[0];
            auth_tx.TLF01_details.product_codes[17].quantity[1] = temp[1];
         }
      }
      else
         memcpy(auth_tx.TLF01_details.interest_rate,&temp[num_spaces],cvc2_len);
   }
   else
   {
      auth_tx.TLF01_details.verify_mag_stripe[0] = 'N';
...
extern void isoutil_priv61_cvt_ascii()
{
   ...
   strncpy(auth_tx.TLF01_details.product_code, ISO_MESSAGE.priv61, 8);
	...
	/*Safe key changes - TF Phani */

	// Store DE61 as it is if the value exists in incoming message
	genutil_bcd_to_asc (ISO_MESSAGE.priv61_len, sizeof(ISO_MESSAGE.priv61_len),  /* JH Apr 30 */
						&templen[0], sizeof(templen));

	
	length = atoi(templen);
	/*if any chnage in dcpiso for storing CAVV, chnages sholud be done in txcntl also as well
		 ,for checking the length*/
	if(length > 0 && length < 100)
	{
		sprintf(auth_tx.int_net_facility_len,"%03d",length);
		memcpy(auth_tx.int_net_facility_data,ISO_MESSAGE.priv61,length);
	}
...
extern BOOLEAN isoutil_priv64_cvt_ascii()
{
	CHAR temp_len[6];
	INT	 tot_len = 0;
	CHAR  tempstr [200] = {0} ;
	...
	if ( ((ISO_MESSAGE.msgtype_bcd[0] == 0x02) || (ISO_MESSAGE.msgtype_bcd[0] == 0x01)) && (ISO_MESSAGE.msgtype_bcd[1] == 0x00) )
	{
	   /* Pin Block can have nulls in it.  So check whole field for data. */
	   for ( i=0; i<8; i++ )
	   {
		  if ( ISO_MESSAGE.priv64[i] != 0x00 )
		  {
			 genutil_bin_to_ascii (ISO_MESSAGE.priv64, sizeof(ISO_MESSAGE.priv64),
								   &tempstr[0], sizeof(tempstr));

			 memcpy(auth_tx.TLF01_details.product_codes[4].amount, &tempstr[0], sizeof(auth_tx.TLF01_details.product_codes[4].amount)-1);
			 memcpy(auth_tx.TLF01_details.product_codes[6].amount, &tempstr[12], sizeof(auth_tx.TLF01_details.product_codes[6].amount)-1);
...
extern BOOLEAN isoutil_RspMovevBCD (enum bitfields fieldid)
{
   ...
   if (isoutil_verify_fieldlen(fieldid, movelen) == false)
   {
      ...
      dcpiso_log_message( 1, 3, Buff,"isoutil_RspMovevBCD",0 );
	  return false;
   }
