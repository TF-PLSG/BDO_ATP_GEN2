...
static BCH01_MP_LIST    bch01_mp_list;
...
CHAR  settle_mp_Erro_warning_Filename[256] = {0};
CHAR  settle_mp_module_error_warning_file_name[256] = {0};
CHAR  settle_mp_error_warning_file_path[256] = {0};
...
void settle_mp_create_Error_Filename(  )
{
   ...
   /* Get system timestamp "YYYY-MM-DD-hh.mm.ss.jjj" */
    ...
    strcpy( settle_mp_Erro_warning_Filename, settle_mp_error_warning_file_path );
    strcat( settle_mp_Erro_warning_Filename, settle_mp_module_error_warning_file_name );
	strcat( settle_mp_Erro_warning_Filename, ".EW." );
    strncat(settle_mp_Erro_warning_Filename, system_date,   4 );  /* YYYY */
    strncat(settle_mp_Erro_warning_Filename, system_date+5, 2 );  /* MM   */
    strncat(settle_mp_Erro_warning_Filename, system_date+8, 2 );  /* DD   */
}
...
void settle_mp_get_error_warning_file_name_path(void )
{
   ...
   CHAR  filename   [80] = {0};
   CHAR  tmpstr     [80] = {0};
   CHAR  settle_mp_error_warning_file_size[5] = {0};
   ...
   rc = GetPrivateProfileString(
								  ...
								  "DB_STATICS_PATH",       	/* points to key name             */
								   "",  					/* Default string                 */
								   settle_mp_error_warning_file_path,              	/* points to destination buffer   */
								   sizeof(settle_mp_error_warning_file_path)-1,   	 /* size of destination buffer     */
   ...
   rc = GetPrivateProfileString(
								  ...
								  "",  					/* Default string                 */
								  settle_mp_module_error_warning_file_name,              	/* points to destination buffer   */
								  sizeof(settle_mp_module_error_warning_file_name)-1,   	 /* size of destination buffer     */
								  filename                  /* points to ini filename         */
								);
   ...
   rc = GetPrivateProfileString(
							  ...
							  "500",  					/* Default string                 */
							  settle_mp_error_warning_file_size,              	/* points to destination buffer   */
							  sizeof(settle_mp_error_warning_file_size)-1,   	 /* size of destination buffer     */
							  filename                  /* points to ini filename         */
							);

	/* File size size conversion */
	Max_File_Size_Defined = atoi(settle_mp_error_warning_file_size);
   ...
   if((strlen(settle_mp_error_warning_file_path) > 0) &&
	  (strlen(settle_mp_module_error_warning_file_name)> 0))
   {
	   settle_mp_create_Error_Filename();
   }
}
...
INT settle_mp_Log_error_warning_to_File(pCHAR Error_Buf,int sev,pCHAR func, int detail)
{
    ...
    len = strlen(settle_mp_Erro_warning_Filename);
    path_len = strlen(settle_mp_error_warning_file_path);
    if( len == 0 ||
    	path_len==0 )
    {
    ...
    if ( 0 != strcmp(&settle_mp_Erro_warning_Filename[len-4], current_mmdd) )
    {
		/* Now create the new filename.*/
    	settle_mp_create_Error_Filename();
    }
	if((fp = fopen(settle_mp_Erro_warning_Filename,"a+b"))==NULL)
	{
		LogEvent(Error_Buf,INFO_MSG);
		return 0;
	}
...
void settle_mp_log_message( INT dest, INT sev, pCHAR msg, pCHAR func, int details )
{
   ...
   settle_mp_Log_error_warning_to_File(text_message,sev,func,details);
   if(monitor_flag == 1)
   {
...
void main(int argc, char *argv[])
{
   ...
   *     Parameter 1    -  settle_mp file base file name
...
void MainProcessor(void)
{
   ...
   INT               ret_code = 0;
   LONG              julian;
   BYTE              bch01_mp_buffer[sizeof(AUTH_TX) + sizeof(BCH01_MP)];
   INT               num_sql;
   INT               err_ctr = 0;
   double            start_time;
   double            end_time;
   ...
   memset(&bch01_mp_buffer,0, sizeof(bch01_mp_buffer));
   ...
   settle_mp_log_message( 2, 1, buffer, "MainProcessor" ,0 );
   ...
   settle_mp_log_message( 2, 1, buffer, "MainProcessor" ,0 );
   settle_mp_get_error_warning_file_name_path();
   ...
   if (dbcommon_connect ("equitable", "equitable", "equitable", "ORCL", err_msg) != PTEMSG_OK)
   {
      ...
      settle_mp_log_message( 2, 3, buffer, "MainProcessor", 0 );
   ...
   /*** Set initial data for BCH01_MP table ***/
   ...
   /* Process all batches identified in the bch01_mp database table */
   if (ret_code =  (db_get_bch01_mp_service_list(&auth_tx_total, &bch01_mp_list, err_msg)) != PTEMSG_OK)
   {
	   sprintf (buffer, "Error processing bch01_mp batches.");
	   sprintf(lError_Msg,"db_get_bch01_mp_service_list: %s",buffer);
	   settle_mp_log_message( 2, 3, lError_Msg, "MainProcessor" ,0 );
   } 
...
BOOLEAN create_raffle_header( pBON03_LIST bon03_ptr )
{
   ...
   { 
      ...
      { 
         ...
         settle_mp_log_message( 2, 3, buffer, "create_reffle_header", 0 );
         write_to_dump_file(buffer);
...
{
   printf("Settle_mp requires four command line parameters to work correctly:                  \n");
   printf("Parameter 1  -  settle_mp file base file name                                       \n");
   ...
   printf("                                                                       			\n");
   printf("In addition, Settle_mp has an optional 5th parameter to indicate which    			\n");
...
void EndProcess(void)
{
   ...
   if ( EndProcessFlag == true )
   {
      sprintf( Buffer, "Shutting down the %s Service, version %s",
               AppName, Version );
      settle_mp_log_message( 2, 1, Buffer, "EndProcess" ,0 );
      strcat( Buffer, "\n" );
      PRINT( Buffer );
   }
   else
...
BOOLEAN get_currency_code()
{
   ...
   if ( rc != PTEMSG_OK )
   {
      memset( buffer, 0x00,sizeof(buffer) );
      sprintf( buffer,
              "DB Err getting Currency Code from Merchant record: %s TID %s ERR MSG %s ",
			  tc_totals.merchant_id,tc_totals.terminal_id,err_msg );
      settle_mp_log_message( 2, 3, buffer, "create_reffle_header", 0 );
      retval = false;
   }
   return( retval );
...
{   
   ...
   settle_mp_log_message( 2, 1, buffer, "open_credit_file", 0 );
   /* Send message to DOS window in debug mode */
   strcat(buffer,"\n");
   PRINT(buffer);
   ...
   {  
      ...
      settle_mp_log_message( 2, 2, buffer, "open_credit_settle_file", 0 );
	  write_to_dump_file(buffer);
...
{   
   ...
   settle_mp_log_message( 2, 1, buffer, "open_debit_file", 0 );
   /* Send message to DOS window in debug mode */
   strcat(buffer,"\n");
   PRINT(buffer);
   ...
   {  
      ...
      settle_mp_log_message( 2, 3, buffer, "open_debit_settle_file", 0 );
	  write_to_dump_file(buffer);
...
BOOLEAN open_empty_raffle_file()
{
   ...
   settle_mp_log_message( 2, 1, buffer, "open_empty_raffle_file", 0 );
   /* Send message to DOS window in debug mode */
   strcat(buffer,"\n");
   PRINT(buffer);
   ...
   {  
      ...
      settle_mp_log_message( 2, 3, buffer, "open_empty_raffle_file", 0 );
	  write_to_dump_file(buffer);
...
BOOLEAN open_raffle_file()
{
   ...
   settle_mp_log_message( 2, 1, buffer, "open_raffle_file", 0 );
   /* Send message to DOS window in debug mode */
   strcat(buffer,"\n");
   PRINT(buffer);
...
BOOLEAN open_exception_file()
{
   ...
   settle_mp_log_message( 2, errtype, buffer, "open_exception_file" ,0 );
   return( retval );
...
/*   strcpy(buffer,"Writing to credit settlement file ");
   settle_mp_log_message( 2, 1, buffer, "write_to_credit_settle_file", 0 );
*/
   /* Send message to DOS window in debug mode */
/*   strcat(buffer,"\n");
   PRINT(buffer);
*/
   ...
   if (fwrite (c_record, sizeof(c_record) - 1, 1, c_file_write) < 0)
   {
      ...
      settle_mp_log_message( 2, 3, buffer, "write_to_credit_settle_file", 0 );
	  write_to_dump_file(buffer);
...
BOOLEAN write_to_except_settle_file()
{
   ...
   if (fputs (e_record, e_file_write) < 0)
   {
      ...
      strcat(buffer, tempstr);
      settle_mp_log_message( 2, 2, buffer, "write_to_except_settle_file" ,0 );
      write_to_dump_file(buffer);
...
void write_to_dump_file(CHAR* dump_record)
{
   ...
   if (dumpfile_ptr == NULL_PTR)
   {
       ...
       settle_mp_log_message( 2, 1, buffer, "open_dump_file", 0 );
       /* Send message to DOS window in debug mode */
       strcat(buffer,"\n");
       PRINT(buffer);
	   ...
	   {  
		  ...
		  settle_mp_log_message( 2, 3, buffer, "open_dump_file", 0 );
   ...
   if (fputs (dump_record, dumpfile_ptr) < 0)
   {
      ...
      settle_mp_log_message( 2, 3, buffer, "write_to_dump_file", 0 );
   }
   return;
...
BOOLEAN write_to_debit_settle_file()
{
   ...
   settle_mp_log_message( 2, 1, buffer, "write_to_debit_settle_file", 0 );
   /* Send message to DOS window in debug mode */
   strcat(buffer,"\n");
   PRINT(buffer);
...
BOOLEAN write_to_raffle_file()
{
   ...
   settle_mp_log_message( 2, 1, buffer, "write_to_raffle_file", 0 );
   /* Send message to DOS window in debug mode */
   strcat(buffer,"\n");
   PRINT(buffer);
   ...
   if (fputs (r_record, r_file_write) < 0)
   {
      ...
      strcat(buffer, tempstr);
      settle_mp_log_message( 2, 3, buffer, "write_to_raffle_file", 0 );
	  write_to_dump_file(buffer);
...
BOOLEAN bch01_mp_handler()
{
   ...
   if (get_currency_code() == false)
   {
      ...
      settle_mp_log_message( 2, 1, buffer, "bch01_mp_handler", 0 );
   ...
   else
   {
      ...
      if (strcmp(auth_tx_total.batch_status, "C") == 0 || strcmp(auth_tx_total.batch_status, "P") == 0 || strcmp(auth_tx_total.batch_status, "W") == 0)
      {
		...
		//else if(strcmp(auth_tx_total.batch_status, "P") == 0)
         ...
         if (db_update_bch01_mp_move_bch10tobch11_delete_bch10(&auth_tx_total,err_msg) != PTEMSG_OK)
         {
            dbcommon_rollback ();
            ...
            settle_mp_log_message( 2, 2, buffer, "update_bch01_mp_move_bch10tobch11_delete_bch10", 0 );
	        /*Write Batch Information to dump file*/
	        memset(&dump_record, 0x00, sizeof(dump_record));
 ...
 *                45 days.  These are put into a separate settle_mp file (an
...
void process_exception_txns()
{
   ...
   settle_mp_log_message( 2, 1, msgbuf, "process_exception_txns" ,0 );

   /* Get number of records that should go in the exception file.
    * These are records that are not part of a batch in BCH01_MP.
    */
   TempExRecCnt   = 0;
   ...
   if ( rec_cnt == -1 )
   {
      ...
      settle_mp_log_message( 2, 2, "Unable to select number of exception records from BCH10",
    		  	  	  	  	  "process_exception_txns" ,0 );
      write_to_dump_file( msgbuf );
   }
   else if ( rec_cnt > 0 )
   {
