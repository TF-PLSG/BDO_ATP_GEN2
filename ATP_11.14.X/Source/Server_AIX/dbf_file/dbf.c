...
void MainProcessor(void)
{
	...
	dbf_log_message(2,2,buffer,"MainProcessor");
...

	{
		...
		if (ret_code =  (db_get_tlf01_count(totalCount, err_msg, cutoff_date)) != PTEMSG_OK)
		{
			...
			dbf_log_message(2,3,lError_Msg,"MainProcessor");
...
BOOLEAN WriteTSVFooter(long lTotalRecords)
{
	if(dbfFilePtr != NULL)
	{
		fprintf(dbfFilePtr, "[(*)DBF TLF File created by DBF_FILE | version %s | Total Records %d]\n", Version, lTotalRecords);
	}
	return TRUE;
}
...
BOOLEAN WriteTLF01TableIntoTSV(DBF_TLF01 *pte_tlf01, FTLF01 *pte_ftlf01, PTSN01 *pte_ptsn01, char *local_kek)
{
	...
	nbr_of_prod_codes_len = strlen(pte_tlf01->nbr_of_prod_codes);
	if ( nbr_of_prod_codes_len > 0  )
	{
	...
	/*Tranid*/
	if(true == Dbf_Check_If_AMEX_Transaction (pte_tlf01))
	{
	...
	else if(true == Dbf_Check_If_VISA_Transaction (pte_tlf01))
	{
		WriteString(&pte_tlf01->visa_tran_id , 25, FILLERCHAR, SEPERATORCHAR, &dbfFilePtr, "Scheme_tran_ID");
	}
	else if(true == Dbf_Check_If_MC_Transaction (pte_tlf01))
	{
		WriteString(&pte_tlf01->mcard_banknet , 25, FILLERCHAR, SEPERATORCHAR, &dbfFilePtr, "Scheme_tran_ID");
	}
	else if( true == Dbf_Check_If_CUP_Transaction (pte_tlf01) ||
			 true == Dbf_Check_If_JCB_Transaction (pte_tlf01))
	{
	...
	if(true == Dbf_Check_If_MC_Transaction (pte_tlf01))
	{
	...
	if(true == Dbf_Check_If_VISA_Transaction (pte_tlf01))
	{
	...
	if(true == Dbf_Check_If_MC_Transaction(pte_tlf01) && strcmp(pte_tlf01->message_type, "0100") == 0 && pte_tlf01->product14_code[0] != '\0')
	{
		memset(str, 0, sizeof(str));
		memcpy(str, pte_tlf01->product14_code, 2);
		WriteString(str , 2, FILLERCHAR, SEPERATORCHAR, &dbfFilePtr, "Brand_Tran_Type_Identifier");
	}
	else if(true == Dbf_Check_If_VISA_Transaction(pte_tlf01) && strlen(pte_tlf01->product17_amount) >= 10)
	{
		memset(str, 0, sizeof(str));
...
BOOLEAN WriteString(char *str, int size, char fillerChar, char seperator, FILE** fptr, char *header)
{
	...
	else
	{
		...
		dbf_log_message(3,3,buffer,"WriteString");
...
BOOLEAN WriteTLF01TableIntoDBF(DBF_TLF01 *pte_tlf01)
{
	...
	memcpy(str1, pte_tlf01->card_num,64);
  ...
  *******************************************************************************/
  void EndProcess(void)
  {
	  ...
	  if ( EndProcessFlag == true )
	  {
		  ...
		  //               AppName, Version );
		  // 	dbf_log_message(2,1,Buffer,"EndProcess");
		  //      strcat( Buffer, "\n" );
		  //      PRINT( Buffer );
	  }
	  else
                                   ...
                                   pCHAR appname, pCHAR errbuf )
{
   ...
   CHAR  dbf_error_warning_file_size[5] = {0};
   ...
   rc = GetPrivateProfileString(
							  ...
							  "500",  					/* Default string                 */
							  dbf_error_warning_file_size,              	/* points to destination buffer   */
							  sizeof(dbf_error_warning_file_size)-1,   	 /* size of destination buffer     */
							  filename                  /* points to ini filename         */
							);

	/* File size size conversion */
	Max_File_Size_Defined = atoi(dbf_error_warning_file_size);
 ...
 *
 ******************************************************************************/
void dbf_log_message( INT dest, INT sev, pCHAR msg, pCHAR func )
{
   INT  monitor_flag;
   INT  eventlog_flag;
   INT  msg_type;
   ...
   dbf_Log_error_warning_to_File(text_message,sev,func);
...
INT dbf_Log_error_warning_to_File(pCHAR Error_Buf,int sev,pCHAR func)
{
	INT   len=0;
	INT   path_len = 0;
	CHAR  time_date[25]  ={0};
...
INT Dbf_Check_If_CUP_Transaction (DBF_TLF01 *pte_tlf01)
{
...
INT Dbf_Check_If_VISA_Transaction (DBF_TLF01 *pte_tlf01)
{
...
INT Dbf_Check_If_MC_Transaction (DBF_TLF01 *pte_tlf01)
{
...
INT Dbf_Check_If_JCB_Transaction (DBF_TLF01 *pte_tlf01)
{
...
INT Dbf_Check_If_AMEX_Transaction (DBF_TLF01 *pte_tlf01)
{
	if((pte_tlf01->card_num[0]=='3') &&
...
BOOLEAN DBF_Check_is_Acquiring_Transaction(DBF_TLF01 *pte_tlf01)
{
