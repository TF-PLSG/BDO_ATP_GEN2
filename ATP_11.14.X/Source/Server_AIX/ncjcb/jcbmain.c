...
extern char jcb_scheme_timeout_rc [3];
extern char jcb_scheme_fai_rc [3];
...
CHAR  ncjcb_Error_warning_Filename[256]={0};
CHAR  ncjcb_module_error_warning_file_name[256]={0};
CHAR  ncjcb_error_warning_file_path[256]={0};
   ...
   switch( msgtype )
   {
      ...
      case MT_SYS_MONITOR:
         ...
         switch( ptemsg_get_msg_subtype1(p_cur_msg) )
         {
            ...
            case ST1_ECHOTEST:
                ...
                memset (&auth_tx, 0, Auth_Tx_Size);
                //HCTS1-D
                ncjcb_log_message( 1, 1, "Sending ECHO TEST request to host." , "PinnacleMsgHandler" , 0 ,"");
            ...
            case ST1_NETWORK_QUERY:
 	             ...
 	             if (ConnectionId[0] != 0)
                {
                   ...
                   if ( 0 > ret_code )
                   {
                      ...
                      ncjcb_log_message( 3, 3, Buff , "PinnacleMsgHandler" , 0 ,"");
                      free( p_msg_ins );
                   }
                }
                free( p_msg_ins );
      ...
      case MT_SYSTEM_REQUEST: 
               ...
               if ( false == flush_file(ppTraceFile_fp) )
               {
                  ...
                  sprintf( Buff,
                    "%s: Unable to flush trace file buffer. Turning Trace off.",
                     ServiceName );
                  ncjcb_log_message( 3, 1, Buff , "PinnacleMsgHandler" , 0 ,"");
         ...
         if(ST1_LOAD_JCB3DS2_RECORDS == ptemsg_get_msg_subtype1(p_msg_in))
        {
        	...
        	sprintf( Buff, "JCB3DS2 records reloaded for JCB" );
        	ncjcb_log_message( 3, 1,Buff, "PinnacleMsgHandler", 0 ,"");
        }
         else if(ST1_LOAD_TO_RC_VALUES == ptemsg_get_msg_subtype1(p_msg_in))
         {
        	 ncjcb_read_TO_rc_from_tf_ini();
        	 send_trace_response(p_msg_in);
        	 sprintf( Buff,
        	          "jcb_scheme_timeout_rc: %s",
					  jcb_scheme_timeout_rc);
        	 ncjcb_log_message( 1, 1, Buff, "PinnacleMsgHandler" ,0,"");
        }
         else if(ST1_LOAD_RC_FOR_FAI == ptemsg_get_msg_subtype1(p_msg_in))
         {
        	 ncjcb_read_fai_rc_from_tf_ini();
        	 send_trace_response(p_msg_in);
        	 sprintf( Buff,
        	          "jcb_scheme_fai_rc: %s",
					  jcb_scheme_fai_rc);
      ...
      case MT_INCOMING_GUI:
         /* GUI is initiating an 0302 message for JCB: Online File Update.
		   NCJBC2 can also send the 0302 message .TF Phani */
         (void)process_file_update_request( p_msg_in );
         break;

      default :
         sprintf( err_mesg, "Unknown msg type received: %d", (INT)msgtype );
         ncjcb_log_message( 3, 1, err_mesg , "PinnacleMsgHandler" , 0 ,"");
         break;
   }  /* switch msg_type */
...
void MainProcessor()
{
   ...
   sprintf( Buffer,
           "Starting the Network Controller Service: %s, version %s",
            ServiceName, Version );
   ncjcb_get_error_warning_file_name_path();
   ncjcb_log_message( 2, 1, Buffer , "MainProcessor" , 0 ,"");
   printf( "%s\n", Buffer );
...
#endif
	{
      ncjcb_log_message( 2, 3, "Failed to create XIPC queues" , "MainProcessor" , 0 ,"");
   ...
   if (strlen(AppName) > (sizeof(ncf01_i.primary_key.network_id)-2) )
	{
      ncjcb_log_message( 2, 2, Buffer , "PinnacleMsgHandler" , 0 ,"");
#ifdef _DEBUG
      pteipc_shutdown_single_instance_app(); 
#else
      pteipc_shutdown_multiple_instance_app();
   ...
   else if ( 0 == ret_code )
   {
      ...
      if ( 0 != ret_code)
      {
         ...
         sprintf( Buffer, "Unable to write to shared memory table: %s.",
                  SHARED_MEMORY_TBL );
         ncjcb_log_message( 2, 2, Buffer , "MainProcessor" , 0 ,"");
      }
   }
   else
   {
         ...
         sprintf( Buffer, "Unable to create shared memory table: %s.",
                  SHARED_MEMORY_TBL );
         ncjcb_log_message( 3, 2, Buffer , "MainProcessor" , 0 ,"");
   }
      ...
      if (false == open_trace_file(AppName,DirPath,ppTraceFile_fp,TraceFile))
      {
         ...
         sprintf( ErrorMsg,
                 "Unable to open trace file %s. Tracing is off",
                  TraceFile );
         ncjcb_log_message( 3, 1, ErrorMsg , "MainProcessor" , 0 ,"");
      }
      else
    	  ncjcb_log_message( 3, 1, "Tracing is turned ON" , "MainProcessor" , 0 ,"");
   }
   else
   {
      Tracing = TRACE_OFF;
      ...
      ncjcb_log_message( 3, 1, "Tracing is turned off" , "MainProcessor" , 0 ,"");
...
//         {
//         	  ncjcb_log_message( 3, 1, "Error executing host Echo Test" , "PinnacleMsgHandler" , 0 ,"");
    	  //}
      } 
   }
...
void EndProcess()
{
   ...
   if ( Tracing == TRACE_ON )
   {
      if ( false == close_trace_file( ppTraceFile_fp ))
      {
         ...
         ncjcb_log_message( 2, 1, Buffer , "EndProcess" , 0 ,"");
      }
   }
