...
* $Log:   N:\POS\PVCS6.6\EPICPORTZ\PTE\Equitable\ncjcb\jcbtran.c  $
       ...
       Rev 1.142  Jul 13  2009            TF-Ajay
   Modified the code that gets the original transaction for an auth
   advice (0120). Need tp copy the STIP txn status.
   

		Rev 1.141   May 04 2007            dirby
   Modified the code that gets the original transaction for an auth
   advice (0120). Need to use the RRN instead of the STAN to find it.
   SCR 24846

      Rev 1.140   Jul 05 2006 17:29:34   dirby
   Modified to store EMV response info into the future fields in the
   EMV block.  This is so dcpiso can read the response info correctly
   when there is a large amount of EMV response info.
   SCR 22170
   
      Rev 1.139   Jun 07 2005 11:31:44   dirby
   Removed an extraneous clear/set echo timer command.
   JCB EMV Certification Issues
   
      Rev 1.138   Jun 06 2005 21:33:00   dirby
   Modified to send voids to JCB as reversals.
   SCR 12190
   
      Rev 1.137   Jun 03 2005 17:49:54   dirby
   Modified to combine host EMV response info with the request EMV
   data and send it back to the originator.
   SCR 15844
   
      Rev 1.136   May 17 2005 17:45:42   dirby
   Modified to stop standing in for Auth and Cash Overrides.
   SCR 15420
   
      Rev 1.135   Feb 16 2005 10:17:28   dirby
   Modified to copy error messages into TLF01 response text.
   SCR 12340
   
      Rev 1.134   Jan 19 2005 21:23:56   dirby
   Modified to cull CAV2 using EBCDIC subfield tags and lengths.
   JCB Cert.
   
      Rev 1.133   Oct 07 2004 17:42:36   dirby
   Added function 'find_cav2' to cull the CAV2 value out of field 48.
   SCR 12462
   
      Rev 1.132   Jun 08 2004 13:12:34   dirby
   Added a log message when host is inactive.
   Release 430-1
   
      Rev 1.131   Jan 07 2004 16:11:14   dirby
   Added an include file for currency conversion.
   SCR 1293
   
      Rev 1.130   Oct 16 2002 15:17:08   dirby
   Modified processing of echo timer when it expires to
   immediately set the echo timer again.  This is to prevent
   warning messages in the syslog that occur when the 0800
   echo msg is sent.  Any time a msg is sent to the host, the echo 
   timer is cleared then reset.  The 'clear' command caused the
   warning because the timer has already been cleared when it
   timed out.
   There is no SCR associated with this.  It will go out next time
   ncjcb is delivered.
   
      Rev 1.129   Aug 02 2002 14:58:34   dirby
   Removed code so that time out of echoes do not increment
   the timeout counter.  They do not cause us to go offline.
   SCR 594
   
      Rev 1.128   Jul 26 2002 14:51:34   dirby
   Modified to allow echoes to continue to go out when offline and
   set host state online if a response is received.
   SCR 594
   
      Rev 1.127   Jul 11 2002 11:53:36   dirby
   Modified to update number of time outs for Monitor.  It was not
   doing so because the response code was '68', not '19'.  Also
   modified to not send out a reversal or repeat of the max time
   outs has been reached.
   SCR 813
   
      Rev 1.126   Jul 10 2002 13:55:44   dirby
   Modified code to always increment consecutive time out counter
   on a txn time out.  Previously only did it when network record
   was configured to more than zero consecutive time outs.
   SCR 813
   
      Rev 1.125   Jul 07 2002 03:23:36   dirby
   Fixed bug - Return false if host is not online when checking status.
   SCR 594
   
      Rev 1.124   Jul 07 2002 02:17:02   dirby
   1.  Reimplemented going offline when max time outs occur.
   2.  Modified to use precalculated sizeof values instead of
   recalculating structure sizes over and over and over.
   SCR 594
   
      Rev 1.123   Jul 06 2002 22:39:24   dirby
   Added code to display a message for time outs and reversals.
   SCR 621
   
      Rev 1.122   Jun 19 2002 13:59:42   dirby
   Added response tx_key AUTH_QUASI_CASH_RESPONSE.
   JCB does not support Quasi-Cash, so if one is send in from
   txcntl, ncjcb treats it like an authorization.  But the tx_key still
   needs to be Quasi-Cash.
   SCR 792
   
      Rev 1.121   Apr 17 2002 13:17:42   dirby
   Removed currency code from function 'alter_transaction_data'.
   This way, the original currency code does not get overwritten.
   SCR 704
   
      Rev 1.120   Apr 15 2002 14:55:28   dirby
   Modified to set response to 68 for repeat reversals after time outs.
   Modified to create a stan after time out that does not have a decimal in it.
   SCR 700
   
      Rev 1.119   Jan 23 2002 14:42:52   dirby
   Initialized buffers in transaction statistic functions.
   
      Rev 1.118   Jan 16 2002 16:26:40   dirby
   Added transaction tracing functionality.
   SCR 546
   
      Rev 1.117   Jan 14 2002 18:17:30   dirby
   ...
   SCR 484
      ...
      Rev 1.116   Jan 07 2002 17:23:06   dirby
   Modified error / info messages to be consistent with other network controllers.
   SCR 87
   
      Rev 1.115   Dec 11 2001 10:29:34   dirby
   Modified to always forward requests to host, even if offline.
   Modified to ignore Max Active and Max Timeout counts
   SCR 488 and 547
   
      Rev 1.114   Nov 20 2001 11:39:52   dirby
   Added some code to function 'alter_transaction_data' for 0302
   responses to copy information from the response into auth_tx
   for inquiries so it can be displayed on the GUI.     SCR 251
   
      Rev 1.113   Nov 14 2001 16:30:28   dirby
   Replaced code that creates a lookup key for the timer with a
   function to do the same thing.     SCR 251
   
      Rev 1.112   Nov 13 2001 16:03:52   dirby
   Added support for JCB Online File Update processing - 0302 transactions.     SCR 251
   
      Rev 1.111   Jun 21 2001 14:40:48   dirby
   1.  In function 'get_orig_data_from_NCF30', added AUTH_ADVICE_REPEAT
   to the AUTH_ADVICE 'IF' statement where stan is copied.
   Whatever happens for an advice needs to happen for an
   advice repeat.
   
   2.  Took AMOUNT out of the requirements to find an original
   txn by stan.  This is because the amount in NCF30 might be
   a converted amount.
   
      Rev 1.110   Apr 30 2001 11:15:02   SYARLAGA
   Modified the logon messages
   
      Rev 1.109   Apr 18 2001 17:28:40   SYARLAGA
   Echo messages has been cleared
   
      Rev 1.108   Dec 13 2000 09:24:22   dirby
   Modified to not clear echo timer if echo-timer value is zero.
   
      Rev 1.107   Dec 01 2000 17:27:22   dirby
   1.  Changed echo lookup key from 'VISA' to 'JCB'.
   2.  Modified to not clear timer on a timeout.  No need to do this.
   3.  Modified to set rc=19 on timeouts of auths, then send a reversal.
       This was being done, but not for industry code of 901 - ATMs.  It 
       needs to be done for ATMs.
   
      Rev 1.106   Oct 28 2000 15:55:06   dirby
   Removed processing that selects NCF01 records during online transactions and logons.  These
   records are retrieved at startup.
   
      Rev 1.105   Oct 18 2000 14:42:38   dirby
   1.  Removed duplicate function prototype of get_orig_data_from_NCF30.
   2.  Removed commented out code.
   3.  Put calculations of tran_start_time and tran_finish_time in more efficient places.
   
      Rev 1.104   Aug 28 2000 09:52:08   dirby
   Changed the MONITOR_STATS structure from 'long's to 'char's.
   Unix seems to translate longs differently than NT does.
   
   
      Rev 1.103   Aug 22 2000 16:39:22   dirby
   Added code to support transaction statistics for Monitor.
   
   
      Rev 1.102   Aug 18 2000 13:12:28   dirby
   Added code to validate expiration date prior to sending out a request.
   Removed some obsolete code.
   
      Rev 1.101   Aug 11 2000 16:39:28   dirby
   Modified to send a response of "01" when host is unavailable - per EBs request.
   Also corrected lots of indentation problems.
   
      Rev 1.100   Aug 08 2000 14:57:12   dirby
   Cleaned up some redundant and useless stuff.
   
      Rev 1.99   Aug 07 2000 11:15:38   dirby
   Removed some messages that were not being displayed anywhere.
   
      Rev 1.98   Jul 14 2000 09:04:34   dirby
   Modified to not translate PINs for reversals.
   
      Rev 1.97   Jul 13 2000 16:15:54   dirby
   Modified the standin processing function to not
   overwrite the auth_number with the time if the auth
   number already exists.
   
      Rev 1.96   Jun 02 2000 10:33:38   gdaniels
   fix field 33 and response back for reversal
   when original not found
   
      Rev 1.95   Jun 01 2000 15:04:54   gdaniels
   fix field90 and reversals
   
      Rev 1.94   Mar 21 2000 17:31:06   ddabberu
   update database for resp codes 00/55
   
      Rev 1.93   Mar 17 2000 15:38:08   dperson
   Put call to TxUtils_Send_Msg_To_Operator
   back in but changed flags to 0 to suppress
   writing to the System Monitor and syslog.
   
      Rev 1.91   Mar 09 2000 02:07:16   ddabberu
   1.transmission_timestamp in process_timeout_ncf01_reply() chaged.
   2.build_host_structure() changed.
   
      Rev 1.90   Mar 08 2000 15:08:06   ddabberu
   modified update_auth_tx_with_ncf30_data() to echo the original fld 7 and fld 12 data in the response
   
      Rev 1.89   Mar 07 2000 16:26:14   ddabberu
   1.for any reversal initiated from terminal, we send 17 as response code.
   2.Nomore logging into TLF01 for reveral_advices sent to host.
   
      Rev 1.88   Mar 06 2000 18:11:54   ddabberu
   jcb reversals for switched_in txns
   
      Rev 1.87   Mar 06 2000 10:22:56   ddabberu
   Fix for SCR#465
   
      Rev 1.86   Feb 29 2000 15:22:20   ddabberu
   Modified to support reversals from the terminal
   SCR# 447,448,449,450,451,452
   
      Rev 1.85   Feb 28 2000 10:22:22   ddabberu
   reversals from terminal:
   incoming_terminal_message() modified
   
      Rev 1.84   Feb 16 2000 13:39:26   svelaga
   Incorporated the design changes to maintian
   the transaction integrity as well as database integrity.
   
   
      Rev 1.83   Feb 05 2000 14:35:16   ddabberu
   recon_update() updated with transmission_timestamp
   
      Rev 1.82   Jan 31 2000 10:09:56   svelaga
   Fix to the SCR #347 ( part 2 ).
   
      Rev 1.81   Jan 27 2000 09:56:12   svelaga
   Added new echo logic.
   
   
      Rev 1.80   Jan 25 2000 10:24:48   dirby
   Added standin processing for void transactions.
   
      Rev 1.79   Jan 24 2000 16:17:46   ddabberu
   restored back the clear_timer request
   in the proecess_timeout function
   
      Rev 1.78   Jan 18 2000 11:03:30   ddabberu
   ...
   2.  Modified the function that converts request tx_keys to responses so that the
      ...
      Rev 1.77   Jan 14 2000 15:28:02   dirby
   Set default to original tx_key in the function that converts tx_key
   from a request to a response.
   
      Rev 1.76   Jan 12 2000 15:11:10   svelaga
   Commented the check for validating the
   card expiry date.
   
   
      Rev 1.75   Jan 11 2000 15:25:34   ddabberu
   modified alter_transaction_data to fix  SCR#75
   
      Rev 1.74   Jan 03 2000 17:08:16   ddabberu
   Modified process_auth_advice to restore the tx_key
   Modified the incoming_other_host_response_db_reply
   
      Rev 1.73   Jan 03 2000 14:18:42   ddabberu
   code modifications related to new Auth_advice process
   and AUTHORIZATION .
   
      Rev 1.72   Dec 29 1999 09:12:48   svelaga
   Changed the code : the way to get the original transaction 
   from NCF30.
   
      Rev 1.71   Dec 28 1999 13:30:10   ddabberu
   process_auth_advices renamed as process_auth_advice
   
      Rev 1.70   Dec 23 1999 15:32:06   ddabberu
   update_ncf30_details: orig_stan replaced with sys_trace_audit_nbr
   
      Rev 1.69   Dec 23 1999 14:16:58   ddabberu
   reconupdate: currency code 
   
      Rev 1.68   Dec 22 1999 14:05:24   dirby
   Modified to separate advice processing from reversal processing.
   
      Rev 1.67   Dec 11 1999 11:24:58   ddabberu
   Integration Test Completed
   
      Rev 1.66   Nov 29 1999 17:08:32   ddabberu
    
   
      Rev 1.65   Nov 22 1999 15:34:32   ddabberu
   added code for reversals and advices
   
      Rev 1.64   Nov 22 1999 15:31:58   ddabberu
    
   
      Rev 1.63   Aug 25 1999 17:02:18   egable
   Implement the new reversal logic.
   
      Rev 1.62   Aug 23 1999 10:34:12   egable
   Use timerds instead of netds for timer.
   
      Rev 1.61   Aug 16 1999 13:32:22   egable
   1) Update the host state when receive 0810
   from host only if response = 00.
   2) Correct paths that respond to a logon from 
   host
   3) Don't update the host state when sending
   0800 to host, wait for 0810 back.
   
      Rev 1.60   Aug 13 1999 14:59:06   egable
   Don't insert NCF30 for 0800 messages.
   
      Rev 1.59   Aug 13 1999 14:45:32   egable
   Insert NCF30 in the switched out path if
   response code = 00
   
      Rev 1.58   Aug 12 1999 16:49:44   egable
   Change recon_update
   
      Rev 1.57   Aug 12 1999 13:36:24   egable
   Correct the initialization of reversal and actual
   amounts for reversals.
   
      Rev 1.56   Aug 10 1999 19:23:44   egable
   Automatically send a reversal when there is
   a timeout (POS).  Send a response code of 19.
   (Per Equitable/JCB).
   
      Rev 1.55   Aug 10 1999 11:07:02   egable
   Don't auto send a reversal for POS terminals.
   For ATMs, send a response code of TO, which
   nceqit will use stop the response from going
   back to the host.
   
      Rev 1.54   Aug 09 1999 13:23:40   egable
   For the timeout path, send the timed out 
   ATM transaction back to the originator.
   
      Rev 1.53   Aug 06 1999 14:54:16   egable
   Add print statement for debugging.
   
      Rev 1.52   Aug 04 1999 18:55:44   egable
   Uncomment the sys trace audit number code.
   
      Rev 1.51   Aug 04 1999 10:51:30   egable
   Get the original processing code for reversals.
   Get the original transaction by STAN, not RRN.
   
      Rev 1.50   Aug 04 1999 10:26:26   egable
   Correct frees.
   
      Rev 1.49   Jul 28 1999 18:16:50   egable
   Get the orig trans by RRN for reversals.
   
      Rev 1.48   Jul 27 1999 16:03:58   egable
   Don't output a reversal for ATMs when the
   transaction times out.
   
      Rev 1.47   Jul 26 1999 16:26:00   egable
   Correct a function return.
   
      Rev 1.46   Jul 26 1999 10:19:02   egable
   Don't set processing code = 000000 for 
   reversals.
   
      Rev 1.45   Jul 23 1999 15:48:16   egable
   When getting a response from the host, use
   the original processing code.
   
      Rev 1.44   Jul 22 1999 14:02:26   egable
   Put all messages to both the monitor and syslog.
   
      Rev 1.43   Jul 15 1999 13:40:40   egable
   Verify that the card is not expired before
   sending it to the host.
   
      Rev 1.42   Jul 12 1999 19:25:00   egable
   Remove the echo interval code.
   
      Rev 1.41   Jul 12 1999 14:42:28   egable
   Set the tx_key for the incoming response 
   message to be a response to the original 
   message.
   
      Rev 1.40   Jul 11 1999 20:08:58   egable
   Don't ever set field 38 to 000000 for reversals
   that were found in TLF01.
   
      Rev 1.39   02 Jul 1999 15:40:02   dirby
   Added a line to copy TPDU ID into auth_tx for timeout messages.
   It should already be there, but for some unknown reason, timeouts
   are not sending 0420s to the host.
   
      Rev 1.38   Jun 24 1999 14:44:32   egable
   Change the response code from "TO" to "19"
   for timeouts.
   
      Rev 1.37   Jun 24 1999 14:41:40   egable
   Undo the previous two changes.  Now we
   will send out the 0400 when a 0100 times
   out.  
   
      Rev 1.36   Jun 24 1999 14:36:06   egable
   Change the way advice_repeat_timer is
   used.
   
      Rev 1.35   Jun 23 1999 14:47:42   egable
   Don't send a 0420 when a 0100 times out.
   
      Rev 1.34   Jun 18 1999 16:46:02   egable
   Correct the advice repeat counter.
   
      Rev 1.33   Jun 16 1999 16:28:14   egable
   For responses from the host, keep the
   original message type in TLF01.  This is
   for the HOST/2 interface.
   
   
      Rev 1.32   Jun 10 1999 09:17:54   egable
   For reversals, if rc=17, bit 38 = 000000 and
   don't copy the original bit 7.  If rc = 68, bit 38
   should equal the original bit 38.
   
      Rev 1.31   Jun 09 1999 13:08:00   egable
   For reversals, output a response code of 17
   if the doesn't respond, output a response
   code of 68 if the terminal times out.  Correct 
   field 12.
   
      Rev 1.30   Jun 04 1999 13:19:50   egable
   Create response text for voice auths and
   update field 12 with original time for reversals.
   
      Rev 1.29   Jun 02 1999 12:08:46   egable
   Correct pos entry mode for reversals, correct
   an error in the issuing path for reversals.
   
      Rev 1.28   May 27 1999 13:19:16   egable
   Update data for field 90 when reversing
   a timed out 0100 message.
   
      Rev 1.27   May 25 1999 13:58:34   egable
   Pull the original times from TLF01 for 
   reversals.
   
      Rev 1.26   May 18 1999 11:45:04   egable
   Correct the lookup key
   
      Rev 1.25   May 12 1999 16:58:56   egable
   Correct 0800 messages.
   
      Rev 1.24   May 07 1999 14:39:48   egable
   Update original data from TLF01 for reversals.
   
      Rev 1.23   May 05 1999 14:04:42   egable
   In error processing function, create date,
   time and trans id only if they are blank.
   
      Rev 1.22   May 04 1999 16:14:08   egable
   Correct currency code error path.
   
      Rev 1.21   Apr 29 1999 14:44:06   egable
   Correct check for max consecutive timeouts.
   
      Rev 1.20   Apr 21 1999 01:15:08   egable
   Correct retrieval reference number.
   
      Rev 1.19   Apr 20 1999 13:58:36   egable
   Correct echo test response, correct reversals.
   
      Rev 1.18   19 Apr 1999 10:09:30   dirby
   Added function 'create_rrn'.
   
      Rev 1.17   16 Apr 1999 10:09:52   dirby
   1. Added 3 offline cases for OVERRIDE transactions from voice authorization.
   2. Made mod when creating stan to bypass decimal point in newly formatted gmt_time
       in function 'perform_op_net_mngmnt_request_ncf01_reply'.
   
      Rev 1.16   Apr 12 1999 14:45:42   egable
   Change the sleep command for non-windows
   platforms.  Remove message when the pin
   translation fails.
   
      Rev 1.15   Apr 09 1999 15:34:42   egable
   Correct RRN...again.
   
      Rev 1.14   Apr 09 1999 15:10:34   egable
   Correct RRN.
   
      Rev 1.13   Apr 09 1999 11:48:18   egable
   Create RRN differently. 
   
      Rev 1.12   Apr 07 1999 08:20:34   egable
   Delay between the messages to monitor.
   
      Rev 1.11   Apr 01 1999 13:49:38   egable
   Remove debugging prints.
   
      Rev 1.10   Mar 31 1999 15:35:20   egable
   Change send_status_to_network to update the
   'A' and 'I' items.
   
      Rev 1.9   Mar 30 1999 15:56:22   egable
    
   
      Rev 1.8   Mar 30 1999 14:07:58   egable
   Only set the timer if the host message is
   built successfully.
   
      Rev 1.7   Mar 26 1999 15:19:34   egable
   Correct NCF30 and gmt.
   
      Rev 1.6   Mar 25 1999 14:03:20   egable
   Update host_start_time, host_finish_time, 
   correct NCF30 and add test messages.
   
      Rev 1.5   Mar 17 1999 17:21:42   egable
   check for valid currency code
   
      Rev 1.4   Mar 16 1999 13:07:22   egable
   Correct errors found in unit testing.
   
      Rev 1.3   Mar 07 1999 10:50:06   egable
   Correct errors found in unit testing.  (With
   reversal advices and auth advices.)
   
      Rev 1.2   Mar 04 1999 16:12:44   egable
   Remove check for ncf01.status = A
   
      Rev 1.1   Mar 02 1999 16:48:22   egable
   Changes made while unit testing.
...
extern void bcd_to_asc (BYTE  bstring[],INT blen,pCHAR astr,INT astr_len);
/* global variables */
...
BOOLEAN send_it_back_to_terminal(BYTE resp_code[3], pAUTH_TX p_auth_tx )
{
	...
	if ( NULL_PTR == p_msg_term_handler_out )
	{
		...
		ncjcb_log_message( 3, 2, Log_Bufr , "send_it_back_to_terminal" , 0 ,p_auth_tx->TLF01_details.card_num);
	...
	if ( ret_code < 0 )
	{
		...
		ncjcb_log_message( 3, 3, Buffer , "send_it_back_to_terminal" , 0 ,p_auth_tx->TLF01_details.card_num);
	...
	else
	{
		...
		sprintf(Log_Bufr," Response sent successfully to terminal handler Term Id:%s ,Merch Id: %s RRN: %s ",
						p_auth_tx->TLF01_details.terminal_id,p_auth_tx->TLF01_details.merchant_id,p_auth_tx->TLF01_details.retrieval_ref_num);
		strcat(Buffer,Log_Bufr);
		ncjcb_log_message( 3, 1, Buffer , "send_it_back_to_terminal" , 0 ,p_auth_tx->TLF01_details.card_num);
	}
...
/*   AUTH_TX         auth_data_in;
   ...
   if( 0L == ret_code )
   {
	 ...
	 sprintf(Log_Bufr," Error received from crypto process Term Id:%s ,Merch Id: %s RRN: %s ",
						p_auth_tx->TLF01_details.terminal_id,p_auth_tx->TLF01_details.merchant_id,p_auth_tx->TLF01_details.retrieval_ref_num);
	 strcat(Buffer,Log_Bufr);
	 ncjcb_log_message( 3, 2, Buffer , "incoming_translate_request" , 0 ,p_auth_tx->TLF01_details.card_num);
   ...
   if(ret_code < 0)
   {
	 ...
	 ncjcb_log_message( 3, 2, ErrorMsg , "incoming_translate_request" , 0 ,p_auth_tx->TLF01_details.card_num);
...
INT incoming_terminal_request(pAUTH_TX p_auth_tx)
{
   ...
   switch ( temp_tx_key)
   {
   	  ...
   	  case AUTH_VOID_PRE_AUTHORIZATION:
      {
         ...
         sprintf( Buffer,"Preauth Cancellation going to JCB. ");

         memset(Log_Bufr,0x00,sizeof(Log_Bufr));
         sprintf(Log_Bufr," Term Id:%s ,Merch Id: %s RRN: %s ",
         					p_auth_tx->TLF01_details.terminal_id,p_auth_tx->TLF01_details.merchant_id,p_auth_tx->TLF01_details.retrieval_ref_num);
         strcat(Buffer,Log_Bufr);
         ncjcb_log_message( 3, 2, Buffer , "incoming_terminal_request" , 0 ,p_auth_tx->TLF01_details.card_num);
      }
	  break;
#endif
      ...
      {  /* Void Sales will be sent to JCB as Reversals. */
         ...
         if(0 == strncmp(p_auth_tx->TLF01_details.handler_queue,"nceqit",5) ||
           (0 == strncmp(p_auth_tx->TLF01_details.handler_queue,"neqit",4)))
         {
        	 strcpy(p_auth_tx->TLF01_details.response_code,ATM_CANCELLATION);
         }
         else
         {
      ...
      { /* Create transaction type string and store it in the auth_tx_block */
          ...
          */
         memset( Buffer, 0x00, sizeof(Buffer) );
         sprintf( Buffer,"Reversal going to JCB. ");
   ...
   if (false == verify_host_app_status(p_auth_tx))
   {
      ...
      return(true);
                                       ...
                                       p_auth_tx->TLF01_details.tx_key,Buffer))
   {
      /* Build response_for_terminal */
      memcpy( p_auth_tx->TLF01_details.response_text, Buffer, 40 );
      memset(Log_Bufr,0x00,sizeof(Log_Bufr));
...
BOOLEAN process_logon_response(CHAR nmi_code[5], BYTE response_code[3])
{
   ...
   {
      ncjcb_log_message( 3, 1, "Received Log ON response. Setting state online." , "process_logon_response" , 0 ,"");
   ...
   {
	   ncjcb_log_message( 3, 1, "Received Log OFF response. Setting state online." , "process_logon_response" , 0 ,"");
      if (0 == strcmp("00",response_code))
      {
         ...
         {
        	 ncjcb_log_message( 3, 3, "Error clearing the echo_timer" , "process_logon_response" , 0 ,"");
         }
      }
   }
   else if (strncmp(LOGON_NMI_ECHO_TEST,nmi_code,4) == 0)
   {
      ...
      if ( current_state[0] == cOFFLINE )
      {
         ...
         update_flg = true;
         ncjcb_log_message( 3, 1, "Received response to echo. Going back Online", "process_logon_response" , 0 ,"");
      }
 ...
 BOOLEAN process_reversal_advice(pAUTH_TX p_auth_tx)
{
 ...
 BOOLEAN process_reversal_only(pAUTH_TX p_auth_tx)
 {
	  ...
	  (void)hhutil_transfer_ctl_to_auth_app(p_auth_tx);

	  return true;
 }

 BOOLEAN process_reversal_and_advice_txns(pAUTH_TX p_auth_tx)
 {
	   pPTE_MSG  p_msg_ins = NULL_PTR;
	   NCF30     ncf30;
	   BYTE 	tx_key = 0;
	  ...
	  /* Tell txcntl to go process the reversal. */
	   ...
	   switch(tx_key)
	   {
			...
			case AUTH_REVERSAL_ADVICE_REPEAT:
				...
				p_auth_tx->TLF01_details.tx_key = AUTH_REVERSAL_ADVICE_REPEAT_RESPONSE ;
			...
			case AUTH_ADVICE_REPEAT:
				...
				p_auth_tx->TLF01_details.tx_key = AUTH_ADVICE_REPEAT_RESPONSE ;
	  ...
	  (void)incoming_other_host_response_db_reply( p_auth_tx );
	   ...
	   p_auth_tx->TLF01_details.tx_key = tx_key;
	  (void)hhutil_transfer_ctl_to_auth_app(p_auth_tx);
   ...
   */
   ...
   if ( ret_code < 0 )
   {
		...
		sprintf(Log_Bufr," Term Id:%s ,Merch Id: %s RRN: %s ",
						p_auth_tx->TLF01_details.terminal_id,p_auth_tx->TLF01_details.merchant_id,p_auth_tx->TLF01_details.retrieval_ref_num);
		strcat(Buffer,Log_Bufr);
		ncjcb_log_message( 3, 3, Buffer , "send_response_to_host_with_rc_success" , 0 ,p_auth_tx->TLF01_details.card_num);
		free(p_msg_host_out);
		return (false);
   }
   else
   {
	   ncjcb_log_message( 2, 1, "Response sent successfully to host" , "send_response_to_host_with_rc_success" , 0 ,"");
   }
...
BOOLEAN get_orig_data_from_NCF30(pAUTH_TX p_auth_tx,pNCF30 pncf30)
{
   ...
   {
      ncjcb_log_message( 3, 2, "Insufficient Memory for select ncf30 message" , "get_orig_data_from_NCF30" , 0 ,"");
      retval = false;
   }
   else
   {
      ...
      if ( 0 > ret_code )
      {
         ...
         ncjcb_log_message( 3, 2, Buff , "get_orig_data_from_NCF30" , 0 ,p_auth_tx->TLF01_details.card_num);
...
{  
   ...
   if ( echo_timer_length > 0 )
   {
      ...
      if ( false == set_echo_timer(echo_lookup_key, echo_timer_length,
                                  (pPTE_MSG *)NULL_PTR,application_que_name,
                                   SEND_TO_TIMERDS) )
      {
         ncjcb_log_message( 3, 2, "Error setting the echo_timer" , "incoming_host_message" , 0 ,"");
         return false;
      }
   }
   ...
   {
	   ncjcb_log_message( 3, 2, "Invalid message with data length more than 1500 byte received" , "incoming_host_message" , 0 ,"");
   }
   ...
   if(AUTH_INVALID == auth_tx.TLF01_details.tx_key)
   {
      ...
      strcat(Buffer,Log_Bufr);
      ncjcb_log_message( 3, 3, Buffer , "incoming_host_message" , 0 ,auth_tx.TLF01_details.card_num);
   ...
   {  
      return (process_reversal_only(&auth_tx));
   }
   ...
   */
   ...
   if(ncjcb_Check_Reversal_Advice_Iss_txns(&auth_tx) == true)
   {
	   return (process_reversal_and_advice_txns(&auth_tx));
   }
   if (AUTH_ADMIN_ADVICE == auth_tx.TLF01_details.tx_key)
   {
      sprintf(Buffer,"Advice Message Received");
      ncjcb_log_message( 3, 1, Buffer , "incoming_host_message" , 0 ,"");
   ...
   { 
      ...
      sprintf(Log_Bufr," Insufficient Memory to build read timer message Term Id:%s ,Merch Id: %s RRN: %s ",
      					auth_tx.TLF01_details.terminal_id,auth_tx.TLF01_details.merchant_id,auth_tx.TLF01_details.retrieval_ref_num);
      ncjcb_log_message( 3, 2, Log_Bufr , "incoming_host_message" , 0 ,auth_tx.TLF01_details.card_num);
      return(false);
   }
...
{  
   ...
   else
   {
      ...
      ncjcb_log_message( 3, 2, Buffer , "incoming_host_message_timer_reply" , 0 ,"");
      return(false);
   }
   ...
   else
   {
      sprintf( Buffer,
              "Did not receive chained host response data with timer reply. Key = %s",
               lookup_key );
      ncjcb_log_message( 1, 2, Buffer , "incoming_host_message_timer_reply" , 0 ,"");
      memcpy( auth_tx.TLF01_details.response_text, Buffer, 40 );
      return(false);
   }
   ...
   if ( NULL_PTR == p_msg_timer_out )
   {
      ncjcb_log_message( 1, 2, "Insufficient Memory to build clear timer message" , "incoming_host_message_timer_reply" , 0 ,"");
      return(false);
   }
   ...
   {   /* failed getting timer data */
       ...
       ncjcb_log_message( 3, 3, Buffer , "incoming_host_message_timer_reply" , 0 ,"");
       memcpy( auth_tx.TLF01_details.response_text, Buffer, 40 );
       return(false);
   }
   ...
   if ((strncmp(temp_auth_tx.TLF01_details.handler_queue, parvbh_que_name, 7)!= 0) &&
   ...
   else if (false == send_response_to_term_app(&temp_auth_tx))
   {
      ...
      sprintf(Log_Bufr," Unable to send message to authorizer Term Id:%s ,Merch Id: %s RRN: %s ",
      					auth_tx.TLF01_details.terminal_id,auth_tx.TLF01_details.merchant_id,auth_tx.TLF01_details.retrieval_ref_num);
      ncjcb_log_message( 3, 2, Log_Bufr , "incoming_host_message_timer_reply" , 0 ,auth_tx.TLF01_details.card_num);
...
INT process_timeout_msg(pPTE_MSG p_msg)
{
   ...
   if(strcmp(lookup_key,echo_lookup_key) == 0)
   {
      ...
      if ( false == set_echo_timer(echo_lookup_key, echo_timer_length,
                                   ...
                                   SEND_TO_TIMERDS) )
      {
         ncjcb_log_message( 3, 3, "Error setting the echo_timer" , "process_timeout_msg" , 0 ,"");
      }
      ...
      {
         ncjcb_log_message( 3, 1, "Error executing host Echo Test" , "process_timeout_msg" , 0 ,"");
      }
      return( true );
   }
   ...
   else
   {
      if (timer_reply_size == sizeof(buffer2))
      {
         ...
         memset( Buffer, 0x00, sizeof(Buffer) );
         sprintf( Buffer,
                 "Txn to JCB timed out. Merchant %s",
         ...
         sprintf(Log_Bufr," Term Id:%s ,Merch Id: %s RRN: %s ",
         					auth_tx.TLF01_details.terminal_id,auth_tx.TLF01_details.merchant_id,auth_tx.TLF01_details.retrieval_ref_num);
         strcat(Buffer,Log_Bufr);
         ncjcb_log_message( 3, 1, Buffer , "process_timeout_msg" , 0 ,auth_tx.TLF01_details.card_num);
      }
      else
      {
         sprintf( ErrorMsg,
         ...
         ncjcb_log_message( 3, 2, ErrorMsg , "process_timeout_msg" , 0 ,"");
         return(false);
      }
   }
   ...
   if ( MaxConsecTimeouts > 0 )
   {
      if (atoi(net_consec_tmouts_ctr) >= MaxConsecTimeouts)
      {
         ...
         sprintf(Log_Bufr," Maximum consecutive timeouts reached. Term Id:%s ,Merch Id: %s RRN: %s ",
         					auth_tx.TLF01_details.terminal_id,auth_tx.TLF01_details.merchant_id,auth_tx.TLF01_details.retrieval_ref_num);
         ncjcb_log_message( 3, 2, Log_Bufr , "process_timeout_msg" , 0 ,auth_tx.TLF01_details.card_num);
      }
   }
        ...
        (AUTH_VOID_SALE       == auth_tx.TLF01_details.tx_key)  )
   {
      if ( update_flag == false )
      {
          ...
          */
         if ((count = atoi(auth_tx.advice_repeat_count)) <=
             atoi(ncf01_i.advice_repeat_counter))  
         {
...
INT send_response_to_term_app(pAUTH_TX p_auth_tx)
{
   ...
   if(strncmp(p_auth_tx->TLF01_details.handler_queue, parvbh_que_name, 7) == 0)
   {
	   strcpy(temp_orig_que, parvbh_que_name);
   }
   else
#endif
   {
...
//      strcpy(temp_orig_que, p_auth_tx->TLF01_details.last_host_app_queue);
   ...
   if ( ret_code < 0 )
   {
      ...
      sprintf(Log_Bufr," Term Id:%s ,Merch Id: %s RRN: %s ",
      					p_auth_tx->TLF01_details.terminal_id,p_auth_tx->TLF01_details.merchant_id,p_auth_tx->TLF01_details.retrieval_ref_num);
      strcat(Buffer,Log_Bufr);
      ncjcb_log_message( 3, 3, Buffer , "send_response_to_term_app" , 0 ,p_auth_tx->TLF01_details.card_num);
      memcpy( p_auth_tx->TLF01_details.response_text, Buffer, 40 );
      free(p_msg_term_handler_out);
      return(false);
   }
   ...
   if (Build_Host_Request( p_auth_tx ))
   {
      ...
      if ( NULL_PTR == p_msg_set_timer )
      {
          ...
          sprintf(Log_Bufr," Insufficient Memory to build timer message Term Id:%s ,Merch Id: %s RRN: %s ",
          					p_auth_tx->TLF01_details.terminal_id,p_auth_tx->TLF01_details.merchant_id,p_auth_tx->TLF01_details.retrieval_ref_num);
          ncjcb_log_message( 3, 2, Log_Bufr , "send_request_to_host" , 0 ,p_auth_tx->TLF01_details.card_num);
          return(false);
      }    
      ...
      {
         ncjcb_log_message( 3, 3, "Error chaining the timer request with clear & set the echo_timer" , "send_request_to_host" , 0 ,"");
      }
      ...
      if( ret_code < 0 )
      {
         ...
         sprintf(Log_Bufr," Term Id:%s ,Merch Id: %s RRN: %s ",
         					p_auth_tx->TLF01_details.terminal_id,p_auth_tx->TLF01_details.merchant_id,p_auth_tx->TLF01_details.retrieval_ref_num);
         strcat(Buffer,Log_Bufr);
         ncjcb_log_message( 3, 3, Buffer , "send_request_to_host" , 0 ,p_auth_tx->TLF01_details.card_num);
         free( p_msg_host_out );
         return(false);
      }
      else
...
INT  verify_host_app_status(pAUTH_TX p_auth_tx)
{
   ...
   if ( (MaxConsecTimeouts > 0) && (ret_code == true) )
   {
      if (atoi(net_consec_tmouts_ctr) > MaxConsecTimeouts )
      {
         ...
         ncjcb_log_message( 1, 2, err_buf , "verify_host_app_status" , 0 ,p_auth_tx->TLF01_details.card_num);
      }
   }
   ...
   switch (source)
	{
		case SOURCE_IS_FSVISA:
		{
			if (2 == atoi(current_state))
			{
				...
				ncjcb_log_message( 2, 1, "Sending logon request to host:" , "perform_host_logon_request" , 0 ,"");
		...
		case SOURCE_IS_STATE:
		{
			...
			{
				ncjcb_log_message( 2, 1, "Sending ECHO request to host:" , "perform_host_logon_request" , 0 ,"");
				p_auth_tx->TLF01_details.tx_key = AUTH_LOGON;
      ...
      case SOURCE_IS_OPERATOR:
      {
		...
		case SOURCE_IS_HOST:
		{
			...
			{
				ncjcb_log_message( 1, 1, "Received Log ON request from host." , "perform_host_logon_request" , 0 ,"");
			...
			{
            	ncjcb_log_message( 1, 1, "Received Log OFF request from host." , "perform_host_logon_request" , 0,"" );
 				p_auth_tx->TLF01_details.tx_key = AUTH_LOGON_RESPONSE;
			...
			{
            	//ncjcb_log_message( 1, 1, "Received network request from host:" , "perform_host_logon_request" , 0 ,"");
            	p_auth_tx->TLF01_details.tx_key = AUTH_LOGON_RESPONSE;
				strcpy(vLogon_NMI_Code, LOGON_NMI_ECHO_TEST);
...
INT send_network_status_to_monitor(void)
{
   ...
   if ( NULL_PTR == p_msg_ins )
   {
      ncjcb_log_message( 3, 2, "Insufficient Memory to create network query message" , "send_network_status_to_monitor" , 0 ,"");
      return( false );
   }
	...
	if (ConnectionId[0] != 0)
   {
      ...
      if ( 0 > ret_code )
      {
         ...
         ncjcb_log_message( 3, 3, Buff , "send_network_status_to_monitor" , 0 ,"");
         free( p_msg_ins );
         return( false );
      }
   }
	...
	if (ConnectionId[0] != 0)
   {
      ...
      if ( 0 > ret_code )
      {
         pteipc_get_errormsg( ret_code, err_mesg );
         sprintf(Buff,"Failed transmit to que: %s. %s",applnk_que_name,err_mesg);
         ncjcb_log_message( 3, 2, Buff , "send_network_status_to_monitor" , 0 ,"");
         free( p_msg_ins );
         return( false );
      }
   }
...
INT perform_op_net_mngmnt_request( char request_type[], pAUTH_TX p_auth_tx )
{
   ...
   {
      ncjcb_log_message( 1, 1, "Sending LOG ON request to host." , "perform_op_net_mngmnt_request" , 0 ,"");
      strcpy(vLogon_NMI_Code, LOGON_NMI_LOGON);
   }
   else if (0 == strcmp(request_type, LOGOFF_REQUEST))
   {
       ncjcb_log_message( 1, 1, "Sending LOG OFF request to host." , "perform_op_net_mngmnt_request" , 0 ,"");
       strcpy(vLogon_NMI_Code, LOGON_NMI_LOGOFF);
   }
   else 
   {
      ...
      if ( NULL_PTR == p_msg_set_timer )
      {
         ...
         ncjcb_log_message( 3, 2, Log_Bufr , "perform_op_net_mngmnt_request" , 0 ,p_auth_tx->TLF01_details.card_num);
      ...
      if(false == chain_echo_host( echo_lookup_key, echo_timer_length,
                                   application_que_name, p_msg_set_timer) )
      {
         ncjcb_log_message( 3, 3, "Error chaining the timer request with clear & set the echo_timer" , "perform_op_net_mngmnt_request" , 0 ,"");
      }
   }
...
INT perform_op_net_mngmnt_response(char nmi_code[], pAUTH_TX p_auth_tx)
{
   ...
   {
      ncjcb_log_message( 1, 1, "Sending Log ON response to host." , "perform_op_net_mngmnt_request" , 0 ,"");
      strcpy(vLogon_Resp_Indicator, LOGON_RESP_INDICATOR);  
   }
   else if (0 == strncmp(nmi_code, LOGON_NMI_LOGOFF, 4))
   {
      ncjcb_log_message( 1, 1, "Sending Log OFF response to host." , "perform_op_net_mngmnt_request" , 0 ,"");
   }
...
{     
   ...
   if ( NULL_PTR == p_msg_term_handler_out  )
   {
      ...
      sprintf(Log_Bufr," Unable to forward txn to authorizer. Err: Insufficient Memory to build PTE message Term Id:%s ,Merch Id: %s RRN: %s ",
      					p_auth_tx->TLF01_details.terminal_id,p_auth_tx->TLF01_details.merchant_id,p_auth_tx->TLF01_details.retrieval_ref_num);
      ncjcb_log_message( 3, 2, Log_Bufr , "perform_standin_processing" , 0 ,p_auth_tx->TLF01_details.card_num);
   }
   else
   {
      ret_code = pteipc_send (p_msg_term_handler_out, authorizer_que_name); 
      if ( ret_code < 0 )
      {
         ...
         sprintf(Log_Bufr," Term Id:%s ,Merch Id: %s RRN: %s ",
         					p_auth_tx->TLF01_details.terminal_id,p_auth_tx->TLF01_details.merchant_id,p_auth_tx->TLF01_details.retrieval_ref_num);
         strcat(Buffer,Log_Bufr);
         ncjcb_log_message( 3, 2, Buffer , "perform_standin_processing" , 0 ,p_auth_tx->TLF01_details.card_num);
...
void ncjcb_parse_DE48(BYTE src_len, pCHAR src, pAUTH_TX p_auth_tx)
{
	   ...
	   for( MainIndex=0; MainIndex<TotalLen && MainIndex<FIELD48_MAX_LEN; )
	   {
	      ...
	      if ( 0 == strncmp( ascii_tag, DE48_TAG30,2 ) )
	      {
				...
				{
		 			p_auth_tx->TLF01_details.product_codes[2].amount[i] = ascii_ebcdic_conv (src[MainIndex+i  ], E2A);
				}
	      }
	      else if ( 0 == strncmp( ascii_tag, DE48_TAG31,2 ) )
	      {
		 		for( i=0; i<sub_field_len; i++ )
				{
		 			p_auth_tx->TLF01_details.product_codes[5].amount[i] = ascii_ebcdic_conv (src[MainIndex+i  ], E2A);
				}
	      }
	      MainIndex += sub_field_len;
	   }
 ...
 *
 *                The tags and length bytes are in EBCDIC.  So they must
 *                be converted into ASCII. Tag is 03.
 *
 *  FIELD 48 FORMAT:
 *            -------+---------+---------+---------+---------+-----
 *             Total | First   | First   | First   | Second  |
 ...
 *                src     - Field 48
 *
 *  OUTPUTS:      Only flag for AVS value presence will be filled.
 *
 *  RTRN VALUE:   None
 *
 *  AUTHOR:       Dennis Irby
...
void find_avs( BYTE src_len, pCHAR src, pCHAR flag )
{
   ...
   for( MainIndex=0; MainIndex<TotalLen && MainIndex<FIELD48_MAX_LEN; )
   {
      ...
      if(0 == strcmp( ascii_tag, CAV2_TAG_03 ))
      {
    	  flag[0] = 'A' ;
    	  break;
      }
...
/******************************************************************************
 *
 *  NAME:         find_ECI_CAVV_data
 *
 *  DESCRIPTION:  This function will parse field 48 sent by JCB host.
 *  				It will return the TAG 02 value if it is present.
 *
 *                The tags and length bytes are in EBCDIC.  So they must
 *                be converted into ASCII.
 *
 *  FIELD 48 TAG 02 FORMAT:
 *
 *				Length : 47
 *				XID    : 4 bit BCD, 20 BYTES
 *				CAVV   : 4 bit BCD, 20 BYTES
 *				ECI    : 4 bit BCD, 01 BYTE
 *
 *  INPUTS:       src_len - Length of src
 *                src     - Field 48
 *
 *  OUTPUTS:      dest - The XID,CAVV data & ECI value, or null
 *
 *  RTRN VALUE:   None
 *
 *  AUTHOR:       Raghu
...
void find_ECI_CAVV_data( BYTE src_len, pCHAR src,pCHAR dest_eci,pCHAR CAVV_data, pCHAR cavv, pAUTH_TX p_auth_tx)
{
   ...
   for( MainIndex=0; MainIndex<TotalLen && MainIndex<FIELD48_MAX_LEN; )
   {
      ...
      if ( 0 == strcmp( ascii_tag, CAVV_3D_SECURE_TAG_02 ) )
      {
         /* Found it.  EBCDIC to ASC conersion, Base64 to bin */
    	 // eci_value =  src[MainIndex+40] ; // At 40 the position eci value will be present.
    	  //sprintf(dest_eci,"%02d",eci_value);
    	  /*Convert 20 Byte Binary CAVV value int 40 byte ascii value*/
    	 // genutil_bin_to_ascii (&src[MainIndex+20], 20, CAVV_data, 40);
    	  /* Store 20 byte CAVV without conversion. This data used in racal for validating the spaces.*/
    	 // memcpy(cavv,&src[MainIndex+20], 28);
          //break;
    	  ...
    	  /*if(ascii_len> 47)
    	  {
              	  eci_value =  src[MainIndex+48] ; // At 40 the position eci value will be present.
              	  sprintf(dest_eci,"%02d",eci_value);
              	  strncpy(p_auth_tx->TLF01_details.profile,dest_eci,2);
              	  //Convert 20 Byte Binary CAVV value int 40 byte ascii value
              	 // genutil_bin_to_ascii (&src[MainIndex+20], 20, CAVV_data, 40);
              	  // Store 20 byte CAVV without conversion. This data used in racal for validating the spaces.

              	  memcpy(cavv,&src[MainIndex+20], 28);
              	  ncjcb_Compute_CAVV_Value(CAVV_VALUE,cavv);
              	  strncpy( p_auth_tx->TLF01_details.product_codes[3].amount,CAVV_VALUE,12);
              	  strncpy( p_auth_tx->TLF01_details.product_codes[6].amount,CAVV_VALUE+12,12);
              	  strncpy( p_auth_tx->TLF01_details.product_codes[8].amount,CAVV_VALUE+24,12);
              	  strncpy( p_auth_tx->TLF01_details.product_codes[0].code,CAVV_VALUE+36,4);
                  break;
    	  }*/
    		  ...
    		  strncpy(p_auth_tx->TLF01_details.profile,dest_eci,2);
    		   ...
    		   genutil_bin_to_ascii (&src[MainIndex+20], 20, CAVV_data, 40);
    		      	  /* Store 20 byte CAVV without conversion. This data used in racal for validating the spaces.*/
           	  strncpy( p_auth_tx->TLF01_details.product_codes[3].amount,CAVV_data,12);
           	  strncpy( p_auth_tx->TLF01_details.product_codes[6].amount,CAVV_data+12,12);
           	  strncpy( p_auth_tx->TLF01_details.product_codes[8].amount,CAVV_data+24,12);
           	  strncpy( p_auth_tx->TLF01_details.product_codes[0].code,CAVV_data+36,4);
           	  sprintf (buffer, "JCB3DS2 CAVV Value: %s %s %s %s ",p_auth_tx->TLF01_details.product_codes[3].amount,
           			p_auth_tx->TLF01_details.product_codes[6].amount,p_auth_tx->TLF01_details.product_codes[8].amount,
					 p_auth_tx->TLF01_details.product_codes[0].code);
           	ncjcb_log_message( 2, 1,buffer, "find_ECI_CAVV_data", 0, "" );

    		   // memcpy(cavv,&src[MainIndex+20], 20);
    		            break;
...
void ncjcb_Compute_AVN_Value(char * AVN_value, char cavv)
{
	    char data[29]={0};
		char          *out;
		size_t      out_len;
		...
		out_len = b64_decoded_size(cavv)+1;
		out = malloc(out_len);
		...
		if (!b64_decode( (unsigned char *)cavv, (unsigned char *)out, out_len)) {
			printf("Decode Failure\n");
			return 1;
		}
		out[out_len] = '\0';
		JCB_string2hexString(out,hexValue);
...
INT process_incoming_message(pPTE_MSG p_msg)
{
   ...
   if(!(auth_tx.TLF01_details.tx_key == AUTH_LOGON_RESPONSE))
   {
	   ...
	   {
		   /* Drop the txn in case of invalid Auth Tx_key.. TF Phani*/
		   return (true);
	   }
      else if(//(auth_tx.TLF01_details.tx_key < AUTH_SALE) ||
         (auth_tx.TLF01_details.tx_key > AUTH_INVALID)) // TF PHANI/Nirbhay ..Handle in case of error, Above case.
      {
         Tran_Flow = SWITCHED_IN;
            ...
            (0 == strcmp(auth_tx.TLF01_details.response_code, "55")))
         {
            if ( false == update_database( &auth_tx ))
            {
               ...
               sprintf(Log_Bufr," Unable to update database tables. Term Id:%s ,Merch Id: %s RRN: %s ",
               					auth_tx.TLF01_details.terminal_id,auth_tx.TLF01_details.merchant_id,auth_tx.TLF01_details.retrieval_ref_num);
               ncjcb_log_message( 3, 3, Log_Bufr , "process_incoming_message" , 0 ,auth_tx.TLF01_details.card_num);
               ...
               if( false == incoming_other_host_response_db_reply( &auth_tx ))
               {
                  ...
                  sprintf(Log_Bufr," Unable to send response to host. Term Id:%s ,Merch Id: %s RRN: %s ",
                  					auth_tx.TLF01_details.terminal_id,auth_tx.TLF01_details.merchant_id,auth_tx.TLF01_details.retrieval_ref_num);
                  ncjcb_log_message( 3, 2, Log_Bufr , "process_incoming_message" , 0 ,auth_tx.TLF01_details.card_num);
               }
            }
         }
         else
		...
		)
   {
        ...
        (0 == strncmp(auth_tx.TLF01_details.handler_queue,"neqit",4)))
	  {
		  ...
		  {
			 strcpy( auth_tx.TLF01_details.response_code, ATM_CANCELLATION);
		  }
		  ...
		  if ( 0 == strcmp(ATM_CANCELLATION, auth_tx.TLF01_details.response_code))
		  {
...
INT process_incoming_message_continued( pAUTH_TX p_auth_tx )
{
   ...
   else 
   {
      ...
      sprintf(Log_Bufr," Pin translation failed. Term Id:%s ,Merch Id: %s RRN: %s ",
      					p_auth_tx->TLF01_details.terminal_id,p_auth_tx->TLF01_details.merchant_id,p_auth_tx->TLF01_details.retrieval_ref_num);
      ncjcb_log_message( 3, 2, Log_Bufr , "process_incoming_message_continued" , 0 ,p_auth_tx->TLF01_details.card_num);
...
BOOLEAN update_ncf02(pAUTH_TX p_auth_tx)
{
 ...
 //  CHAR       tran_finish_time[17] = ""; 
   ...
   if (strcmp(p_auth_tx->TLF01_details.response_code,"00"))
   {
      ...
      if(NULL_PTR == p_msg_ins)
      {
         ncjcb_log_message( 3, 2, "Insufficient Memory to build insert NCF02 message" , "update_ncf02" , 0 ,"");
         return(false);
      }
      ...
      if(ret_code < 0)
      {
         ...
         memcpy( p_auth_tx->TLF01_details.response_text, Buff, 40 );
         ncjcb_log_message( 3, 2, Buff , "update_ncf02" , 0 ,"");
         return(false);
      }
   }
   return(true);
...
INT process_auth_advice( pAUTH_TX p_auth_tx, INT status, NCF30 ncf30 )
{
   ...
   if ( true == ret_val )
   {
         ...
         )
      {
         ...
         //strcpy( p_auth_tx->TLF01_details.response_code, "00" );
		 // TF Phani..store response code in voice Auth text and restore in ncjcb2
   ...
   if ( false == ret_val )
   {
      ...
      sprintf(Log_Bufr," Unable to complete processing of an advice (1120). Term Id:%s ,Merch Id: %s RRN: %s ",
      					p_auth_tx->TLF01_details.terminal_id,p_auth_tx->TLF01_details.merchant_id,p_auth_tx->TLF01_details.retrieval_ref_num);
      ncjcb_log_message( 3, 3, Log_Bufr , "process_auth_advice" , 0 ,p_auth_tx->TLF01_details.card_num);
   }
   return( ret_val );
 }
...
INT delete_orig_ncf30( NCF30 ncf30, pAUTH_TX p_auth_tx )
{
   ...
   if ( NULL_PTR == p_msg  )
   {
      ncjcb_log_message( 3, 2, "Insufficient Memory to build NCF30 message" , "delete_orig_ncf30" , 0 ,"");
      return( false );
   }
   ...
   if ( 0 > ret_code )
   {
      ...
      memcpy( p_auth_tx->TLF01_details.response_text, Buff, 40 );
      ncjcb_log_message( 3, 3, Buff , "delete_orig_ncf30" , 0 ,"");
      return( false );
   }
   return(true);
 }
...
INT recon_update( pAUTH_TX p_auth_tx, BYTE subtype_2, pCHAR network_type )
{
   ...
   if ( NULL_PTR == p_msg  )
   {
      ncjcb_log_message( 3, 2, "Insufficient Memory to build update NCF30 message" , "recon_update" , 0 ,"");
      return( false );
   }
   ...
   if ( 0 > ret_code )
   {
      ...
      memcpy( p_auth_tx->TLF01_details.response_text, Buff, 40 );
      ncjcb_log_message( 3, 3, Buff , "recon_update" , 0 ,"");
      return( false );
   }
   return( true );
}
...
INT update_database( pAUTH_TX p_auth_tx )
{
   ...
   if(NULL_PTR == p_msg)
   {
      ...
      sprintf(Log_Bufr," Insufficient Memory to build UPDATE ALL message Term Id:%s ,Merch Id: %s RRN: %s ",
      					p_auth_tx->TLF01_details.terminal_id,p_auth_tx->TLF01_details.merchant_id,p_auth_tx->TLF01_details.retrieval_ref_num);
      ncjcb_log_message( 3, 2, Log_Bufr , "update_database" , 0 ,p_auth_tx->TLF01_details.card_num);
      ret_val = false;
   }
   else
   {
      ...
      if(ret_code < 0)
	  {
         ...
         sprintf(Log_Bufr," Term Id:%s ,Merch Id: %s RRN: %s ",
         					p_auth_tx->TLF01_details.terminal_id,p_auth_tx->TLF01_details.merchant_id,p_auth_tx->TLF01_details.retrieval_ref_num);
         strcat(Buff,Log_Bufr);
         ncjcb_log_message( 3, 3, Buff , "update_database" , 0 ,p_auth_tx->TLF01_details.card_num);
         ret_val = false;
      }
   }
   return( ret_val );
...
INT send_transaction_statistics( pPTE_MSG p_msg )
{
   ...
   if ( false == ret_code  )
   {
      ncjcb_log_message( 3, 2, err_msg , "send_transaction_statistics" , 0 ,"");
   }
   return( ret_code );
}
...
INT reset_transaction_statistics( pPTE_MSG p_msg )
{
   ...
   if ( false == ret_code  )
   {
      ncjcb_log_message( 3, 2, err_msg , "reset_transaction_statistics" , 0 ,"");
   }
   return( ret_code );
}
...
void ncjcb_Compute_CAVV_Value(char * CAVV_value ,char * cavv)
{
		...
		out[out_len] = '\0';
		JCB_string2hexString(out,hexValue);
		strncpy(CAVV_value,hexValue,40);
		free(out);
