...
void process_monitor_request( pPTE_MSG p_msg )
{
   ...
   else if ( nm_type == ST1_ECHOTEST )
   {
	   nmi_type = ECHO;
	   //HCTS1-D
	   ncamaq_log_message( 1, 1, "Sending ECHO TEST request to host.", "process_monitor_request",0 );
   }
   ...
   else
   {
      ...
      sprintf( err_buf,
              "Unsupported request from Monitor. Subtype1 = %c",
               nm_type );
      ncamaq_log_message( 1, 2, err_buf, "process_monitor_request",0 );
   }
...
void issue_1804( INT nm_type )
{
   ...
   else if ( nm_type == LOGON )
   {
      if ( 0 != strcmp( ONLINE, current_state ) )
      {
         ...
         ncamaq_log_message( 1, 1, err_buf, "issue_1804",0 );
   ...
   if ( ret_val == true )
   {
      ...
      if ( false == build_1804_msg(err_buf) )
         ncamaq_log_message( 1, 2, err_buf, "issue_1804" ,0);
      ...
      /* Step 3.  Send request to host. */
         ...
         ncamaq_log_message( 3, 2, err_buf, "issue_1804",0 );
   }
   return;
}
...
INT issue_1814( INT nm_type )
{
   ...
   if ( nm_type == LOGON )
   {
      ncamaq_log_message( 1, 1, "Sending Log ON response to host.", "issue_1814",0 );
   }
   else if ( nm_type == LOGOFF )
   {
      ncamaq_log_message( 1, 1, "Sending Log OFF response to host.", "issue_1814",0 );
   }
   ...
   if ( false == build_1814_msg(err_buf) )
      ncamaq_log_message( 3, 2, err_buf, "issue_1814" ,0);
...
void process_1804_request()
{
   ...
   {
      type = LOGON;
      ncamaq_log_message( 1, 1, "Received Log ON request from host.",
                  "process_1804_request",0 );
      if ( 0 == strcmp(ONLINE, current_state) )
         ncamaq_log_message( 1, 1, "Already online.", "process_1804_request",0 );
      else
   ...
   {
      type = LOGOFF;
      ncamaq_log_message( 1, 1, "Received Log OFF request from host.",
                  "process_1804_request",0 );
      if ( 0 == strcmp(OFFLINE, current_state) )
         ncamaq_log_message( 1, 1, "Not online.", "process_1804_request" ,0);
      else
   ...
   else
   {
      ...
      if ( 0 == strcmp(NMI_KEYCHANGE, NMI_Code) )
         ncamaq_log_message( 1, 1, "Key Change request from host not supported.",
                     "process_1804_request",0 );

      else if ( 0 == strcmp(NMI_CUTOFF, NMI_Code) )
         ncamaq_log_message( 1, 1, "Cutoff (1804) request from host not supported.",
                     "process_1804_request",0 );

      else if ( 0 == strcmp(NMI_SAF, NMI_Code) )
         ncamaq_log_message( 1, 1, "SAF (1804) request from host not supported.",
                     "process_1804_request",0 );
      ...
      else
      {
         ...
         sprintf( buffer,
                 "Received unsupported 1804 request from host. NMI = %s",
                  NMI_Code );
         ncamaq_log_message( 1, 1, buffer, "process_1804_request",0 );
      }
   }
      ...
      if ( true == issue_1814( type ) )
      {
         if ( 0 != strcmp(new_state, same_state) )
         {
            ...
            if ( false == set_host_state( new_state, err_buf ) )
            {
               ...
               ncamaq_log_message( 1, 2, buffer, "process_1804_request",0 );
            }
         }
      }
   }
...
void process_1814_request()
{
   ...
   if ( 0 == strcmp(NMI_LOGON, NMI_Code) )
   {
      ...
      if ( 0 != strcmp(ONLINE, current_state) )
      {
         ncamaq_log_message( 1, 1, "Received Log ON response. Setting state online.",
         ...
         if ( true == set_host_state( ONLINE, err_buf ) )
         {
            ...
            if ( 0 == strcmp( AppName, "nceqit" ) )
            {
               ...
               {
                  ncamaq_log_message( 1, 1, "Issuing 1804 Key Change message.",
                              "process_1814_request",1 );
         ...
         else
         {
            ...
            strcat( buffer, err_buf );
            ncamaq_log_message( 1, 2, buffer, "process_1814_request" ,1);
         }
      }
   ...
   else if ( 0 == strcmp(NMI_LOGOFF, NMI_Code) )
   {
      ...
      if ( 0 != strcmp(OFFLINE, current_state) )
      {
         ncamaq_log_message( 1, 1, "Received Log OFF response. Setting state offline.",
         ...
         if ( false == set_host_state( OFFLINE, err_buf ) )
         {
            ...
            strcat( buffer, err_buf );
            ncamaq_log_message( 1, 2, buffer, "process_1814_request",0 );
         }
      }
   }
   else if ( 0 == strcmp(NMI_ECHO, NMI_Code) )
   {
      ...
      if ( false == clear_request_timer(echo_req_timer_key) )
      {
         ...
         ncamaq_log_message(1, 2, err_buf, "process_1814_request",0 );
      }
      ...
      if ( 0 == strcmp(OFFLINE, current_state) )
      {
         ncamaq_log_message( 1, 1, "Received response to echo. Going back Online",
                     "process_1814_request",0 );
...
void process_net_management_from_host()
{
   ...
   else
   {
      ...
      ncamaq_log_message( 1, 2, buffer, "process_net_management_from_host",0 );
   }
   return;
}
...
INT send_18nn_to_host( pCHAR err_buf )
{
   ...
   ret_val = send_message_to_host( err_buf,true );//praneeth added true as para
...
void process_monitor_query( pPTE_MSG p_msg )
{
   ...
   {
      ncamaq_log_message( 1, 3, "Unable to build PTE message for Monitor query",
   ...
   else
   {
      ...
      if (ConnectionId[0] != 0)
      {
         ...
         if ( false == send_msg( p_msg_query, applnk_que_name, err_buf ))
            ncamaq_log_message( 1, 3, err_buf, "process_monitor_query",0 );
      }
      free( p_msg_query );
   }
   return;
