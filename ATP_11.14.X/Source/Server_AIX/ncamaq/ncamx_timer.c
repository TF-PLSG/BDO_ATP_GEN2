 ...
 * $Log:   N:\PVCS\PTE\Equitable\ncjcb\ncamx_timer.c  $
      ...
      Rev 1.10   Oct 15 2004 16:37:18   dirby
   Modified building of timer lookup key for reversals to use TID for
   ATM txns and use MID for POS txns.  TID is not used in POS and
   MID is not used in ATM reversals.
   SCR 12817
   
      Rev 1.9   Dec 02 2003 14:42:06   dirby
   Modified to create a timer key for Online Updates using the last
   eight digits of the card number instead of the terminal Id.  This
   is because Online Updates do not use a terminal Id.
   SCR 1200
   
      Rev 1.8   Oct 29 2003 11:41:58   dirby
   Added voids and ATM Confirms to the list of transactions that
   use card number instead of stan when creating the timer key.
   This is because they are also sent to the host as reversals.
   SCR 1262
   
      Rev 1.7   Oct 28 2003 14:37:52   dirby
   Modified to use card number as part of the timer lookup key for
   reversals.  This is because reversals send out a different stan
   to the host than the stan that comes from the terminal; Amex
   requires a unique stan.  So the stan cannot be used in the key
   because the one in the host response is different than the one
   in auth_tx.
   SCR 1262
   
      Rev 1.6   Oct 07 2003 16:29:24   svelaga
   ...
   Automatic revesal transaction is also logged into TLF01.
      ...
      Rev 1.5   Oct 03 2003 17:27:58   svelaga
   4.2.0.9: Routing of ATM transactions to Amex ATM Acquiring network controller ( ncatma ).
   
      Rev 1.4   Sep 19 2003 17:07:18   svelaga
   1. Field 22 according to Equitable supplied mapping.
   2. Bug fix with timer_key due to merchant_id truncation.
   3. Some more logging fixes.
   
      Rev 1.3   Sep 10 2003 15:22:32   svelaga
   Fixes to Field 25, Field 22, Field 42, Field 43 and Field 49.
   Changed version to 4.2.0.1
   
      Rev 1.2   Aug 28 2003 11:18:50   svelaga
      ...
      if ( timer_reply_size != Auth_Tx_Size )
	   {
         sprintf( err_buf,
                 "Error retrieving auth_tx from timer message. Lookup key = %s",
                  lookup_key );
         ncamaq_log_message( 1, 2, err_buf, "process_timeout_msg",0 );
	   }
	   else
      {
         ...
         sprintf( err_buf,
                 "Txn to Amex timed out.  Merchant %s",
                  Auth_Tx.TLF01_details.merchant_id );
         ncamaq_log_message( 1, 1, err_buf, "process_timeout_msg",0 );
...
INT set_timer( pCHAR lookup_key, INT timer_length, INT type_of_timer )
{
   ...
   if ( false == ret_val )
      ncamaq_log_message( 1, 2, err_buf, "set_timer",0 );
...
INT clear_timer( pCHAR lookup_key )
{
   ...
   if ( false == ret_val )
      ncamaq_log_message( 1, 2, err_buf, "clear_timer",0 );
