...
* All Rights Reserved.
*
* MODULE:      ncpul_validation.c
*  
* TITLE:      ncpul Network Control Data Validation Functions
*  
 ...
 *
 ******************************************************************************/
INT ncpul_validate_date_yymm( pBYTE date, pCHAR err_buf )
{
   INT  ret_val = false;
   INT  iYear = 0;
   INT  iMonth = 0;
   ...
   if ( true != ncpul_isnum(temp_year) )
      sprintf( err_buf, "Invalid year - not numeric: %s", temp_year );
   else if ( true != ncpul_isnum(temp_month) )
...
INT ncpul_validate_date_yyyymmdd( pBYTE date_str, pCHAR err_buf )
{
   ...
   if ( (true == ncpul_isnum(date_str)) && (8 == strlen(date_str)) )
   {
      /* Validate month */
...
INT ncpul_validate_time( pBYTE time_str )
{
   ...
   if ( (true == ncpul_isnum(time_str)) && (6 == strlen(time_str)) )
   {
      /* Validate hour */
...
INT ncpul_validate_date_time( pBYTE date_time )
{
   ...
   ret_val = ncpul_validate_date_mmdd( date );
   if ( ret_val == true )
   {
      memcpy( time, date_time+DINERS_MMDD_LEN, DINERS_HHMMSS_LEN );
      ret_val = ncpul_validate_time( time );
   }
   return( ret_val );
}
...
/******************************************************************************
 *
 *  NAME:         ncpul_validate_YY_date_time
 *
 *  DESCRIPTION:  This function validates that a date in the format of MMDD
 *                has a valid day (numeric and ranging from 01 to 31) and a
 *                valid month (numeric and ranging from 01 to 12).  It also
 ...
 *
 ******************************************************************************/
INT ncpul_validate_YY_date_time( pBYTE date_time )
{
   INT   ret_val = false;
   CHAR  err_buf[100]={0};
   BYTE  date[9] = {0};
...
INT ncpul_validate_date_mmdd( pBYTE date_str )
{
   ...
   /* Validate date is numeric. */
   if ( (true == ncpul_isnum(date_str)) && (DINERS_MMDD_LEN == strlen(date_str)) )
   {
      /* Validate month */
      ...
      {
         /* Month is valid; check the day. */
         memcpy( temp_day, date_str+ DINERS_MM_LEN, DINERS_DD_LEN );
         iday = atoi(temp_day);
         if ( (1 <= iday) && (iday <= 31) )
         {
            if ( iday > 29 )
...
INT ncpul_validate_host_msg()
{
   ...
   if ( ret_val == false )
      ncpul_log_message( 2, 2, err_buf, "validate_host_msg",0);
 ...
 *                is reported.
 *
 *  INPUTS:      ncpul_HOST_MESSAGE.msgtype - (GLOBAL) Message type from host
 *
 *  OUTPUTS:      msgtype - Message type fromncpul_HOST_MESSAGE
...
INT ncpul_validate_msgtype( pCHAR msgtype, pCHAR err_buf )
{
   ...
   do
   {
      memcpy( msgtype,ncpul_HOST_MESSAGE.msgtype, 4 );
...
void ncpul_check_end_of_year_rollover( pBYTE txn_mmdd, pBYTE ATP_date_yyyymmdd )
{
   ...
   ncpul_get_date( time_date, date_yyyymmdd );
