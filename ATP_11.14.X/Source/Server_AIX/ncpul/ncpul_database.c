 ...
 *
 ******************************************************************************/
INT ncpul_insert_db_record( BYTE data_type )
{
	INT    ret_val = true;
	INT    length = 0;
	...
	{
		ret_val = ncpul_build_and_send_request_message( MT_DB_REQUEST,
														ST1_DB_INSERT,
														0,
														updateds_que_name,
														db_buffer,
		...
		if ( ret_val == false )
		{
			...
			sprintf( Buffer,
					 ...
					 db_table, err_buf );
			ncpul_log_message( 3, 2, Buffer, "ncpul_insert_db_record",1 );
		}
	}
 ...
 *  
 ******************************************************************************/
INT ncpul_get_db_record( BYTE app_data_type, BYTE subtype2, pBYTE network_type )
{
	...
	if ( ret_val == false )
	{
		ncpul_log_message( 3, 3, error_msg, "get_db_record",0 );
		memset( err_buf, 0x00, sizeof(err_buf) );
		sprintf( err_buf,
				"Unable to select record from database for table type %d.",
				app_data_type );
		ncpul_log_message( 3, 3, err_buf, "get_db_record",0 );
	}
...
INT ncpul_update_database()
{
	...
	if ( ret_val == false )
	{
		...
		sprintf( Buffer,
				 ...
				 err_buf );
		ncpul_log_message( 3, 2, Buffer, "ncpul_update_database",1 );
	}
...
void ncpul_process_db_reply( pPTE_MSG p_msg )
{
	...
	{
		ncpul_process_db_ok( p_msg );
	}
	else
	{
		ncpul_process_db_error( p_msg );
	}
	return;
}
...
void ncpul_process_db_ok( pPTE_MSG p_msg )
{
	...
	if ( ST1_DB_UPDATE == subtype1 )
	{
		if ( ALL_DATA == app_data_type )
		{
			...
			ncpul_log_message( 2, 1, "Response Send to ncpul Host", "ncpul_process_db_ok",0 );
		}
		else if(NCF01_DATA == app_data_type)
		{
							...
							PULSE_MESSAGE_TYPE_SIZE))
			{
				...
				ncpul_hhutil_send_net_mngmnt_response(&Auth_Tx);
				ncpul_log_message( 2, 1, "Updated key in to NCF01 table", "ncpul_process_db_ok",0 );
			...
			else
			{
				...
				ncpul_log_message( 2, 1, Errorbuf,"ncpul_process_db_ok",0 );
			}
		}
	}
	...
	else  if ( ST1_DB_SELECT== subtype1 )
	{
		...
		else if (NCF30_DATA == app_data_type)
		{
			...
			if (false == ncpul_process_incoming_message_ncf30_reply(p_data_stream))
			{
				ncpul_log_message( 1, 2, "Unable to send the request to the authorizor application",
										"ncpul_process_db_ok",1 );
			}
		}
		else
		{
			ncpul_log_message( 1, 2, "Invalid incoming select message type",
									  "ncpul_process_db_ok",1 );
		}
...
void ncpul_process_db_error( pPTE_MSG p_msg )
{
	...
	if ( ST1_DB_SELECT == subtype1 )
	{
		...
		{
			ncpul_log_message( 1, 2, "Failed to fetch TLF01 for revrsal, copying default value to DE63", "process_db_error",0 );
			memset(Auth_Tx.TLF01_details.ncf01_retired_cwk,0x00,sizeof(Auth_Tx.TLF01_details.ncf01_retired_cwk));
			strncpy(Auth_Tx.TLF01_details.ncf01_retired_cwk,"017D0076100PULPRCD44",20);
			(void)ncpul_process_request_for_host_after_security();
		}
	}
	...
	else if ( ST1_DB_UPDATE == subtype1 )
	{
		if ( ALL_DATA == app_data_type )
		{
			...
			*/
			TranFlow = PULSE_SWITCHED_IN;
			strcpy( Auth_Tx.TLF01_details.response_code, PULSE_SYSTEM_ERROR );
			Auth_Tx.TLF01_details.auth_number[0] = 0x00;
			ncpul_send_response_to_host( PULSE_SEND_IF_NOT_REVERSAL );
			sprintf( buffer,
					 "Unable to update DB with txn info. Error = %s",
					 error_msg );
			ncpul_log_message( 3, 3, buffer, "process_db_error",0 );
		}
		else if(NCF01_DATA == app_data_type)
		{
							...
							PULSE_MESSAGE_TYPE_SIZE))
			{
				...
				Auth_Tx.TLF01_details.tx_key = AUTH_PIN_CHANGE_RESPONSE;
				ncpul_hhutil_send_net_mngmnt_response(&Auth_Tx);
				ncpul_log_message( 1, 3, "Failed to updated key in to NCF01 table",
													  "ncpul_process_db_error",1 );
			}
			else
			{
				sprintf(error_msg,"NCF01 Operation is Failed : %s", Auth_Tx.TLF01_details.tx_key);
				ncpul_log_message( 1, 3, error_msg, "ncpul_process_db_error",0 );
			}
		}
	}
	return;
...
/*****************************************************************************
  ...
  Function:    ncpul_get_ncf01_with_wait
  ...
  Return values:
      ...
      false     - Error
******************************************************************************/
INT ncpul_get_ncf01_with_wait( pNCF01 pncf01, pCHAR error_msg )
{
   ...
   {
      if(PTEMSG_OK == (rcode = ncpul_send_interactive_db_request(p_msg_out, &p_msg_in,
                                                           netds_que_name,
                                                           error_msg )) )
      {
...
/*****************************************************************************
  ...
  Function:    ncpul_send_interactive_db_request
