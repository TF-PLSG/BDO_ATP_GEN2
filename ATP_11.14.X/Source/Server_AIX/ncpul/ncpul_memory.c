 ...
 * All Rights Reserved.
 *
 * MODULE:      ncpul_memory.c
 *  
 * TITLE:       ncpul Network Controller Shared Memory Processor
 *  
...
/******************************************************************************
 *
 *  NAME:         ncpul_create_shared_mem_table
 *
...
INT ncpul_create_shared_mem_table( pCHAR errbuf )
{
   ...
   else if ( 0 == ret_code )
   {
       ...
       * Initialize host counters and state upon ncpul_ncpul1_startup.
       */
     ret_val = ncpul_write_to_shared_mem_table( "0", "0", OFFLINE, errbuf );
   }
   else
   {
...
/******************************************************************************
 *
 *  NAME:         ncpul_write_to_shared_mem_table
 *
                               ...
                               pCHAR errbuf )
{
   ...
   ret_code = ncpul_WriteMemTable( table_name, consec_time_out_count,
...
/******************************************************************************
 *
 *  NAME:         ncpul_read_shared_mem_table
 *
                           ...
                           pCHAR errbuf )
{
   ...
   ret_code = ncpul_ReadMemTable( table_name, consec_time_out_count,
                            active_tran_count, host_state );
   if ( 0 != ret_code )
   {
      ret_val = false;
...
INT ncpul_set_host_state( pBYTE host_state, pCHAR err_buf )
{
   ...
   ret_val = ncpul_read_shared_mem_table( timeout_cnt, active_cnt,
   ...
   if ( true == ret_val )
   {
      ...
      if ( 0 != strcmp(host_state, prev_host_state) )
      {
         ...
         ret_val = ncpul_write_to_shared_mem_table( timeout_cnt, active_cnt,
                                              host_state,  err_buf );
         if ( true == ret_val )
         {
            ...
            if ( 0 == strcmp(host_state, ONLINE) )
            {
               ...
               if ( false == ncpul_set_echo_timer(err_buf) )
                  ncpul_log_message( 3, 2, err_buf, "ncpul_set_host_state",0 );
            }
            else if ( 0 == strcmp(host_state, DOWN) )
            {
               ...
               if ( false == ncpul_clear_echo_timer(err_buf) )
                  ncpul_log_message( 3, 2, err_buf, "ncpul_set_host_state",0 );
            }
            else if ( 0 == strcmp(host_state, OFFLINE) )
            {
                ...
                */
               (void)ncpul_clear_echo_timer(err_buf);
...
/******************************************************************************
 *
 *  NAME:         ncpul_decrement_active_count
 *
...
INT ncpul_decrement_active_count( pCHAR errbuf )
{
   ...
   if ( true == ret_val )
   {
      ...
      if ( 0 == strcmp(host_state,QUERYING) )
      {
         ...
         {
            ncpul_set_host_state( ONLINE, errbuf );
...
/******************************************************************************
 *
 *  NAME:         ncpul_increment_active_count
 *
...
INT ncpul_increment_active_count( pCHAR errbuf )
{
   ...
   if ( true == ret_val )
   {
      ...
      if ( (tempint >= MaxActiveTxns) && (MaxActiveTxns > 0) )
      {
          ...
          */
         (void)ncpul_set_host_state( QUERYING, errbuf );
         strcpy( errbuf,
                "Active transaction count too high. Continuing to process.");

         ncpul_log_message( 1, 2, errbuf, "ncpul_increment_active_count",0 );
      }
      else
      {
...
/******************************************************************************
 *
 *  NAME:         ncpul_increment_timeout_count
 *
 *  DESCRIPTION:  This function reads the shared memory table, increments the
 ...
 *
 ******************************************************************************/
INT ncpul_increment_timeout_count( pCHAR err_buf )
{
   INT      ret_val;
   BYTE     active_cnt[4] = {0};
   BYTE     timeout_cnt[4] = {0};
   ...
   if ( true == ret_val )
   {
      ...
      /* Update the values into shared memory. */
      (void)ncpul_write_to_shared_mem_table( timeout_cnt, active_cnt,
                                       host_state, err_buf );
   }
   return( ret_val );
}
...
/******************************************************************************
 *
 *  NAME:         ncpul_reset_timeout_count
 *
...
INT ncpul_reset_timeout_count( pCHAR errbuf )
{
   ...
   if ( true == ret_val )
   {
      ...
      ret_val = ncpul_write_to_shared_mem_table( "0", active_cnt,
                                           host_state, errbuf);
   }
   return( ret_val );
}
