 ...
 * All Rights Reserved.
 *
 * MODULE:      ncpul_timer.c
 *  
 * TITLE:       ncpul Network Controller Timer Processor
 *  
...
/******************************************************************************
 *
 *  NAME:         ncpul_process_timeout_msg
 *
 ...
 *
 ******************************************************************************/
void ncpul_process_timeout_msg( pPTE_MSG p_msg )
{
	...
	CHAR		   err_buf[100] = {0};
	CHAR        echo_timer_key[PULSE_LOOKUP_KEY_SIZE] = {0};
...
/* Determine what timed out: echo timer or txn request. */
	...
	ncpul_create_echo_timer_key( echo_timer_key );
	ncpul_create_echo_req_timer_key( echo_req_timer_key );
   ...
   */
		Echo_Timer_Flag = PULSE_ECHO_TIMER_IS_CLEARED;
		//HCTS1-A, was commented below line
		(void)ncpul_issue_req_08X4( PULSE_ECHO );
	}
  ...
  /* --------------------------------------------------- */
		(void)ncpul_decrement_active_count( err_buf );
	}
	else
	{
		if ( timer_reply_size != Auth_Tx_Size )
		{
			sprintf( err_buf,
			 "Error retrieving auth_tx from timer message. Lookup key = %s",
			  lookup_key );
			ncpul_log_message( 3, 2, err_buf, "process_timeout_msg",1 );
		}
		else
		{
			...
			strcpy( Auth_Tx.TLF01_details.response_code, PULSE_TO );
			...
			*/
			...
			strcpy( err_buf, "Txn to ncpul timed out" );
			ncpul_log_message( 3, 1, err_buf, "process_timeout_msg",1 );
			...
			if ( repeat_ctr == 0 )
			{
				...
				*/
				if ( 0 != strcmp(app_data2, PULSE_TIMER_REVERSAL) )
				{
					temp_tx_key = Auth_Tx.TLF01_details.tx_key;
					if ( false == ncpul_send_timeout_response(err_buf) )
					ncpul_log_message( 3, 2, err_buf, "process_timeout_msg",1 );
			...
			*/
			if ( false == ncpul_increment_timeout_count(err_buf) )
			{
			...
			else
			{
				...
				if ( ncpul_is_reversal() )
				{
					...
					if ( repeat_ctr < max_repeats )
					{
						...
						/* Use PULSE_TIMER_REVERSAL so response does not go to txcntl. */
						(void)ncpul_send_request_to_host( PULSE_TIMER_REVERSAL );
					}
				}
				else
				{
					...
					*/
					...
					{
						(void)ncpul_transform_sale_into_0420();
						(void)ncpul_send_request_to_host( PULSE_TIMER_REVERSAL );
					}
				}
			}
...
/******************************************************************************
 *
 *  NAME:         ncpul_send_timeout_response
 *
...
INT ncpul_send_timeout_response( pCHAR err_buf )
{
   ...
   ret_val = ncpul_perform_standin_processing( PULSE_TO, GS_TXN_TIMEDOUT, err_buf );
   if (ret_val == true )
      ret_val = ncpul_decrement_active_count( err_buf );
...
/******************************************************************************
 *
 *  NAME:         ncpul_transform_sale_into_0420
 *
...
void ncpul_transform_sale_into_0420()
{
   ...
   strcpy( Auth_Tx.TLF01_details.response_code, PULSE_TIMEOUT );
 ...
 *
 *                Its values: PULSE_TIMER_REVERSAL
 *                            PULSE_TIMER_FINANCIAL
 *                            TIMER_0800
 *                            PULSE_TIMER_ECHO
 *
 *  OUTPUTS:      None
...
INT ncpul_set_timer( pCHAR lookup_key, INT timer_length, INT type_of_timer )
{
   ...
   if ( type_of_timer == PULSE_REQUEST_TIMER )
   {
   ...
   {
      ret_val = ncpul_send_msg(p_msg_set_timer, timerds_que_name, err_buf);
      free( p_msg_set_timer );
   }

   if ( false == ret_val )
      ncpul_log_message( 2, 2, err_buf, "ncpul_set_timer",0 );
 ...
 *
 ******************************************************************************/
INT ncpul_clear_timer( pCHAR lookup_key )
{
   ...
   {
      /* Clear Timer message  */
      ret_val = ncpul_send_msg(p_msg, timerds_que_name, err_buf);
      free( p_msg );
   }

   if ( false == ret_val )
      ncpul_log_message( 2, 2, err_buf, "ncpul_clear_timer",1 );
...
/******************************************************************************
 *
 *  NAME:         ncpul_clear_request_timer
 *
...
INT ncpul_clear_request_timer( pCHAR lookup_key )
{
   ...
   else
   {
      ...
      else
      {
          ...
          */
         ret_val = ncpul_send_msg( p_chain_msg, timerds_que_name, err_buf );
         free( p_chain_msg );
      }
   }
...
INT ncpul_reset_echo_timer( pCHAR err_buf )
{
   ...
   {
      ret_val = ncpul_clear_echo_timer( err_buf );
      if ( true == ret_val )
         ret_val = ncpul_set_echo_timer( err_buf );
   }
   return( ret_val );
}
...
INT ncpul_set_echo_timer( pCHAR err_buf )
{
   ...
   if ( Echo_Timer_Flag == PULSE_ECHO_TIMER_IS_CLEARED )
   {
      echo_timer_seconds = atoi( Ncf01_I.echo_interval );
      if ( echo_timer_seconds > 0 )
      {
         ...
         if ( ret_val == true )
         {
            if( 0 == strcmp( current_state, ONLINE ) )
            {
               ...
               ncpul_create_echo_timer_key( echo_timer_key );
               strcpy( TimerAppData2, PULSE_TIMER_ECHO );
               if (false == ncpul_set_timer(echo_timer_key,echo_timer_seconds,PULSE_ECHO_TIMER))
               {
                  strcpy( err_buf, "Unable to set Echo timer." );
                  ret_val = false;
               }
               else
                  Echo_Timer_Flag = PULSE_ECHO_TIMER_IS_SET;
            }
...
INT ncpul_clear_echo_timer( pCHAR err_buf )
{
   ...
   if ( Echo_Timer_Flag == PULSE_ECHO_TIMER_IS_SET )
   {
      echo_timer_seconds = atoi( Ncf01_I.echo_interval );
      if ( echo_timer_seconds > 0 )
      {
         memset( echo_timer_key, 0x00, sizeof(echo_timer_key) );
         ncpul_create_echo_timer_key( echo_timer_key );
         if ( false == ncpul_clear_timer(echo_timer_key) )
         {
...
/******************************************************************************
 *
 *  NAME:         ncpul_create_echo_timer_key
 *
...
/******************************************************************************
 *
 *  NAME:         ncpul_create_request_timer_key
 *
...
void ncpul_create_request_timer_key( pCHAR timer_key )
{
   ...
   ncpul_trim_trailing_spaces( tid );
   ...
   {
	   /* Copying the last 8 bytes of rrn for reversal lookup key
	    */
	   strncpy(temp_rrn,Auth_Tx.TLF01_details.retrieval_ref_num+4,8);
	   strcat( timer_key, temp_rrn );
   }
   else
   {
...
/******************************************************************************
 *
 *  NAME:         ncpul_create_echo_req_timer_key
 *
