/******************************************************************************
   ...
   * MODULE:	  ncpul_main.c
   *  
   * TITLE: 	  ncpul Network Controller Database Processor
   *  
...
CHAR  ncpul_Erro_warning_Filename[256] = {0};
CHAR  ncpul_module_error_warning_file_name[256] = {0};
CHAR  ncpul_error_warning_file_path[256] = {0};
							   
/******************************************************************************
 *
 *  NAME:         ncpul_ncpul1_PinnacleMsgHandler
 *
 *  DESCRIPTION:  This function acts as a switch for messages coming into ncpul2
 ...
 *
 ******************************************************************************/
void ncpul_PinnacleMsgHandler( pPTE_MSG p_msg_in )
{
   BYTE            subtype1;
   BYTE            msgtype;
   CHAR            buffer[256];      
	 ...
	 case MT_AUTH_RESPONSE: 
      ...
      case MT_TIMER_REPLY:
         ...
         if ( subtype1 == ST1_TIMER_CLEAR )
         {
             ...
             */
        	 ncpul_process_clear_timer_reply( p_msg_in );
         }
         break;
      ...
      case MT_TIMER_TIMEOUT: 
         ...
         /* One of the timers expired. */
    	  ncpul_process_timeout_msg( p_msg_in );
         break;

      case MT_SYS_MONITOR:

         /* Process a request from Monitor. */
    	  ncpul_process_monitor_request( p_msg_in );
         break;  

      case MT_GET_STATS:

         /* Send transaction counts to whoever requested them. */
         (void)ncpul_send_transaction_statistics( p_msg_in );
         break;

      case MT_RESET_STATS:

         /* Reset transaction counts to zero. */
         (void)ncpul_reset_transaction_statistics( p_msg_in );
         break;
         ...
         if ( ST1_SYS_PING == ptemsg_get_msg_subtype1(p_msg_in) )
         {
               ...
               if ( false == flush_file(ppTraceFile_fp) )
               {
                  ...
                  sprintf( buffer,
                    "%s: Unable to flush trace file buffer. Turning Trace off.",
                     AppName );
                  ncpul_log_message( 2, 1, buffer, "PingMsgHandler",0 );
            ...
            sprintf( buffer,
                    "Pinged -> Network Controller Service: %s, version %s",
                     ServiceName, Version );
            ncpul_log_message( 3, 1, buffer, "PingMsgHandler",0 );
         }
      break;
      ...
      default :
         ...
         ncpul_log_message( 3, 2, buffer, "PingMsgHandler",0 );
      break;
   }  /* switch msg_type */
...
void MainProcessor()
{
   ...
   ncpul_get_error_warning_file_name_path();
   sprintf( buffer,
           "Starting the Network Controller Service: %s,AppName: %s, version %s",
            ServiceName,AppName, Version );
   ncpul_log_message( 1, 1,buffer,"MainProcessor",0 );
   PRINT2( "%s\n", buffer );

 
   if( !pteipc_init_single_instance_app( AppName, "pte_ipc" ) ) 
   {
      ncpul_log_message( 2, 3, "Failed to create XIPC queues.", "MainProcessor",0 );
   ...
   {
      /* Perform any ncpul_ncpul1_startup initialization. */
      if ( false == ncpul_startup() )
      {
	      pteipc_shutdown_single_instance_app(); 
      ...
      else
      {
         ...
         while( !EndProcessSignalled )
         {
            ...
            if ( NULL_PTR != p_msg )
            {
               ncpul_PinnacleMsgHandler( p_msg );
               free( p_msg );
            }
            else if ( QUE_ER_TIMEOUT != ret_code ) 
            {
               ...
               ncpul_log_message( 2, 3, buffer, "MainProcessor" ,0);
            }
         }
      }

      /* Shutting down, need to change state before exiting Xipc. */
      (void)ncpul_set_host_state( DOWN, ErrorMsg );
...
void EndProcess()
{
   ...
   if ( Tracing == PULSE_TRACE_ON )
   {
      if ( false == close_trace_file( ppTraceFile_fp ))
      {
         ...
         ncpul_log_message( 2, 1, Buffer, "ncpul_ncpul1_startup" ,0);
      }
   }

   sprintf( Buffer, "Shutting down the %s Service, version %s",
            ServiceName, Version );
   ncpul_log_message( 1, 1,Buffer,"EndProcess",0 );
   strcat( Buffer, "\n" );
   PRINT( Buffer );
}
...
INT ncpul_startup()
{
    ...
    */
   ret_val = ncpul_create_shared_mem_table( err_buf );
   if ( true == ret_val )
   {
      ...
      ret_val = ncpul_get_ncf01_with_wait( &Ncf01_I, Buffer );
	  if ( false == ret_val )
      {
   ...
   if ( true == ret_val )
   {
      ...
      ncpul_init_character_code_maps();
         ...
         if (false == open_trace_file(AppName,DirPath,ppTraceFile_fp,TraceFile))
         {
            ...
            sprintf( err_buf,
                    "Unable to open trace file %s. Tracing is off",
                     TraceFile );
            ncpul_log_message( 2, 1, err_buf, "ncpul_ncpul1_startup",0 );
         }
         else
            ncpul_log_message( 2, 1, "Tracing is turned ON", "ncpul_ncpul1_startup",0 );
      }
      else
      {
         Tracing = PULSE_TRACE_OFF;
         ...
         ncpul_log_message( 2, 1, "Tracing is turned off", "ncpul_ncpul1_startup",0 );
      }
	  ...
	  */
      if (0 == strcmp(Ncf01_I.auto_logon,"1"))
      {
         /* Issue a LOGON request to the host. */
    	  ...
    	  ncpul_issue_req_08X4( PULSE_ECHO );
      }
      (void)ncpul_send_network_status_to_monitor();
   }
   else
   {
      sprintf( Buffer, "Unable to ncpul_ncpul1_startup %s.  %s.", AppName, err_buf );
      ncpul_log_message( 2, 3, Buffer, "Startup",0 );
   }
