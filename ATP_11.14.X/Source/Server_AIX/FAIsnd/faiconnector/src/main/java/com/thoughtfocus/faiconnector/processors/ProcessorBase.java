...
public abstract class ProcessorBase {
	...
	protected ProcessorBase(CamelContext camelContext, XcomManager xcommManager) {
	...
	protected String getMessage() {
		...
		if ((message.length() == State.CURRENT.getMessageLength()) || (message.length() == State.CURRENT.getRealTimeMessageLength()) || (message.length() == State.CURRENT.getExt10Length()))
			return message;
		if ((message.length() == State.CURRENT.getMessageLength() - 1) || (message.length() == State.CURRENT.getRealTimeMessageLength() - 1))
			return message + " ";
		
		log.warn("Received Message Length = " + message.length() + ", Standard Message Length For Online = " + State.CURRENT.getMessageLength() + ",Standard Message Length For Real-Time = " + State.CURRENT.getRealTimeMessageLength() + ",Standard Message Length For EXT10 = " + State.CURRENT.getExt10Length());
