...
public class XcomToActiveMqRoute extends RouteBuilder {
	...
	public XcomToActiveMqRoute(@Value("${fai.input-queue-name}") String activeMqInputQueueName,
		...
		State.CURRENT.setExt10Length(ext10Length);
	...
	public void handleException(Exchange exchange) {
		...
		if (requestType == true) {
			String response = SYSTEM_ERROR + message.substring(1, 101);
	...
	public void processMessage(Exchange exchange) {
		...
		if (checkForDuplicateTransactions(message)) {
			...
			if (message.startsWith(REAL_TIME)) {
				requestType = true;
				log.info("FaisndRT: Received the request from xcom size: {} Request: {}", message.length(),
				...
				log.info("Service name: " + State.CURRENT.getServiceName()
						+ ", FaisndRT:  Sending request to ActiveMq Size: " + message.length() + " TranId+RRN: "
				...
				log.info("Received the request from xcom for EXT10: {}",
						(displayTransactionMsgRT == 1) ? message : message.substring(1, 101));
				exchange.getIn().setBody(message.substring(1));
				log.info("Sending request to ActiveMq in EXT10: {}", message.substring(1, 101));
			}
		}
	}
