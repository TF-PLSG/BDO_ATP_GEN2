...
BYTE Process_Incoming_Message_From_Terminal (pPTE_MSG p_msg_in)
{
   ...
   if ( retCode != DEVICE_OK )
   {
      ...
      dcpimp_log_message( 1, 3, strTemp," Process_Incoming_Message_From_Terminal ", 0 );
      return( retCode );
   }
   ...
   {
      dcpimp_log_message( 1, 3, "Cannot parse terminal request message",
   ...
   if (!If_Transaction_Cannot_Be_Declined ())
   {
    ...
    /*Check if its a CUP Transaction*/
	/* CUP transaction coming on this NII value should be forwarded dcpiso*/
	if (true == dcpimp_check_If_CUP_Retail_Transaction())
    {
    	// We have to send to dcpiso 
		retCode = Send_Request_To_Service (  "dcpisoA",
														MT_INCOMING_DCPIMP, 
														0, 
														0, 
														0, 
    ...
    else
    {
		...
		strcpy(auth_tx.TLF01_details.acquirer_id,nciftr_module_name);
		//For MP & Fleet Acquiring Id will be NCIFTR
		strcpy(auth_tx.TLF01_details.acquiring_id, "NCIFTR");
		if((0 != strncmp(auth_tx.TLF01_details.card_num, "7",1)))
		{
			/*We are using this flag to make sure that only  MP */
			/*Privalte lable transaction like fleet will not have this flag*/
			strncpy(auth_tx.TLF01_details.product_codes[2].quantity,"1",1);
    	}
	}
...
BYTE Process_incoming_dcpiso_message(pPTE_MSG p_msg_in)
{
   ...
   dataLength = ptemsg_get_pte_msg_data_data_length( p_msg_data );
	...
	if (retCode != DEVICE_OK)
	{
	 ...
	 /*Check what message type is coming from dcpiso module  */
   ...
   {
   		/*dcpiso module has done duplicate check.*/
		/* Get device details for this module and process for settlement*/
...
BYTE Copy_Message_Fields_From_Global_Structure_To_Auth_Tx (void)
{
   ...
   if ( temp_len > 0)
   {
	...
	if ((ISO_MESSAGE.priv63_len[0] != 0x00) || (ISO_MESSAGE.priv63_len[1] != 0x00))
	{
		...
		{
			dcpimp_log_message( 1, 2, "Error while parsing private field 63",
...
BYTE Populate_Sections_Of_Auth_Tx (BYTE tx_type, pPTE_MSG p_msg_in)
{
   ...
   {
   strcpy (auth_tx.TLF01_details.response_code, ATP_pos_default_response_code) ;
   }
