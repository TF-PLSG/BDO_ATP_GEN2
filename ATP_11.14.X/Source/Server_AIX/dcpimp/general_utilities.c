...
void create_rrn( pCHAR rrn )
{
   ...
   /* NEW FORMAT YDDDSSSSSCCI - TF PHANI */
     ...
     if(strcmp(ServiceName,"dcpimp") == 0)
		 strcpy(instanceNo,"0");
...
INT send_transaction_statistics( pPTE_MSG p_msg )
{
   ...
   if ( false == ret_code  )
   {
      dcpimp_log_message( 1, 2, err_msg, " send_transaction_statistics ", 0 );
   }
   return( ret_code );
}
      ...
      if ( rc < 0 )
      {
         ...
         sprintf( err_buf,
                 "Error logging entry to Txn Trace File = %s",
                  TraceFile );
         dcpimp_log_message( 1, 3, err_buf, " write_to_txn_file ", 0 );
         if ( rc == -1 )
            strcpy( err_buf,
         ...
         if ( false == flush_file(ppTraceFile_fp) )
         {
            ...
            sprintf( err_buf,
                    "%s: Unable to flush trace file buffer. Turning Trace off.",
                     ServiceName );
             dcpimp_log_message( 1, 2, err_buf, " write_to_txn_file ", 0 );
...
BYTE Check_Is_Host_Up(BYTE* destination_name)
{
	...
	if (destination_name[0] != 0)
	{
		...
		if (ReturnCode == MEMACCESSERROR)
		{
			...
			dcpimp_log_message( 1, 2, strTemp, " Check_Is_Host_Up ", 0 );
		}
		else
		{
			 ...
			 */
			/*
					we are hard coding the status as ONLINE for testing perpose. we have to remove this before releasing
					Abhishek
					
			*/

			//////////////////////////////////////////////////////////////////
				//strncpy(state,"ONLINE",6);
				//ret_val = true;
				//return ret_val;
