...
* All Rights Reserved.
*
* MODULE:      ncdci_validation.c
*  
* TITLE:      ncdci Network Control Data Validation Functions
*  
 ...
 *
 ******************************************************************************/
INT ncdci_validate_date_yymm( pBYTE date, pCHAR err_buf )
{
   INT  ret_val = false;
   INT  iYear = 0;
   INT  iMonth = 0;
   ...
   if ( true != ncdci_isnum(temp_year) )
      sprintf( err_buf, "Invalid year - not numeric: %s", temp_year );
   else if ( true != ncdci_isnum(temp_month) )
...
/******************************************************************************
 *
 *  NAME:         ncdci_validate_date_yyyymmdd
 *
...
INT ncdci_validate_date_yyyymmdd( pBYTE date_str, pCHAR err_buf )
{
   ...
   if ( (true == ncdci_isnum(date_str)) && (8 == strlen(date_str)) )
   {
      /* Validate month */
...
INT ncdci_validate_time( pBYTE time_str )
{
   ...
   INT  isecond = 0;
   CHAR temp_hour[3]   = {0};
   CHAR temp_minute[3] = {0};
   CHAR temp_second[3] = {0};
   CHAR err_buf[100]   = {0};

   /* Validate time is numeric. */
   if ( (true == ncdci_isnum(time_str)) && (6 == strlen(time_str)) )
   {
      /* Validate hour */
...
INT ncdci_validate_date_time( pBYTE date_time )
{
   ...
   memcpy( date, date_time, DINERS_MMDD_LEN );
   ret_val = ncdci_validate_date_mmdd( date );
   if ( ret_val == true )
   {
      memcpy( time, date_time+DINERS_MMDD_LEN, DINERS_HHMMSS_LEN );
      ret_val = ncdci_validate_time( time );
   }
   return( ret_val );
}
...
/******************************************************************************
 *
 *  NAME:         ncdci_validate_YY_date_time
 *
 *  DESCRIPTION:  This function validates that a date in the format of MMDD
 *                has a valid day (numeric and ranging from 01 to 31) and a
 *                valid month (numeric and ranging from 01 to 12).  It also
 ...
 *
 ******************************************************************************/
INT ncdci_validate_YY_date_time( pBYTE date_time )
{
   INT   ret_val = false;
   CHAR  err_buf[100]={0};
   BYTE  date[9] = {0};
   ...
   memcpy( date, date_time + DINERS_YY_LEN, DINERS_MMDD_LEN );
   ret_val = ncdci_validate_date_mmdd( date );
   if ( ret_val == true )
   {
      memcpy( time, date_time+DINERS_MMDD_LEN, DINERS_HHMMSS_LEN );
...
INT ncdci_validate_date_mmdd( pBYTE date_str )
{
   ...
   /* Validate date is numeric. */
   if ( (true == ncdci_isnum(date_str)) && (DINERS_MMDD_LEN == strlen(date_str)) )
   {
      /* Validate month */
      memcpy( temp_month, date_str, DINERS_MM_LEN );
      imonth = atoi(temp_month);
      if ( (1 <= imonth) && (imonth <= DINERS_MAX_MONTH_IN_YEAR) )
      {
         /* Month is valid; check the day. */
         memcpy( temp_day, date_str+ DINERS_DD_LEN, DINERS_DD_LEN );
         iday = atoi(temp_day);
         if ( (1 <= iday) && (iday <= 31) )
         {
            if ( iday > 29 )
 ...
 *                as sale 's need card numbers and amounts, etc.  reversal's need
 *                originator information, and so on.  This function is only
 *                called for sale and reversal requests.
 *
...
INT ncdci_validate_host_msg()
{
   ...
   if ( ret_val == false )
      ncdci_log_message( 2, 2, err_buf, "validate_host_msg" ,0);
 ...
 *                is reported.
 *
 *  INPUTS:      ncdci_HOST_MESSAGE.msgtype - (GLOBAL) Message type from host
 *
 *  OUTPUTS:      msgtype - Message type fromncdci_HOST_MESSAGE
...
INT ncdci_validate_msgtype( pCHAR msgtype, pCHAR err_buf )
{
   ...
   do
   {
      memcpy( msgtype,ncdci_HOST_MESSAGE.msgtype, 4 );
...
/******************************************************************************
 *
 *  NAME:         ncdci_check_end_of_year_rollover
 *
...
void ncdci_check_end_of_year_rollover( pBYTE txn_mmdd, pBYTE ATP_date_yyyymmdd )
{
   ...
   ncdci_get_date( time_date, date_yyyymmdd );
