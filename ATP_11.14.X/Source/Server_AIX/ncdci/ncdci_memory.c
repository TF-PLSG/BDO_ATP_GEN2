 ...
 * All Rights Reserved.
 *
 * MODULE:      ncdci_memory.c
 *  
 * TITLE:       ncdci Network Controller Shared Memory Processor
 *  
...
/******************************************************************************
 *
 *  NAME:         ncdci_create_shared_mem_table
 *
...
INT ncdci_create_shared_mem_table( pCHAR errbuf )
{
   ...
   else if ( 0 == ret_code )
   {
       ...
       * Initialize host counters and state upon ncdci_ncdci1_startup.
       */
     ret_val = ncdci_write_to_shared_mem_table( "0", "0", OFFLINE, errbuf );
   }
   else
   {
...
/******************************************************************************
 *
 *  NAME:         ncdci_write_to_shared_mem_table
 *
 ...
 *  MODIFICATIONS:
 ******************************************************************************/
INT ncdci_write_to_shared_mem_table( pBYTE consec_time_out_count,
                               ...
                               pCHAR errbuf )
{
   ...
   ret_code = ncdci_WriteMemTable( table_name, consec_time_out_count,
...
/******************************************************************************
 *
 *  NAME:         ncdci_read_shared_mem_table
 *
 ...
 *  MODIFICATIONS:
 ******************************************************************************/
INT ncdci_read_shared_mem_table( pBYTE consec_time_out_count,
                           pBYTE active_tran_count,
                           pBYTE host_state,
                           pCHAR errbuf )
{
   ...
   ret_code = ncdci_ReadMemTable( table_name, consec_time_out_count,
                            active_tran_count, host_state );
   if ( 0 != ret_code )
   {
      ret_val = false;
...
INT ncdci_set_host_state( pBYTE host_state, pCHAR err_buf )
{
   ...
   ret_val = ncdci_read_shared_mem_table( timeout_cnt, active_cnt,
   ...
   if ( true == ret_val )
   {
      ...
      if ( 0 != strcmp(host_state, prev_host_state) )
      {
         ...
         ret_val = ncdci_write_to_shared_mem_table( timeout_cnt, active_cnt,
                                              host_state,  err_buf );
         if ( true == ret_val )
         {
            ...
            if ( 0 == strcmp(host_state, ONLINE) )
            {
               ...
               if ( false == ncdci_set_echo_timer(err_buf) )
                  ncdci_log_message( 3, 2, err_buf, "ncdci_set_host_state", 0 );
            }
            else if ( 0 == strcmp(host_state, DOWN) )
            {
               ...
               if ( false == ncdci_clear_echo_timer(err_buf) )
                  ncdci_log_message( 3, 2, err_buf, "ncdci_set_host_state", 0 );
            }
            else if ( 0 == strcmp(host_state, OFFLINE) )
            {
                ...
                */
               //(void)ncdci_clear_echo_timer(err_buf);
               if ( MaxConsecTimeouts > 0 )
               {
                  ...
                  {
                     //if ( false == ncdci_set_echo_timer(err_buf) )
                       // ncdci_log_message( 3, 2, err_buf, "ncdci_set_host_state" );
                  }
               }
...
/******************************************************************************
 *
 *  NAME:         ncdci_decrement_active_count
 *
...
INT ncdci_decrement_active_count( pCHAR errbuf )
{
   ...
   if ( true == ret_val )
   {
      ...
      if ( 0 == strcmp(host_state,QUERYING) )
      {
         ...
         {
            ncdci_set_host_state( ONLINE, errbuf );
...
/******************************************************************************
 *
 *  NAME:         ncdci_increment_active_count
 *
...
INT ncdci_increment_active_count( pCHAR errbuf )
{
   ...
   if ( true == ret_val )
   {
      ...
      if ( (tempint >= MaxActiveTxns) && (MaxActiveTxns > 0) )
      {
          ...
          */
         (void)ncdci_set_host_state( QUERYING, errbuf );
         strcpy( errbuf,
                "Active transaction count too high. Continuing to process.");

         ncdci_log_message( 1, 2, errbuf, "ncdci_increment_active_count",0 );
      }
      else
      {
...
/******************************************************************************
 *
 *  NAME:         ncdci_increment_timeout_count
 *
 *  DESCRIPTION:  This function reads the shared memory table, increments the
 ...
 *
 ******************************************************************************/
INT ncdci_increment_timeout_count( pCHAR err_buf )
{
   INT      ret_val;
   BYTE     active_cnt[4] = {0};
   BYTE     timeout_cnt[4] = {0};
   ...
   if ( true == ret_val )
   {
      ...
      /* Update the values into shared memory. */
      (void)ncdci_write_to_shared_mem_table( timeout_cnt, active_cnt,
                                       host_state, err_buf );
   }
   return( ret_val );
}
...
/******************************************************************************
 *
 *  NAME:         ncdci_reset_timeout_count
 *
...
INT ncdci_reset_timeout_count( pCHAR errbuf )
{
   ...
   if ( true == ret_val )
   {
      ...
      ret_val = ncdci_write_to_shared_mem_table( "0", active_cnt,
                                           host_state, errbuf);
   }
   return( ret_val );
}
