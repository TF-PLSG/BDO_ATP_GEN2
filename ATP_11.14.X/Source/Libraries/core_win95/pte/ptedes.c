...
DWORD pte_pwd_hash_calc ( pBYTE p_data_str );
...
INT encrypt_key ( pBYTE key_in, pBYTE key_out )
{
  ...
  DES (ukey, hard_key, 1, work);

  /* put key to it place... */
  for (i=0; i<4; i++)
    sprintf(key_out+(4*i), "%04X", ukey[i]);
...
{
	DWORD    i = 0;
	add_log_ascii ("\n\n*****GUI Password Encryption*****", buffer, buffer_len);
	//i = simple_encryption (buffer, buffer_len);
	i = pte_pwd_hash_calc(buffer);
	sprintf(buffer,"%032ld",i);
	add_log ("after encryption ", buffer, i);
	return (i);
}
...
/******************************************************************************/
INT des_decryption_ex ( pBYTE buffer, INT buffer_len, pWORD des_key, INT mode )
{
  return des_decryption(buffer,buffer_len,des_key,mode);
}
...
INT password_cryption ( pBYTE data, INT encrypt )
{
  ...
  DES(buffer, hard_key1, encrypt, work);
