/******************************************************************************
   ...
   $Log:   N:\PVCS\PTE\CORE\PTE_LIB\PTELLIST.C  $
   
      Rev 1.8   Oct 15 1999 11:12:24   MSALEH
   Move SSL_SUPPORT to preprocessot definition
   section of project files
   
      Rev 1.7   Oct 14 1999 08:36:56   MSALEH
   integrate SSL modifications
   
      Rev 1.6   Dec 03 1998 10:53:26   drodden
   Added a new function to allow you to go to an element relative to your 
   current position.  Also removed old, unused fields from the linklist records.
...
*
******************************************************************************/
pLIST_ELEMENT ptellist_goto_relative ( pLIST_ELEMENT p_element,
                                       LONG          how_many   )
{
   LONG           index;
   ...
   {
      how_many = labs(how_many);

      for (index=0; index < how_many; index++)
      {
         p_element = p_element->p_prev_element;
...
*
******************************************************************************/
void ptellist_init_ssl_list ( pSSL_LINKED_LIST p_list )
{
   if (p_list != NULL_PTR)
   {
      p_list->num_elements = 0;
...
*
******************************************************************************/
pSSL_LIST_ELEMENT ptellist_first_ssl_element ( pSSL_LINKED_LIST p_list )
{
   if (p_list != NULL_PTR)
      return (p_list->p_first);
...
/*****************************************************************************
*
*  This function will append a new ssl element to the end of the linked list.
*
*  Paramaters:
*     p_list........pointer to the linked list to append the new element to.
*     *stored_ssl...ssl structure to store.
*     data_size.....number of bytes of data at the p_data location.
*
******************************************************************************/
pSSL_LIST_ELEMENT ptellist_append_ssl_element ( pSSL_LINKED_LIST p_list,
                                                SSL			     *store_ssl,
                                                LONG             data_size )
{
   pSSL_LIST_ELEMENT  p_new_elem;
   pSSL_LIST_ELEMENT  p_prev_elem;


   if (p_list == NULL_PTR)
      return (NULL_PTR);

   p_new_elem = (pSSL_LIST_ELEMENT) malloc (sizeof(struct SSL_LIST_ELEMENT) - 1 + data_size);
   ...
   if ( p_new_elem != NULL_PTR )
   {
       ...
       * Null fill the fixed portion of the element.
       */
      memset (p_new_elem, 0, sizeof(struct SSL_LIST_ELEMENT));
       ...
       */
      p_new_elem->data_size = data_size;
      memcpy (&p_new_elem->stored_ssl, store_ssl, data_size);
   }
...
*
******************************************************************************/
pSSL_LIST_ELEMENT ptellist_delete_ssl_element ( pSSL_LINKED_LIST  p_list,
                                                pSSL_LIST_ELEMENT p_cur_elem )
{
   pSSL_LIST_ELEMENT  p_prev_elem;
   pSSL_LIST_ELEMENT  p_next_elem;
...
*
******************************************************************************/
pSSL_LIST_ELEMENT ptellist_next_ssl_element ( pSSL_LIST_ELEMENT p_element )
{
