...
{
   BYTE  num_returned             [5];
   CCL01 ccl01_record             [GUI_MAX_LIST_SIZE];
...
typedef struct
{
	BYTE card_brand					[11];
	BYTE bdk_key					[65];
	BYTE future_field1				[33];
	BYTE future_field2				[33];
	BYTE future_field3				[37];
...
{
   BYTE  num_returned             [5];
   BDKKEY01 bdkkey01_record       [GUI_MAX_LIST_SIZE];
...
typedef struct
{
	...
	BYTE Cvk_Mag_Keyblock		[129];
	BYTE Cvk_Acq_3DS_Keyblock	[129];
	BYTE Cvk_Pin_Verification	[129];
	BYTE Cvk_Mkac_Keyblock		[129];
	BYTE Csck_Iss_3ds_Keyblock	[129];
	BYTE Csck_Mag_Keyblock		[129];
	BYTE Dci_3ds_Keyblock		[129];
	BYTE Cvk_3ds_Cak_Keyblock	[129];
	BYTE Future_Field1			[129];
	BYTE Future_Field2			[129];
	BYTE Future_Field3			[129];
	BYTE Future_Field4			[129];
	BYTE Future_Field5			[129];
...
typedef struct
{
   ...
   BYTE  Credit_limit  					   		   [11];       /* TF Phani - For Falcon */
   BYTE  Custr_nbr      					   	   [13];
   BYTE  APP_Source     					   	   [11];
   BYTE  COMP_Name      					   	   [101];
...
/*Added Archive Db password structure for Archive DB password changes in tf.ini file*/
typedef struct
{
