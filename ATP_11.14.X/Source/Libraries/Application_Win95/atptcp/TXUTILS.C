/* $Log:   N:\PVCS\PTE\Equitable\App_lib\Source\TXUTILS.C  $
									  ...
									  BYTE* strCardNumber, BYTE* strMerchant)
{
	...
	if (strCardNumber != NULL_PTR)
		if (strCardNumber[0] != NULL_PTR)
		{
			...
			strcat (strTempMessage, strCardNumber) ;
		}
...
void reset_stats( pTXN_COUNTS p_txn_cnts )
{
   ...
   p_txn_cnts->stip_sent_count = 0UL;	//TF Anna
   p_txn_cnts->stip_recv_count = 0UL;	//TF Anna
   return;
}
