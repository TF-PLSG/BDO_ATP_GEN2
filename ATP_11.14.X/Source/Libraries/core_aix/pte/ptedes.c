...
/******************************************************************************/
INT des_encryption_ex1 ( pBYTE buffer, INT buffer_len, pWORD des_key, INT mode )
{
	if(buffer_len > 10000)
		return 0;
	char out[10001] = {0};
	INT ret = 0;
		...
		{
			memset(temp,0,512);
			strncpy(temp, buffer+n, 128);
			n = n + strlen(temp);
			ret = des_encryption((unsigned char *)temp, strlen(temp), des_key, mode);
			if(ret == 0)
			{
				return 0;
			}
...
******************************************************************************/
INT des_decryption_ex1 ( pBYTE buffer, INT buffer_len, pWORD des_key, INT mode )
{
	if(buffer_len > 10000)
		return 0;
	INT ret = 0;
	char out[10001] = {0};
		...
		{
			memset(temp,0,512);
			strncpy(temp, buffer+n, 256);
			n = n + strlen(temp);
			ret = des_decryption((unsigned char *)temp, strlen(temp), des_key, mode);
			if(ret == 0)
			{
				return 0;
			}
