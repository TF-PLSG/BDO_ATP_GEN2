...
extern BOOLEAN tpsr_tcp_convert(BYTE connection_type  , WORD tcp_port,
...
int update_dm_config()
{
    ...
    char   tpsRate[10], autoModeStr[5];

    GetAscendentConfigDirectory( filename );
    strcat( filename, "tf.ini" );

    GetPrivateProfileString( "TPSRTOOL",
                             "SERVER",
                             "",
                             hostNameOrIP,
                             sizeof( hostNameOrIP ),
    ...
    strcat ( flowListFileName, filename2 );
    ...
    strcat ( deviceMerchantListFileName, filename4 );	// Built the file name, including path, for list of valid device-merchant combination
    ...
    strcpy(dm_config.db_com_port        , "");
    strcpy(dm_config.db_baud_rate       , "");
    strcpy(dm_config.db_byte_size       , "");
    strcpy(dm_config.db_parity          , "");
    strcpy(dm_config.db_stop_bits       , "");

    strcpy(db_dm_config->db_host_name       , hostNameOrIP);
    strcpy(db_dm_config->db_tcp_port        , dialogPort);
