/******************************************************************************
  ...
  $Log:   N:\PVCS\PTE\CORE\ptecom_lib\SERIAL.C  $
      ...
      Rev 1.10   Oct 06 1999 09:38:22   MSALEH
   use pteipc_sleep() instead of sleep()
   
      Rev 1.9   Apr 06 1999 16:23:00   MSALEH
   ...
   is a racal thing only
      ...
      Rev 1.6   Feb 15 1999 11:41:06   MSALEH
   Added mods to handle the parameter db_tcp_header_length = 0
   in that case, a raw message in the format
   <STX>DATA<ETX> is handled, notice that
   there is no LRC computation, this was done for 
   testing with racal
     ...
     err = GetLastError();
      ...
      else
      {
        // some other error occurred
         LogEvent("ASCENDENT: serial get byte error occurred", ERROR_MSG);
         ClearCommError( COMDEV( com ), &dwErrorFlags, &ComStat ) ;
         *len = *ch = 0x00;
...
BOOLEAN serial_put_bytes(pBYTE lpBuf, const DWORD dwLen)
{
   ...
   {  
      ...
      if(err == ERROR_IO_PENDING)
      {
        // We should wait for the completion of the write operation
         ...
         while(!GetOverlappedResult(COMDEV( com ), &(WRITE_OS( com )), &outlen, TRUE))
         {
            ...
            {
               LogEvent("ASCENDENT: serial_put_byte, overlapped result failed", ERROR_MSG);
               ClearCommError( COMDEV( com ), &dwErrorFlags, &ComStat ) ;
      ...
      {
         // some other error occurred
         LogEvent("ASCENDENT: serial_put_byte. other error occured", ERROR_MSG);
         ClearCommError( COMDEV( com ), &dwErrorFlags, &ComStat ) ;
...
void serial_read(pIPC_INFO ipc_info)
{
  ...
  {
    LogEvent( "ASCENDENT: serial_read: Unable to allocate memory for incoming message", ERROR_MSG );
    return;
  }
         ...
         if (!bytesread) break;
          ...
          switch(bState)
          {
           case STATE_STX:
             if(bTmp == STX)
             {
               if (ipc_info->messageheaderlength)
                  bState = STATE_DATA_LEN1;
               else
               {
                  wBufidx = 0;
                  wLength = 0;
                  lpByteBuffer = p_incoming_buffer;
                  bState = STATE_DATA;
               }
             }
             else if(bTmp == ENQ)
             ...
             else if(bTmp != ACK) // Acknowledge from terminal emulator
             {
               sprintf(cStr, "ASCENDENT: serial_read communication error, expecting STX, received %0.2X.", bTmp);
           ...
           case STATE_DATA:
             ...
             {
               /* 
                If we don't have a twp byte header length, then the LRC
                calculation has to begin with the first and second positions
                of the data buffer itself, be sure not to stop the ETX.
                Also, since we have no length in front of the message, we'll
                have to assume that the data itself could NOT be one of the
               ...
               */
               ...
               {
                  bState = STATE_STX;
                  pfNotify(p_incoming_buffer, wBufidx, ipc_info);
               }
             }
             break;
           ...
           case STATE_ETX:
             if(bTmp != ETX)
             {
               sprintf(cStr, "ASCENDENT: serial_read communication error, expecting ETX, received %0.2X.", bTmp);
               LogEvent(cStr, ERROR_MSG);
           ...
           case STATE_LRC:
             if((BYTE)bTmp != bLrc)
             {
               LogEvent("ASCENDENT: serial_read communication error, bad LRC", ERROR_MSG);
               bState = STATE_RESTART;
               break;
             }
             else
             {
               // Valid message!
               bState = STATE_STX;
               pfNotify(p_incoming_buffer, wLength, ipc_info);
               break;
             }
           ...
           case STATE_DATA_LEN2:
             ...
             if(wLength > MAX_INCOMING_BUFFER_SIZE)
               {
                  LogEvent("ASCENDENT: serial_read, communication error. maximum message length exceeded", ERROR_MSG);
                  bState = STATE_RESTART;
                  break;
               }
  ...
  LogEvent("ASCENDENT: serial_read, exit loop, disconnected", INFO_MSG);
...
BOOLEAN serial_write(const WORD len, const void * lpData, pIPC_INFO ipc_info)
{
   ...
   if (ipc_info->messageheaderlength)
   {
      /* Add the two byte length to the calculation of the LRC */
   ...
   {
      /* Send STX ONLY */
      if(serial_put_bytes(bHeader, 1) == FALSE)
         return(FALSE);
      ...
      /* Send ETX ONLY */
      if(serial_put_bytes(bTrailer, 1) == FALSE)
         return(FALSE);
   }
...
BOOLEAN serial_init(const USER_SERIAL_STRUCT * ud, pIPC_INFO ipc_info)
{
   ...
   if(!ud)
   {
    LogEvent("ASCENDENT: serial_init, bad user data pointer passed", ERROR_MSG);
    return(FALSE);
   }
   else if(!ud->pfNotify)
   {
    LogEvent("ASCENDENT: serial_init, bad Function/Buffer pointer passed", ERROR_MSG);
    return(FALSE);
   }
   ...
   if (CONNECTED( com ) == TRUE)
   {
    LogEvent("ASCENDENT: serial_init, connection already established", WARN_MSG); 
    return(FALSE);
   }
   ...
   {
    err = GetLastError();
    LogEvent("ASCENDENT: serial_ init read create event error", ERROR_MSG);
    return ( FALSE ) ;
   }
   ...
   if (NULL == WRITE_OS( com ).hEvent)
   {
    CloseHandle( READ_OS( com ).hEvent ) ;
    err = GetLastError();
    LogEvent("ASCENDENT: serial_init write create event error", ERROR_MSG);
    return ( FALSE ) ;
   }
   ...
   {
    err = GetLastError();
    LogEvent("ASCENDENT: serial_init, serial create file failed", ERROR_MSG);
    return(FALSE);
   }
   ...
   {
      err = GetLastError();
      LogEvent("ASCENDENT: serial_init, get commstate failed", ERROR_MSG);
      return(FALSE);
   }
   ...
   {
      err = GetLastError();
      LogEvent("ASCENDENT: serial_init, set commstate failed", ERROR_MSG);
      CloseAllHandles();
      return(FALSE);
   }
   ...
   lhandle = CreateThread(  NULL,
                            0,
                            (LPTHREAD_START_ROUTINE)serial_read, 
  			   				    ipc_info,
                            0,
                            &lthread_id );
   if( lhandle == NULL )
   {
      LogEvent( "ASCENDENT: serial_init, Unable to create read thread", ERROR_MSG );
      serial_close(0);
      return (FALSE);
   }
