/******************************************************************************
   ...
   $Log:   N:\PVCS\PTE\CORE\cdsapi\CDSAPI.C  $
      ...
      Rev 1.14   Nov 04 1999 14:02:58   MSALEH
   add functions to support cm_config
   
      Rev 1.13   Oct 15 1999 11:15:00   MSALEH
   Move SSL_SUPPORT to preprocessor definitions
   in the project files
   
      Rev 1.12   Oct 14 1999 10:34:24   MSALEH
   Integrating SSL modifications
   
      Rev 1.11   Sep 30 1999 10:21:02   MSALEH
   AIX Modifications
   
      Rev 1.10   Sep 28 1999 14:45:18   MSALEH
   AIX Modifications
   
      Rev 1.9   Jun 16 1999 15:30:32   MSALEH
   Disable sending to tflogger/tfreply
...
BYTE cdsapi_get_cm_config(pCM_CONFIG cm_config, pCHAR error_msg)
{
   ...
   p_msg_out = ptemsg_build_msg(MT_DB_REQUEST,
                                ...
                                coreds_que_name,
                                interactive_que_name,
                                (BYTE *)cm_config->db_subsystem_name,
                                sizeof( cm_config->db_subsystem_name),
                                CM_CONFIG_DATA);
   ...
   memcpy(cm_config, p_data, data_len);
   cdsapi_free(p_msg_in);
   return (PTEMSG_OK);
}
...
BYTE cdsapi_get_cmconfig_table( pCM_CONFIG_LIST cmconfig_list, pCHAR error_msg )
{
   ...
   memcpy( cmconfig_list, p_data, data_len );
	cdsapi_free( p_msg_in );
   return ( PTEMSG_OK );
}
...
BYTE cdsapi_parse_cm_config( pCM_CONFIG cm_config, pPTE_MSG p_msg_in )
{
  ...
  if (p_msg_in == NULL_PTR) return (PTEMSG_NOT_FOUND);
	...
	memcpy( cm_config, (pCM_CONFIG)p_data, data_len );
	return ( PTEMSG_OK );
}
...
BYTE cdsapi_parse_cmconfig_table( pCM_CONFIG_LIST cmconfig_list, pPTE_MSG p_msg_in)
{
  ...
  if (p_msg_in == NULL_PTR) return(PTEMSG_NOT_FOUND);
	...
	memcpy( cmconfig_list, (pCM_CONFIG_LIST)p_data, data_len );
	return ( PTEMSG_OK );
}
...
/************************************************************************/
/************************************************************************/
BYTE cdsapi_update_cm_config_snd( pCM_CONFIG cm_config, pCHAR error_msg )
{
   pPTE_MSG    p_msg_out;
   BYTE        rcode;
