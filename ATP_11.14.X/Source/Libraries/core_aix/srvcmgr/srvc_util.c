...
INT xipc_start( pCHAR xipc_instance )
{
              ...
              XIPC_START_NETWORK ) ) >= 0  )
        {
          // printf( "XIPC instance %s started\n", xipc_instance );
           ...
           srvcmgr_log_message(1,buffer,"xipc_start");
...
INT dependency_start()
{
	...
	if( retcode != ERROR_SUCCESS )
	{
        ...
        sprintf( Buffer,
        		"Unable to open Dependency key. Registry not Initialized for service manager\n");
        srvcmgr_log_message(3,Buffer,"dependency_start");
		return 0;
	}
	...
	if( !hScm )
	{
        ...
        sprintf( Buffer,
        		"Unable to establish connection with the Service Control Manager");
        srvcmgr_log_message(3,Buffer,"dependency_start");
		return 0;
	}
...
INT get_services_table( SERVICE_INFO serv_table[], pINT num_services )
{
    ...
    if( srvc_file == NULL )
    {
        ...
        srvcmgr_log_message(3,err_msg,"get_services_table");
        return 0;
    } 
...
void save_services_table( SERVICE_INFO serv_table[], INT num_services )
{
   ...
   CHAR  buffer[256], err_msg[256];
    ...
    if( srvc_file == NULL )
    {
        sprintf( err_msg, "Error opening Ascendent services file: %d-%s",
                          errno, sys_errlist[errno] );
        srvcmgr_log_message(3,err_msg,"save_services_table");
        //printf( "%s\n", buffer );
        //LogEvent( err_msg, ERROR_MSG );
        return;
    } 
...
void save_service_info( pSERVICE_INFO service_info, INT line_num )
{
    ...
    if( srvc_file == NULL )
    {
        sprintf( err_msg, "Error opening Ascendent services file: %d-%s",
                          errno, sys_errlist[errno] );
        srvcmgr_log_message(3,err_msg,"save_service_info");
       // printf( "%s\n", buffer );
        //LogEvent( err_msg, ERROR_MSG );
        return;
    } 
...
BYTE start_service( pSERVICE_INFO service_info )
{
   ...
   /* open the handle to service control manager */
	...
	if( !hScm )
	{
      ...
      srvcmgr_log_message(3,err_msg,"start_service");
		//printf( "%s\n", buffer );
		//LogEvent( buffer, ERROR_MSG );
...
BYTE stop_service( pSERVICE_INFO service_info )
{
   ...
   DWORD err_code;
	...
	if( !hScm )
	{
      err_code = GetLastError();
      sprintf( buffer, "Error opening NT Service Control Database - %d", err_code );
      srvcmgr_log_message(3,err_msg,"stop_service");
		//printf( "%s\n", buffer );
		//LogEvent( buffer, ERROR_MSG );
...
BYTE ping_service( pSERVICE_INFO service_info )
{
   ...
   /*  build the outgoing message */
	...
	if( p_msg_out == NULL_PTR )
	{
		...
		srvcmgr_log_message(3,buffer,"ping_service");
		//LogEvent( buffer, ERROR_MSG );
                printf("Failed\n");
...
BYTE trace_service( pSERVICE_INFO service_info, BYTE flag )
{
	...
	if( p_msg_out == NULL_PTR )
	{
	   sprintf( buffer, "Unable to build trace message to %s", 
				 service_info->service_name );
	   srvcmgr_log_message(3,buffer,"trace_service");
       	   //printf( "%s\n", buffer );
		//LogEvent( buffer, ERROR_MSG );
	   return PTEMSG_INSUFFICIENT_MEMORY;
	}
...
/* ************************************************************************** */
BYTE mra_service( pSERVICE_INFO service_info, BYTE flag )
{
	pPTE_MSG p_msg_out, p_msg_in;
	BYTE trace_flag, result;
	LONG rcode;
	...
	if( p_msg_out == NULL_PTR )
	{
	   sprintf( buffer, "Unable to build trace message to %s", 
				 service_info->service_name );
	   srvcmgr_log_message(3,buffer,"mra_service");
       	   //printf( "%s\n", buffer );
		//LogEvent( buffer, ERROR_MSG );
	   return PTEMSG_INSUFFICIENT_MEMORY;
	}
	...
	if( result == PTEMSG_OK )
	{
		if(flag == ST1_SRVC_MRAON)
			service_info->mra_on = '1';
		else
			service_info->mra_on = '0';
	}
...
BYTE falcon_service( pSERVICE_INFO service_info, BYTE flag )
{
	pPTE_MSG p_msg_out, p_msg_in;
	BYTE trace_flag, result;
	LONG rcode;
	...
	if( p_msg_out == NULL_PTR )
	{
	   sprintf( buffer, "Unable to build trace message to %s", 
				 service_info->service_name );
	   srvcmgr_log_message(3,buffer,"falcon_service");
       	  // printf( "%s\n", buffer );
		//LogEvent( buffer, ERROR_MSG );
	   return PTEMSG_INSUFFICIENT_MEMORY;
	}
	...
	if( result == PTEMSG_OK )
	{
		if(flag == ST1_SRVC_FALCONON)
			service_info->falcon_on = '1';
		else
			service_info->falcon_on = '0';
	}
...
BYTE encrypt_trace_service( pSERVICE_INFO service_info, BYTE flag )
{
	pPTE_MSG p_msg_out, p_msg_in;
	BYTE trace_flag, result;
	LONG rcode;
	...
	if( p_msg_out == NULL_PTR )
	{
	   sprintf( buffer, "Unable to build trace message to %s", 
				 service_info->service_name );
	   srvcmgr_log_message(3,buffer,"encrypt_trace_service");
       	   //printf( "%s\n", buffer );
		//LogEvent( buffer, ERROR_MSG );
	   return PTEMSG_INSUFFICIENT_MEMORY;
	}
	...
	if( result == PTEMSG_OK )
	{
		if(flag == ST1_SRVC_ENCON)
			service_info->encryption_on = '1';
		else
			service_info->encryption_on = '0';
	}
