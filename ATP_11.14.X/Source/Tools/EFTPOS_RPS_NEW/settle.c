	...
	*/
	...
	else
	{
	 ...
	 /* Segment 10 */
	  ...
	  sprintf(c_record, "%1s%4s%-15s%-019s%6s%6s%2s%4s%012s%03s%03s%-031s%4s%10s%1s%11s%15s%03s%8s%-15s%72s",
         "1",
		 auth_tx_detail.message_type,
         term_id,         
         auth_tx_detail.card_nbr,
                   ...
                   auth_tx_detail.processing_code[2] == '4' )
               {
                       ...
                       ( credit_txns_exist == true ) )
                  {
					   ...
					   {						   
                           ...
                           TxUtils_Send_Msg_To_Operator (1,   /* 1 - write to sys. mon., 0 - not */
							 buffer,                          /* the text of the message */
					    	 1,                               /* 1 - write to event log, 0 - not */
							 INFO_MSG,                        /* message type */
							 "create_settle_files",           /* name of the function */
							 2,                               /* severity */
							 WARNING_ERROR,                   /* message type */
							 auth_tx_detail.tran_amount,      /* Transaction Amount */
							 NULL_PTR,                        
							 NULL_PTR           
							            );  
				      ...
				      ctemp_amt=atoi(auth_tx_detail_arc.tran_amount); /* To Check whether amount is proper or not */
					   ...
					   if((ctemp_amt==INT_MAX) || (ctemp_amt==INT_MIN))
					   {
                           ...
                           TxUtils_Send_Msg_To_Operator (1,   /* 1 - write to sys. mon., 0 - not */
							 ...
							 2,                               /* severity */
							 WARNING_ERROR,                   /* message type */
							 auth_tx_detail_arc.tran_amount,      /* Transaction Amount */
							 NULL_PTR,                        
							 NULL_PTR           
							            );  
 ...
 Girija Y, TF Feb 19, 2009 */
	if (get_currency_code() == false)
   {
      ...
      TxUtils_Send_Msg_To_Operator (0,    
		...
		1,                               
         INFO_MSG,                        
         "BCH01 Handler/get_currency_code", 
         2,                               
         WARNING_ERROR,                  
         tc_totals.terminal_id,           
         NULL_PTR,                       
