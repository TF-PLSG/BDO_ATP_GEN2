...
public class MessageReciever {
	...
	private final static Logger logger = Logger.getLogger(MessageReciever.class);
	...
	/*
	 * This method is responsible for reading the property file which is
	 * configured in the environment variable ("ASCENDENTROOT") if
	 * (falconHandler.getLibFileVersion() == currentVersion) condition satisfies
	 * this will initialize the connection with HOST Server
	 * 
	 */
	...
	/*
	 * Once connection is initialize this will read the message from the queue
	 */
	...
	public void readMessage() {
		...
		if (isConnAvailable) {
			...
			try {
				...
				if (!isanyMessages) {
					/*
					 * if(MsgSenderThread.logsCreate == true && logsenalble ==
					 * true){ logger.
					 * info("There are no messages inside JMS Queue=========");
					 * }
					 */
				}
				/*
	...
	public static void enableReceiverLogs() {
		try {
			String propFilePath = System.getenv("ASCENDENTROOT");
			if (propFilePath != null && MsgSenderThread.logsCreate == true) {
				if (!logsenalble) {
					logsenalble = true;
					...
					// logger.info("enabled Receiver Logs===================");
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
