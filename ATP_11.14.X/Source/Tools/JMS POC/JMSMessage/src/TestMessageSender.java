...
public class TestMessageSender {
	 ...
	 * @param args
	 */
	private final static Logger logger = Logger.getLogger(TestMessageSender.class);

	public static void main(String[] args) {

		/*
		 * this portion was commented starts from here
		 */
		...
		try {
			...
			/*
			 * Here propFilePath is the path for the ASCENDENTROOT where we can
			 * define the path for trace file to generate the log for falcon
			 * request respose
			 */
			...
			if (args.length > 0) {
				...
				/*fstream = (InputStream) TestMessageSender.class.getResourceAsStream("D:\\epicportz\\CTRANS_msg_cards.txt");
				FileInputStream fstream = new FileInputStream("D:\\epicportz\\CTRANS_msg_cards.txt");
				DataInputStream in = new DataInputStream(fstream);
				BufferedReader br = new BufferedReader(new InputStreamReader(in));
				String strLine;
				while ((strLine = br.readLine()) != null) {
					logger.info("Message Sent====================" + strLine);
				}
				FileReader fr = new FileReader("D:\\epicportz\\CTRANS_msg_cards.txt");
		...
		try {
			...
			MessageSender messageSender = new MessageSender();
			messageSender.setup();
			String msgTextFile = propFilePath + "/trace/Falcon_Archive.txt";
			...
			/*
			 * while (br.readLine()!=null) { i++; //Thread.sleep(1000);
			 * //messageSender.sendMessage(br.readLine());
			 * System.out.println("Message Sent===================="+i+":"+br.
			 * readLine());
			 * //logger.info("Message Sent===================="+i+":"+br.
			 * readLine()); }
			 */
			messageSender.closeSession();
		} catch (Exception e) {
