...
#define DINERS_RESPONSE_CODE_LEN 3
...
************************************/
   ...
   MainProcessor(connection_type);
	...
	else
	{
		...
		strncpy(RespCodeinTxt,valuefromRCfile,DINERS_RESPONSE_CODE_LEN);
		sprintf( global_err_str, "Value of Diners Response Code is %s", RespCodeinTxt);
		AfxMessageBox(global_err_str,MB_ICONINFORMATION | MB_OK);

		GetPrivateProfileString1(
		"AUTH_CODE",	            /*  points to section name  */
		"DINERSAC",                   /*  points to key name  */
		"0",                     /*  points to default string  */
		...
		sprintf( global_err_str, "Value of Diners Auth Code is %s", AuthCodeinTxt);
		AfxMessageBox(global_err_str,MB_ICONINFORMATION | MB_OK);

		GetPrivateProfileString1(
		"RRN",	            /*  points to section name  */
		"DINERSRRN",                   /*  points to key name  */
		"0",                     /*  points to default string  */
		...
		sprintf( global_err_str, "Value of Diners RRN is %s", RRNinTxt);
		AfxMessageBox(global_err_str,MB_ICONINFORMATION | MB_OK);
    ...
    while( !feof( src_file ) )
    {
        ...
        if( strncmp( key_name, buffer, strlen( key_name ) ) == 0 )
        {
           ...
           // strcpy( dest_buffer, &buffer[temp] );
			//strncpy(RespCodeBuffer, &buffer[temp],3);dest_buffer
			if(strncmp(full_section_name,"[RESPONSE_CODE]",15)==0)
			{
				strncpy(dest_buffer, &buffer[temp],DINERS_RESPONSE_CODE_LEN);
			}
