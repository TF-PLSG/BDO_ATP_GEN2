...
#define RACAL_RESPONSE_CODE_LEN 2
...
************************************/
   ...
   MainProcessor(connection_type);
	...
	else
	{
		...
		sprintf( global_err_str, "Value of RACAL Response Code is %s", RespCodeinTxt);
		AfxMessageBox(global_err_str,MB_ICONINFORMATION | MB_OK);

		GetPrivateProfileString1(
		"AUTH_CODE",	            /*  points to section name  */
		"RACALAC",                   /*  points to key name  */
		"0",                     /*  points to default string  */
		...
		sprintf( global_err_str, "Value of RACAL Auth Code is %s", AuthCodeinTxt);
		AfxMessageBox(global_err_str,MB_ICONINFORMATION | MB_OK);

		GetPrivateProfileString1(
		"RRN",	            /*  points to section name  */
		"RACALRRN",                   /*  points to key name  */
		"0",                     /*  points to default string  */
		...
		sprintf( global_err_str, "Value of RACAL RRN is %s", RRNinTxt);
		AfxMessageBox(global_err_str,MB_ICONINFORMATION | MB_OK);
                             ...
                             char * file_name)
{
    ...
    while( !feof( src_file ) )
    {
        ...
        if( strncmp( key_name, buffer, strlen( key_name ) ) == 0 )
        {
			...
			{
				strncpy(dest_buffer, &buffer[temp],RACAL_RESPONSE_CODE_LEN);
			}
			...
			//strncpy(dest_buffer, &buffer[temp],2);
