...
#define AMEX_RESPONSE_CODE_LEN 3
...
************************************/
   ...
   MainProcessor(connection_type);
	...
	else
	{
		...
		strncpy(RespCodeinTxt,valuefromRCfile,AMEX_RESPONSE_CODE_LEN);
		...
		sprintf( global_err_str, "Value of AMEX Auth Code is %s", AuthCodeinTxt);
		AfxMessageBox(global_err_str,MB_ICONINFORMATION | MB_OK);

		GetPrivateProfileString1(
		"RRN",	            /*  points to section name  */
		"AMEXRRN",                   /*  points to key name  */
		"0",                     /*  points to default string  */
		...
		sprintf( global_err_str, "Value of AMEX RRN is %s", RRNinTxt);
		AfxMessageBox(global_err_str,MB_ICONINFORMATION | MB_OK);
                             ...
                             char * file_name)
{
    ...
    while( !feof( src_file ) )
    {
        ...
        if( strncmp( key_name, buffer, strlen( key_name ) ) == 0 )
        {
			...
			{
				strncpy(dest_buffer, &buffer[temp],AMEX_RESPONSE_CODE_LEN);
			}
