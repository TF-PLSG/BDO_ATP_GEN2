...
#define NCIFTR_RESPONSE_CODE_LEN 2
...
*/
	...
	else
	{
		...
		strncpy(RespCodeinTxt,valuefromRCfile,NCIFTR_RESPONSE_CODE_LEN);
		sprintf( global_err_str, "Value of NCIFTR Response Code is %s", RespCodeinTxt);
		AfxMessageBox(global_err_str,MB_ICONINFORMATION | MB_OK);

		GetPrivateProfileString1(
		"AUTH_CODE",	            /*  points to section name  */
		"NCIFTRAC",                   /*  points to key name  */
		"0",                     /*  points to default string  */
		...
		sprintf( global_err_str, "Value of NCIFTR Auth Code is %s", AuthCodeinTxt);
		AfxMessageBox(global_err_str,MB_ICONINFORMATION | MB_OK);

		GetPrivateProfileString1(
		"RRN",	            /*  points to section name  */
		"NCIFTRRRN",                   /*  points to key name  */
		"0",                     /*  points to default string  */
		...
		sprintf( global_err_str, "Value of NCIFTR RRN is %s", RRNinTxt);
		AfxMessageBox(global_err_str,MB_ICONINFORMATION | MB_OK);
                             ...
                             char * file_name)
{
    ...
    while( !feof( src_file ) )
    {
        ...
        if( strncmp( key_name, buffer, strlen( key_name ) ) == 0 )
        {
			...
			{
				strncpy(dest_buffer, &buffer[temp],NCIFTR_RESPONSE_CODE_LEN);
			}
			...
			/*strncpy(RespCodeBuffer, &buffer[temp],2);
