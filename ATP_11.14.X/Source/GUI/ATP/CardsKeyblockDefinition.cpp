...
IMPLEMENT_DYNCREATE(CCardsKeyblockDefinition, CPropertyPage)

CCardsKeyblockDefinition::CCardsKeyblockDefinition() : CPropertyPage(CCardsKeyblockDefinition::IDD)
{
...
CCardsKeyblockDefinition::~CCardsKeyblockDefinition()
{
}

void CCardsKeyblockDefinition::DoDataExchange(CDataExchange* pDX)
{
	CPropertyPage::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CCardsFleetDefinition)
	DDX_Control(pDX, IDC_VIEW_3DS2_BUTTON, m_ViewButton);
	DDX_Control(pDX, IDC_UPDATE_3DS2_BUTTON, m_UpdateButton);
	DDX_Control(pDX, IDC_LIST_3DS2_BUTTON, m_ListButton);
	DDX_Control(pDX, IDC_DELETE_3DS2_BUTTON, m_DeleteButton);
	DDX_Control(pDX, IDC_ADD_3DS2_BUTTON, m_AddButton);
...
BEGIN_MESSAGE_MAP(CCardsKeyblockDefinition, CPropertyPage)
//{{AFX_MSG_MAP(CCardsKeyblockDefinition)
ON_BN_CLICKED(IDC_ADD_3DS2_BUTTON, OnCardsAddButton)
ON_BN_CLICKED(IDC_DELETE_3DS2_BUTTON, OnCardsDeleteButton)
ON_BN_CLICKED(IDC_LIST_3DS2_BUTTON, OnCardsListButton)
ON_BN_CLICKED(IDC_UPDATE_3DS2_BUTTON, OnCardsUpdateButton)
ON_BN_CLICKED(IDC_VIEW_3DS2_BUTTON, OnCardsViewButton)
//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
BOOL CCardsKeyblockDefinition::OnInitDialog() 
{  
...
void CCardsKeyblockDefinition::OnCardsAddButton() 
{
    ...
    KEY3DS01 sKey3ds01;
	CAF01_KEYBLOCK sCaf01_keyblk01;
	...
	memset(strFilePrimaryKey, 0x00, sizeof(strFilePrimaryKey));
	...
	ZeroMemory(&sCaf01_keyblk01,sizeof(CAF01_KEYBLOCK));
    ...
    memcpy ((LPSTR)(sKey3ds01.caf_profile_id),m_StrProfileId_Fld ,sizeof (sKey3ds01.caf_profile_id));
 	memcpy ((LPSTR)(sCaf01_keyblk01.primary_key.caf_profile_id),m_StrProfileId_Fld ,sizeof (sCaf01_keyblk01.primary_key.caf_profile_id));
   ...
   /* if (m_nCardMode == MODE_FLEET)
    ...
    if (m_nCardMode == MODE_CREDIT)
    { 		   
        memcpy ((LPSTR)(sCaf01_keyblk01.primary_key.type),"C" ,sizeof (sCaf01_keyblk01.primary_key.type));  
    ...
    int rcode = txdsapi_get_record ( (pBYTE)&sCaf01_keyblk01, sizeof (CAF01_KEYBLOCK), CAF01_KEYBLOCK_DATA, strErrorMessage );
    if(rcode == PTEMSG_OK )
    {
...
void CCardsKeyblockDefinition::OnCardsDeleteButton() 
{
	CString strMessage;
    int nReturnCode;
    CAF01 sProfileId;
	...
	memcpy ((LPTSTR)sCaf01_keyblk01.primary_key.caf_profile_id,m_StrProfileId_Fld,sizeof (sCaf01_keyblk01.primary_key.caf_profile_id)) ;

  
	if (m_nCardMode == MODE_CREDIT)
	{
		memcpy ((LPTSTR)sCaf01_keyblk01.primary_key.type,"C",sizeof (sCaf01_keyblk01.primary_key.type)) ;
    ...
    int rcode = txdsapi_delete_record ((pBYTE)&sCaf01_keyblk01,
        sizeof (CAF01_KEYBLOCK), CAF01_KEYBLOCK_DATA, strErrorMessage);
...
void CCardsKeyblockDefinition::OnCardsListButton() 
{
    CNameList listDlg;    
    int nReturnCode;
    ...
    listDlg.m_nIndicator = MODE_CAF01_KEYBLOCK;
...
void CCardsKeyblockDefinition::OnCardsUpdateButton() 
{    
...
/*    pMerchantLimitPage->m_nDialogMode = MODE_EDIT;
	...
	memcpy ((LPSTR)(sCaf01_keyblkOld.primary_key.caf_profile_id),
		      m_StrProfileId_Fld,sizeof (sCaf01_keyblkOld.primary_key.caf_profile_id) );		  
   ...
   /* if (m_nCardMode == MODE_FLEET)
    ...
    if (m_nCardMode == MODE_CREDIT)
    {         
		memcpy((LPSTR)(sCaf01_keyblkOld.primary_key.type),
			"C",sizeof (sCaf01_keyblkOld.primary_key.type));	      
    ...
    int rcode = txdsapi_get_record((pBYTE)&sCaf01_keyblkOld,
		sizeof (CAF01_KEYBLOCK),CAF01_KEYBLOCK_DATA, strErrorMessage);
	if (rcode != PTEMSG_OK)
	{        
		m_StrMessageLbl = strErrorMessage;
    ...
    strcpy ( strFilePrimaryKey, (char *)sCaf01_keyblkOld.primary_key.caf_profile_id);
	...
	PopulateProfileIdDialogFromDatabase ( sCaf01_keyblkOld);
...
void CCardsKeyblockDefinition::OnCardsViewButton() 
{
/*CAF01 sProfileId;
FAF01 sFleetCard;
HST01 sHistoryRecord;
...
/*	memset(&sEMVProfileOld, 0, sizeof(sEMVProfileOld));
	memset(&sKey3ds01Old, 0, sizeof(sKey3ds01Old)); */
	memset(&sCaf01_keyblkOld, 0, sizeof(sCaf01_keyblkOld));
...
BOOL CCardsKeyblockDefinition::OnApply() 
{

	CMyUtils myUtilsObject;

	CAF01_KEYBLOCK	sCaf01_kblk01DatabaseStructure;
	...
	*/
	...
	{
		if (( myUtilsObject.IsEmpty (pProcessingPage->m_StrCvk_Mag_KeyblockFld,this)) )
		{
			((CPropertySheet*) this->GetParent())->SetActivePage(1);
            pProcessingPage->m_CntlCvk_Mag_KeyblockFld.SetFocus();
			pProcessingPage->m_StrMessageLbl = "Please enter CVK MAG keyblock ";
			pProcessingPage->UpdateData(FALSE);
			return FALSE;
		}
		if (( myUtilsObject.IsEmpty (pProcessingPage->m_StrCvk_Acq_3DS_KeyblockFld,this)) )
		{
			((CPropertySheet*) this->GetParent())->SetActivePage(1);
            pProcessingPage->m_CntlCvk_Acq_3DS_KeyblockFld.SetFocus();
			pProcessingPage->m_StrMessageLbl = "Please enter CVK ACQ 3DS keyblock ";
			pProcessingPage->UpdateData(FALSE);
			return FALSE;
		}
		if (( myUtilsObject.IsEmpty (pProcessingPage->m_StrCvk_Pin_VerificationFld,this)) )
		{
			((CPropertySheet*) this->GetParent())->SetActivePage(1);
            pProcessingPage->m_CntlCvk_Pin_VerificationFld.SetFocus();
			pProcessingPage->m_StrMessageLbl = "Please enter CVK PIN Verification ";
			pProcessingPage->UpdateData(FALSE);
			return FALSE;
		}
		if ( myUtilsObject.IsEmpty(pProcessingPage->m_StrCvk_Mkac_KeyblockFld, this)) 
		{
			((CPropertySheet*) this->GetParent())->SetActivePage(1);
            pProcessingPage->m_CntlCvk_Mkac_KeyblockFld.SetFocus();
			pProcessingPage->m_StrMessageLbl = "Please enter CVK MKAC keyblock";
			pProcessingPage->UpdateData(FALSE);
			return FALSE;
		}
		if (( myUtilsObject.IsEmpty (pProcessingPage->m_StrCsck_Iss_3ds_KeyblockFld,this)) )
		{
			((CPropertySheet*) this->GetParent())->SetActivePage(1);
            pProcessingPage->m_CntlCsck_Iss_3ds_KeyblockFld.SetFocus();
			pProcessingPage->m_StrMessageLbl = "Please enter CSCK ISS 3DS keyblock ";
			pProcessingPage->UpdateData(FALSE);
			return FALSE;
		}
		if (( myUtilsObject.IsEmpty (pProcessingPage->m_StrCsck_Mag_KeyblockFld,this)) )
		{
			((CPropertySheet*) this->GetParent())->SetActivePage(1);
            pProcessingPage->m_CntlCsck_Mag_KeyblockFld.SetFocus();
			pProcessingPage->m_StrMessageLbl = "Please enter CSCK MAG keyblock ";
			pProcessingPage->UpdateData(FALSE);
			return FALSE;
		}
		if (( myUtilsObject.IsEmpty (pProcessingPage->m_StrDci_3ds_Keyblock_Fld,this)) )
		{
			((CPropertySheet*) this->GetParent())->SetActivePage(1);
            pProcessingPage->m_CntlDci_3ds_Keyblock_Fld.SetFocus();
			pProcessingPage->m_StrMessageLbl = "Please enter DCI 3DS keyblock ";
			pProcessingPage->UpdateData(FALSE);
			return FALSE;
		}
		if (( myUtilsObject.IsEmpty (pProcessingPage->m_StrCvk_3ds_Cak_KeyblockFld,this)) )
		{
			((CPropertySheet*) this->GetParent())->SetActivePage(1);
            pProcessingPage->m_CntlCvk_3ds_Cak_KeyblockFld.SetFocus();
			pProcessingPage->m_StrMessageLbl = "Please enter CVK 3DS CAK keyblock ";
			pProcessingPage->UpdateData(FALSE);
			return FALSE;
		}
	}
	...
	{ 
		...
		memset(&sCaf01_kblk01DatabaseStructure, 0, sizeof(sCaf01_kblk01DatabaseStructure));
		
        PopulateProfileIdDatabaseStructureFromDialog(&sCaf01_kblk01DatabaseStructure);
        ...
        int rcode = txdsapi_update_record  ((pBYTE)&sCaf01_kblk01DatabaseStructure, sizeof (CAF01_KEYBLOCK), CAF01_KEYBLOCK_DATA, strErrorMessage);
		if (rcode != PTEMSG_OK)
		{    	
			m_StrMessageLbl = strErrorMessage;
            m_nDialogMode = MODE_DONE;
        ...
        else
		{
            ...
            /*if (pMerchantLimitPage->m_bMerchant )
            {	
				//	pMerchantLimitPage->CompareStructures(&sCaf01_kblk01DatabaseStructure);
				pMerchantLimitPage->CleanMerchantDialog();				
                pMerchantLimitPage->EnableMerchantDialog(FALSE);			
            }
			if (pCardsProfilePage->m_bProcessing)
	...
	if (m_nDialogMode == MODE_ADD)
    {
        ...
        if (!(PopulateProfileIdDatabaseStructureFromDialog( &sCaf01_kblk01DatabaseStructure)))
        {		
			return FALSE;
		}
        /*if (m_nCardMode == MODE_FLEET)
        ...
        int rcode = txdsapi_insert_record  ((pBYTE)&sCaf01_kblk01DatabaseStructure, sizeof (CAF01_KEYBLOCK), CAF01_KEYBLOCK_DATA, strErrorMessage); 
        if (rcode != PTEMSG_OK)
		{   	
			m_StrMessageLbl = strErrorMessage;
            m_nDialogMode = MODE_DONE;
...
BOOL CCardsKeyblockDefinition::PopulateProfileIdDialogFromDatabase (  CAF01_KEYBLOCK sCaf01_keyblk01)
{   
	
	if (!pProcessingPage->m_bProcessing)
		memcpy ( &pProcessingPage->sCaf01_keyblkOld, &sCaf01_keyblk01, sizeof ( CAF01_KEYBLOCK));
	else
		pProcessingPage->PopulateProcessingDialogFromDatabase( sCaf01_keyblk01 );
...
BOOL CCardsKeyblockDefinition::PopulateProfileIdDatabaseStructureFromDialog ( pCAF01_KEYBLOCK psCaf01_keyblk01) 
{

    ZeroMemory (psCaf01_keyblk01, sizeof(CAF01_KEYBLOCK)) ;			
	
	if ( m_nDialogMode == MODE_EDIT )
	{

		memcpy (psCaf01_keyblk01,&sCaf01_keyblkOld, sizeof(CAF01_KEYBLOCK));		
    ...
    memcpy ((LPTSTR) psCaf01_keyblk01->primary_key.caf_profile_id,m_StrProfileId_Fld, sizeof (psCaf01_keyblk01->primary_key.caf_profile_id));
   ...
   /* pTime1Page->PopulateDatabaseFromTime1Dialog( psProfileId );
    ...
    if (m_nCardMode == MODE_CREDIT)
    {
		memcpy ((LPTSTR) psCaf01_keyblk01->primary_key.type,"C", sizeof (psCaf01_keyblk01->primary_key.type));
	...
	/*if (m_nCardMode == MODE_DEBIT)
    {
	    ...
	    pProcessingPage->PopulateDatabaseFromProcessingDialog( psCaf01_keyblk01 );  
	return TRUE;
}
...
void CCardsKeyblockDefinition::PopulateHistoryTable()
{
    HST01 sHistoryData;
    CHAR dest_str[18];   
	CMyUtils myUtilsObject;
...
BOOL CCardsKeyblockDefinition::CheckDialogForCompleteness()
{
    CMyUtils myUtilsObject;
...
void CCardsKeyblockDefinition::OnCancel() 
{      
    if ( pChangeHistoryPage->m_bChangeHistory )
		pChangeHistoryPage->CleanChangeHistory();
...
BOOL CCardsKeyblockDefinition::OnCommand(WPARAM wParam, LPARAM lParam) 
{
